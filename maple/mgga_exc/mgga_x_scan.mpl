...
scan_h1x := x -> 1 + params_a_k1*x/(params_a_k1 + x):

scan_b2 := sqrt(5913/405000):
scan_b1 := (511/13500)/(2*scan_b2):
scan_b3 := 1/2:
scan_b4 := MU_GE^2/params_a_k1 - 1606/18225 - scan_b1^2:
scan_y  := (x, a) -> MU_GE*mgga_p(x) + scan_b4*mgga_p(x)^2*exp(-scan_b4*mgga_p(x)/MU_GE)
  + (scan_b1*mgga_p(x) + scan_b2*(1 - a)*exp(-scan_b3*(1 - a)^2))^2:

scan_a1 := 4.9479:
scan_gx := x -> -xc_expm1(-scan_a1/sqrt(gga_s(x))):
...
scan_f_a := (x, a) ->
  (scan_h1x(scan_y(x, a))*(1 - scan_f_alpha(a)) + scan_h0x*scan_f_alpha(a))*scan_gx(x):

scan_f   := (x, u, t) -> scan_f_a(x, mgga_alpha(x, t)):
