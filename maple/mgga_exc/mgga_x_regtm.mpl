...
regtm_one_minus_w := zp -> (1 - zp)*(1 + zp - zp^3 - zp^4 - zp^5) / (1 + zp^3)^2:

(* Collect all the pieces together; override both halves of the tm_w
   pair so the SC weighting in tm_f stays cancellation-free. *)
tm_w           := (x, t) -> regtm_w          (regtm_zp(mgga_alpha(x, t), tm_p(x))):
tm_one_minus_w := (x, t) -> regtm_one_minus_w(regtm_zp(mgga_alpha(x, t), tm_p(x))):
