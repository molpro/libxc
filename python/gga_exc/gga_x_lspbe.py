...
def lspbe_f0(s):
    return (
        Integer(1) + params_a_kappa*(params_a_mu*s**Integer(2)/(params_a_kappa + params_a_mu*s**Integer(2)))
        - (params_a_kappa+Integer(1))*(-xc_expm1(-params_a_alpha*s**Integer(2)))
    )

@helper
def lspbe_f(x):
    return lspbe_f0(gga_s(x))

def f(rs, z, xt, xs0, xs1):
    return gga_exchange(lspbe_f, rs, z, xs0, xs1)

TYPE = "gga_exc"
PARAMS_STRUCT = "gga_x_lspbe_params"
