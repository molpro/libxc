...
def test_hyb_gga_xc_hflyp_N_restr_1_zk():
    # Prepare the input
    ...
    tgt = out["zk"].flatten()
    ref_tgt = ns.full_like(tgt, ns.asarray([-6.123759716085011e-02, -6.106943218190817e-02, -6.081943604492026e-02, -6.044227018373092e-02, -5.986087099053470e-02, -5.893593649785146e-02, -5.739479701126333e-02, -5.464958299167929e-02, -4.931574669191897e-02, -3.851942047736291e-02, -2.757576257670772e-02, -4.120734741489564e-02, -4.101764338883682e-02, -3.454254934263504e-02, -2.190251291391559e-02, -1.275465908408974e-03, 1.220269948652408e-02, -3.235727194607824e-03, -9.108326858672549e-04, -1.023961420889946e-04]))
    error = ns.max(ns.abs(tgt-ref_tgt))/(1.0+ns.max(ns.asarray([ns.abs(tgt), ns.abs(ref_tgt)])))
    assert error < 5e-08


def test_hyb_gga_xc_hflyp_N_restr_1_vrho():
    # Prepare the input
    inp = test_data["N_restr"]
    ...
    ref_tgt = ns.full_like(tgt, ns.asarray([-6.771727150669461e-02, -6.778345481435906e-02, -6.788577411908817e-02, -6.804878610813661e-02, -6.831954359079714e-02, -6.879558392420027e-02, -6.969836957749311e-02, -7.158237585696089e-02, -7.593437783597867e-02, -8.592098919032358e-02, -9.057159877107156e-02, -5.576530627018002e-02, -5.043208237525430e-02, -5.126692518551532e-02, -5.269384736361098e-02, -5.373170688312724e-02, -3.313754987452720e-02, -4.487673348321921e-03, -1.208820589539621e-03, -1.364571241821357e-04]))
    error = ns.max(ns.abs(tgt-ref_tgt))/(1.0+ns.max(ns.asarray([ns.abs(tgt), ns.abs(ref_tgt)])))
    assert error < 5e-05


def test_hyb_gga_xc_hflyp_N_restr_1_vsigma():
    # Prepare the input
    inp = test_data["N_restr"]
...
def test_hyb_gga_xc_hflyp_N_restr_1_v2rho2():
    # Prepare the input
    inp = test_data["N_restr"]
    ...
    ref_tgt = ns.full_like(tgt, ns.asarray([4.154264052837645e-05, 4.614629922922016e-05, 5.365765080907261e-05, 6.660814600963132e-05, 9.077752114118106e-05, 1.413164961951366e-04, 2.656664883177741e-04, 6.509389233611789e-04, 2.284692995646671e-03, 1.206557180125359e-02, 5.785137551190032e-02, 1.668792801830256e-02, 2.877957921629364e-03, 5.361922106487729e-02, 4.495121081192578e-01, 4.513854003329142e+00, 2.335171472742378e+01, -7.377231245543088e+01, -1.378206526628154e+03, -1.175077579757397e+05]))
    error = ns.max(ns.abs(tgt-ref_tgt))/(1.0+ns.max(ns.asarray([ns.abs(tgt), ns.abs(ref_tgt)])))
    assert error < 5e-05


def test_hyb_gga_xc_hflyp_N_restr_1_v2rhosigma():
    # Prepare the input
    inp = test_data["N_restr"]
...
def test_hyb_gga_xc_hflyp_N_restr_1_v2sigma2():
    # Prepare the input
    inp = test_data["N_restr"]
