/*
  Generated from python/gga_exc/gga_x_dk87.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py gga_x_dk87
*/

#ifndef _GGA_X_DK87_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _GGA_X_DK87_KERNEL_BODY
#define _KMAX 0
#define _GGA_X_DK87_HELPER_BODIES
#include "gga_x_dk87.c"
#undef _GGA_X_DK87_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _GGA_X_DK87_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_GGA_X_DK87_HELPER_BODIES)
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(dk87_f_pf_k, _KMAX)(const xc_func_type *p, double x2, double *out) {
  const gga_x_dk87_params *params = (const gga_x_dk87_params *)(p->params);

  const double _t1 = params->b1*x2;
  const double _t2 = _t1 + 0.1e1;
  const double _t3 = (0.1e1 / _t2);
  const double _t4 = pow(x2, (0.1e1 / 0.2e1)*params->alpha);
  const double _t5 = _t4*params->a1;
  const double _t6 = _t5 + 0.1e1;
  const double _t7 = xc_powr(0.18e2, 1, 3);
  const double _t8 = (0.1e1 / (M_PI * M_CBRTPI * M_CBRTPI));
  const double _t9 = (0.1e1 / M_CBRT3);
  const double _t10 = M_CBRTPI*_t7*_t8*_t9;
  const double _t11 = _t10*_t6;
  const double _t12 = (0.7e1 / 0.1944e4)*_t11;
  const double _t13 = _t12*_t3;
#if _KMAX >= 1
  const double _t14 = _t10*_t5;
  const double _t15 = _t14*params->alpha;
  const double _t16 = _t15*_t3;
  const double _t17 = (0.7e1 / 0.3888e4)*_t16;
  const double _t18 = (0.1e1 / (_t2 * _t2));
#endif
#if _KMAX >= 2
  const double _t19 = _t18*params->b1;
  const double _t20 = _t15*_t19;
  const double _t21 = (0.7e1 / 0.972e3)*_t11;
  const double _t22 = (0.1e1 / x2);
  const double _t23 = _t14*_t3;
  const double _t24 = (0.7e1 / 0.7776e4)*_t23;
  const double _t25 = ((params->alpha) * (params->alpha));
  const double _t26 = _t22*_t25;
  const double _t27 = (0.1e1 / (_t2 * _t2 * _t2));
  const double _t28 = ((params->b1) * (params->b1));
  const double _t29 = _t27*_t28;
#endif
#if _KMAX >= 3
  const double _t30 = (0.1e1 / (x2 * x2));
  const double _t31 = ((params->alpha) * (params->alpha) * (params->alpha));
  const double _t32 = ((params->b1) * (params->b1) * (params->b1))/(_t2 * _t2 * _t2 * _t2);
  const double _t33 = _t14*_t19;
#endif
#if _KMAX >= 4
  const double _t34 = (0.1e1 / (x2 * x2 * x2));
  const double _t35 = (0.7e1 / 0.81e2)*_t11;
  const double _t36 = _t24*_t34;
#endif

  const double f = _t13*x2 + 0.1e1;
  out[0] = f;
#if _KMAX >= 1
  const double df_dx2 = -_t1*_t12*_t18 + _t13 + _t17;
  out[1] = df_dx2;
#endif
#if _KMAX >= 2
  const double d2f_dx22 = _t17*_t22 - _t19*_t21 - 0.7e1 / 0.1944e4*_t20 + _t21*_t29*x2 + _t24*_t26;
  out[2] = d2f_dx22;
#endif
#if _KMAX >= 3
  const double d3f_dx23 = (0.7e1 / 0.648e3)*M_CBRTPI*_t27*_t28*_t4*_t7*_t8*_t9*params->a1*params->alpha + (0.7e1 / 0.324e3)*M_CBRTPI*_t27*_t28*_t6*_t7*_t8*_t9 + (0.7e1 / 0.15552e5)*M_CBRTPI*_t3*_t30*_t31*_t4*_t7*_t8*_t9*params->a1 - 0.7e1 / 0.324e3*_t11*_t32*x2 - _t17*_t30 - 0.7e1 / 0.1296e4*_t20*_t22 - 0.7e1 / 0.2592e4*_t26*_t33;
  out[3] = d3f_dx23;
#endif
#if _KMAX >= 4
  const double d4f_dx24 = (0.7e1 / 0.648e3)*_t14*_t26*_t29 + (0.7e1 / 0.324e3)*_t15*_t22*_t29 - 0.7e1 / 0.162e3*_t15*_t32 + (0.7e1 / 0.1944e4)*_t16*_t34 + (0.7e1 / 0.972e3)*_t20*_t30 + (0.7e1 / 0.31104e5)*_t23*_t34*((params->alpha) * (params->alpha) * (params->alpha) * (params->alpha)) - _t25*_t36 - 0.7e1 / 0.3888e4*_t30*_t31*_t33 - _t31*_t36 - _t32*_t35 + _t35*((params->b1) * (params->b1) * (params->b1) * (params->b1))*x2/xc_powi(_t2, 5);
  out[4] = d4f_dx24;
#endif
}
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(xchan_k, _KMAX)(const xc_func_type *p, double n_s, double sigma_ss, double *out) {
  ...
  if(!(_t4)) XC_CAT(dk87_f_pf_k, _KMAX)(p, _t1*sigma_ss, _hc0);
