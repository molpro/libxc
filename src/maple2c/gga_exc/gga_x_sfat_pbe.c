/*
  Generated from python/gga_exc/gga_x_sfat_pbe.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py gga_x_sfat_pbe
*/

#ifndef _GGA_X_SFAT_PBE_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _GGA_X_SFAT_PBE_KERNEL_BODY
#define _KMAX 0
#define _GGA_X_SFAT_PBE_HELPER_BODIES
#include "gga_x_sfat_pbe.c"
#undef _GGA_X_SFAT_PBE_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _GGA_X_SFAT_PBE_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_GGA_X_SFAT_PBE_HELPER_BODIES)
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(pbe_f0_k, _KMAX)(const xc_func_type *p, double s, double *out) {
  ...
  const double _t2 = 0.2195149727645171*_t1 + 0.80400000000000005;
...
#endif
#if _KMAX >= 3
  const double _t8 = 0.92981294177559784*_t5;
  const double _t9 = (0.1e1 / (_t2 * _t2 * _t2 * _t2));
#endif

  const double f = 0.17649003810267176*_t1*_t3 + 0.1e1;
  out[0] = f;
#if _KMAX >= 1
  const double df_ds = 0.35298007620534352*_t3*s - 0.077484411814633154*_t4*_t5;
  out[1] = df_ds;
#endif
#if _KMAX >= 2
  const double d2f_ds2 = -0.38742205907316576*_t1*_t5 + 0.35298007620534352*_t3 + 0.068035954196655295*_t6*_t7;
  out[2] = d2f_ds2;
#endif
#if _KMAX >= 3
  const double d3f_ds3 = 0.61232358776989765*_t4*_t7 - _t8*s - 0.089609463794920321*_t9*xc_powi(s, 5);
  out[3] = d3f_ds3;
#endif
#if _KMAX >= 4
  const double d4f_ds4 = 2.6534022136695565*_t1*_t7 - 1.2545324931288845*_t6*_t9 - _t8 + 0.15736495203507932*xc_powi(s, 6)/xc_powi(_t2, 5);
  out[4] = d4f_ds4;
#endif
}
