/*
  Generated from python/gga_exc/gga_x_ityh_pbe.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py gga_x_ityh_pbe
*/

#ifndef _GGA_X_ITYH_PBE_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _GGA_X_ITYH_PBE_KERNEL_BODY
#define _KMAX 0
#define _GGA_X_ITYH_PBE_HELPER_BODIES
#include "gga_x_ityh_pbe.c"
#undef _GGA_X_ITYH_PBE_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _GGA_X_ITYH_PBE_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_GGA_X_ITYH_PBE_HELPER_BODIES)
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(pbe_f0_k, _KMAX)(const xc_func_type *p, double s, double *out) {
  const gga_x_ityh_pbe_params *params = (const gga_x_ityh_pbe_params *)(p->params);

  const double _t1 = (s * s);
  const double _t2 = _t1*params->mu;
  const double _t3 = _t2 + params->kappa;
  const double _t4 = (0.1e1 / _t3);
  const double _t5 = _t4*params->kappa;
#if _KMAX >= 1
  const double _t6 = (s * s * s);
  const double _t7 = params->kappa*((params->mu) * (params->mu))/(_t3 * _t3);
#endif
#if _KMAX >= 2
  const double _t8 = params->kappa*(s * s * s * s);
  const double _t9 = (0.1e1 / (_t3 * _t3 * _t3));
  const double _t10 = ((params->mu) * (params->mu) * (params->mu));
  const double _t11 = _t10*_t9;
#endif
#if _KMAX >= 3
  const double _t12 = 0.24e2*_t7;
  const double _t13 = ((params->mu) * (params->mu) * (params->mu) * (params->mu))/(_t3 * _t3 * _t3 * _t3);
#endif

  const double f = _t2*_t5 + 0.1e1;
  out[0] = f;
#if _KMAX >= 1
  const double df_ds = 0.2e1*_t4*params->kappa*params->mu*s - 0.2e1*_t6*_t7;
  out[1] = df_ds;
#endif
#if _KMAX >= 2
  const double d2f_ds2 = -0.10e2*_t1*_t7 + 0.8e1*_t11*_t8 + 0.2e1*_t5*params->mu;
  out[2] = d2f_ds2;
#endif
#if _KMAX >= 3
  const double d3f_ds3 = 0.72e2*_t10*_t6*_t9*params->kappa - _t12*s - 0.48e2*_t13*params->kappa*xc_powi(s, 5);
  out[3] = d3f_ds3;
#endif
#if _KMAX >= 4
  const double d4f_ds4 = 0.312e3*_t1*_t11*params->kappa - _t12 - 0.672e3*_t13*_t8 + 0.384e3*params->kappa*xc_powi((params->mu), 5)*xc_powi(s, 6)/xc_powi(_t3, 5);
  out[4] = d4f_ds4;
#endif
}
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(ityh_enhancement_k, _KMAX)(const xc_func_type *p, double xs, double *out) {
  ...
  double _hc0[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(pbe_f_k, _KMAX)(p, xs, _hc0);
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(ityh_attenuation_k, _KMAX)(const xc_func_type *p, double a, double *out) {
  ...
  double _hc0[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(attenuation_erf_k, _KMAX)(p, a, _hc0);
