/*
  Generated from python/gga_exc/gga_x_bayesian.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py gga_x_bayesian
*/

#ifndef _GGA_X_BAYESIAN_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _GGA_X_BAYESIAN_KERNEL_BODY
#define _KMAX 0
#define _GGA_X_BAYESIAN_HELPER_BODIES
#include "gga_x_bayesian.c"
#undef _GGA_X_BAYESIAN_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _GGA_X_BAYESIAN_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_GGA_X_BAYESIAN_HELPER_BODIES)
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(bayesian_f0_k, _KMAX)(const xc_func_type *p, double s, double *out) {
...
#endif
#if _KMAX >= 2
  const double _t5 = (0.1e1 / (_t2 * _t2 * _t2 * _t2));
#endif
#if _KMAX >= 3
  const double _t6 = xc_powi(_t2, -5);
#endif

  const double f = _t1*_t3;
  out[0] = f;
#if _KMAX >= 1
  const double df_ds = -0.2e1*_t1*_t4 + 0.2e1*_t3*s;
  out[1] = df_ds;
#endif
#if _KMAX >= 2
  const double d2f_ds2 = 0.6e1*_t1*_t5 + 0.2e1*_t3 - 0.8e1*_t4*s;
  out[2] = d2f_ds2;
#endif
#if _KMAX >= 3
  const double d3f_ds3 = -0.24e2*_t1*_t6 - 0.12e2*_t4 + 0.36e2*_t5*s;
  out[3] = d3f_ds3;
#endif
#if _KMAX >= 4
  const double d4f_ds4 = 0.120e3*_t1/xc_powi(_t2, 6) + 0.72e2*_t5 - 0.192e3*_t6*s;
  out[4] = d4f_ds4;
#endif
}

GPU_DEVICE_FUNCTION static inline void XC_CAT(bayesian_f_k, _KMAX)(const xc_func_type *p, double x, double *out) {
...
#endif
  double _hc0[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(bayesian_f0_k, _KMAX)(p, _t3*x, _hc0);
  const double _t4 = 1.8962000000000001*_hc0[0] + 0.19259999999999999;
#if _KMAX >= 2
  const double _t6 = _hc0[2]*_t5;
#endif
#if _KMAX >= 3
  const double _t8 = _hc0[3]*_t7;
#endif
#if _KMAX >= 4
  const double _t10 = _hc0[4]*_t9;
#endif

  const double f = _hc0[0]*_t4 + 1.0007999999999999;
  out[0] = f;
#if _KMAX >= 1
  const double df_dx = 0.15801666666666667*_hc0[0]*_hc0[1]*_t2 + _hc0[1]*_t3*_t4;
  out[1] = df_dx;
#endif
#if _KMAX >= 2
  const double d2f_dx2 = 0.079008333333333319*_hc0[0]*_t6 + 0.15801666666666664*((_hc0[1]) * (_hc0[1]))*_t5 + (0.1e1 / 0.24e2)*_t4*_t6;
  out[2] = d2f_dx2;
#endif
#if _KMAX >= 3
  const double d3f_dx3 = 0.03950416666666666*_hc0[0]*_t8 + 0.23702499999999999*_hc0[1]*_hc0[2]*_t7 + (0.1e1 / 0.48e2)*_t4*_t8;
  out[3] = d3f_dx3;
#endif
#if _KMAX >= 4
  const double d4f_dx4 = 0.0032920138888888882*_hc0[0]*_t10 + 0.026336111111111109*_hc0[1]*_hc0[3]*_t9 + 0.01975208333333333*((_hc0[2]) * (_hc0[2]))*_t9 + (0.1e1 / 0.576e3)*_t10*_t4;
  out[4] = d4f_dx4;
#endif
}
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(xchan_k, _KMAX)(const xc_func_type *p, double n_s, double sigma_ss, double *out) {
  ...
  if(!(_t6)) XC_CAT(bayesian_f_k, _KMAX)(p, _t1*_t2, _hc0);
