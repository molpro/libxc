/*
  Generated from python/gga_exc/gga_x_am05.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py gga_x_am05
*/

#ifndef _GGA_X_AM05_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _GGA_X_AM05_KERNEL_BODY
#define _KMAX 0
#define _GGA_X_AM05_HELPER_BODIES
#include "gga_x_am05.c"
#undef _GGA_X_AM05_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _GGA_X_AM05_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_GGA_X_AM05_HELPER_BODIES)
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(am05_csi_k, _KMAX)(const xc_func_type *p, double s, double *out) {

  const double _t1 = xc_powr(s, 3, 2);
  const double _t2 = (0.1e1 / 0.12e2)*xc_powr(0.6e1, 1, 2)*_t1;
  const double _t3 = LambertW(_t2);
  const double _t4 = M_CBRT2*xc_powr(0.3e1, 2, 3);
#if _KMAX >= 1
  const double _t5 = xc_powr(s, 1, 2);
  const double _t6 = xc_lambertw_d1(_t2);
  const double _t7 = xc_powr(_t3, -1, 3);
  const double _t8 = xc_powr(0.2e1, 5, 6)*xc_powr(0.3e1, 1, 6);
  const double _t9 = _t7*_t8;
  const double _t10 = _t6*_t9;
#endif
#if _KMAX >= 2
  const double _t11 = xc_lambertw_d2(_t2);
...
#endif
#if _KMAX >= 3
  const double _t18 = (0.3e1 / 0.64e2)*_t7;
  const double _t19 = xc_lambertw_d3(_t2);
  const double _t20 = _t19*_t9;
  const double _t21 = _t1*_t8;
  const double _t22 = xc_powr(_t3, -7, 3);
  const double _t23 = _t22*(_t6 * _t6 * _t6);
  const double _t24 = _t11*_t15*_t6;
#endif
#if _KMAX >= 4
  const double _t25 = (0.1e1 / s);
  const double _t26 = (s * s);
  const double _t27 = _t26*_t4;
  const double _t28 = _t16*_t26;
  const double _t29 = _t5*_t8;
#endif

  const double f = (0.1e1 / 0.2e1)*xc_powr(_t3, 2, 3)*_t4;
  out[0] = f;
#if _KMAX >= 1
  const double df_ds = (0.1e1 / 0.8e1)*_t10*_t5;
  out[1] = df_ds;
#endif
#if _KMAX >= 2
  const double d2f_ds2 = (0.1e1 / 0.16e2)*_t10/_t5 + (0.1e1 / 0.32e2)*_t13*s - 0.1e1 / 0.96e2*_t17*s;
  out[2] = d2f_ds2;
#endif
#if _KMAX >= 3
  const double d3f_ds3 = (0.3e1 / 0.256e3)*_t1*_t20 + _t12*_t18 - 0.1e1 / 0.64e2*_t17 + (0.1e1 / 0.192e3)*_t21*_t23 - 0.3e1 / 0.256e3*_t21*_t24 - 0.1e1 / 0.32e2*_t10/_t1;
  out[3] = d3f_ds3;
#endif
#if _KMAX >= 4
  const double d4f_ds4 = -0.3e1 / 0.1024e4*(_t11 * _t11)*_t28 + (0.1e1 / 0.128e3)*_t12*_t14*_t22*_t26 - 0.1e1 / 0.128e3*_t13*_t25 + (0.1e1 / 0.384e3)*_t17*_t25 + _t18*_t6*_t8/xc_powr(s, 5, 2) - 0.1e1 / 0.256e3*_t19*_t28*_t6 + (0.9e1 / 0.256e3)*_t20*_t5 + (0.1e1 / 0.64e2)*_t23*_t29 - 0.9e1 / 0.256e3*_t24*_t29 + (0.3e1 / 0.1024e4)*_t27*_t7*xc_lambertw_d4(_t2) - 0.7e1 / 0.2304e4*_t27*(_t6 * _t6 * _t6 * _t6)/xc_powr(_t3, 10, 3);
  out[4] = d4f_ds4;
#endif
}

GPU_DEVICE_FUNCTION static inline void XC_CAT(am05_fb_k, _KMAX)(const xc_func_type *p, double s, double *out) {

  double _hc0[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(am05_csi_k, _KMAX)(p, s, _hc0);
  const double _t1 = (0.1e1 / (_hc0[0]));
  const double _t2 = ((_hc0[0]) * (_hc0[0]));
  const double _t3 = _t2 + 28.23705740248932;
  const double _t4 = xc_powr(_t3, -1, 4);
  const double _t5 = (0.1e1 / 0.3e1)*M_PI;
  const double _t6 = _t4*_t5;
#if _KMAX >= 1
  const double _t7 = xc_powr(_t3, -5, 4);
  const double _t8 = _hc0[1]*_t7;
  const double _t9 = (0.1e1 / 0.6e1)*s;
  const double _t10 = M_PI*_t9;
  const double _t11 = _hc0[1]*s;
  const double _t12 = (0.1e1 / _t2);
  const double _t13 = _t12*_t6;
#endif
#if _KMAX >= 2
  const double _t14 = M_PI*_hc0[2];
  const double _t15 = _t14*_t7;
  const double _t16 = _t12*_t4;
  const double _t17 = (0.2e1 / 0.3e1)*M_PI;
  const double _t18 = _t13*s;
  const double _t19 = ((_hc0[1]) * (_hc0[1]));
  const double _t20 = xc_powr(_t3, -9, 4);
  const double _t21 = ((_hc0[0]) * (_hc0[0]) * (_hc0[0]));
  const double _t22 = (0.1e1 / _t21);
#endif
#if _KMAX >= 3
  const double _t23 = _hc0[3]*_t7;
  const double _t24 = M_PI*((_hc0[1]) * (_hc0[1]) * (_hc0[1]));
  const double _t25 = _t4/((_hc0[0]) * (_hc0[0]) * (_hc0[0]) * (_hc0[0]));
  const double _t26 = _t24*_t25;
  const double _t27 = _t12*_t24;
  const double _t28 = xc_powr(_t3, -13, 4);
  const double _t29 = _t2*_t28;
  const double _t30 = _t24*_t29;
#endif
#if _KMAX >= 4
  const double _t31 = _hc0[0]*_t20;
  const double _t32 = _hc0[1]*_t14;
  const double _t33 = M_PI*_hc0[3];
  const double _t34 = _t11*_t33;
  const double _t35 = _t22*_t4;
  const double _t36 = M_PI*s;
  const double _t37 = ((_hc0[1]) * (_hc0[1]) * (_hc0[1]) * (_hc0[1]))*_t36;
  const double _t38 = ((_hc0[2]) * (_hc0[2]))*_t36;
  const double _t39 = 0.2e1*_t7;
  const double _t40 = _t19*s;
  const double _t41 = _t14*_t40;
#endif

  const double f = _t1*_t6*s;
  out[0] = f;
#if _KMAX >= 1
  const double df_ds = (0.1e1 / 0.3e1)*M_PI*_t1*_t4 - _t10*_t8 - _t11*_t13;
  out[1] = df_ds;
#endif
#if _KMAX >= 2
  const double d2f_ds2 = (0.5e1 / 0.12e2)*M_PI*_hc0[0]*_t19*_t20*s - _hc0[1]*_t16*_t17 - _hc0[2]*_t18 + (0.1e1 / 0.6e1)*M_PI*_t1*_t19*_t7*s - _t15*_t9 + (0.2e1 / 0.3e1)*M_PI*_t19*_t22*_t4*s - _t5*_t8;
  out[2] = d2f_ds2;
#endif
#if _KMAX >= 3
  const double d3f_ds3 = (0.5e1 / 0.4e1)*M_PI*_hc0[0]*_hc0[1]*_hc0[2]*_t20*s + (0.5e1 / 0.4e1)*M_PI*_hc0[0]*_t19*_t20 + (0.1e1 / 0.2e1)*M_PI*_hc0[1]*_hc0[2]*_t1*_t7*s + 0.2e1*M_PI*_hc0[1]*_hc0[2]*_t22*_t4*s - _hc0[3]*_t18 + (0.1e1 / 0.2e1)*M_PI*_t1*_t19*_t7 - _t10*_t23 - _t14*_t16 - 0.1e1 / 0.2e1*_t15 + 0.2e1*M_PI*_t19*_t22*_t4 - 0.2e1*_t26*s - 0.1e1 / 0.2e1*_t27*_t7*s - 0.15e2 / 0.8e1*_t30*s;
  out[3] = d3f_ds3;
#endif
#if _KMAX >= 4
  const double d4f_ds4 = -0.15e2 / 0.4e1*_hc0[0]*_t28*_t37 + _hc0[3]*_t1*_t17*_t8*s - _hc0[4]*_t10*_t7 - _hc0[4]*_t18 + 0.2e1*_t1*_t14*_t8 + (0.5e1 / 0.4e1)*_t1*_t20*_t37 + (0.1e1 / 0.2e1)*_t1*_t38*_t7 - 0.3e1*_t12*_t15*_t40 - 0.4e1 / 0.3e1*_t16*_t33 - _t17*_t23 + (0.195e3 / 0.16e2)*_t21*_t37/xc_powr(_t3, 17, 4) + _t22*_t37*_t39 - 0.12e2*_t25*_t41 - 0.8e1*_t26 - _t27*_t39 - 0.45e2 / 0.4e1*_t29*_t41 - 0.15e2 / 0.2e1*_t30 + 0.5e1*_t31*_t32 + (0.5e1 / 0.3e1)*_t31*_t34 + (0.5e1 / 0.4e1)*_t31*_t38 + 0.8e1*_t32*_t35 + (0.8e1 / 0.3e1)*_t34*_t35 + 0.2e1*_t35*_t38 + 0.8e1*_t37*_t4/xc_powi((_hc0[0]), 5);
  out[4] = d4f_ds4;
#endif
}

GPU_DEVICE_FUNCTION static inline void XC_CAT(am05_flaa_k, _KMAX)(const xc_func_type *p, double s, double *out) {
  const gga_x_am05_params *params = (const gga_x_am05_params *)(p->params);

  double _hc0[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(am05_fb_k, _KMAX)(p, s, _hc0);
  const double _t1 = (s * s);
  const double _t2 = _t1*params->c;
  const double _t3 = _t2 + 0.1e1;
  const double _t4 = (0.1e1 / (_hc0[0]));
  const double _t5 = _t2*_t4 + 0.1e1;
  const double _t6 = (0.1e1 / _t5);
#if _KMAX >= 1
  const double _t7 = 0.2e1*params->c;
  const double _t8 = _t6*_t7;
  const double _t9 = _t4*_t7;
  const double _t10 = (0.1e1 / ((_hc0[0]) * (_hc0[0])));
  const double _t11 = _hc0[1]*_t1*_t10*params->c - _t9*s;
  ...
  const double _t17 = _t4*params->c;
  const double _t18 = 0.4e1*_t17;
  const double _t19 = 0.2e1*_hc0[1]*_t1*_t10*params->c - _t18*s;
  ...
  const double _t25 = 0.4e1*_hc0[1]*_t10*params->c*s + _hc0[2]*_t1*_t10*params->c - 0.2e1*_t24 - _t9;
#endif
#if _KMAX >= 3
  const double _t26 = 0.6e1*params->c;
  const double _t27 = _t26*s;
  const double _t28 = _t15*_t20;
  const double _t29 = _t12*_t25;
  const double _t30 = 0.6e1*_t17;
  const double _t31 = 0.3e1*_hc0[1]*_t1*_t10*params->c - _t30*s;
  const double _t32 = (0.1e1 / (_t5 * _t5 * _t5 * _t5));
  const double _t33 = _t20*_t3*_t32;
  const double _t34 = 0.8e1*_hc0[1]*_t10*params->c*s + 0.2e1*_hc0[2]*_t1*_t10*params->c - _t18 - 0.4e1*_t24;
  const double _t35 = _t11*_t16;
  const double _t36 = _t16*_t19;
  const double _t37 = _hc0[1]*_t10;
  const double _t38 = _hc0[2]*_t10;
  const double _t39 = _t10*_t2;
  const double _t40 = _hc0[3]*_t39;
  const double _t41 = _t23*params->c;
  const double _t42 = 0.6e1*_t2;
  const double _t43 = _t21*_t42;
  const double _t44 = _hc0[1]*_hc0[2];
  const double _t45 = (0.1e1 / ((_hc0[0]) * (_hc0[0]) * (_hc0[0]) * (_hc0[0])));
  const double _t46 = ((_hc0[1]) * (_hc0[1]) * (_hc0[1]))*_t45;
  const double _t47 = _t26*_t37 + _t27*_t38 + _t40 - 0.12e2*_t41*s + _t42*_t46 - _t43*_t44;
#endif
#if _KMAX >= 4
  const double _t48 = 0.12e2*params->c;
  const double _t49 = 0.8e1*s;
  const double _t50 = _t49*params->c;
  const double _t51 = params->c*s;
  const double _t52 = _t11*_t34;
  const double _t53 = _t3*_t31;
  const double _t54 = 0.3e1*_t25;
  const double _t55 = _t32*_t53;
  const double _t56 = _t38*_t48;
  const double _t57 = 0.24e2*_t41;
  const double _t58 = 0.12e2*_t2;
  const double _t59 = _t21*_t44;
  const double _t60 = 0.48e2*_t51;
#endif

  const double f = _t3*_t6;
  out[0] = f;
#if _KMAX >= 1
  const double df_ds = _t11*_t13 + _t8*s;
  out[1] = df_ds;
#endif
#if _KMAX >= 2
  const double d2f_ds2 = _t13*_t25 + 0.4e1*_t14*params->c*s + _t16*_t20 + _t8;
  out[2] = d2f_ds2;
#endif
#if _KMAX >= 3
  const double d3f_ds3 = _t13*_t47 + _t14*_t26 + 0.2e1*_t25*_t36 + _t27*_t28 + _t27*_t29 + _t31*_t33 + _t34*_t35;
  out[3] = d3f_ds3;
#endif
#if _KMAX >= 4
  const double d4f_ds4 = _t12*_t47*_t50 + _t13*(-0.8e1*_hc0[1]*_hc0[3]*_t2*_t21 - ((_hc0[2]) * (_hc0[2]))*_t43 + 0.36e2*_hc0[2]*_t2*_t22*_t45 + _hc0[3]*_t10*_t50 + _hc0[4]*_t39 + _t46*_t60 + _t56 - _t57 - _t59*_t60 - 0.24e2*((_hc0[1]) * (_hc0[1]) * (_hc0[1]) * (_hc0[1]))*_t2/xc_powi((_hc0[0]), 5)) + 0.16e2*_t15*_t19*_t25*_t51 + _t15*_t50*_t52 + _t16*_t34*_t54 + _t19*_t54*_t55 + _t20*_t31*_t32*_t50 + _t20*_t53*(0.4e1*_hc0[1]*_t1*_t10*params->c - _t17*_t49)/xc_powi(_t5, 5) + _t28*_t48 + _t29*_t48 + _t33*(0.12e2*_hc0[1]*_t10*params->c*s + 0.3e1*_hc0[2]*_t1*_t10*params->c - 0.6e1*_t24 - _t30) + _t35*(_t37*_t48 + 0.2e1*_t40 + _t46*_t58 + _t56*s - _t57*s - _t58*_t59) + 0.3e1*_t36*_t47 + 0.2e1*_t52*_t55;
  out[4] = d4f_ds4;
#endif
}

GPU_DEVICE_FUNCTION static inline void XC_CAT(am05_XX_k, _KMAX)(const xc_func_type *p, double s, double *out) {
  const gga_x_am05_params *params = (const gga_x_am05_params *)(p->params);

  const double _t1 = (s * s);
  const double _t2 = _t1*params->alpha + 0.1e1;
#if _KMAX >= 1
  const double _t3 = 0.2e1*params->alpha/(_t2 * _t2);
#endif
#if _KMAX >= 2
  const double _t4 = (0.1e1 / (_t2 * _t2 * _t2));
  const double _t5 = ((params->alpha) * (params->alpha));
  const double _t6 = _t4*_t5;
#endif
#if _KMAX >= 3
  const double _t7 = ((params->alpha) * (params->alpha) * (params->alpha))/(_t2 * _t2 * _t2 * _t2);
#endif

  const double f = (0.1e1 / _t2);
  out[0] = f;
#if _KMAX >= 1
  const double df_ds = -_t3*s;
  out[1] = df_ds;
#endif
#if _KMAX >= 2
  const double d2f_ds2 = 0.8e1*_t1*_t6 - _t3;
  out[2] = d2f_ds2;
#endif
#if _KMAX >= 3
  const double d3f_ds3 = 0.24e2*_t4*_t5*s - 0.48e2*_t7*(s * s * s);
  out[3] = d3f_ds3;
#endif
#if _KMAX >= 4
  const double d4f_ds4 = -0.288e3*_t1*_t7 + 0.24e2*_t6 + 0.384e3*((params->alpha) * (params->alpha) * (params->alpha) * (params->alpha))*(s * s * s * s)/xc_powi(_t2, 5);
  out[4] = d4f_ds4;
#endif
}

GPU_DEVICE_FUNCTION static inline void XC_CAT(am05_one_minus_XX_k, _KMAX)(const xc_func_type *p, double s, double *out) {
  const gga_x_am05_params *params = (const gga_x_am05_params *)(p->params);
  ...
  const double _t6 = ((params->alpha) * (params->alpha))/(_t3 * _t3);
#endif
#if _KMAX >= 2
  const double _t7 = (s * s * s * s);
  const double _t8 = (0.1e1 / (_t3 * _t3 * _t3));
  const double _t9 = ((params->alpha) * (params->alpha) * (params->alpha));
  const double _t10 = _t8*_t9;
#endif
#if _KMAX >= 3
  const double _t11 = 0.24e2*_t6;
  const double _t12 = ((params->alpha) * (params->alpha) * (params->alpha) * (params->alpha))/(_t3 * _t3 * _t3 * _t3);
#endif

  const double f = _t2*_t4;
  out[0] = f;
#if _KMAX >= 1
  const double df_ds = 0.2e1*_t4*params->alpha*s - 0.2e1*_t5*_t6;
  out[1] = df_ds;
#endif
#if _KMAX >= 2
  const double d2f_ds2 = -0.10e2*_t1*_t6 + 0.8e1*_t10*_t7 + 0.2e1*_t4*params->alpha;
  out[2] = d2f_ds2;
#endif
#if _KMAX >= 3
  const double d3f_ds3 = -_t11*s - 0.48e2*_t12*xc_powi(s, 5) + 0.72e2*_t5*_t8*_t9;
  out[3] = d3f_ds3;
#endif
#if _KMAX >= 4
  const double d4f_ds4 = 0.312e3*_t1*_t10 - _t11 - 0.672e3*_t12*_t7 + 0.384e3*xc_powi((params->alpha), 5)*xc_powi(s, 6)/xc_powi(_t3, 5);
  out[4] = d4f_ds4;
#endif
}

GPU_DEVICE_FUNCTION static inline void XC_CAT(am05_f_k, _KMAX)(const xc_func_type *p, double x, double *out) {

  const double _t1 = xc_powr(0.6e1, 2, 3);
  const double _t2 = (0.1e1 / 0.12e2)*_t1/(M_CBRTPI * M_CBRTPI);
  const double _t3 = _t2*x;
#if _KMAX >= 2
  const double _t4 = xc_powr(0.6e1, 1, 3)/(M_PI * M_CBRTPI);
  const double _t5 = (0.1e1 / 0.24e2)*_t4;
#endif
#if _KMAX >= 3
  const double _t6 = (0.1e1 / (M_PI * M_PI));
  const double _t7 = (0.1e1 / 0.48e2)*_t6;
  const double _t8 = (0.1e1 / 0.16e2)*_t6;
#endif
#if _KMAX >= 4
  const double _t9 = _t1/(M_PI * M_PI * M_CBRTPI * M_CBRTPI);
  const double _t10 = (0.1e1 / 0.576e3)*_t9;
  const double _t11 = (0.1e1 / 0.144e3)*_t9;
#endif
  double _hc0[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(am05_flaa_k, _KMAX)(p, _t3, _hc0);
  double _hc1[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(am05_one_minus_XX_k, _KMAX)(p, _t3, _hc1);
  double _hc2[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(am05_XX_k, _KMAX)(p, _t3, _hc2);

  const double f = _hc0[0]*_hc1[0] + _hc2[0];
  out[0] = f;
#if _KMAX >= 1
  const double df_dx = _hc0[0]*_hc1[1]*_t2 + _hc0[1]*_hc1[0]*_t2 + _hc2[1]*_t2;
  out[1] = df_dx;
#endif
#if _KMAX >= 2
  const double d2f_dx2 = _hc0[0]*_hc1[2]*_t5 + (0.1e1 / 0.12e2)*_hc0[1]*_hc1[1]*_t4 + _hc0[2]*_hc1[0]*_t5 + _hc2[2]*_t5;
  out[2] = d2f_dx2;
#endif
#if _KMAX >= 3
  const double d3f_dx3 = _hc0[0]*_hc1[3]*_t7 + _hc0[1]*_hc1[2]*_t8 + _hc0[2]*_hc1[1]*_t8 + _hc0[3]*_hc1[0]*_t7 + _hc2[3]*_t7;
  out[3] = d3f_dx3;
#endif
#if _KMAX >= 4
  const double d4f_dx4 = _hc0[0]*_hc1[4]*_t10 + _hc0[1]*_hc1[3]*_t11 + (0.1e1 / 0.96e2)*_hc0[2]*_hc1[2]*_t9 + _hc0[3]*_hc1[1]*_t11 + _hc0[4]*_hc1[0]*_t10 + _hc2[4]*_t10;
  out[4] = d4f_dx4;
#endif
}
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(xchan_k, _KMAX)(const xc_func_type *p, double n_s, double sigma_ss, double *out) {
  ...
  if(!(_t6)) XC_CAT(am05_f_k, _KMAX)(p, _t1*_t2, _hc0);
