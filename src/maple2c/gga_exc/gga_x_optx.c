/*
  Generated from python/gga_exc/gga_x_optx.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py gga_x_optx
*/

#ifndef _GGA_X_OPTX_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _GGA_X_OPTX_KERNEL_BODY
#define _KMAX 0
#define _GGA_X_OPTX_HELPER_BODIES
#include "gga_x_optx.c"
#undef _GGA_X_OPTX_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _GGA_X_OPTX_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_GGA_X_OPTX_HELPER_BODIES)
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(optx_f_pf_k, _KMAX)(const xc_func_type *p, double x2, double *out) {
  const gga_x_optx_params *params = (const gga_x_optx_params *)(p->params);

  const double _t1 = params->b*(x2 * x2);
  const double _t2 = params->gamma*x2 + 0.1e1;
  const double _t3 = (0.1e1 / (_t2 * _t2));
  const double _t4 = ((params->gamma) * (params->gamma));
  const double _t5 = _t3*_t4;
#if _KMAX >= 1
  const double _t6 = ((params->gamma) * (params->gamma) * (params->gamma))/(_t2 * _t2 * _t2);
#endif
#if _KMAX >= 2
  const double _t7 = _t6*params->b;
  const double _t8 = (0.1e1 / (_t2 * _t2 * _t2 * _t2));
  const double _t9 = ((params->gamma) * (params->gamma) * (params->gamma) * (params->gamma));
  const double _t10 = _t8*_t9;
#endif
#if _KMAX >= 3
  const double _t11 = xc_powi((params->gamma), 5)/xc_powi(_t2, 5);
#endif

  const double f = _t1*_t5 + params->a;
  out[0] = f;
#if _KMAX >= 1
  const double df_dx2 = -0.2e1*_t1*_t6 + 0.2e1*_t3*_t4*params->b*x2;
  out[1] = df_dx2;
#endif
#if _KMAX >= 2
  const double d2f_dx22 = 0.6e1*_t1*_t10 + 0.2e1*_t5*params->b - 0.8e1*_t7*x2;
  out[2] = d2f_dx22;
#endif
#if _KMAX >= 3
  const double d3f_dx23 = -0.24e2*_t1*_t11 - 0.12e2*_t7 + 0.36e2*_t8*_t9*params->b*x2;
  out[3] = d3f_dx23;
#endif
#if _KMAX >= 4
  const double d4f_dx24 = 0.120e3*_t1*xc_powi((params->gamma), 6)/xc_powi(_t2, 6) + 0.72e2*_t10*params->b - 0.192e3*_t11*params->b*x2;
  out[4] = d4f_dx24;
#endif
}
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(xchan_k, _KMAX)(const xc_func_type *p, double n_s, double sigma_ss, double *out) {
  ...
  XC_CAT(optx_f_pf_k, _KMAX)(p, _t3*sigma_ss, _hc1);
