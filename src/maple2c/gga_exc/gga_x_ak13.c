/*
  Generated from python/gga_exc/gga_x_ak13.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py gga_x_ak13
*/

#ifndef _GGA_X_AK13_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _GGA_X_AK13_KERNEL_BODY
#define _KMAX 0
#define _GGA_X_AK13_HELPER_BODIES
#include "gga_x_ak13.c"
#undef _GGA_X_AK13_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _GGA_X_AK13_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_GGA_X_AK13_HELPER_BODIES)
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(ak13_f0_k, _KMAX)(const xc_func_type *p, double s, double *out) {
  const gga_x_ak13_params *params = (const gga_x_ak13_params *)(p->params);

  const double _t1 = xc_log1p(s);
  const double _t2 = _t1*params->B1;
  const double _t3 = params->B2*xc_log1p(_t1);
#if _KMAX >= 1
  const double _t4 = s + 0.1e1;
  const double _t5 = (0.1e1 / _t4);
  const double _t6 = params->B2*s;
  const double _t7 = _t1 + 0.1e1;
  const double _t8 = (0.1e1 / _t7);
#endif
#if _KMAX >= 2
  const double _t9 = (0.1e1 / (_t4 * _t4));
  const double _t10 = _t9*params->B1;
  const double _t11 = (0.1e1 / (_t7 * _t7));
  const double _t12 = _t6*_t9;
#endif
#if _KMAX >= 3
  const double _t13 = (0.1e1 / (_t4 * _t4 * _t4));
  const double _t14 = 0.3e1*_t9;
  const double _t15 = (0.1e1 / (_t7 * _t7 * _t7));
  const double _t16 = _t13*_t6;
  ...
  const double _t21 = _t18*_t6;
#endif

  const double f = _t2*s + _t3*s + 0.1e1;
  out[0] = f;
#if _KMAX >= 1
  const double df_ds = _t2 + _t3 + _t5*_t6*_t8 + _t5*params->B1*s;
  out[1] = df_ds;
#endif
#if _KMAX >= 2
  const double d2f_ds2 = -_t10*s - _t11*_t12 - _t12*_t8 + 0.2e1*_t5*_t8*params->B2 + 0.2e1*_t5*params->B1;
  out[2] = d2f_ds2;
#endif
#if _KMAX >= 3
  const double d3f_ds3 = -0.3e1*_t10 - _t11*_t14*params->B2 + 0.3e1*_t11*_t16 + 0.2e1*_t13*params->B1*s - _t14*_t8*params->B2 + _t15*_t17 + _t17*_t8;
  out[3] = d3f_ds3;
#endif
#if _KMAX >= 4
  const double d4f_ds4 = 0.12e2*_t11*_t13*params->B2 - 0.11e2*_t11*_t21 + 0.8e1*_t13*_t15*params->B2 + 0.8e1*_t13*_t8*params->B2 + 0.8e1*_t13*params->B1 - 0.12e2*_t15*_t21 - _t19*params->B1*s - _t20*_t8 - _t20/(_t7 * _t7 * _t7 * _t7);
  out[4] = d4f_ds4;
#endif
}

GPU_DEVICE_FUNCTION static inline void XC_CAT(ak13_f_k, _KMAX)(const xc_func_type *p, double x, double *out) {

  const double _t1 = xc_powr(0.6e1, 2, 3);
  const double _t2 = (0.1e1 / 0.12e2)*_t1/(M_CBRTPI * M_CBRTPI);
  double _hc0[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(ak13_f0_k, _KMAX)(p, _t2*x, _hc0);
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(xchan_k, _KMAX)(const xc_func_type *p, double n_s, double sigma_ss, double *out) {
  ...
  if(!(_t6)) XC_CAT(ak13_f_k, _KMAX)(p, _t1*_t2, _hc0);
