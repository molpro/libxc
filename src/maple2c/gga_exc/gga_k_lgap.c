/*
  Generated from python/gga_exc/gga_k_lgap.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py gga_k_lgap
*/

#ifndef _GGA_K_LGAP_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _GGA_K_LGAP_KERNEL_BODY
#define _KMAX 0
#define _GGA_K_LGAP_HELPER_BODIES
#include "gga_k_lgap.c"
#undef _GGA_K_LGAP_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _GGA_K_LGAP_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_GGA_K_LGAP_HELPER_BODIES)

/* ---- helpers (one graded body -> <name>_k<order>) ---- */

GPU_DEVICE_FUNCTION static inline void XC_CAT(lgap_f0_k, _KMAX)(const xc_func_type *p, double s, double *out) {
  const gga_k_lgap_params *params = (const gga_k_lgap_params *)(p->params);

  const double _t1 = (s * s);
  const double _t2 = xc_expm1(-_t1*params->mu[1] - params->mu[0]*s - params->mu[2]*(s * s * s));
#if _KMAX >= 1
  const double _t3 = 0.2e1*params->mu[1];
  const double _t4 = -0.3e1*_t1*params->mu[2] - _t3*s - params->mu[0];
  const double _t5 = _t2 + 0.1e1;
  const double _t6 = _t5*params->kappa;
  const double _t7 = _t4*_t6;
#endif
#if _KMAX >= 2
  const double _t8 = params->mu[2]*s;
  const double _t9 = -_t3 - 0.6e1*_t8;
  const double _t10 = _t6*_t9;
  const double _t11 = (_t4 * _t4)*_t6;
#endif
#if _KMAX >= 3
  const double _t12 = -0.12e2*_t8 - 0.4e1*params->mu[1];
#endif

  const double f = -_t2*params->kappa + 0.1e1;
  out[0] = f;
#if _KMAX >= 1
  const double df_ds = -_t7;
  out[1] = df_ds;
#endif
#if _KMAX >= 2
  const double d2f_ds2 = -_t10 - _t11;
  out[2] = d2f_ds2;
#endif
#if _KMAX >= 3
  const double d3f_ds3 = -_t12*_t7 - (_t4 * _t4 * _t4)*_t6 + 0.6e1*_t5*params->kappa*params->mu[2] - _t7*_t9;
  out[3] = d3f_ds3;
#endif
#if _KMAX >= 4
  const double d4f_ds4 = -_t10*_t12 - _t11*_t12 - _t11*_t9 - _t11*(-0.18e2*_t8 - 0.6e1*params->mu[1]) - (_t4 * _t4 * _t4 * _t4)*_t6 + 0.24e2*_t4*_t5*params->kappa*params->mu[2] - _t6*(_t9 * _t9);
  out[4] = d4f_ds4;
#endif
}

GPU_DEVICE_FUNCTION static inline void XC_CAT(lgap_f_k, _KMAX)(const xc_func_type *p, double x, double *out) {

  const double _t1 = xc_powr(0.6e1, 2, 3);
  const double _t2 = (0.1e1 / 0.12e2)*_t1/(M_CBRTPI * M_CBRTPI);
  double _hc0[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(lgap_f0_k, _KMAX)(p, _t2*x, _hc0);
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(xchan_k, _KMAX)(const xc_func_type *p, double n_s, double sigma_ss, double *out) {
  ...
  if(!(_t3)) XC_CAT(lgap_f_k, _KMAX)(p, _t1*_t2, _hc1);
