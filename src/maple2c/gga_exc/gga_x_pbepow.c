/*
  Generated from python/gga_exc/gga_x_pbepow.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py gga_x_pbepow
*/

#ifndef _GGA_X_PBEPOW_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _GGA_X_PBEPOW_KERNEL_BODY
#define _KMAX 0
#define _GGA_X_PBEPOW_HELPER_BODIES
#include "gga_x_pbepow.c"
#undef _GGA_X_PBEPOW_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _GGA_X_PBEPOW_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_GGA_X_PBEPOW_HELPER_BODIES)
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(pbepow_f0_small_k, _KMAX)(const xc_func_type *p, double s, double *out) {

  const double _t1 = fabs(s);
  const double _t2 = pow(_t1, 200.0);
  const double _t3 = (s * s);
  const double _t4 = 21.95149727645171*_t3 + 0.80400000000000005;
  const double _t5 = pow(_t4, -100.0);
#if _KMAX >= 1
  const double _t6 = pow(_t4, -101.0);
  const double _t7 = 6.1504792222857796e+137*_t2*_t6;
  const double _t8 = (((s) > 0) - ((s) < 0));
  const double _t9 = pow(_t1, 199.0);
#endif
#if _KMAX >= 2
  const double _t10 = pow(_t4, -102.0);
  const double _t11 = _t10*_t2;
  const double _t12 = _t8*_t9;
  const double _t13 = _t12*_t6;
  const double _t14 = pow(_t1, 198.0)*(_t8 * _t8);
#endif
#if _KMAX >= 3
  const double _t15 = (s * s * s);
  const double _t16 = pow(_t4, -103.0);
  const double _t17 = _t16*_t2;
  const double _t18 = pow(_t1, 197.0);
  const double _t19 = (_t8 * _t8 * _t8);
  const double _t20 = _t14*_t6;
#endif

  const double f = 1.4009247626319449e+134*_t2*_t5 - 0.1e1;
  out[0] = f;
#if _KMAX >= 1
  const double df_ds = 2.8018495252638898e+136*_t5*_t8*_t9 - _t7*s;
  out[1] = df_ds;
#endif
#if _KMAX >= 2
  const double d2f_ds2 = 2.7272470035169583e+141*_t11*_t3 - 2.4601916889143118e+140*_t13*s + 5.5756805552751408e+138*_t14*_t5 - _t7;
  out[2] = d2f_ds2;
#endif
#if _KMAX >= 3
  const double d3f_ds3 = 8.1817410105508751e+141*_t10*_t2*s + 1.636348202110175e+144*_t10*_t3*_t8*_t9 - 3.6902875333714677e+140*_t13 - 1.2212899654662374e+145*_t15*_t17 + 1.1039847499444779e+141*_t18*_t19*_t5 - 7.3436721914092208e+142*_t20*s;
  out[3] = d3f_ds3;
#endif
#if _KMAX >= 4
  const double d4f_ds4 = 2.1748499573906214e+143*pow(_t1, 196.0)*_t5*(_t8 * _t8 * _t8 * _t8) + 6.5453928084407001e+144*_t10*_t12*s + 6.5126658443984965e+146*_t10*_t14*_t3 + 8.1817410105508751e+141*_t11 - 9.7703197237298993e+147*_t12*_t15*_t16 - 7.3277397927974245e+145*_t17*_t3 - 1.9387294585320343e+145*_t18*_t19*_t6*s + 5.5226835302421222e+148*_t2*pow(_t4, -104.0)*(s * s * s * s) - 1.4687344382818442e+143*_t20;
  out[4] = d4f_ds4;
#endif
}

GPU_DEVICE_FUNCTION static inline void XC_CAT(pbepow_f0_large_k, _KMAX)(const xc_func_type *p, double s, double *out) {

  const double _t1 = 0.036626203209495167/(s * s);
  const double _t2 = xc_expm1(-100.0*xc_log1p(_t1));
#if _KMAX >= 1
...
#endif
#if _KMAX >= 2
  const double _t6 = xc_powi(s, -6);
  const double _t7 = (0.1e1 / (_t3 * _t3));
#endif
#if _KMAX >= 3
  const double _t8 = _t4/(_t3 * _t3 * _t3);
#endif

  const double f = _t2;
  out[0] = f;
#if _KMAX >= 1
  const double df_ds = 7.3252406418990335*_t5/(s * s * s);
  out[1] = df_ds;
#endif
#if _KMAX >= 2
  const double d2f_ds2 = 54.195741966346657*_t4*_t6*_t7 - 21.9757219256971*_t5/(s * s * s * s);
  out[2] = d2f_ds2;
#endif
#if _KMAX >= 3
  const double d3f_ds3 = -487.76167769711991*_t4*_t7/xc_powi(s, 7) + 87.902887702788401*_t5/xc_powi(s, 5) + 404.93678870315068*_t8/xc_powi(s, 9);
  out[3] = d3f_ds3;
#endif
#if _KMAX >= 4
  const double d4f_ds4 = 4064.6806474759993*_t4*_t7/xc_powi(s, 8) - 439.51443851394201*_t5*_t6 - 7288.8621966567123*_t8/xc_powi(s, 10) + 3055.2472046686528*_t4/((_t3 * _t3 * _t3 * _t3)*xc_powi(s, 12));
  out[4] = d4f_ds4;
#endif
}

GPU_DEVICE_FUNCTION static inline void XC_CAT(pbepow_f0_k, _KMAX)(const xc_func_type *p, double s, double *out) {

  const double _t1 = s > 0.1e1;
  const double _t2 = s <= 0.1e1;
  const double _t4 = 0.2195149727645171*(s * s);
#if _KMAX >= 1
  const double _t6 = my_piecewise3(_t1, 0, 0.1e1);
  const double _t7 = my_piecewise3(_t1, 0.1e1, 0);
#endif
  double _hc0[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1] = {0.};
  if(_t2) XC_CAT(pbepow_f0_small_k, _KMAX)(p, my_piecewise3(_t1, 0.1e1, s), _hc0);
  double _hc1[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(pbepow_f0_large_k, _KMAX)(p, my_piecewise3(_t1, s, 0.1e1), _hc1);
  const double _t3 = my_piecewise3(_t2, _hc0[0], _hc1[0]);
#if _KMAX >= 1
  const double _t5 = 0.4390299455290342*_t3;
  const double _t8 = my_piecewise3(_t2, _hc0[1]*_t6, _hc1[1]*_t7);
#endif
#if _KMAX >= 2
  const double _t9 = my_piecewise3(_t2, _hc0[2]*(_t6 * _t6), _hc1[2]*(_t7 * _t7));
#endif
#if _KMAX >= 3
  const double _t10 = my_piecewise3(_t2, _hc0[3]*(_t6 * _t6 * _t6), _hc1[3]*(_t7 * _t7 * _t7));
#endif

  const double f = -_t3*_t4 + 0.1e1;
  out[0] = f;
#if _KMAX >= 1
  const double df_ds = -_t4*_t8 - _t5*s;
  out[1] = df_ds;
#endif
#if _KMAX >= 2
  const double d2f_ds2 = -_t4*_t9 - _t5 - 0.87805989105806841*_t8*s;
  out[2] = d2f_ds2;
#endif
#if _KMAX >= 3
  const double d3f_ds3 = -_t10*_t4 - 1.3170898365871026*_t8 - 1.3170898365871026*_t9*s;
  out[3] = d3f_ds3;
#endif
#if _KMAX >= 4
  const double d4f_ds4 = -1.7561197821161368*_t10*s - _t4*my_piecewise3(_t2, _hc0[4]*(_t6 * _t6 * _t6 * _t6), _hc1[4]*(_t7 * _t7 * _t7 * _t7)) - 2.6341796731742052*_t9;
  out[4] = d4f_ds4;
#endif
}

GPU_DEVICE_FUNCTION static inline void XC_CAT(pbepow_f_k, _KMAX)(const xc_func_type *p, double x, double *out) {

  const double _t1 = xc_powr(0.6e1, 2, 3);
  const double _t2 = (0.1e1 / 0.12e2)*_t1/(M_CBRTPI * M_CBRTPI);
  double _hc0[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(pbepow_f0_k, _KMAX)(p, _t2*x, _hc0);
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(xchan_k, _KMAX)(const xc_func_type *p, double n_s, double sigma_ss, double *out) {
  ...
  if(!(_t6)) XC_CAT(pbepow_f_k, _KMAX)(p, _t4*_t5, _hc1);
