/*
  Generated from python/gga_exc/gga_c_pbe.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py gga_c_pbe
*/

#ifndef _GGA_C_PBE_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _GGA_C_PBE_KERNEL_BODY
#define _KMAX 0
#define _GGA_C_PBE_HELPER_BODIES
#include "gga_c_pbe.c"
#undef _GGA_C_PBE_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _GGA_C_PBE_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_GGA_C_PBE_HELPER_BODIES)
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(mbeta_k, _KMAX)(const xc_func_type *p, double rs, double t, double *out) {
  const gga_c_pbe_params *params = (const gga_c_pbe_params *)(p->params);
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(tp_k, _KMAX)(const xc_func_type *p, double rs, double z, double xt, double *out) {
  ...
  const double _t2 = xc_powr(0.2e1, 2, 3)*params->tscale;
  ...
  const double _t17 = _t2/(_t1 * _t1 * _t1);
  ...
  const double _t34 = _t2*_t32*_t33;
  ...
  out[23] = d4f_dz3_dxt;
  const double d4f_dz4 = (0.3e1 / 0.8e1)*_t27*_t44 + (0.1e1 / 0.4e1)*_t29*_t34*_t4*_t50 + _t30*xt*(-_hc0[9] + _hc1[9]) + _t32*_t49*(-0.3e1 / 0.2e1*_hc0[5] - 0.3e1 / 0.2e1*_hc1[5]) + _t45*_t47 + _t47*_t48 + _t9*xt*(-0.1e1 / 0.2e1*_hc0[14] - 0.1e1 / 0.2e1*_hc1[14]) + _t33*_t49*(-0.2e1*_hc0[2] + 0.2e1*_hc1[2])/xc_powi(_t1, 5);
  out[24] = d4f_dz4;
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(A_k, _KMAX)(const xc_func_type *p, double rs, double z, double t, double *out) {
  ...
  const double _t3 = (0.1e1 / (params->gamma));
  const double _t4 = _t2*_t3;
  const double _t5 = xc_expm1(-_hc2[0]*_t4);
  const double _t6 = _hc3[0]*_t3;
#if _KMAX >= 1
  const double _t7 = _t5 + 0.1e1;
  const double _t8 = _t7/(_t5 * _t5);
  const double _t9 = _hc2[1]*_t4;
  const double _t10 = -0.3e1 / 0.2e1*_hc0[2] + (0.3e1 / 0.2e1)*_hc1[2];
  const double _t11 = (0.1e1 / (_t1 * _t1 * _t1 * _t1));
  const double _t12 = _t11*_t3;
  const double _t13 = _hc2[0]*_t12;
  const double _t14 = _t10*_t13;
  const double _t15 = -_t14 - _t9;
  const double _t16 = _t15*_t6*_t8;
  const double _t17 = (0.1e1 / ((params->gamma) * (params->gamma)));
  const double _t18 = _hc3[0]*_t8;
  const double _t19 = _t17*_t18;
  const double _t20 = _t19*_t2;
  const double _t21 = _hc2[2]*_t20;
#endif
#if _KMAX >= 2
  const double _t22 = (_t15 * _t15);
  const double _t23 = _t6*_t8;
  const double _t24 = _t22*_t23;
  const double _t25 = (0.1e1 / (_t5 * _t5 * _t5));
  const double _t26 = (_t7 * _t7);
  const double _t27 = _hc2[3]*_t4;
  const double _t28 = -0.3e1 / 0.2e1*_hc0[5] - 0.3e1 / 0.2e1*_hc1[5];
  const double _t29 = _t13*_t28;
  const double _t30 = _hc2[1]*_t12;
  const double _t31 = 0.2e1*_t10;
  const double _t32 = xc_powi(_t1, -5);
  const double _t33 = _t10*_t32;
  const double _t34 = -0.2e1*_hc0[2] + 0.2e1*_hc1[2];
  const double _t35 = _hc2[0]*_t3;
  const double _t36 = _t34*_t35;
  const double _t37 = _t33*_t36;
  const double _t38 = -_t27 - _t29 - _t30*_t31 - _t37;
  const double _t39 = _t23*_t38;
  const double _t40 = _hc2[4]*_t20;
  const double _t41 = _t11*_t19;
  const double _t42 = _hc2[2]*_t41;
  const double _t43 = _t15*_t21;
  const double _t44 = _hc2[2]*_t15;
  const double _t45 = _t25*_t26;
  const double _t46 = 0.2e1*_t45;
  const double _t47 = _hc3[0]*_t46;
  const double _t48 = _t17*_t2;
  const double _t49 = _t47*_t48;
  const double _t50 = _t44*_t49;
  const double _t51 = _hc2[5]*_t20;
  const double _t52 = xc_powi(_t1, -6);
  const double _t53 = (0.1e1 / ((params->gamma) * (params->gamma) * (params->gamma)));
  const double _t54 = _t52*_t53;
  const double _t55 = ((_hc2[2]) * (_hc2[2]));
  const double _t56 = _t18*_t55;
  const double _t57 = _t54*_t56;
  const double _t58 = _hc3[0]*_t54;
  const double _t59 = _t55*_t58;
  const double _t60 = _t46*_t59;
#endif
#if _KMAX >= 3
  const double _t61 = (_t15 * _t15 * _t15);
  const double _t62 = (_t7 * _t7 * _t7)/(_t5 * _t5 * _t5 * _t5);
  const double _t63 = 0.6e1*_t62;
  const double _t64 = _t6*_t61*_t63;
  const double _t65 = -0.2e1*_t14 - 0.2e1*_t9;
  const double _t66 = 0.4e1*_t30;
  const double _t67 = _t32*_t36;
  const double _t68 = -_t10*_t66 - 0.2e1*_t27 - 0.2e1*_t29 - _t31*_t67;
  const double _t69 = _hc2[6]*_t4;
  const double _t70 = -0.3e1 / 0.2e1*_hc0[9] + (0.3e1 / 0.2e1)*_hc1[9];
  const double _t71 = _t13*_t70;
  const double _t72 = _t28*_t30;
  const double _t73 = _hc2[3]*_t12;
  const double _t74 = 0.3e1*_t10;
  const double _t75 = -0.2e1*_hc0[5] - 0.2e1*_hc1[5];
  const double _t76 = _t33*_t35;
  const double _t77 = _t28*_t67;
  const double _t78 = _hc2[1]*_t3;
  const double _t79 = _t33*_t34;
  const double _t80 = 0.3e1*_t79;
  const double _t81 = -0.5e1 / 0.2e1*_hc0[2] + (0.5e1 / 0.2e1)*_hc1[2];
  const double _t82 = _t52*_t81;
  const double _t83 = _t10*_t36;
  const double _t84 = -_t69 - _t71 - 0.3e1*_t72 - _t73*_t74 - _t75*_t76 - 0.2e1*_t77 - _t78*_t80 - _t82*_t83;
  const double _t85 = _hc2[7]*_t20;
  const double _t86 = _t28*_t42;
  const double _t87 = _hc2[4]*_t41;
  const double _t88 = _hc2[2]*_t19;
  const double _t89 = 0.2e1*_t15;
  const double _t90 = _hc2[4]*_t15;
  const double _t91 = 0.4e1*_t45;
  const double _t92 = _hc3[0]*_t48;
  const double _t93 = _t91*_t92;
  const double _t94 = _hc2[2]*_t49;
  const double _t95 = _t22*_t94;
  const double _t96 = _hc2[2]*_t92;
  const double _t97 = _t22*_t63;
  const double _t98 = _t96*_t97;
  const double _t99 = _t15*_t31;
  const double _t100 = 0.4e1*_t10;
  const double _t101 = _hc3[0]*_t45;
  const double _t102 = _t11*_t17;
  const double _t103 = _t101*_t102;
  const double _t104 = _t100*_t103;
  const double _t105 = _hc2[8]*_t20;
  const double _t106 = _t18*_t54;
  const double _t107 = 0.2e1*_t106;
  const double _t108 = _hc2[2]*_hc2[4];
  const double _t109 = _t58*_t91;
  const double _t110 = _hc2[5]*_t41;
  const double _t111 = xc_powi(_t1, -7);
  const double _t112 = -0.3e1*_hc0[2] + 0.3e1*_hc1[2];
  const double _t113 = _t111*_t112;
  const double _t114 = _t53*_t56;
  const double _t115 = _t113*_t114;
  const double _t116 = _t113*_t53;
  const double _t117 = _t47*_t55;
  const double _t118 = _hc2[5]*_t49;
  const double _t119 = _t118*_t15;
  const double _t120 = _t15*_t60;
  const double _t121 = _t59*_t63;
  const double _t122 = _t121*_t15;
  const double _t123 = _hc2[9]*_t20;
  const double _t124 = ((_hc2[2]) * (_hc2[2]) * (_hc2[2]));
  const double _t125 = (0.1e1 / ((params->gamma) * (params->gamma) * (params->gamma) * (params->gamma)));
  const double _t126 = _t125/xc_powi(_t1, 9);
  const double _t127 = _t124*_t126;
  const double _t128 = _t127*_t18;
  const double _t129 = 0.3e1*_t106;
  const double _t130 = _hc2[2]*_hc2[5];
  const double _t131 = 0.6e1*_t45;
  const double _t132 = _t131*_t58;
  const double _t133 = _t130*_t132;
  const double _t134 = _hc3[0]*_t127;
  const double _t135 = _t131*_t134;
  const double _t136 = _t134*_t63;
#endif
#if _KMAX >= 4
  const double _t137 = (_t15 * _t15 * _t15 * _t15);
  const double _t138 = xc_powi(_t5, -5);
  const double _t139 = (_t7 * _t7 * _t7 * _t7);
  const double _t140 = -0.3e1*_t14 - 0.3e1*_t9;
  const double _t141 = (_t65 * _t65);
  const double _t142 = (_t38 * _t38);
  const double _t143 = 0.6e1*_t10;
  const double _t144 = -_t143*_t30 - 0.3e1*_t27 - 0.3e1*_t29 - 0.3e1*_t37;
  const double _t145 = _t22*_t6*_t63;
  const double _t146 = 0.6e1*_t73;
  const double _t147 = _t35*_t75;
  const double _t148 = _t147*_t31;
  const double _t149 = 0.6e1*_t79;
  const double _t150 = _t36*_t82;
  const double _t151 = -_t10*_t146 - _t148*_t32 - _t149*_t78 - _t150*_t31 - 0.2e1*_t69 - 0.2e1*_t71 - 0.6e1*_t72 - 0.4e1*_t77;
  const double _t152 = _t28*_t32;
  const double _t153 = _t34*_t78;
  const double _t154 = _t33*_t75;
  const double _t155 = 0.3e1*_t28;
  const double _t156 = _t34*_t88;
  const double _t157 = _t19*_t80;
  const double _t158 = 0.3e1*_t15;
  const double _t159 = 0.3e1*_t40;
  const double _t160 = _t131*_t92;
  const double _t161 = _t61*_t96;
  const double _t162 = 0.24e2*_t138*_t139;
  const double _t163 = _hc2[4]*_t160;
  const double _t164 = 0.18e2*_hc2[4];
  const double _t165 = _t164*_t62;
  const double _t166 = _t42*_t74;
  const double _t167 = _hc3[0]*_t102;
  const double _t168 = _hc2[2]*_t22;
  const double _t169 = _t103*_t143;
  const double _t170 = 0.18e2*_t62;
  const double _t171 = _hc2[2]*_t38;
  const double _t172 = _t44*_t92;
  const double _t173 = _t172*_t63;
  const double _t174 = _hc2[2]*_hc2[7];
  const double _t175 = ((_hc2[4]) * (_hc2[4]));
  const double _t176 = _t108*_t116;
  const double _t177 = _t111*(-0.3e1*_hc0[5] - 0.3e1*_hc1[5]);
  const double _t178 = _t117*_t53;
  const double _t179 = _t112*(-0.7e1 / 0.2e1*_hc0[2] + (0.7e1 / 0.2e1)*_hc1[2])/xc_powi(_t1, 8);
  const double _t180 = 0.4e1*_t106;
  const double _t181 = _hc2[4]*_t44;
  const double _t182 = 0.8e1*_t45*_t58;
  const double _t183 = _hc3[0]*_t116;
  const double _t184 = _t183*_t55;
  const double _t185 = _t15*_t184;
  const double _t186 = _hc2[2]*_hc2[8];
  const double _t187 = _hc2[4]*_hc2[5];
  const double _t188 = _t126*_t56;
  const double _t189 = _t126*_t55;
  const double _t190 = _t101*_t189;
  const double _t191 = _hc3[0]*_t189;
  const double _t192 = _t124*_t125*(-0.9e1 / 0.2e1*_hc0[2] + (0.9e1 / 0.2e1)*_hc1[2])/xc_powi(_t1, 10);
  const double _t193 = _hc3[0]*_t192;
  const double _t194 = _hc2[5]*_t44;
  const double _t195 = _t134*_t15;
  const double _t196 = _hc2[2]*_hc2[9];
  const double _t197 = ((_hc2[2]) * (_hc2[2]) * (_hc2[2]) * (_hc2[2]))/(xc_powi(_t1, 12)*xc_powi((params->gamma), 5));
  const double _t198 = ((_hc2[5]) * (_hc2[5]));
  const double _t199 = _hc3[0]*_t197;
  const double _t200 = 0.36e2*_t62;
#endif

  const double f = _t6/_t5;
  out[0] = f;
#if _KMAX >= 1
  const double df_dt = 0;
  out[1] = df_dt;
  const double df_dz = -_t16;
  out[2] = df_dz;
  const double df_drs = _t21;
  out[3] = df_drs;
#endif
#if _KMAX >= 2
  const double d2f_dt2 = 0;
  ...
  out[5] = d2f_dz_dt;
  const double d2f_dz2 = 0.2e1*_hc3[0]*_t22*_t25*_t26*_t3 - _t24 - _t39;
  out[6] = d2f_dz2;
  const double d2f_drs_dt = 0;
  out[7] = d2f_drs_dt;
  const double d2f_drs_dz = _t10*_t42 + _t40 + _t43 - _t50;
  out[8] = d2f_drs_dz;
  const double d2f_drs2 = _t51 - _t57 + _t60;
  out[9] = d2f_drs2;
#endif
#if _KMAX >= 3
  const double d3f_dt3 = 0;
  ...
  out[12] = d3f_dz2_dt;
  const double d3f_dz3 = 0.2e1*_hc3[0]*_t15*_t25*_t26*_t3*_t38 + 0.2e1*_hc3[0]*_t15*_t25*_t26*_t3*_t68 + 0.2e1*_hc3[0]*_t22*_t25*_t26*_t3*_t65 + 0.2e1*_hc3[0]*_t25*_t26*_t3*_t61 - _t16*_t38 - _t16*_t68 - _t23*_t61 - _t23*_t84 - _t64;
  out[13] = d3f_dz3;
  const double d3f_drs_dt2 = 0;
  out[14] = d3f_drs_dt2;
  const double d3f_drs_dz_dt = 0;
  out[15] = d3f_drs_dz_dt;
  const double d3f_drs_dz2 = -_t104*_t44 + _t21*_t22 + _t21*_t38 + _t31*_t87 - _t38*_t94 + _t40*_t89 + _t42*_t99 - _t50*_t65 + _t79*_t88 + _t85 + _t86 - _t90*_t93 - _t95 + _t98;
  out[16] = d3f_drs_dz2;
  const double d3f_drs2_dt = 0;
  out[17] = d3f_drs2_dt;
  const double d3f_drs2_dz = _t10*_t110 + _t105 - _t107*_t108 + _t108*_t109 - _t115 + _t116*_t117 - _t119 + _t120 - _t122 + _t15*_t51 - _t15*_t57 + _t60*_t65;
  out[18] = d3f_drs2_dz;
  const double d3f_drs3 = _t123 + _t128 - _t129*_t130 + _t133 - _t135 + _t136;
  out[19] = d3f_drs3;
#endif
#if _KMAX >= 4
  const double d4f_dt4 = 0;
  ...
  out[23] = d4f_dz3_dt;
  const double d4f_dz4 = 0.24e2*_hc3[0]*_t137*_t138*_t139*_t3 + 0.2e1*_hc3[0]*_t137*_t25*_t26*_t3 + 0.2e1*_hc3[0]*_t141*_t22*_t25*_t26*_t3 + 0.2e1*_hc3[0]*_t142*_t25*_t26*_t3 + 0.2e1*_hc3[0]*_t144*_t22*_t25*_t26*_t3 + 0.2e1*_hc3[0]*_t15*_t151*_t25*_t26*_t3 + 0.2e1*_hc3[0]*_t15*_t25*_t26*_t3*_t38*_t65 + 0.4e1*_hc3[0]*_t15*_t25*_t26*_t3*_t65*_t68 + 0.4e1*_hc3[0]*_t15*_t25*_t26*_t3*_t84 + 0.2e1*_hc3[0]*_t22*_t25*_t26*_t3*_t38 + 0.4e1*_hc3[0]*_t22*_t25*_t26*_t3*_t68 + 0.2e1*_hc3[0]*_t25*_t26*_t3*_t38*_t68 + 0.2e1*_hc3[0]*_t25*_t26*_t3*_t61*_t65 - _t137*_t23 - _t137*_t6*_t63 - _t140*_t64 - _t142*_t23 - _t144*_t145 - _t144*_t24 - _t145*_t38 - _t145*_t68 - _t151*_t16 - 0.2e1*_t16*_t84 - _t23*(-_hc2[10]*_t4 - _hc2[3]*_t149*_t3 - _hc2[6]*_t100*_t12 - _t100*_t153*_t82 - _t113*_t81*_t83 - _t13*(-0.3e1 / 0.2e1*_hc0[14] - 0.3e1 / 0.2e1*_hc1[14]) - _t146*_t28 - 0.3e1*_t147*_t152 - _t148*_t82 - _t150*_t155 - 0.8e1*_t152*_t153 - 0.4e1*_t154*_t78 - _t52*_t83*(-0.5e1 / 0.2e1*_hc0[5] - 0.5e1 / 0.2e1*_hc1[5]) - _t66*_t70 - 0.3e1*_t67*_t70 - _t76*(-0.2e1*_hc0[9] + 0.2e1*_hc1[9])) - _t24*_t38 - _t24*_t68 - _t39*_t68 - _t64*_t65;
  out[24] = d4f_dz4;
  const double d4f_drs_dt3 = 0;
  out[25] = d4f_drs_dt3;
  ...
  const double d4f_drs_dz3 = _hc2[11]*_t20 + _hc2[4]*_t157 - _hc2[7]*_t15*_t160 + _hc2[7]*_t41*_t74 - 0.12e2*_t10*_t103*_t90 + _t10*_t156*_t82 + _t10*_t167*_t168*_t170 - _t101*_t149*_t17*_t44 - _t131*_t167*_t28*_t44 + _t140*_t98 - _t141*_t50 + _t143*_t41*_t90 + 0.2e1*_t152*_t156 + _t154*_t88 + _t155*_t87 + _t157*_t44 + _t158*_t85 + _t158*_t86 + _t159*_t22 + _t159*_t38 - _t160*_t65*_t90 - _t161*_t162 + _t161*_t63 - _t163*_t22 - _t163*_t38 + _t165*_t22*_t92 + _t166*_t22 + _t166*_t38 - _t168*_t169 - _t169*_t171 - _t169*_t44*_t65 - _t171*_t65*_t93 - _t172*_t68*_t91 + _t173*_t38 + _t173*_t68 + _t21*_t61 + _t21*_t84 + _t38*_t43 - _t38*_t50 + _t42*_t70 + _t43*_t68 - _t61*_t94 - _t65*_t95 + _t65*_t98 - _t84*_t94;
  out[28] = d4f_drs_dz3;
  const double d4f_drs2_dt2 = 0;
  out[29] = d4f_drs2_dt2;
  const double d4f_drs2_dz_dt = 0;
  out[30] = d4f_drs2_dz_dt;
  const double d4f_drs2_dz2 = _hc2[12]*_t20 - _hc2[5]*_t104*_t15 + _hc2[5]*_t19*_t79 + _hc2[5]*_t92*_t97 - _hc2[8]*_t15*_t93 + _hc2[8]*_t31*_t41 + 0.8e1*_t101*_t176 + _t105*_t89 - _t107*_t174 - _t107*_t175 + _t108*_t182*_t65 + _t109*_t174 + _t109*_t175 + _t110*_t28 + _t110*_t99 - _t114*_t177 - _t114*_t179 - _t115*_t89 - _t118*_t22 - _t118*_t38 - _t119*_t65 + _t120*_t65 - _t121*_t38 - _t122*_t140 - _t122*_t65 + _t141*_t60 + _t162*_t22*_t59 - 0.4e1*_t176*_t18 + _t177*_t178 + _t178*_t179 - _t180*_t181 + _t181*_t182 - 0.24e2*_t181*_t58*_t62 + _t184*_t65*_t91 - 0.12e2*_t185*_t62 + _t185*_t91 + _t22*_t51 - _t22*_t57 + _t22*_t60 + _t38*_t51 - _t38*_t57 + _t38*_t60 - _t59*_t97 + _t60*_t68;
  out[31] = d4f_drs2_dz2;
  const double d4f_drs3_dt = 0;
  out[32] = d4f_drs3_dt;
  const double d4f_drs3_dz = _hc2[13]*_t20 + 0.3e1*_hc2[4]*_t188 + _hc2[9]*_t10*_t41 - _hc2[9]*_t15*_t49 - 0.3e1*_t116*_t130*_t18 + _t123*_t15 + _t128*_t15 - _t129*_t186 - _t129*_t187 - _t129*_t194 + _t130*_t131*_t183 - _t131*_t193 + _t132*_t186 + _t132*_t187 + _t132*_t194 + _t133*_t65 - _t135*_t65 + _t136*_t140 - _t162*_t195 - _t164*_t190 + _t165*_t191 - _t170*_t194*_t58 + _t170*_t195 + _t18*_t192 + _t193*_t63 - _t195*_t46;
  out[33] = d4f_drs3_dz;
  const double d4f_drs4 = _hc2[14]*_t20 + 0.6e1*_hc2[5]*_t188 - 0.36e2*_hc2[5]*_t190 + _hc2[5]*_t191*_t200 - _t129*_t198 + _t132*_t198 + _t162*_t199 - _t18*_t197 - _t180*_t196 + _t182*_t196 - _t199*_t200 + 0.14e2*_t199*_t45;
  out[34] = d4f_drs4;
#endif
}
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(f1_k, _KMAX)(const xc_func_type *p, double rs, double z, double t, double *out) {
  ...
  const double _t2 = params->BB*(t * t * t * t);
#if _KMAX >= 1
  const double _t3 = _hc0[0]*params->BB;
  const double _t4 = (t * t * t);
  const double _t5 = 0.4e1*_t4;
#endif
#if _KMAX >= 2
  const double _t6 = 0.12e2*_t1;
  const double _t7 = _t4*params->BB;
  const double _t8 = 0.8e1*_t7;
  const double _t9 = _t5*params->BB;
#endif
#if _KMAX >= 3
  const double _t10 = 0.24e2*_t3;
  const double _t11 = _t1*params->BB;
  const double _t12 = 0.36e2*_t11;
  const double _t13 = 0.12e2*_t7;
  const double _t14 = _t6*params->BB;
#endif
#if _KMAX >= 4
  const double _t15 = params->BB*t;
  const double _t16 = 0.24e2*_t15;
#endif

  const double f = _hc0[0]*_t2 + _t1;
  out[0] = f;
#if _KMAX >= 1
  const double df_dt = _hc0[1]*_t2 + _t3*_t5 + 0.2e1*t;
  out[1] = df_dt;
...
#endif
#if _KMAX >= 2
  const double d2f_dt2 = _hc0[1]*_t8 + _hc0[4]*_t2 + _t3*_t6 + 0.2e1;
  out[4] = d2f_dt2;
  const double d2f_dz_dt = _hc0[2]*_t9 + _hc0[5]*_t2;
  out[5] = d2f_dz_dt;
  const double d2f_dz2 = _hc0[6]*_t2;
  out[6] = d2f_dz2;
  const double d2f_drs_dt = _hc0[3]*_t9 + _hc0[7]*_t2;
  out[7] = d2f_drs_dt;
...
#endif
#if _KMAX >= 3
  const double d3f_dt3 = _hc0[1]*_t12 + _hc0[10]*_t2 + _hc0[4]*_t13 + _t10*t;
  out[10] = d3f_dt3;
  const double d3f_dz_dt2 = _hc0[11]*_t2 + _hc0[2]*_t14 + _hc0[5]*_t8;
  out[11] = d3f_dz_dt2;
  const double d3f_dz2_dt = _hc0[12]*_t2 + _hc0[6]*_t9;
  out[12] = d3f_dz2_dt;
  const double d3f_dz3 = _hc0[13]*_t2;
  out[13] = d3f_dz3;
  const double d3f_drs_dt2 = _hc0[14]*_t2 + _hc0[3]*_t14 + _hc0[7]*_t8;
  out[14] = d3f_drs_dt2;
  const double d3f_drs_dz_dt = _hc0[15]*_t2 + _hc0[8]*_t9;
  out[15] = d3f_drs_dz_dt;
  const double d3f_drs_dz2 = _hc0[16]*_t2;
  out[16] = d3f_drs_dz2;
  const double d3f_drs2_dt = _hc0[17]*_t2 + _hc0[9]*_t9;
  out[17] = d3f_drs2_dt;
...
#endif
#if _KMAX >= 4
  const double d4f_dt4 = 0.96e2*_hc0[1]*_t15 + 0.16e2*_hc0[10]*_t7 + _hc0[20]*_t2 + 0.72e2*_hc0[4]*_t11 + _t10;
  out[20] = d4f_dt4;
  const double d4f_dz_dt3 = _hc0[11]*_t13 + _hc0[2]*_t16 + _hc0[21]*_t2 + _hc0[5]*_t12;
  out[21] = d4f_dz_dt3;
  const double d4f_dz2_dt2 = _hc0[12]*_t8 + _hc0[22]*_t2 + _hc0[6]*_t14;
  out[22] = d4f_dz2_dt2;
  const double d4f_dz3_dt = _hc0[13]*_t9 + _hc0[23]*_t2;
  out[23] = d4f_dz3_dt;
  const double d4f_dz4 = _hc0[24]*_t2;
  out[24] = d4f_dz4;
  const double d4f_drs_dt3 = _hc0[14]*_t13 + _hc0[25]*_t2 + _hc0[3]*_t16 + _hc0[7]*_t12;
  out[25] = d4f_drs_dt3;
  const double d4f_drs_dz_dt2 = _hc0[15]*_t8 + _hc0[26]*_t2 + _hc0[8]*_t14;
  out[26] = d4f_drs_dz_dt2;
  const double d4f_drs_dz2_dt = _hc0[16]*_t9 + _hc0[27]*_t2;
  out[27] = d4f_drs_dz2_dt;
  const double d4f_drs_dz3 = _hc0[28]*_t2;
  out[28] = d4f_drs_dz3;
  const double d4f_drs2_dt2 = _hc0[17]*_t8 + _hc0[29]*_t2 + _hc0[9]*_t14;
  out[29] = d4f_drs2_dt2;
  const double d4f_drs2_dz_dt = _hc0[18]*_t9 + _hc0[30]*_t2;
  out[30] = d4f_drs2_dz_dt;
  const double d4f_drs2_dz2 = _hc0[31]*_t2;
  out[31] = d4f_drs2_dz2;
  const double d4f_drs3_dt = _hc0[19]*_t9 + _hc0[32]*_t2;
  out[32] = d4f_drs3_dt;
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(f2_k, _KMAX)(const xc_func_type *p, double rs, double z, double t, double *out) {
  ...
  const double _t2 = _hc2[0]/params->gamma;
  const double _t3 = _t2/_t1;
#if _KMAX >= 1
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(fH_k, _KMAX)(const xc_func_type *p, double rs, double z, double t, double *out) {
  ...
  const double _t4 = _t3*params->gamma;
#if _KMAX >= 1
  ...
  const double _t9 = (_t2 * _t2);
  const double _t10 = _t9*params->gamma;
  const double _t11 = _t1*_t10;
#endif
#if _KMAX >= 2
  const double _t12 = ((_hc2[1]) * (_hc2[1]));
  const double _t13 = (0.1e1 / (_t5 * _t5));
  const double _t14 = _t13*_t4;
  const double _t15 = _hc2[1]*_t14;
  const double _t16 = _t10*_t8;
  const double _t17 = _hc2[1]*_t6;
  const double _t18 = ((_hc2[2]) * (_hc2[2]));
  const double _t19 = (0.3e1 / 0.2e1)*_hc0[5] + (0.3e1 / 0.2e1)*_hc1[5];
  const double _t20 = _hc0[2] - _hc1[2];
  const double _t21 = _t20*_t8*params->gamma;
  const double _t22 = _t1*_t2;
  const double _t23 = _t16*_t6;
  const double _t24 = 0.2e1*_t23;
  const double _t25 = _hc2[2]*_t14;
  const double _t26 = ((_hc2[3]) * (_hc2[3]));
#endif
#if _KMAX >= 3
  const double _t27 = 0.3e1*_t15;
  const double _t28 = ((_hc2[1]) * (_hc2[1]) * (_hc2[1]));
  const double _t29 = (0.1e1 / (_t5 * _t5 * _t5));
  const double _t30 = _t29*_t4;
  const double _t31 = 0.2e1*_t30;
  const double _t32 = 0.2e1*_hc2[5];
  const double _t33 = _t12*_t31;
  const double _t34 = _t13*_t16;
  const double _t35 = _t18*_t31;
  const double _t36 = _t10*_t19;
  const double _t37 = _hc2[1]*_t34;
  const double _t38 = 0.2e1*_hc2[2];
  const double _t39 = _t2*_t21;
  const double _t40 = 0.3e1*_hc2[6];
  const double _t41 = ((_hc2[2]) * (_hc2[2]) * (_hc2[2]));
  const double _t42 = (0.3e1 / 0.2e1)*_hc0[9] - 0.3e1 / 0.2e1*_hc1[9];
  const double _t43 = _hc0[5] + _hc1[5];
  const double _t44 = _t22*params->gamma;
  const double _t45 = _t44*_t8;
  const double _t46 = (0.1e1 / 0.2e1)*_hc0[2] - 0.1e1 / 0.2e1*_hc1[2];
  const double _t47 = _t1*_t21;
  const double _t48 = _t19*_t20;
  const double _t49 = 0.2e1*_t48;
  const double _t50 = _t36*_t6;
  const double _t51 = 0.3e1*_t50;
  const double _t52 = 0.3e1*_t34;
  const double _t53 = _t39*_t6;
  const double _t54 = 0.3e1*_hc2[2];
  const double _t55 = 0.2e1*_hc2[7];
  const double _t56 = _hc2[3]*_t14;
  const double _t57 = _hc2[3]*_t34;
  const double _t58 = 0.2e1*_hc2[8];
  const double _t59 = _hc2[3]*_t6;
  const double _t60 = _t26*_t31;
  const double _t61 = 0.3e1*_t56;
  const double _t62 = ((_hc2[3]) * (_hc2[3]) * (_hc2[3]));
#endif
#if _KMAX >= 4
  const double _t63 = 0.6e1*_t4/(_t5 * _t5 * _t5 * _t5);
  const double _t64 = 0.3e1*_t14;
  const double _t65 = _hc2[5]*_t64;
  const double _t66 = _t28*_t63;
  const double _t67 = _hc2[4]*_t14;
  const double _t68 = 0.2e1*_t15;
  const double _t69 = 0.2e1*_t25;
  const double _t70 = 0.2e1*_t14;
  const double _t71 = _t12*_t63;
  const double _t72 = _t13*_t36;
  const double _t73 = _t34*_t38;
  const double _t74 = _t13*_t39;
  const double _t75 = 0.3e1*_hc2[12];
  const double _t76 = 0.6e1*_hc2[6];
  const double _t77 = _hc2[2]*_t30;
  const double _t78 = _t41*_t63;
  const double _t79 = 0.6e1*_hc2[5];
  const double _t80 = _t18*_t30;
  const double _t81 = _t10*_t42;
  const double _t82 = _t43*_t8;
  const double _t83 = _t2*params->gamma;
  const double _t84 = _t17*_t83;
  const double _t85 = _t54*_t72;
  const double _t86 = _t21*_t46;
  const double _t87 = _hc2[2]*_t34;
  const double _t88 = _t16*_t29;
  const double _t89 = 0.6e1*_t18;
  const double _t90 = _t88*_t89;
  const double _t91 = 0.3e1*_t53;
  const double _t92 = _t54*_t74;
  const double _t93 = 0.4e1*_hc2[13];
  const double _t94 = 0.12e2*_hc2[6];
  const double _t95 = _t1*_t46*params->gamma;
  const double _t96 = 0.3e1*_t44;
  const double _t97 = 0.4e1*_hc2[2]*_t6;
  const double _t98 = _t82*_t83;
  const double _t99 = _hc2[7]*_t64;
  const double _t100 = _hc2[5]*_t70;
  const double _t101 = _hc2[6]*_t14;
  const double _t102 = _hc2[1]*_hc2[3];
  const double _t103 = _t18*_t63;
  const double _t104 = 0.3e1*_hc2[16];
  const double _t105 = _hc2[8]*_t64;
  const double _t106 = 0.6e1*_hc2[8];
  const double _t107 = 0.2e1*_t56;
  const double _t108 = _t62*_t63;
#endif

  const double f = _t1*_t4;
  out[0] = f;
#if _KMAX >= 1
  const double df_dt = _hc2[1]*_t7;
  out[1] = df_dt;
  const double df_dz = _hc2[2]*_t7 + _t11*_t8;
  out[2] = df_dz;
  const double df_drs = _hc2[3]*_t7;
  out[3] = df_drs;
#endif
#if _KMAX >= 2
  const double d2f_dt2 = _hc2[4]*_t3*_t6*params->gamma - _t12*_t14;
  out[4] = d2f_dt2;
  const double d2f_dz_dt = -_hc2[2]*_t15 + _hc2[5]*_t7 + _t16*_t17;
  out[5] = d2f_dz_dt;
  const double d2f_dz2 = _hc2[2]*_t24 + _hc2[6]*_t7 + _t11*_t19 - _t14*_t18 + _t21*_t22;
  out[6] = d2f_dz2;
  const double d2f_drs_dt = -_hc2[3]*_t15 + _hc2[7]*_t3*_t6*params->gamma;
  out[7] = d2f_drs_dt;
  const double d2f_drs_dz = _hc2[3]*_t23 - _hc2[3]*_t25 + _hc2[8]*_t7;
  out[8] = d2f_drs_dz;
  const double d2f_drs2 = _hc2[9]*_t3*_t6*params->gamma - _t14*_t26;
  out[9] = d2f_drs2;
#endif
#if _KMAX >= 3
  const double d3f_dt3 = _hc2[10]*_t7 - _hc2[4]*_t27 + _t28*_t31;
  out[10] = d3f_dt3;
  const double d3f_dz_dt2 = _hc2[11]*_t7 + _hc2[2]*_t33 + _hc2[4]*_t23 - _hc2[4]*_t25 - _t12*_t34 - _t15*_t32;
  out[11] = d3f_dz_dt2;
  const double d3f_dz2_dt = _hc2[1]*_t35 + _hc2[12]*_t7 + _hc2[5]*_t24 - _hc2[6]*_t15 + _t17*_t36 + _t17*_t39 - _t25*_t32 - _t37*_t38;
  out[12] = d3f_dz2_dt;
  const double d3f_dz3 = _hc2[13]*_t7 + _hc2[2]*_t51 + _t11*_t42 - _t18*_t52 + _t23*_t40 - _t25*_t40 + _t31*_t41 + _t43*_t45 + _t44*_t49 + _t46*_t47 + _t53*_t54;
  out[13] = d3f_dz3;
  const double d3f_drs_dt2 = _hc2[14]*_t7 + _hc2[3]*_t33 - _hc2[4]*_t56 - _t15*_t55;
  out[14] = d3f_drs_dt2;
  const double d3f_drs_dz_dt = 0.2e1*_hc2[1]*_hc2[2]*_hc2[3]*_t29*_t3*params->gamma - _hc2[1]*_t57 + _hc2[15]*_t3*_t6*params->gamma - _hc2[5]*_t56 - _hc2[7]*_t25 + _hc2[7]*_t6*_t8*_t9*params->gamma - _hc2[8]*_t15;
  out[15] = d3f_drs_dz_dt;
  const double d3f_drs_dz2 = _hc2[16]*_t7 + _hc2[3]*_t35 + _hc2[3]*_t50 - _hc2[6]*_t56 + _hc2[8]*_t24 - _t25*_t58 - _t38*_t57 + _t39*_t59;
  out[16] = d3f_drs_dz2;
  const double d3f_drs2_dt = _hc2[1]*_t60 + _hc2[17]*_t7 - _hc2[9]*_t15 - _t55*_t56;
  out[17] = d3f_drs2_dt;
  const double d3f_drs2_dz = _hc2[18]*_t7 + _hc2[2]*_t60 + _hc2[9]*_t23 - _hc2[9]*_t25 - _t26*_t34 - _t56*_t58;
  out[18] = d3f_drs2_dz;
  const double d3f_drs3 = _hc2[19]*_t7 - _hc2[9]*_t61 + _t31*_t62;
  out[19] = d3f_drs3;
#endif
#if _KMAX >= 4
  const double d4f_dt4 = -((_hc2[1]) * (_hc2[1]) * (_hc2[1]) * (_hc2[1]))*_t63 - 0.4e1*_hc2[10]*_t15 + _hc2[20]*_t3*_t6*params->gamma - ((_hc2[4]) * (_hc2[4]))*_t64 + 0.12e2*_hc2[4]*_t12*_t29*_t3*params->gamma;
  out[20] = d4f_dt4;
  const double d4f_dz_dt3 = 0.6e1*_hc2[1]*_hc2[2]*_hc2[4]*_t29*_t3*params->gamma - _hc2[1]*_hc2[4]*_t52 - _hc2[10]*_t25 + _hc2[10]*_t6*_t8*_t9*params->gamma - _hc2[11]*_t27 - _hc2[2]*_t66 + _hc2[21]*_t3*_t6*params->gamma - _hc2[4]*_t65 + 0.6e1*_hc2[5]*_t12*_t29*_t3*params->gamma + 0.2e1*_t28*_t29*_t8*_t9*params->gamma;
  out[21] = d4f_dz_dt3;
  const double d4f_dz2_dt2 = 0.8e1*_hc2[1]*_hc2[2]*_hc2[5]*_t29*_t3*params->gamma + 0.2e1*_hc2[11]*_t6*_t8*_t9*params->gamma - _hc2[11]*_t69 - _hc2[12]*_t68 + 0.4e1*_hc2[2]*_t12*_t29*_t8*_t9*params->gamma + _hc2[22]*_t3*_t6*params->gamma + 0.2e1*_hc2[4]*_t18*_t29*_t3*params->gamma + _hc2[4]*_t19*_t6*_t9*params->gamma + _hc2[4]*_t2*_t20*_t6*_t8*params->gamma - _hc2[4]*_t73 - ((_hc2[5]) * (_hc2[5]))*_t70 - 0.4e1*_hc2[5]*_t37 + 0.2e1*_hc2[6]*_t12*_t29*_t3*params->gamma - _hc2[6]*_t67 - _t12*_t72 - _t12*_t74 - _t18*_t71;
  out[22] = d4f_dz2_dt2;
  const double d4f_dz3_dt = _hc2[1]*_t76*_t77 - _hc2[1]*_t78 - _hc2[1]*_t85 + _hc2[1]*_t90 - _hc2[1]*_t92 - _hc2[13]*_t15 + _hc2[23]*_t7 + _hc2[5]*_t51 + _hc2[5]*_t91 - _hc2[6]*_t65 + _t17*_t81 + _t17*_t86 + _t23*_t75 - _t25*_t75 - _t37*_t40 + _t49*_t84 + _t79*_t80 - _t79*_t87 + _t82*_t84;
  out[23] = d4f_dz3_dt;
  const double d4f_dz4 = -((_hc2[2]) * (_hc2[2]) * (_hc2[2]) * (_hc2[2]))*_t63 + 0.8e1*_hc2[2]*_t48*_t6*_t83 + _hc2[24]*_t7 - ((_hc2[6]) * (_hc2[6]))*_t64 + _t11*((0.3e1 / 0.2e1)*_hc0[14] + (0.3e1 / 0.2e1)*_hc1[14]) + _t19*_t43*_t96 + _t20*_t42*_t96 + _t23*_t93 - _t25*_t93 + 0.8e1*_t41*_t88 + _t45*(_hc0[9] - _hc1[9]) + _t47*((0.1e1 / 0.2e1)*_hc0[5] + (0.1e1 / 0.2e1)*_hc1[5]) + 0.3e1*_t48*_t95 + _t50*_t76 + _t53*_t76 - _t72*_t89 - _t74*_t89 + _t80*_t94 + _t81*_t97 + 0.2e1*_t82*_t95 + _t86*_t97 - _t87*_t94 + _t97*_t98;
  out[24] = d4f_dz4;
  const double d4f_drs_dt3 = 0.6e1*_hc2[1]*_hc2[3]*_hc2[4]*_t29*_t3*params->gamma - _hc2[10]*_t56 - _hc2[14]*_t27 + _hc2[25]*_t3*_t6*params->gamma - _hc2[3]*_t66 - _hc2[4]*_t99 + 0.6e1*_hc2[7]*_t12*_t29*_t3*params->gamma;
  out[25] = d4f_drs_dt3;
  const double d4f_drs_dz_dt2 = 0.4e1*_hc2[1]*_hc2[2]*_hc2[7]*_t29*_t3*params->gamma + 0.4e1*_hc2[1]*_hc2[3]*_hc2[5]*_t29*_t3*params->gamma - _hc2[11]*_t56 - _hc2[14]*_t25 + _hc2[14]*_t6*_t8*_t9*params->gamma - _hc2[15]*_t68 + 0.2e1*_hc2[2]*_hc2[3]*_hc2[4]*_t29*_t3*params->gamma - _hc2[2]*_hc2[3]*_t71 + _hc2[26]*_t3*_t6*params->gamma + 0.2e1*_hc2[3]*_t12*_t29*_t8*_t9*params->gamma - _hc2[4]*_t57 - _hc2[7]*_t100 + 0.2e1*_hc2[8]*_t12*_t29*_t3*params->gamma - _hc2[8]*_t67 - _t37*_t55;
  out[26] = d4f_drs_dz_dt2;
  const double d4f_drs_dz2_dt = 0.4e1*_hc2[1]*_hc2[2]*_hc2[3]*_t29*_t8*_t9*params->gamma + 0.4e1*_hc2[1]*_hc2[2]*_hc2[8]*_t29*_t3*params->gamma + 0.2e1*_hc2[1]*_hc2[3]*_hc2[6]*_t29*_t3*params->gamma - _hc2[12]*_t56 + 0.2e1*_hc2[15]*_t6*_t8*_t9*params->gamma - _hc2[15]*_t69 - _hc2[16]*_t15 + 0.4e1*_hc2[2]*_hc2[3]*_hc2[5]*_t29*_t3*params->gamma + _hc2[27]*_t3*_t6*params->gamma - _hc2[7]*_t101 + 0.2e1*_hc2[7]*_t18*_t29*_t3*params->gamma + _hc2[7]*_t19*_t6*_t9*params->gamma + _hc2[7]*_t2*_t20*_t6*_t8*params->gamma - _hc2[8]*_t100 - _t102*_t103 - _t102*_t72 - _t102*_t74 - _t32*_t57 - _t37*_t58 - _t55*_t87;
  out[27] = d4f_drs_dz2_dt;
  const double d4f_drs_dz3 = -_hc2[13]*_t56 + _hc2[28]*_t7 + _hc2[3]*_t76*_t77 - _hc2[3]*_t78 - _hc2[3]*_t85 + _hc2[3]*_t90 - _hc2[3]*_t92 - _hc2[6]*_t105 + _hc2[8]*_t51 + _hc2[8]*_t91 + _t104*_t23 - _t104*_t25 + _t106*_t80 - _t106*_t87 - _t40*_t57 + _t49*_t59*_t83 + _t59*_t81 + _t59*_t86 + _t59*_t98;
  out[28] = d4f_drs_dz3;
  const double d4f_drs2_dt2 = 0.8e1*_hc2[1]*_hc2[3]*_hc2[7]*_t29*_t3*params->gamma - _hc2[14]*_t107 - _hc2[17]*_t68 + _hc2[29]*_t3*_t6*params->gamma + 0.2e1*_hc2[4]*_t26*_t29*_t3*params->gamma - ((_hc2[7]) * (_hc2[7]))*_t70 + 0.2e1*_hc2[9]*_t12*_t29*_t3*params->gamma - _hc2[9]*_t67 - _t26*_t71;
  out[29] = d4f_drs2_dt2;
  const double d4f_drs2_dz_dt = 0.2e1*_hc2[1]*_hc2[2]*_hc2[9]*_t29*_t3*params->gamma - _hc2[1]*_hc2[2]*_t26*_t63 + 0.4e1*_hc2[1]*_hc2[3]*_hc2[8]*_t29*_t3*params->gamma + 0.2e1*_hc2[1]*_t26*_t29*_t8*_t9*params->gamma - _hc2[15]*_t107 - _hc2[17]*_t25 + _hc2[17]*_t6*_t8*_t9*params->gamma - _hc2[18]*_t15 + 0.4e1*_hc2[2]*_hc2[3]*_hc2[7]*_t29*_t3*params->gamma + _hc2[30]*_t3*_t6*params->gamma - _hc2[5]*_hc2[9]*_t14 + 0.2e1*_hc2[5]*_t26*_t29*_t3*params->gamma - _hc2[7]*_hc2[8]*_t70 - _hc2[9]*_t37 - _t55*_t57;
  out[30] = d4f_drs2_dz_dt;
  const double d4f_drs2_dz2 = -_hc2[16]*_t107 + 0.2e1*_hc2[18]*_t6*_t8*_t9*params->gamma - _hc2[18]*_t69 + 0.8e1*_hc2[2]*_hc2[3]*_hc2[8]*_t29*_t3*params->gamma + 0.4e1*_hc2[2]*_t26*_t29*_t8*_t9*params->gamma + _hc2[31]*_t3*_t6*params->gamma + 0.2e1*_hc2[6]*_t26*_t29*_t3*params->gamma - ((_hc2[8]) * (_hc2[8]))*_t70 - 0.4e1*_hc2[8]*_t57 - _hc2[9]*_t101 + 0.2e1*_hc2[9]*_t18*_t29*_t3*params->gamma + _hc2[9]*_t19*_t6*_t9*params->gamma + _hc2[9]*_t2*_t20*_t6*_t8*params->gamma - _hc2[9]*_t73 - _t103*_t26 - _t26*_t72 - _t26*_t74;
  out[31] = d4f_drs2_dz2;
  const double d4f_drs3_dt = 0.6e1*_hc2[1]*_hc2[3]*_hc2[9]*_t29*_t3*params->gamma - _hc2[1]*_t108 - _hc2[17]*_t61 - _hc2[19]*_t15 + _hc2[32]*_t3*_t6*params->gamma + 0.6e1*_hc2[7]*_t26*_t29*_t3*params->gamma - _hc2[9]*_t99;
  out[32] = d4f_drs3_dt;
  const double d4f_drs3_dz = -_hc2[18]*_t61 - _hc2[19]*_t25 + _hc2[19]*_t6*_t8*_t9*params->gamma + 0.6e1*_hc2[2]*_hc2[3]*_hc2[9]*_t29*_t3*params->gamma - _hc2[2]*_t108 - _hc2[3]*_hc2[9]*_t52 + _hc2[33]*_t3*_t6*params->gamma + 0.6e1*_hc2[8]*_t26*_t29*_t3*params->gamma - _hc2[9]*_t105 + 0.2e1*_t29*_t62*_t8*_t9*params->gamma;
  out[33] = d4f_drs3_dz;
  const double d4f_drs4 = -0.4e1*_hc2[19]*_t56 - ((_hc2[3]) * (_hc2[3]) * (_hc2[3]) * (_hc2[3]))*_t63 + _hc2[34]*_t3*_t6*params->gamma - ((_hc2[9]) * (_hc2[9]))*_t64 + 0.12e2*_hc2[9]*_t26*_t29*_t3*params->gamma;
  out[34] = d4f_drs4;
#endif
}
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(tp_z0_k, _KMAX)(const xc_func_type *p, double rs, double xt, double *out) {
  ...
  const double _t1 = xc_powr(0.2e1, 2, 3)*params->tscale;
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(A_z0_k, _KMAX)(const xc_func_type *p, double rs, double t, double *out) {
...
#if _KMAX >= 1
  const double _t3 = _hc0[1]*_hc1[0];
  const double _t4 = _t2 + 0.1e1;
  const double _t5 = _t4/(_t2 * _t2);
  const double _t6 = _t5/((params->gamma) * (params->gamma));
#endif
#if _KMAX >= 2
  const double _t7 = _hc1[0]*_t6;
  const double _t8 = _hc1[0]*_t5;
  const double _t9 = ((_hc0[1]) * (_hc0[1]));
  const double _t10 = (0.1e1 / ((params->gamma) * (params->gamma) * (params->gamma)));
  const double _t11 = _t10*_t9;
  const double _t12 = (_t4 * _t4)/(_t2 * _t2 * _t2);
  const double _t13 = _hc1[0]*_t12;
#endif
#if _KMAX >= 3
  const double _t14 = (0.1e1 / ((params->gamma) * (params->gamma) * (params->gamma) * (params->gamma)));
  const double _t15 = ((_hc0[1]) * (_hc0[1]) * (_hc0[1]))*_t14;
  const double _t16 = _t10*_t3;
  const double _t17 = _hc0[2]*_t16;
  const double _t18 = 0.6e1*_t15;
  const double _t19 = _hc1[0]*(_t4 * _t4 * _t4)/(_t2 * _t2 * _t2 * _t2);
#endif
#if _KMAX >= 4
  const double _t20 = _hc0[3]*_t16;
  const double _t21 = ((_hc0[1]) * (_hc0[1]) * (_hc0[1]) * (_hc0[1]))/xc_powi((params->gamma), 5);
  const double _t22 = ((_hc0[2]) * (_hc0[2]))*_t10;
  const double _t23 = _hc0[2]*_t14*_t9;
  const double _t24 = 0.36e2*_t19;
#endif

  const double f = _hc1[0]*_t1/_t2;
  out[0] = f;
#if _KMAX >= 1
  const double df_dt = 0;
  out[1] = df_dt;
  ...
  const double d2f_drs_dt = 0;
  out[4] = d2f_drs_dt;
  const double d2f_drs2 = _hc0[2]*_t7 + 0.2e1*_t11*_t13 - _t11*_t8;
  out[5] = d2f_drs2;
#endif
#if _KMAX >= 3
  const double d3f_dt3 = 0;
  ...
  const double d3f_drs2_dt = 0;
  out[8] = d3f_drs2_dt;
  const double d3f_drs3 = _hc0[3]*_t7 + 0.6e1*_t12*_t17 - _t13*_t18 + _t15*_t8 - 0.3e1*_t17*_t5 + _t18*_t19;
  out[9] = d3f_drs3;
#endif
#if _KMAX >= 4
  const double d4f_dt4 = 0;
  ...
  const double d4f_drs3_dt = 0;
  out[13] = d4f_drs3_dt;
  const double d4f_drs4 = _hc0[4]*_t7 + 0.24e2*_hc1[0]*_t21*(_t4 * _t4 * _t4 * _t4)/xc_powi(_t2, 5) + 0.8e1*_t12*_t20 + 0.14e2*_t13*_t21 + 0.6e1*_t13*_t22 - 0.36e2*_t13*_t23 - 0.4e1*_t20*_t5 - _t21*_t24 - _t21*_t8 - 0.3e1*_t22*_t8 + _t23*_t24 + 0.6e1*_t23*_t8;
  out[14] = d4f_drs4;
#endif
}
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(f1_z0_k, _KMAX)(const xc_func_type *p, double rs, double t, double *out) {
  ...
  const double _t7 = _t4*params->BB;
  const double _t8 = 0.8e1*_t7;
  const double _t9 = _hc0[2]*params->BB;
#endif
#if _KMAX >= 3
  const double _t10 = 0.24e2*_t3;
  const double _t11 = _t1*params->BB;
  const double _t12 = 0.36e2*_t11;
  const double _t13 = 0.12e2*_t7;
  const double _t14 = _hc0[5]*params->BB;
#endif
...
#endif
#if _KMAX >= 2
  const double d2f_dt2 = _hc0[1]*_t8 + _hc0[3]*_t2 + _t3*_t6 + 0.2e1;
  out[3] = d2f_dt2;
  const double d2f_drs_dt = _hc0[4]*_t2 + _t5*_t9;
  out[4] = d2f_drs_dt;
  const double d2f_drs2 = _hc0[5]*_t2;
  out[5] = d2f_drs2;
#endif
#if _KMAX >= 3
  const double d3f_dt3 = _hc0[1]*_t12 + _hc0[3]*_t13 + _hc0[6]*_t2 + _t10*t;
  out[6] = d3f_dt3;
  const double d3f_drs_dt2 = _hc0[4]*_t8 + _hc0[7]*_t2 + _t6*_t9;
  out[7] = d3f_drs_dt2;
  const double d3f_drs2_dt = _hc0[8]*_t2 + _t14*_t5;
  out[8] = d3f_drs2_dt;
  const double d3f_drs3 = _hc0[9]*_t2;
  out[9] = d3f_drs3;
#endif
#if _KMAX >= 4
  const double d4f_dt4 = 0.96e2*_hc0[1]*params->BB*t + _hc0[10]*_t2 + 0.72e2*_hc0[3]*_t11 + 0.16e2*_hc0[6]*_t7 + _t10;
  out[10] = d4f_dt4;
  const double d4f_drs_dt3 = _hc0[11]*_t2 + _hc0[4]*_t12 + _hc0[7]*_t13 + 0.24e2*_t9*t;
  out[11] = d4f_drs_dt3;
  const double d4f_drs2_dt2 = _hc0[12]*_t2 + _hc0[8]*_t8 + _t14*_t6;
  out[12] = d4f_drs2_dt2;
  const double d4f_drs3_dt = _hc0[13]*_t2 + _hc0[9]*_t5*params->BB;
  out[13] = d4f_drs3_dt;
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(fH_z0_k, _KMAX)(const xc_func_type *p, double rs, double t, double *out) {
  ...
  const double _t3 = _t2*params->gamma;
#endif
#if _KMAX >= 2
  const double _t4 = ((_hc0[1]) * (_hc0[1]));
  const double _t5 = params->gamma/(_t1 * _t1);
  ...
  const double _t10 = ((_hc0[1]) * (_hc0[1]) * (_hc0[1]));
  const double _t11 = (0.1e1 / (_t1 * _t1 * _t1));
  const double _t12 = 0.2e1*_t11*params->gamma;
  const double _t13 = 0.2e1*_t5;
  const double _t14 = _hc0[1]*_t13;
  const double _t15 = _hc0[5]*_t5;
  const double _t16 = 0.2e1*_t6;
  const double _t17 = 0.3e1*_t6;
  const double _t18 = ((_hc0[2]) * (_hc0[2]) * (_hc0[2]));
#endif
#if _KMAX >= 4
  const double _t19 = _hc0[1]*_t5;
  const double _t20 = 0.6e1*params->gamma/(_t1 * _t1 * _t1 * _t1);
#endif

  const double f = params->gamma*xc_log1p(_hc0[0]);
  out[0] = f;
#if _KMAX >= 1
  const double df_dt = _hc0[1]*_t3;
  out[1] = df_dt;
  const double df_drs = _hc0[2]*_t3;
  out[2] = df_drs;
#endif
#if _KMAX >= 2
  const double d2f_dt2 = _hc0[3]*_t2*params->gamma - _t4*_t5;
  out[3] = d2f_dt2;
  const double d2f_drs_dt = -_hc0[1]*_t6 + _hc0[4]*_t2*params->gamma;
  out[4] = d2f_drs_dt;
  const double d2f_drs2 = _hc0[5]*_t2*params->gamma - _t5*_t7;
  out[5] = d2f_drs2;
#endif
#if _KMAX >= 3
  const double d3f_dt3 = -_hc0[3]*_t9 + _hc0[6]*_t3 + _t10*_t12;
  out[6] = d3f_dt3;
  const double d3f_drs_dt2 = _hc0[2]*_t12*_t4 - _hc0[3]*_t6 - _hc0[4]*_t14 + _hc0[7]*_t3;
  out[7] = d3f_drs_dt2;
  const double d3f_drs2_dt = _hc0[1]*_t12*_t7 - _hc0[1]*_t15 - _hc0[4]*_t16 + _hc0[8]*_t3;
  out[8] = d3f_drs2_dt;
  const double d3f_drs3 = -_hc0[5]*_t17 + _hc0[9]*_t3 + _t12*_t18;
  out[9] = d3f_drs3;
#endif
#if _KMAX >= 4
  const double d4f_dt4 = -((_hc0[1]) * (_hc0[1]) * (_hc0[1]) * (_hc0[1]))*_t20 + _hc0[10]*_t2*params->gamma - ((_hc0[3]) * (_hc0[3]))*_t8 + 0.12e2*_hc0[3]*_t11*_t4*params->gamma - 0.4e1*_hc0[6]*_t19;
  out[10] = d4f_dt4;
  const double d4f_drs_dt3 = 0.6e1*_hc0[1]*_hc0[2]*_hc0[3]*_t11*params->gamma + _hc0[11]*_t2*params->gamma - _hc0[2]*_t10*_t20 - _hc0[3]*_hc0[4]*_t8 + 0.6e1*_hc0[4]*_t11*_t4*params->gamma - _hc0[6]*_t6 - _hc0[7]*_t9;
  out[11] = d4f_drs_dt3;
  const double d4f_drs2_dt2 = 0.8e1*_hc0[1]*_hc0[2]*_hc0[4]*_t11*params->gamma + _hc0[12]*_t2*params->gamma + 0.2e1*_hc0[3]*_t11*_t7*params->gamma - _hc0[3]*_t15 - ((_hc0[4]) * (_hc0[4]))*_t13 + 0.2e1*_hc0[5]*_t11*_t4*params->gamma - _hc0[7]*_t16 - _hc0[8]*_t14 - _t20*_t4*_t7;
  out[12] = d4f_drs2_dt2;
  const double d4f_drs3_dt = 0.6e1*_hc0[1]*_hc0[2]*_hc0[5]*_t11*params->gamma - _hc0[1]*_t18*_t20 + _hc0[13]*_t2*params->gamma + 0.6e1*_hc0[4]*_t11*_t7*params->gamma - 0.3e1*_hc0[4]*_t15 - _hc0[8]*_t17 - _hc0[9]*_t19;
  out[13] = d4f_drs3_dt;
  const double d4f_drs4 = _hc0[14]*_t2*params->gamma - ((_hc0[2]) * (_hc0[2]) * (_hc0[2]) * (_hc0[2]))*_t20 - ((_hc0[5]) * (_hc0[5]))*_t8 + 0.12e2*_hc0[5]*_t11*_t7*params->gamma - 0.4e1*_hc0[9]*_t6;
  out[14] = d4f_drs4;
#endif
}
