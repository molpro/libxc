/*
  Generated from python/gga_exc/gga_x_b86.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py gga_x_b86
*/

#ifndef _GGA_X_B86_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _GGA_X_B86_KERNEL_BODY
#define _KMAX 0
#define _GGA_X_B86_HELPER_BODIES
#include "gga_x_b86.c"
#undef _GGA_X_B86_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _GGA_X_B86_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_GGA_X_B86_HELPER_BODIES)
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(b86_f_pf_k, _KMAX)(const xc_func_type *p, double x2, double *out) {
  const gga_x_b86_params *params = (const gga_x_b86_params *)(p->params);

  const double _t1 = params->gamma*x2;
  const double _t2 = _t1 + 0.1e1;
  const double _t3 = pow(_t2, -params->omega);
  const double _t4 = _t3*params->beta;
  const double _t5 = _t4*x2;
#if _KMAX >= 1
  const double _t6 = _t4*params->omega;
  const double _t7 = _t6/_t2;
#endif
#if _KMAX >= 2
  const double _t8 = (0.1e1 / (_t2 * _t2));
  const double _t9 = ((params->gamma) * (params->gamma));
  const double _t10 = _t8*_t9;
  const double _t11 = _t5*params->omega;
  const double _t12 = ((params->omega) * (params->omega));
  const double _t13 = _t12*_t5;
#endif
#if _KMAX >= 3
  const double _t14 = ((params->gamma) * (params->gamma) * (params->gamma))/(_t2 * _t2 * _t2);
  const double _t15 = ((params->omega) * (params->omega) * (params->omega));
  const double _t16 = _t15*_t5;
#endif
#if _KMAX >= 4
  const double _t17 = ((params->gamma) * (params->gamma) * (params->gamma) * (params->gamma))/(_t2 * _t2 * _t2 * _t2);
  const double _t18 = 0.6e1*_t17;
#endif

  const double f = _t5 + 0.1e1;
  out[0] = f;
#if _KMAX >= 1
  const double df_dx2 = -_t1*_t7 + _t3*params->beta;
  out[1] = df_dx2;
#endif
#if _KMAX >= 2
  const double d2f_dx22 = _t10*_t11 + _t10*_t13 - 0.2e1*_t7*params->gamma;
  out[2] = d2f_dx22;
#endif
#if _KMAX >= 3
  const double d3f_dx23 = -0.2e1*_t11*_t14 + 0.3e1*_t12*_t3*_t8*_t9*params->beta - 0.3e1*_t13*_t14 - _t14*_t16 + 0.3e1*_t3*_t8*_t9*params->beta*params->omega;
  out[3] = d3f_dx23;
#endif
#if _KMAX >= 4
  const double d4f_dx24 = _t11*_t18 - 0.12e2*_t12*_t14*_t4 + 0.11e2*_t13*_t17 - 0.4e1*_t14*_t15*_t4 - 0.8e1*_t14*_t6 + _t16*_t18 + _t17*_t5*((params->omega) * (params->omega) * (params->omega) * (params->omega));
  out[4] = d4f_dx24;
#endif
}
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(xchan_k, _KMAX)(const xc_func_type *p, double n_s, double sigma_ss, double *out) {
  ...
  if(!(_t4)) XC_CAT(b86_f_pf_k, _KMAX)(p, _t1*sigma_ss, _hc0);
