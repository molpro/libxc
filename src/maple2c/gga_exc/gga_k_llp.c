/*
  Generated from python/gga_exc/gga_k_llp.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py gga_k_llp
*/

#ifndef _GGA_K_LLP_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _GGA_K_LLP_KERNEL_BODY
#define _KMAX 0
#define _GGA_K_LLP_HELPER_BODIES
#include "gga_k_llp.c"
#undef _GGA_K_LLP_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _GGA_K_LLP_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_GGA_K_LLP_HELPER_BODIES)

/* ---- helpers (one graded body -> <name>_k<order>) ---- */

GPU_DEVICE_FUNCTION static inline void XC_CAT(llp_f_k, _KMAX)(const xc_func_type *p, double x, double *out) {
  const gga_k_llp_params *params = (const gga_k_llp_params *)(p->params);

  const double _t1 = params->beta*params->gamma;
  const double _t2 = _t1*xc_asinh(x);
  const double _t3 = _t2*x + 1.0;
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(xchan_k, _KMAX)(const xc_func_type *p, double n_s, double sigma_ss, double *out) {
  ...
  if(!(_t3)) XC_CAT(llp_f_k, _KMAX)(p, _t1*_t2, _hc1);
