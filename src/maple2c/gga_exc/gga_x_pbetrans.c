/*
  Generated from python/gga_exc/gga_x_pbetrans.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py gga_x_pbetrans
*/

#ifndef _GGA_X_PBETRANS_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _GGA_X_PBETRANS_KERNEL_BODY
#define _KMAX 0
#define _GGA_X_PBETRANS_HELPER_BODIES
#include "gga_x_pbetrans.c"
#undef _GGA_X_PBETRANS_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _GGA_X_PBETRANS_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_GGA_X_PBETRANS_HELPER_BODIES)
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(pbetrans_fermi_k, _KMAX)(const xc_func_type *p, double s, double *out) {

  const double _t1 = M_CBRT3;
  const double _t2 = (M_CBRTPI * M_CBRTPI);
  const double _t3 = _t1*_t2*(s - 0.3e1);
  const double _t4 = exp(-0.2e1*_t3);
  const double _t5 = _t4 + 0.1e1;
#if _KMAX >= 1
  const double _t6 = _t4/(_t5 * _t5);
  const double _t7 = _t1*_t6;
#endif
#if _KMAX >= 2
  const double _t8 = xc_powr(0.3e1, 2, 3);
  const double _t9 = (M_PI * M_CBRTPI);
  const double _t10 = exp(-0.4e1*_t3);
  const double _t11 = (0.1e1 / (_t5 * _t5 * _t5));
#endif
#if _KMAX >= 3
  const double _t12 = (M_PI * M_PI);
  const double _t13 = 0.144e3*_t12;
  const double _t14 = exp(-0.6e1*_t3)/(_t5 * _t5 * _t5 * _t5);
#endif
#if _KMAX >= 4
  const double _t15 = (M_PI * M_PI * M_CBRTPI * M_CBRTPI);
#endif

  const double f = (0.1e1 / _t5);
  out[0] = f;
#if _KMAX >= 1
  const double df_ds = 0.2e1*_t2*_t7;
  out[1] = df_ds;
#endif
#if _KMAX >= 2
  const double d2f_ds2 = 0.8e1*_t10*_t11*_t8*_t9 - 0.4e1*_t6*_t8*_t9;
  out[2] = d2f_ds2;
#endif
#if _KMAX >= 3
  const double d3f_ds3 = -_t10*_t11*_t13 + 0.24e2*_t12*_t6 + _t13*_t14;
  out[3] = d3f_ds3;
#endif
#if _KMAX >= 4
  const double d4f_ds4 = 0.672e3*_t1*_t10*_t11*_t15 - 0.1728e4*_t1*_t14*_t15 + 0.1152e4*_t1*_t15*exp(-0.8e1*_t3)/xc_powi(_t5, 5) - 0.48e2*_t15*_t7;
  out[4] = d4f_ds4;
#endif
}

GPU_DEVICE_FUNCTION static inline void XC_CAT(pbetrans_kappa_k, _KMAX)(const xc_func_type *p, double s, double *out) {

  double _hc0[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(pbetrans_fermi_k, _KMAX)(p, s, _hc0);

  const double f = 1.2270000000000001 - 0.41300000000000014*_hc0[0];
  out[0] = f;
#if _KMAX >= 1
  const double df_ds = -0.41300000000000014*_hc0[1];
  out[1] = df_ds;
#endif
#if _KMAX >= 2
  const double d2f_ds2 = -0.41300000000000014*_hc0[2];
  out[2] = d2f_ds2;
#endif
#if _KMAX >= 3
  const double d3f_ds3 = -0.41300000000000014*_hc0[3];
  out[3] = d3f_ds3;
#endif
#if _KMAX >= 4
  const double d4f_ds4 = -0.41300000000000014*_hc0[4];
  out[4] = d4f_ds4;
#endif
}

GPU_DEVICE_FUNCTION static inline void XC_CAT(pbetrans_f0_k, _KMAX)(const xc_func_type *p, double s, double *out) {

  double _hc0[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(pbetrans_kappa_k, _KMAX)(p, s, _hc0);
  const double _t1 = _hc0[0] + 0.219*(s * s);
  const double _t2 = (0.1e1 / _t1);
  const double _t3 = -_hc0[0]*_t2 + 0.1e1;
#if _KMAX >= 1
  const double _t4 = -_hc0[1] - 0.438*s;
  const double _t5 = (0.1e1 / (_t1 * _t1));
  const double _t6 = _hc0[0]*_t5;
  const double _t7 = -_hc0[1]*_t2 - _t4*_t6;
#endif
#if _KMAX >= 2
  const double _t8 = 0.2e1*_hc0[1];
  const double _t9 = -_hc0[2] - 0.438;
  const double _t10 = _t4*_t5;
  const double _t11 = (0.1e1 / (_t1 * _t1 * _t1));
  const double _t12 = -_t8 - 0.876*s;
  const double _t13 = _t11*_t12;
  const double _t14 = _hc0[0]*_t4;
  const double _t15 = -_hc0[2]*_t2 - _t10*_t8 - _t13*_t14 - _t6*_t9;
#endif
#if _KMAX >= 3
  const double _t16 = 0.3e1*_hc0[2];
  const double _t17 = 0.3e1*_hc0[1];
  const double _t18 = _t5*_t9;
  const double _t19 = -0.2e1*_hc0[2] - 0.876;
  const double _t20 = _t11*_t19;
  const double _t21 = _t13*_t9;
  const double _t22 = _t13*_t4;
  const double _t23 = -_t17 - 1.3140000000000001*s;
  const double _t24 = (0.1e1 / (_t1 * _t1 * _t1 * _t1));
  const double _t25 = _t12*_t14*_t24;
  const double _t26 = _hc0[0]*_hc0[3]*_t5 - 0.2e1*_hc0[0]*_t21 - _hc0[3]*_t2 - _t10*_t16 - _t14*_t20 - _t17*_t18 - _t17*_t22 - _t23*_t25;
#endif
#if _KMAX >= 4
  const double _t27 = 0.4e1*_hc0[3];
  const double _t28 = 0.6e1*_hc0[2];
  const double _t29 = 0.4e1*_hc0[1];
  const double _t30 = 0.3e1*_hc0[0]*_t9;
  const double _t31 = _t29*_t4;
  ...
  out[0] = f;
#if _KMAX >= 1
  const double df_ds = _hc0[0]*_t7 + _hc0[1]*_t3;
  out[1] = df_ds;
#endif
#if _KMAX >= 2
  const double d2f_ds2 = _hc0[0]*_t15 + _hc0[2]*_t3 + _t7*_t8;
  out[2] = d2f_ds2;
#endif
#if _KMAX >= 3
  const double d3f_ds3 = _hc0[0]*_t26 + _hc0[3]*_t3 + _t15*_t17 + _t16*_t7;
  out[3] = d3f_ds3;
#endif
#if _KMAX >= 4
  const double d4f_ds4 = _hc0[0]*(0.3e1*_hc0[0]*_hc0[3]*_t11*_t12 + 0.2e1*_hc0[0]*_hc0[3]*_t11*_t4 + _hc0[0]*_hc0[4]*_t5 + 0.4e1*_hc0[1]*_hc0[3]*_t5 - 0.8e1*_hc0[1]*_t21 - _hc0[4]*_t2 - _t10*_t27 - 0.2e1*_t14*_t19*_t32 - _t18*_t28 - _t20*_t30 - _t20*_t31 - _t22*_t28 - _t25*(-_t16 - 1.3140000000000001) - _t30*_t33 - _t31*_t33 - _t12*_t14*_t23*(-_t29 - 1.752*s)/xc_powi(_t1, 5)) + _hc0[4]*_t3 + _t15*_t28 + _t26*_t29 + _t27*_t7;
  out[4] = d4f_ds4;
#endif
}

GPU_DEVICE_FUNCTION static inline void XC_CAT(pbetrans_f_k, _KMAX)(const xc_func_type *p, double x, double *out) {

  const double _t1 = xc_powr(0.6e1, 2, 3);
  const double _t2 = (0.1e1 / 0.12e2)*_t1/(M_CBRTPI * M_CBRTPI);
  double _hc0[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(pbetrans_f0_k, _KMAX)(p, _t2*x, _hc0);
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(xchan_k, _KMAX)(const xc_func_type *p, double n_s, double sigma_ss, double *out) {
  ...
  if(!(_t6)) XC_CAT(pbetrans_f_k, _KMAX)(p, _t4*_t5, _hc1);
