/*
  Generated from python/gga_exc/gga_x_bpccac.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py gga_x_bpccac
*/

#ifndef _GGA_X_BPCCAC_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _GGA_X_BPCCAC_KERNEL_BODY
#define _KMAX 0
#define _GGA_X_BPCCAC_HELPER_BODIES
#include "gga_x_bpccac.c"
#undef _GGA_X_BPCCAC_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _GGA_X_BPCCAC_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_GGA_X_BPCCAC_HELPER_BODIES)
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(pbe_f0_k, _KMAX)(const xc_func_type *p, double s, double *out) {
  ...
  const double _t2 = 0.2195149727645171*_t1 + 1.2270000000000001;
...
#endif
#if _KMAX >= 3
  const double _t8 = 1.4190055715903714*_t5;
  const double _t9 = (0.1e1 / (_t2 * _t2 * _t2 * _t2));
#endif

  const double f = 0.2693448715820625*_t1*_t3 + 0.1e1;
  out[0] = f;
#if _KMAX >= 1
  const double df_ds = 0.538689743164125*_t3*s - 0.11825046429919761*_t4*_t5;
  out[1] = df_ds;
#endif
#if _KMAX >= 2
  const double d2f_ds2 = -0.59125232149598807*_t1*_t5 + 0.538689743164125*_t3 + 0.10383098980011946*_t6*_t7;
  out[2] = d2f_ds2;
#endif
#if _KMAX >= 3
  const double d3f_ds3 = 0.93447890820107517*_t4*_t7 - _t8*s - 0.13675474138851646*_t9*xc_powi(s, 5);
  out[3] = d3f_ds3;
#endif
#if _KMAX >= 4
  const double d4f_ds4 = 4.0494086022046591*_t1*_t7 - 1.9145663794392304*_t6*_t9 - _t8 + 0.24015770665055016*xc_powi(s, 6)/xc_powi(_t2, 5);
  out[4] = d4f_ds4;
#endif
}
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(pw91_den_k, _KMAX)(const xc_func_type *p, double s, double *out) {
  ...
  const double _t1 = (s * s * s * s);
  const double _t2 = 0.19645000000000001*xc_asinh(7.7956000000000003*s);
#if _KMAX >= 1
  const double _t3 = (s * s * s);
  const double _t4 = (s * s);
  const double _t5 = 60.771379360000005*_t4 + 0.1e1;
  const double _t6 = xc_powr(_t5, -1, 2);
#endif
#if _KMAX >= 2
  const double _t7 = xc_powr(_t5, -3, 2);
#endif
#if _KMAX >= 3
  const double _t8 = 372.2722509689217*_t7;
  const double _t9 = xc_powr(_t5, -5, 2);
#endif

  const double f = 0.0040000000000000001*_t1 + _t2*s + 0.1e1;
  out[0] = f;
#if _KMAX >= 1
  const double df_ds = _t2 + 0.016*_t3 + 1.5314456200000002*_t6*s;
  out[1] = df_ds;
#endif
#if _KMAX >= 2
  const double d2f_ds2 = -93.068062742230424*_t4*_t7 + 0.048000000000000001*_t4 + 3.0628912400000003*_t6;
  out[2] = d2f_ds2;
#endif
#if _KMAX >= 3
  const double d3f_ds3 = 16967.623641625101*_t3*_t9 - _t8*s + 0.096000000000000002*s;
  out[3] = d3f_ds3;
#endif
#if _KMAX >= 4
  const double d4f_ds4 = -5155729.4658145187*_t1/xc_powr(_t5, 7, 2) + 118773.36549137571*_t4*_t9 - _t8 + 0.096000000000000002;
  out[4] = d4f_ds4;
#endif
}
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(pw91_fnum_k, _KMAX)(const xc_func_type *p, double s, double *out) {
  ...
  const double _t1 = 0.19645000000000001*xc_asinh(7.7956000000000003*s);
  const double _t2 = (s * s);
  const double _t3 = exp(-0.100e3*_t2);
  const double _t4 = 0.27429999999999999 - 0.15079999999999999*_t3;
#if _KMAX >= 1
  const double _t5 = (s * s * s);
  const double _t6 = _t3*_t5;
  const double _t7 = 60.771379360000005*_t2 + 0.1e1;
  const double _t8 = xc_powr(_t7, -1, 2);
#endif
#if _KMAX >= 2
  const double _t9 = (s * s * s * s);
  const double _t10 = xc_powr(_t7, -3, 2);
#endif
#if _KMAX >= 3
  const double _t11 = 372.2722509689217*_t10;
  const double _t12 = xc_powr(_t7, -5, 2);
#endif

  const double f = _t1*s + _t2*_t4 + 0.1e1;
  out[0] = f;
#if _KMAX >= 1
  const double df_ds = _t1 + 0.2e1*_t4*s + 30.159999999999997*_t6 + 1.5314456200000002*_t8*s;
  out[1] = df_ds;
#endif
#if _KMAX >= 2
  const double d2f_ds2 = -93.068062742230424*_t10*_t2 + 150.79999999999998*_t2*_t3 - 6031.9999999999991*_t3*_t9 - 0.30159999999999998*_t3 + 3.0628912400000003*_t8 + 0.54859999999999998;
  out[2] = d2f_ds2;
#endif
#if _KMAX >= 3
  const double d3f_ds3 = -_t11*s + 16967.623641625101*_t12*_t5 + 1206399.9999999998*_t3*xc_powi(s, 5) + 361.91999999999996*_t3*s - 54287.999999999993*_t6;
  out[3] = d3f_ds3;
#endif
#if _KMAX >= 4
  const double d4f_ds4 = -_t11 + 118773.36549137571*_t12*_t2 - 235247.99999999994*_t2*_t3 + 16889599.999999996*_t3*_t9 - 241279999.99999994*_t3*xc_powi(s, 6) + 361.91999999999996*_t3 - 5155729.4658145187*_t9/xc_powr(_t7, 7, 2);
  out[4] = d4f_ds4;
#endif
}
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(bpccac_fab_k, _KMAX)(const xc_func_type *p, double x, double *out) {

  const double _t1 = exp(0.19e2 - x);
  const double _t2 = _t1 + 0.1e1;
#if _KMAX >= 1
  const double _t3 = _t1/(_t2 * _t2);
#endif
#if _KMAX >= 2
  const double _t4 = exp(0.38e2 - 0.2e1*x);
  const double _t5 = (0.1e1 / (_t2 * _t2 * _t2));
#endif
#if _KMAX >= 3
  const double _t6 = _t1*_t4/(_t2 * _t2 * _t2 * _t2);
#endif

  const double f = (0.1e1 / _t2);
  out[0] = f;
#if _KMAX >= 1
  const double df_dx = _t3;
  out[1] = df_dx;
#endif
#if _KMAX >= 2
  const double d2f_dx2 = -_t3 + 0.2e1*_t4*_t5;
  out[2] = d2f_dx2;
#endif
#if _KMAX >= 3
  const double d3f_dx3 = _t3 - 0.6e1*_t4*_t5 + 0.6e1*_t6;
  out[3] = d3f_dx3;
#endif
#if _KMAX >= 4
  const double d4f_dx4 = -_t3 + 0.14e2*_t4*_t5 - 0.36e2*_t6 + 0.24e2*exp(0.76e2 - 0.4e1*x)/xc_powi(_t2, 5);
  out[4] = d4f_dx4;
#endif
}

GPU_DEVICE_FUNCTION static inline void XC_CAT(bpccac_f_k, _KMAX)(const xc_func_type *p, double x, double *out) {

  double _hc0[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(bpccac_fab_k, _KMAX)(p, x, _hc0);
  double _hc1[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(pbe_f_k, _KMAX)(p, x, _hc1);
  double _hc2[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(pw91_f_k, _KMAX)(p, x, _hc2);
  const double _t1 = 0.1e1 - _hc0[0];
#if _KMAX >= 2
  const double _t2 = 0.2e1*_hc0[1];
#endif
#if _KMAX >= 3
  const double _t3 = 0.3e1*_hc0[1];
  const double _t4 = 0.3e1*_hc0[2];
#endif
#if _KMAX >= 4
  const double _t5 = 0.4e1*_hc0[1];
  const double _t6 = 0.6e1*_hc0[2];
  const double _t7 = 0.4e1*_hc0[3];
#endif

  const double f = _hc0[0]*_hc2[0] + _hc1[0]*_t1;
  out[0] = f;
#if _KMAX >= 1
  const double df_dx = _hc0[0]*_hc2[1] - _hc0[1]*_hc1[0] + _hc0[1]*_hc2[0] + _hc1[1]*_t1;
  out[1] = df_dx;
#endif
#if _KMAX >= 2
  const double d2f_dx2 = _hc0[0]*_hc2[2] - _hc0[2]*_hc1[0] + _hc0[2]*_hc2[0] - _hc1[1]*_t2 + _hc1[2]*_t1 + _hc2[1]*_t2;
  out[2] = d2f_dx2;
#endif
#if _KMAX >= 3
  const double d3f_dx3 = _hc0[0]*_hc2[3] - _hc0[3]*_hc1[0] + _hc0[3]*_hc2[0] - _hc1[1]*_t4 - _hc1[2]*_t3 + _hc1[3]*_t1 + _hc2[1]*_t4 + _hc2[2]*_t3;
  out[3] = d3f_dx3;
#endif
#if _KMAX >= 4
  const double d4f_dx4 = _hc0[0]*_hc2[4] - _hc0[4]*_hc1[0] + _hc0[4]*_hc2[0] - _hc1[1]*_t7 - _hc1[2]*_t6 - _hc1[3]*_t5 + _hc1[4]*_t1 + _hc2[1]*_t7 + _hc2[2]*_t6 + _hc2[3]*_t5;
  out[4] = d4f_dx4;
#endif
}
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(xchan_k, _KMAX)(const xc_func_type *p, double n_s, double sigma_ss, double *out) {
  ...
  if(!(_t6)) XC_CAT(bpccac_f_k, _KMAX)(p, _t1*_t2, _hc0);
