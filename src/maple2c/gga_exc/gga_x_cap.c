/*
  Generated from python/gga_exc/gga_x_cap.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py gga_x_cap
*/

#ifndef _GGA_X_CAP_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _GGA_X_CAP_KERNEL_BODY
#define _KMAX 0
#define _GGA_X_CAP_HELPER_BODIES
#include "gga_x_cap.c"
#undef _GGA_X_CAP_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _GGA_X_CAP_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_GGA_X_CAP_HELPER_BODIES)
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(cap_f0_k, _KMAX)(const xc_func_type *p, double s, double *out) {
  const gga_x_cap_params *params = (const gga_x_cap_params *)(p->params);

  const double _t1 = xc_log1p(s);
  const double _t2 = _t1*params->c;
  const double _t3 = _t2 + 0.1e1;
  const double _t4 = (0.1e1 / _t3);
  const double _t5 = _t4*params->alphaoAx;
  const double _t6 = _t1*_t5;
#if _KMAX >= 1
  const double _t7 = s + 0.1e1;
  const double _t8 = (0.1e1 / _t7);
  const double _t9 = _t5*_t8;
  const double _t10 = (0.1e1 / (_t3 * _t3));
#endif
#if _KMAX >= 2
  const double _t11 = (0.1e1 / (_t7 * _t7));
  const double _t12 = _t10*params->alphaoAx;
  const double _t13 = _t12*_t2;
  const double _t14 = _t11*_t13;
  const double _t15 = 0.2e1*s;
  const double _t16 = (0.1e1 / (_t3 * _t3 * _t3));
  const double _t17 = ((params->c) * (params->c));
  const double _t18 = _t16*_t17*params->alphaoAx;
  const double _t19 = _t1*_t18;
#endif
#if _KMAX >= 3
  const double _t20 = _t12*params->c;
  const double _t21 = 0.6e1*_t11;
  const double _t22 = (0.1e1 / (_t7 * _t7 * _t7));
  const double _t23 = _t22*_t5;
  const double _t24 = 0.6e1*s;
  const double _t25 = _t22*_t24;
  const double _t26 = _t18*_t25;
  const double _t27 = (0.1e1 / (_t3 * _t3 * _t3 * _t3));
  const double _t28 = ((params->c) * (params->c) * (params->c));
  const double _t29 = _t1*_t27*_t28*params->alphaoAx;
#endif
#if _KMAX >= 4
  const double _t30 = 0.24e2*_t22;
  const double _t31 = (0.1e1 / (_t7 * _t7 * _t7 * _t7));
  const double _t32 = _t31*s;
#endif

  const double f = -_t6*s + 0.1e1;
  out[0] = f;
#if _KMAX >= 1
  const double df_ds = _t1*_t10*_t8*params->alphaoAx*params->c*s - _t6 - _t9*s;
  out[1] = df_ds;
#endif
#if _KMAX >= 2
  const double d2f_ds2 = 0.2e1*_t1*_t10*_t8*params->alphaoAx*params->c + 0.2e1*_t10*_t11*params->alphaoAx*params->c*s - _t11*_t15*_t19 + _t11*_t4*params->alphaoAx*s - _t14*s - 0.2e1*_t9;
  out[2] = d2f_ds2;
#endif
#if _KMAX >= 3
  const double d3f_ds3 = _t1*_t26 + 0.3e1*_t11*_t5 + _t13*_t15*_t22 - 0.3e1*_t14 - _t15*_t23 - _t19*_t21 + _t20*_t21 - _t20*_t25 + _t25*_t29 - _t26;
  out[3] = d3f_ds3;
#endif
#if _KMAX >= 4
  const double d4f_ds4 = 0.8e1*_t1*_t10*_t22*params->alphaoAx*params->c + 0.24e2*_t1*_t16*_t17*_t22*params->alphaoAx + 0.24e2*_t1*_t22*_t27*_t28*params->alphaoAx - 0.24e2*_t1*_t32*params->alphaoAx*((params->c) * (params->c) * (params->c) * (params->c))/xc_powi(_t3, 5) + 0.22e2*_t10*_t31*params->alphaoAx*params->c*s - _t13*_t24*_t31 + 0.36e2*_t16*_t17*_t31*params->alphaoAx*s - _t18*_t30 - 0.22e2*_t19*_t32 - _t20*_t30 - 0.8e1*_t23 + 0.24e2*_t27*_t28*_t31*params->alphaoAx*s - 0.36e2*_t29*_t32 + 0.6e1*_t31*_t4*params->alphaoAx*s;
  out[4] = d4f_ds4;
#endif
}

GPU_DEVICE_FUNCTION static inline void XC_CAT(cap_f_k, _KMAX)(const xc_func_type *p, double x, double *out) {

  const double _t1 = xc_powr(0.6e1, 2, 3);
  const double _t2 = (0.1e1 / 0.12e2)*_t1/(M_CBRTPI * M_CBRTPI);
  double _hc0[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(cap_f0_k, _KMAX)(p, _t2*x, _hc0);
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(xchan_k, _KMAX)(const xc_func_type *p, double n_s, double sigma_ss, double *out) {
  ...
  if(!(_t6)) XC_CAT(cap_f_k, _KMAX)(p, _t1*_t2, _hc0);
