/*
  Generated from python/gga_exc/gga_x_sogga11.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py gga_x_sogga11
*/

#ifndef _GGA_X_SOGGA11_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _GGA_X_SOGGA11_KERNEL_BODY
#define _KMAX 0
#define _GGA_X_SOGGA11_HELPER_BODIES
#include "gga_x_sogga11.c"
#undef _GGA_X_SOGGA11_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _GGA_X_SOGGA11_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_GGA_X_SOGGA11_HELPER_BODIES)
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(sogga11_f0_pf_k, _KMAX)(const xc_func_type *p, double x2, double *out) {
  const gga_x_sogga11_params *params = (const gga_x_sogga11_params *)(p->params);

  const double _t1 = xc_powr(0.6e1, 1, 3);
  const double _t2 = _t1*x2;
  const double _t3 = (0.1e1 / 0.24e2)*params->mu/((M_PI * M_CBRTPI)*params->kappa);
  const double _t4 = _t2*_t3;
  const double _t5 = _t4 + 0.1e1;
  const double _t6 = (0.1e1 / _t5);
#if _KMAX >= 1
  const double _t7 = xc_powr(0.6e1, 2, 3);
  const double _t8 = _t7*((params->mu) * (params->mu))/((M_PI * M_PI * M_CBRTPI * M_CBRTPI)*(_t5 * _t5)*((params->kappa) * (params->kappa)));
#endif
#if _KMAX >= 2
  const double _t9 = (0.1e1 / (M_PI * M_PI * M_PI * M_PI));
  const double _t10 = (0.1e1 / ((params->kappa) * (params->kappa) * (params->kappa)));
  const double _t11 = ((params->mu) * (params->mu) * (params->mu));
  const double _t12 = (0.1e1 / (_t5 * _t5 * _t5));
#endif
#if _KMAX >= 3
  const double _t13 = ((params->mu) * (params->mu) * (params->mu) * (params->mu))/((M_PI * M_PI * M_PI * M_PI * M_PI * M_CBRTPI)*(_t5 * _t5 * _t5 * _t5)*((params->kappa) * (params->kappa) * (params->kappa) * (params->kappa)));
#endif

  const double f = _t4*_t6;
  out[0] = f;
#if _KMAX >= 1
  const double df_dx2 = _t1*_t3*_t6 - 0.1e1 / 0.576e3*_t8*x2;
  out[1] = df_dx2;
#endif
#if _KMAX >= 2
  const double d2f_dx22 = (0.1e1 / 0.1152e4)*_t10*_t11*_t12*_t9*x2 - 0.1e1 / 0.288e3*_t8;
  out[2] = d2f_dx22;
#endif
#if _KMAX >= 3
  const double d3f_dx23 = (0.1e1 / 0.384e3)*_t10*_t11*_t12*_t9 - 0.1e1 / 0.9216e4*_t13*_t2;
  out[3] = d3f_dx23;
#endif
#if _KMAX >= 4
  const double d4f_dx24 = -0.1e1 / 0.2304e4*_t1*_t13 + (0.1e1 / 0.55296e5)*_t7*xc_powi((params->mu), 5)*x2/((M_PI * M_PI * M_PI * M_PI * M_PI * M_PI * M_CBRTPI * M_CBRTPI)*xc_powi(_t5, 5)*xc_powi((params->kappa), 5));
  out[4] = d4f_dx24;
#endif
}

GPU_DEVICE_FUNCTION static inline void XC_CAT(sogga11_f1_pf_k, _KMAX)(const xc_func_type *p, double x2, double *out) {
  const gga_x_sogga11_params *params = (const gga_x_sogga11_params *)(p->params);

  const double _t1 = xc_powr(0.6e1, 1, 3);
  const double _t2 = (0.1e1 / 0.24e2)*_t1*params->mu/((M_PI * M_CBRTPI)*params->kappa);
  const double _t3 = xc_expm1(-_t2*x2);
#if _KMAX >= 1
  ...
  out[0] = f;
#if _KMAX >= 1
  const double df_dx2 = _t2*_t4;
  out[1] = df_dx2;
#endif
#if _KMAX >= 2
  const double d2f_dx22 = -0.1e1 / 0.576e3*xc_powr(0.6e1, 2, 3)*_t4*((params->mu) * (params->mu))/((M_PI * M_PI * M_CBRTPI * M_CBRTPI)*((params->kappa) * (params->kappa)));
  out[2] = d2f_dx22;
#endif
#if _KMAX >= 3
  const double d3f_dx23 = (0.1e1 / 0.2304e4)*_t4*((params->mu) * (params->mu) * (params->mu))/((M_PI * M_PI * M_PI * M_PI)*((params->kappa) * (params->kappa) * (params->kappa)));
  out[3] = d3f_dx23;
#endif
#if _KMAX >= 4
  const double d4f_dx24 = -0.1e1 / 0.55296e5*_t1*_t4*((params->mu) * (params->mu) * (params->mu) * (params->mu))/((M_PI * M_PI * M_PI * M_PI * M_PI * M_CBRTPI)*((params->kappa) * (params->kappa) * (params->kappa) * (params->kappa)));
  out[4] = d4f_dx24;
#endif
}

GPU_DEVICE_FUNCTION static inline void XC_CAT(sogga11_f_pf_k, _KMAX)(const xc_func_type *p, double x2, double *out) {
  const gga_x_sogga11_params *params = (const gga_x_sogga11_params *)(p->params);

  double _hc0[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(sogga11_f0_pf_k, _KMAX)(p, x2, _hc0);
  double _hc1[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(sogga11_f1_pf_k, _KMAX)(p, x2, _hc1);
  ...
  const double _t5 = ((_hc1[0]) * (_hc1[0]) * (_hc1[0]));
  const double _t6 = ((_hc1[0]) * (_hc1[0]) * (_hc1[0]) * (_hc1[0]));
#if _KMAX >= 1
  const double _t7 = 0.2e1*params->a[2];
  const double _t8 = _hc0[0]*_t7;
  const double _t9 = 0.2e1*params->b[2];
  const double _t10 = _hc1[0]*_t9;
  const double _t11 = _hc0[1]*params->a[3];
  const double _t12 = 0.3e1*_t1;
  const double _t13 = _hc0[1]*params->a[4];
  const double _t14 = 0.4e1*_t2;
  const double _t15 = _hc0[1]*params->a[5];
  const double _t16 = 0.5e1*_t3;
  const double _t17 = _hc1[1]*params->b[3];
  const double _t18 = 0.3e1*_t4;
  const double _t19 = _hc1[1]*params->b[4];
  const double _t20 = 0.4e1*_t5;
  const double _t21 = _hc1[1]*params->b[5];
  const double _t22 = 0.5e1*_t6;
#endif
#if _KMAX >= 2
  const double _t23 = ((_hc0[1]) * (_hc0[1]));
  const double _t24 = ((_hc1[1]) * (_hc1[1]));
  const double _t25 = 0.6e1*params->a[3];
  const double _t26 = _hc0[0]*_t23;
  const double _t27 = _t12*params->a[3];
  const double _t28 = _t14*params->a[4];
  const double _t29 = _t16*params->a[5];
  const double _t30 = 0.6e1*params->b[3];
  const double _t31 = _hc1[0]*_t24;
  const double _t32 = _t18*params->b[3];
  const double _t33 = _t20*params->b[4];
  const double _t34 = _t22*params->b[5];
  const double _t35 = _t1*params->a[4];
  const double _t36 = _t2*params->a[5];
  const double _t37 = _t4*params->b[4];
  const double _t38 = _t5*params->b[5];
#endif
#if _KMAX >= 3
  const double _t39 = 0.6e1*params->a[2];
  const double _t40 = 0.6e1*params->b[2];
  const double _t41 = ((_hc0[1]) * (_hc0[1]) * (_hc0[1]));
  const double _t42 = ((_hc1[1]) * (_hc1[1]) * (_hc1[1]));
  const double _t43 = 0.18e2*_hc0[0];
  const double _t44 = 0.18e2*_hc1[0];
  const double _t45 = 0.24e2*params->a[4];
  const double _t46 = 0.24e2*params->b[4];
  const double _t47 = 0.36e2*_hc0[2];
  const double _t48 = _t1*_t13;
  const double _t49 = _t15*_t2;
  const double _t50 = 0.36e2*_hc1[2];
  const double _t51 = _t19*_t4;
  const double _t52 = _t21*_t5;
  const double _t53 = _t1*params->a[5];
  const double _t54 = _t4*params->b[5];
#endif
#if _KMAX >= 4
  const double _t55 = ((_hc0[1]) * (_hc0[1]) * (_hc0[1]) * (_hc0[1]));
  const double _t56 = ((_hc0[2]) * (_hc0[2]));
  const double _t57 = ((_hc1[1]) * (_hc1[1]) * (_hc1[1]) * (_hc1[1]));
  const double _t58 = ((_hc1[2]) * (_hc1[2]));
#endif

  const double f = xc_powi((_hc0[0]), 5)*params->a[5] + _hc0[0]*params->a[1] + xc_powi((_hc1[0]), 5)*params->b[5] + _hc1[0]*params->b[1] + _t1*params->a[2] + _t2*params->a[3] + _t3*params->a[4] + _t4*params->b[2] + _t5*params->b[3] + _t6*params->b[4] + params->a[0] + params->b[0];
  out[0] = f;
#if _KMAX >= 1
  const double df_dx2 = _hc0[1]*_t8 + _hc0[1]*params->a[1] + _hc1[1]*_t10 + _hc1[1]*params->b[1] + _t11*_t12 + _t13*_t14 + _t15*_t16 + _t17*_t18 + _t19*_t20 + _t21*_t22;
  out[1] = df_dx2;
#endif
#if _KMAX >= 2
  const double d2f_dx22 = _hc0[2]*_t27 + _hc0[2]*_t28 + _hc0[2]*_t29 + _hc0[2]*_t8 + _hc0[2]*params->a[1] + _hc1[2]*_t10 + _hc1[2]*_t32 + _hc1[2]*_t33 + _hc1[2]*_t34 + _hc1[2]*params->b[1] + 0.12e2*_t23*_t35 + 0.20e2*_t23*_t36 + _t23*_t7 + 0.12e2*_t24*_t37 + 0.20e2*_t24*_t38 + _t24*_t9 + _t25*_t26 + _t30*_t31;
  out[2] = d2f_dx22;
#endif
#if _KMAX >= 3
  const double d3f_dx23 = _hc0[0]*_t41*_t45 + _hc0[1]*_hc0[2]*_t39 + _hc0[2]*_t11*_t43 + 0.60e2*_hc0[2]*_t49 + _hc0[3]*_t27 + _hc0[3]*_t28 + _hc0[3]*_t29 + _hc0[3]*_t8 + _hc0[3]*params->a[1] + _hc1[0]*_t42*_t46 + _hc1[1]*_hc1[2]*_t40 + _hc1[2]*_t17*_t44 + 0.60e2*_hc1[2]*_t52 + _hc1[3]*_t10 + _hc1[3]*_t32 + _hc1[3]*_t33 + _hc1[3]*_t34 + _hc1[3]*params->b[1] + _t25*_t41 + _t30*_t42 + 0.60e2*_t41*_t53 + 0.60e2*_t42*_t54 + _t47*_t48 + _t50*_t51;
  out[3] = d3f_dx23;
#endif
#if _KMAX >= 4
  const double d4f_dx24 = 0.24e2*_hc0[0]*_hc0[3]*_t11 + 0.120e3*_hc0[0]*_t55*params->a[5] + 0.8e1*_hc0[1]*_hc0[3]*params->a[2] + 0.360e3*_hc0[2]*_t23*_t53 + 0.144e3*_hc0[2]*_t26*params->a[4] + 0.48e2*_hc0[3]*_t48 + 0.80e2*_hc0[3]*_t49 + _hc0[4]*_t27 + _hc0[4]*_t28 + _hc0[4]*_t29 + _hc0[4]*_t8 + _hc0[4]*params->a[1] + 0.24e2*_hc1[0]*_hc1[3]*_t17 + 0.120e3*_hc1[0]*_t57*params->b[5] + 0.8e1*_hc1[1]*_hc1[3]*params->b[2] + 0.360e3*_hc1[2]*_t24*_t54 + 0.144e3*_hc1[2]*_t31*params->b[4] + 0.48e2*_hc1[3]*_t51 + 0.80e2*_hc1[3]*_t52 + _hc1[4]*_t10 + _hc1[4]*_t32 + _hc1[4]*_t33 + _hc1[4]*_t34 + _hc1[4]*params->b[1] + _t23*_t47*params->a[3] + _t24*_t50*params->b[3] + 0.36e2*_t35*_t56 + 0.60e2*_t36*_t56 + 0.36e2*_t37*_t58 + 0.60e2*_t38*_t58 + _t39*_t56 + _t40*_t58 + _t43*_t56*params->a[3] + _t44*_t58*params->b[3] + _t45*_t55 + _t46*_t57;
  out[4] = d4f_dx24;
#endif
}
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(xchan_k, _KMAX)(const xc_func_type *p, double n_s, double sigma_ss, double *out) {
  ...
  if(!(_t4)) XC_CAT(sogga11_f_pf_k, _KMAX)(p, _t3*sigma_ss, _hc1);
