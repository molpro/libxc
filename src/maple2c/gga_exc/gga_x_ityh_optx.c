/*
  Generated from python/gga_exc/gga_x_ityh_optx.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py gga_x_ityh_optx
*/

#ifndef _GGA_X_ITYH_OPTX_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _GGA_X_ITYH_OPTX_KERNEL_BODY
#define _KMAX 0
#define _GGA_X_ITYH_OPTX_HELPER_BODIES
#include "gga_x_ityh_optx.c"
#undef _GGA_X_ITYH_OPTX_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _GGA_X_ITYH_OPTX_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_GGA_X_ITYH_OPTX_HELPER_BODIES)
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(optx_f_k, _KMAX)(const xc_func_type *p, double x, double *out) {
  const gga_x_ityh_optx_params *params = (const gga_x_ityh_optx_params *)(p->params);

  const double _t1 = (x * x * x * x);
  const double _t2 = (x * x);
  const double _t3 = 6.0*_t2 + 0.1e1;
  const double _t4 = (0.1e1 / (_t3 * _t3));
  const double _t5 = _t4*params->b;
#if _KMAX >= 1
  const double _t6 = (x * x * x);
  const double _t7 = xc_powi(x, 5);
  const double _t8 = params->b/(_t3 * _t3 * _t3);
#endif
#if _KMAX >= 2
  const double _t9 = xc_powi(x, 6);
  const double _t10 = (0.1e1 / (_t3 * _t3 * _t3 * _t3));
  const double _t11 = _t10*params->b;
#endif
#if _KMAX >= 3
  const double _t12 = params->b/xc_powi(_t3, 5);
#endif

  const double f = 36.0*_t1*_t5 + params->a;
  out[0] = f;
#if _KMAX >= 1
  const double df_dx = 144.0*_t4*_t6*params->b - 864.0*_t7*_t8;
  out[1] = df_dx;
#endif
#if _KMAX >= 2
  const double d2f_dx2 = -7776.0*_t1*_t8 + 31104.0*_t11*_t9 + 432.0*_t2*_t5;
  out[2] = d2f_dx2;
#endif
#if _KMAX >= 3
  const double d3f_dx3 = 466560.0*_t10*_t7*params->b - 1492992.0*_t12*xc_powi(x, 7) + 864.0*_t4*params->b*x - 41472.0*_t6*_t8;
  out[3] = d3f_dx3;
#endif
#if _KMAX >= 4
  const double d4f_dx4 = 3825792.0*_t1*_t11 - 32845824.0*_t12*_t9 - 145152.0*_t2*_t8 + 864.0*_t5 + 89579520.0*params->b*xc_powi(x, 8)/xc_powi(_t3, 6);
  out[4] = d4f_dx4;
#endif
}
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(ityh_enhancement_k, _KMAX)(const xc_func_type *p, double xs, double *out) {
  ...
  double _hc0[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(optx_f_k, _KMAX)(p, xs, _hc0);
