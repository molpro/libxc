/*
  Generated from python/gga_exc/gga_x_htbs.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py gga_x_htbs
*/

#ifndef _GGA_X_HTBS_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _GGA_X_HTBS_KERNEL_BODY
#define _KMAX 0
#define _GGA_X_HTBS_HELPER_BODIES
#include "gga_x_htbs.c"
#undef _GGA_X_HTBS_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _GGA_X_HTBS_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_GGA_X_HTBS_HELPER_BODIES)
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(rpbe_f0_k, _KMAX)(const xc_func_type *p, double s, double *out) {
  ...
  const double _t1 = (M_PI * M_PI);
  const double _t2 = (s * s);
  const double _t3 = xc_expm1(-0.02766357819367712*_t1*_t2);
#if _KMAX >= 1
  const double _t4 = _t3 + 0.1e1;
#endif
#if _KMAX >= 2
  const double _t5 = (M_PI * M_PI * M_PI * M_PI)*_t4;
#endif
#if _KMAX >= 3
  const double _t6 = 0.0073833592921927369*_t5;
  const double _t7 = xc_powi(M_PI, 6);
#endif

  const double f = 0.1e1 - 0.80400000000000005*_t3;
  out[0] = f;
#if _KMAX >= 1
  const double df_ds = 0.044483033735432812*_t1*_t4*s;
  out[1] = df_ds;
#endif
#if _KMAX >= 2
  const double d2f_ds2 = 0.044483033735432812*_t1*_t4 - 0.0024611197640642456*_t2*_t5;
  out[2] = d2f_ds2;
#endif
#if _KMAX >= 3
  const double d3f_ds3 = 0.00013616675807439089*_t4*_t7*(s * s * s) - _t6*s;
  out[3] = d3f_ds3;
#endif
#if _KMAX >= 4
  const double d4f_ds4 = 0.00081700054844634534*_t2*_t4*_t7 - 7.5337195187408555e-6*xc_powi(M_PI, 8)*_t4*(s * s * s * s) - _t6;
  out[4] = d4f_ds4;
#endif
}

GPU_DEVICE_FUNCTION static inline void XC_CAT(wc_f0_aux_m_kappa_k, _KMAX)(const xc_func_type *p, double s, double *out) {

  const double _t1 = (s * s);
  const double _t2 = (s * s * s * s);
  const double _t3 = 0.007937469335161821*_t2;
  const double _t4 = exp(-_t1);
  const double _t5 = _t1*_t4;
#if _KMAX >= 1
  const double _t6 = 0.19211636528212062*_t4;
  const double _t7 = (s * s * s);
  const double _t8 = _t3 + 0.1e1;
  const double _t9 = (0.1e1 / _t8);
#endif
#if _KMAX >= 2
  const double _t10 = _t2*_t4;
  const double _t11 = xc_powi(s, 6);
  const double _t12 = (0.1e1 / (_t8 * _t8));
#endif
#if _KMAX >= 3
  const double _t13 = 2.3053963833854475*_t4;
  const double _t14 = xc_powi(s, 5);
  const double _t15 = (0.1e1 / (_t8 * _t8 * _t8));
#endif

  const double f = (0.10e2 / 0.81e2)*_t1 + 0.096058182641060311*_t5 + xc_log1p(_t3);
  out[0] = f;
#if _KMAX >= 1
  const double df_ds = -_t6*_t7 + _t6*s + 0.031749877340647284*_t7*_t9 + (0.20e2 / 0.81e2)*s;
  out[1] = df_ds;
#endif
#if _KMAX >= 2
  const double d2f_ds2 = 0.095249632021941852*_t1*_t9 + 0.38423273056424125*_t10 - 0.0010080547111461478*_t11*_t12 - 0.96058182641060312*_t5 + _t6 + 0.20e2 / 0.81e2;
  out[2] = d2f_ds2;
#endif
#if _KMAX >= 3
  const double d3f_ds3 = -0.0090724924003153305*_t12*_t14 - _t13*s - 0.76846546112848249*_t14*_t4 + 6.4011226863103645e-5*_t15*xc_powi(s, 9) + 3.4580945750781712*_t4*_t7 + 0.1904992640438837*_t9*s;
  out[3] = d3f_ds3;
#endif
#if _KMAX >= 4
  const double d4f_ds4 = 14.985076492005408*_t1*_t4 - 10.758516455798755*_t10 + 1.536930922256965*_t11*_t4 - 0.05141079026845354*_t12*_t2 - _t13 + 0.0011522020835358656*_t15*xc_powi(s, 8) + 0.1904992640438837*_t9 - 6.0970458039836614e-6*xc_powi(s, 12)/(_t8 * _t8 * _t8 * _t8);
  out[4] = d4f_ds4;
#endif
}

GPU_DEVICE_FUNCTION static inline void XC_CAT(wc_f0_aux_k, _KMAX)(const xc_func_type *p, double s, double *out) {

  double _hc0[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(wc_f0_aux_m_kappa_k, _KMAX)(p, s, _hc0);

  const double f = _hc0[0] + 0.80400000000000005;
  out[0] = f;
#if _KMAX >= 1
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(wc_f0_k, _KMAX)(const xc_func_type *p, double s, double *out) {

  double _hc0[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(wc_f0_aux_k, _KMAX)(p, s, _hc0);
  double _hc1[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(wc_f0_aux_m_kappa_k, _KMAX)(p, s, _hc1);
  const double _t1 = (0.1e1 / (_hc0[0]));
  const double _t2 = 0.80400000000000005*_t1;
#if _KMAX >= 1
  const double _t3 = (0.1e1 / ((_hc0[0]) * (_hc0[0])));
  const double _t4 = _hc0[1]*_t3;
  const double _t5 = 0.80400000000000005*_hc1[0];
#endif
#if _KMAX >= 2
...
#endif
#if _KMAX >= 3
  const double _t10 = _t3*_t5;
  const double _t11 = 4.8239999999999998*_hc1[0];
  const double _t12 = (0.1e1 / ((_hc0[0]) * (_hc0[0]) * (_hc0[0]) * (_hc0[0])));
  const double _t13 = ((_hc0[1]) * (_hc0[1]) * (_hc0[1]))*_t12;
#endif
#if _KMAX >= 4
  const double _t14 = 19.295999999999999*_hc1[1];
#endif

  const double f = _hc1[0]*_t2 + 0.1e1;
  out[0] = f;
#if _KMAX >= 1
  const double df_ds = _hc1[1]*_t2 - _t4*_t5;
  out[1] = df_ds;
#endif
#if _KMAX >= 2
  const double d2f_ds2 = 1.6080000000000001*_hc1[0]*_t9 - 1.6080000000000001*_hc1[1]*_t4 + _hc1[2]*_t2 - _t5*_t6;
  out[2] = d2f_ds2;
#endif
#if _KMAX >= 3
  const double d3f_ds3 = 4.8239999999999998*_hc0[1]*_hc0[2]*_hc1[0]*_t7 - _hc0[3]*_t10 - 2.4119999999999999*_hc1[1]*_t6 + 4.8239999999999998*_hc1[1]*_t7*_t8 - 2.4119999999999999*_hc1[2]*_t4 + 0.80400000000000005*_hc1[3]*_t1 - _t11*_t13;
  out[3] = d3f_ds3;
#endif
#if _KMAX >= 4
  const double d4f_ds4 = _hc0[1]*_hc0[2]*_t14*_t7 + 6.4320000000000004*_hc0[1]*_hc0[3]*_hc1[0]*_t7 + ((_hc0[2]) * (_hc0[2]))*_t11*_t7 - 28.943999999999999*_hc0[2]*_hc1[0]*_t12*_t8 - 3.2160000000000002*_hc0[3]*_hc1[1]*_t3 - _hc0[4]*_t10 - 4.8239999999999998*_hc1[2]*_t6 + 9.6479999999999997*_hc1[2]*_t9 - 3.2160000000000002*_hc1[3]*_t4 + _hc1[4]*_t2 - _t13*_t14 + 19.295999999999999*((_hc0[1]) * (_hc0[1]) * (_hc0[1]) * (_hc0[1]))*_hc1[0]/xc_powi((_hc0[0]), 5);
  out[4] = d4f_ds4;
#endif
}

GPU_DEVICE_FUNCTION static inline void XC_CAT(htbs_g_k, _KMAX)(const xc_func_type *p, double s, double *out) {

  const double _t1 = (s * s);
  const double _t2 = (s * s * s);
  const double _t3 = (s * s * s * s);

  const double f = -4.6800000000000006*_t1 + 4.1750000000000007*_t2 - 1.5*_t3 + 0.1875*xc_powi(s, 5) + 2.2814999999999999*s - 0.40608;
  out[0] = f;
#if _KMAX >= 1
  const double df_ds = 12.525000000000002*_t1 - 6.0*_t2 + 0.9375*_t3 - 9.3600000000000012*s + 2.2814999999999999;
  out[1] = df_ds;
#endif
#if _KMAX >= 2
  const double d2f_ds2 = -18.0*_t1 + 3.75*_t2 + 25.050000000000004*s - 9.3600000000000012;
  out[2] = d2f_ds2;
#endif
#if _KMAX >= 3
  const double d3f_ds3 = 11.25*_t1 - 36.0*s + 25.050000000000004;
  out[3] = d3f_ds3;
#endif
#if _KMAX >= 4
  const double d4f_ds4 = 22.5*s - 36.0;
  out[4] = d4f_ds4;
#endif
}

GPU_DEVICE_FUNCTION static inline void XC_CAT(htbs_f0_k, _KMAX)(const xc_func_type *p, double s, double *out) {

  double _hc0[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(wc_f0_k, _KMAX)(p, s, _hc0);
  double _hc1[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(rpbe_f0_k, _KMAX)(p, s, _hc1);
  double _hc2[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(htbs_g_k, _KMAX)(p, s, _hc2);
  const double _t1 = s <= 0.59999999999999998;
  const double _t2 = s >= 2.6000000000000001;
  const double _t3 = 0.1e1 - _hc2[0];
#if _KMAX >= 2
  const double _t4 = 0.2e1*_hc2[1];
#endif
#if _KMAX >= 3
  const double _t5 = 0.3e1*_hc2[2];
  const double _t6 = 0.3e1*_hc2[1];
#endif
#if _KMAX >= 4
  const double _t7 = 0.4e1*_hc2[3];
  const double _t8 = 0.6e1*_hc2[2];
  const double _t9 = 0.4e1*_hc2[1];
#endif

  const double f = my_piecewise5(_t1, _hc0[0], _t2, _hc1[0], _hc0[0]*_t3 + _hc1[0]*_hc2[0]);
  out[0] = f;
#if _KMAX >= 1
  const double df_ds = my_piecewise5(_t1, _hc0[1], _t2, _hc1[1], -_hc0[0]*_hc2[1] + _hc0[1]*_t3 + _hc1[0]*_hc2[1] + _hc1[1]*_hc2[0]);
  out[1] = df_ds;
#endif
#if _KMAX >= 2
  const double d2f_ds2 = my_piecewise5(_t1, _hc0[2], _t2, _hc1[2], -_hc0[0]*_hc2[2] - _hc0[1]*_t4 + _hc0[2]*_t3 + _hc1[0]*_hc2[2] + _hc1[1]*_t4 + _hc1[2]*_hc2[0]);
  out[2] = d2f_ds2;
#endif
#if _KMAX >= 3
  const double d3f_ds3 = my_piecewise5(_t1, _hc0[3], _t2, _hc1[3], -_hc0[0]*_hc2[3] - _hc0[1]*_t5 - _hc0[2]*_t6 + _hc0[3]*_t3 + _hc1[0]*_hc2[3] + _hc1[1]*_t5 + _hc1[2]*_t6 + _hc1[3]*_hc2[0]);
  out[3] = d3f_ds3;
#endif
#if _KMAX >= 4
  const double d4f_ds4 = my_piecewise5(_t1, _hc0[4], _t2, _hc1[4], -_hc0[0]*_hc2[4] - _hc0[1]*_t7 - _hc0[2]*_t8 - _hc0[3]*_t9 + _hc0[4]*_t3 + _hc1[0]*_hc2[4] + _hc1[1]*_t7 + _hc1[2]*_t8 + _hc1[3]*_t9 + _hc1[4]*_hc2[0]);
  out[4] = d4f_ds4;
#endif
}

GPU_DEVICE_FUNCTION static inline void XC_CAT(htbs_f_k, _KMAX)(const xc_func_type *p, double x, double *out) {

  const double _t1 = xc_powr(0.6e1, 2, 3);
  const double _t2 = (0.1e1 / 0.12e2)*_t1/(M_CBRTPI * M_CBRTPI);
  double _hc0[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(htbs_f0_k, _KMAX)(p, _t2*x, _hc0);
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(xchan_k, _KMAX)(const xc_func_type *p, double n_s, double sigma_ss, double *out) {
  ...
  if(!(_t6)) XC_CAT(htbs_f_k, _KMAX)(p, _t1*_t2, _hc0);
