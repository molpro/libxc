/*
  Generated from python/gga_exc/gga_x_g96.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py gga_x_g96
*/

#ifndef _GGA_X_G96_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _GGA_X_G96_KERNEL_BODY
#define _KMAX 0
#define _GGA_X_G96_HELPER_BODIES
#include "gga_x_g96.c"
#undef _GGA_X_G96_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _GGA_X_G96_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_GGA_X_G96_HELPER_BODIES)
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(f_g96_k, _KMAX)(const xc_func_type *p, double x, double *out) {

  const double _t1 = xc_powr(x, 3, 2);
  const double _t2 = xc_powr(0.2e1, 2, 3)*M_CBRTPI/M_CBRT3;
#if _KMAX >= 1
  const double _t3 = xc_powr(x, 1, 2);
#endif

  const double f = (0.2e1 / 0.411e3)*_t1*_t2 + 0.1e1;
  out[0] = f;
#if _KMAX >= 1
  const double df_dx = (0.1e1 / 0.137e3)*_t2*_t3;
  out[1] = df_dx;
#endif
#if _KMAX >= 2
  const double d2f_dx2 = (0.1e1 / 0.274e3)*_t2/_t3;
  out[2] = d2f_dx2;
#endif
#if _KMAX >= 3
  const double d3f_dx3 = -0.1e1 / 0.548e3*_t2/_t1;
  out[3] = d3f_dx3;
#endif
#if _KMAX >= 4
  const double d4f_dx4 = (0.3e1 / 0.1096e4)*_t2/xc_powr(x, 5, 2);
  out[4] = d4f_dx4;
#endif
}
...
#endif
  double _hc0[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(f_g96_k, _KMAX)(p, _t1*_t2, _hc0);
