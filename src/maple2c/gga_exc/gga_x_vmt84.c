/*
  Generated from python/gga_exc/gga_x_vmt84.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py gga_x_vmt84
*/

#ifndef _GGA_X_VMT84_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _GGA_X_VMT84_KERNEL_BODY
#define _KMAX 0
#define _GGA_X_VMT84_HELPER_BODIES
#include "gga_x_vmt84.c"
#undef _GGA_X_VMT84_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _GGA_X_VMT84_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_GGA_X_VMT84_HELPER_BODIES)
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(vmt_f0_k, _KMAX)(const xc_func_type *p, double s, double *out) {
  const gga_x_vmt84_params *params = (const gga_x_vmt84_params *)(p->params);

  const double _t1 = (s * s);
  const double _t2 = _t1*params->mu;
  const double _t3 = _t2 + 0.1e1;
  const double _t4 = (0.1e1 / _t3);
  const double _t5 = _t1*params->alpha;
  const double _t6 = exp(-_t5);
  const double _t7 = _t4*_t6;
  const double _t8 = _t2*_t7;
#if _KMAX >= 1
  const double _t9 = (s * s * s);
  const double _t10 = _t7*params->mu;
  const double _t11 = 0.2e1*_t10;
  const double _t12 = (0.1e1 / (_t3 * _t3));
  const double _t13 = ((params->mu) * (params->mu));
  const double _t14 = _t12*_t13*_t6;
#endif
#if _KMAX >= 2
  const double _t15 = (s * s * s * s);
  const double _t16 = _t15*_t6;
  const double _t17 = (0.1e1 / (_t3 * _t3 * _t3));
  const double _t18 = ((params->mu) * (params->mu) * (params->mu));
  const double _t19 = _t17*_t18;
  const double _t20 = _t16*_t19;
  const double _t21 = _t14*_t15;
  const double _t22 = ((params->alpha) * (params->alpha));
#endif
#if _KMAX >= 3
  const double _t23 = 0.24e2*_t10*params->alpha;
  const double _t24 = 0.24e2*_t14;
  const double _t25 = ((params->mu) * (params->mu) * (params->mu) * (params->mu))/(_t3 * _t3 * _t3 * _t3);
  const double _t26 = xc_powi(s, 5);
  const double _t27 = 0.48e2*_t26*_t6;
  const double _t28 = ((params->alpha) * (params->alpha) * (params->alpha));
  const double _t29 = _t10*_t28;
#endif
#if _KMAX >= 4
  const double _t30 = _t19*_t6;
  const double _t31 = xc_powi(s, 6);
  const double _t32 = 0.384e3*_t31*_t6;
#endif

  const double f = _t8 + 0.1e1;
  out[0] = f;
#if _KMAX >= 1
  const double df_ds = -_t11*_t9*params->alpha - 0.2e1*_t14*_t9 + 0.2e1*_t4*_t6*params->mu*s;
  out[1] = df_ds;
#endif
#if _KMAX >= 2
  const double d2f_ds2 = -0.10e2*_t1*_t14 + 0.4e1*_t10*_t15*_t22 + _t11 + 0.8e1*_t20 + 0.8e1*_t21*params->alpha - 0.10e2*_t8*params->alpha;
  out[2] = d2f_ds2;
#endif
#if _KMAX >= 3
  const double d3f_ds3 = 0.72e2*_t12*_t13*_t6*_t9*params->alpha + 0.72e2*_t17*_t18*_t6*_t9 - _t19*_t27*params->alpha - _t22*_t24*_t26 + 0.36e2*_t22*_t4*_t6*_t9*params->mu - _t23*s - _t24*s - _t25*_t27 - 0.8e1*_t26*_t29;
  out[3] = d3f_ds3;
#endif
#if _KMAX >= 4
  const double d4f_ds4 = 0.312e3*_t1*_t30 + 0.16e2*_t10*_t31*((params->alpha) * (params->alpha) * (params->alpha) * (params->alpha)) + 0.64e2*_t14*_t28*_t31 + 0.312e3*_t14*_t5 - 0.112e3*_t15*_t29 - 0.672e3*_t16*_t25 - 0.672e3*_t20*params->alpha - 0.336e3*_t21*_t22 + 0.192e3*_t22*_t30*_t31 + 0.156e3*_t22*_t8 - _t23 - _t24 + _t25*_t32*params->alpha + _t32*xc_powi((params->mu), 5)/xc_powi(_t3, 5);
  out[4] = d4f_ds4;
#endif
}

GPU_DEVICE_FUNCTION static inline void XC_CAT(vmt_f_k, _KMAX)(const xc_func_type *p, double x, double *out) {

  const double _t1 = xc_powr(0.6e1, 2, 3);
  const double _t2 = (0.1e1 / 0.12e2)*_t1/(M_CBRTPI * M_CBRTPI);
  double _hc0[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(vmt_f0_k, _KMAX)(p, _t2*x, _hc0);
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(vmt84_f0_k, _KMAX)(const xc_func_type *p, double s, double *out) {
  const gga_x_vmt84_params *params = (const gga_x_vmt84_params *)(p->params);

  const double _t1 = 0.1e1 - s;
  const double _t2 = s + 0.1e1;
  const double _t3 = (s * s);
  const double _t4 = (0.1e1 / _t3);
  const double _t5 = (s * s * s * s);
  const double _t6 = xc_expm1(-_t5*params->alpha);
  const double _t7 = _t4*_t6;
  const double _t8 = _t2*_t7;
#if _KMAX >= 1
  const double _t9 = (s * s * s);
  ...
  const double _t12 = _t1*_t2;
  const double _t13 = _t6 + 0.1e1;
  const double _t14 = _t13*params->alpha;
  const double _t15 = _t14*s;
  const double _t16 = 0.4e1*_t12;
#endif
#if _KMAX >= 2
  const double _t17 = (0.1e1 / _t5);
  const double _t18 = _t17*_t6;
  const double _t19 = ((params->alpha) * (params->alpha));
  const double _t20 = _t13*_t19;
  const double _t21 = _t20*_t5;
#endif
#if _KMAX >= 3
  const double _t22 = 0.12e2*_t14;
  const double _t23 = 0.18e2*_t18;
  const double _t24 = 0.48e2*_t21;
  const double _t25 = xc_powi(s, -5);
  const double _t26 = _t2*_t6;
  const double _t27 = _t1*_t26;
  const double _t28 = (0.1e1 / s);
  const double _t29 = _t14*_t28;
  const double _t30 = _t20*_t9;
  const double _t31 = xc_powi(s, 7);
  const double _t32 = ((params->alpha) * (params->alpha) * (params->alpha));
  const double _t33 = _t31*_t32;
#endif
#if _KMAX >= 4
  const double _t34 = 0.256e3*_t13;
  const double _t35 = xc_powi(s, 6);
#endif

  const double f = -_t1*_t8;
  out[0] = f;
#if _KMAX >= 1
  const double df_ds = -_t1*_t7 + 0.2e1*_t11*_t12 + _t15*_t16 + _t8;
  out[1] = df_ds;
#endif
#if _KMAX >= 2
  const double d2f_ds2 = 0.4e1*_t1*_t10*_t6 + 0.8e1*_t1*_t13*params->alpha*s - 0.4e1*_t11*_t2 - 0.6e1*_t12*_t18 - 0.16e2*_t12*_t21 - _t14*_t16 - 0.8e1*_t15*_t2 + 0.2e1*_t4*_t6;
  out[2] = d2f_ds2;
#endif
#if _KMAX >= 3
  const double d3f_ds3 = -_t1*_t22 - _t1*_t23 - _t1*_t24 - 0.12e2*_t11 + 0.64e2*_t12*_t13*_t33 + 0.24e2*_t12*_t29 - 0.48e2*_t12*_t30 - 0.24e2*_t15 + _t2*_t22 + _t2*_t23 + _t2*_t24 + 0.24e2*_t25*_t27;
  out[3] = d3f_ds3;
#endif
#if _KMAX >= 4
  const double d4f_ds4 = 0.640e3*_t1*_t13*_t2*_t32*_t35 + 0.96e2*_t1*_t13*_t28*params->alpha + 0.256e3*_t1*_t13*_t31*_t32 + 0.96e2*_t1*_t25*_t6 - 0.192e3*_t1*_t30 - 0.120e3*_t12*_t14*_t4 - 0.240e3*_t12*_t20*_t3 - _t12*_t34*((params->alpha) * (params->alpha) * (params->alpha) * (params->alpha))*xc_powi(s, 10) + 0.192e3*_t13*_t19*_t2*_t9 + 0.192e3*_t13*_t19*_t5 + 0.48e2*_t13*params->alpha + 0.72e2*_t17*_t6 - 0.96e2*_t2*_t29 - _t2*_t33*_t34 - 0.96e2*_t25*_t26 - 0.120e3*_t27/_t35;
  out[4] = d4f_ds4;
#endif
}

GPU_DEVICE_FUNCTION static inline void XC_CAT(vmt84_f_k, _KMAX)(const xc_func_type *p, double x, double *out) {

  const double _t1 = xc_powr(0.6e1, 2, 3);
  const double _t2 = (0.1e1 / 0.12e2)*_t1/(M_CBRTPI * M_CBRTPI);
  double _hc0[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(vmt84_f0_k, _KMAX)(p, _t2*x, _hc0);
  double _hc1[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(vmt_f_k, _KMAX)(p, x, _hc1);

  const double f = _hc0[0] + _hc1[0];
  out[0] = f;
#if _KMAX >= 1
  const double df_dx = _hc0[1]*_t2 + _hc1[1];
  out[1] = df_dx;
#endif
#if _KMAX >= 2
  const double d2f_dx2 = (0.1e1 / 0.24e2)*xc_powr(0.6e1, 1, 3)*_hc0[2]/(M_PI * M_CBRTPI) + _hc1[2];
  out[2] = d2f_dx2;
#endif
#if _KMAX >= 3
  const double d3f_dx3 = (0.1e1 / 0.48e2)*_hc0[3]/(M_PI * M_PI) + _hc1[3];
  out[3] = d3f_dx3;
#endif
#if _KMAX >= 4
  const double d4f_dx4 = (0.1e1 / 0.576e3)*_hc0[4]*_t1/(M_PI * M_PI * M_CBRTPI * M_CBRTPI) + _hc1[4];
  out[4] = d4f_dx4;
#endif
}
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(xchan_k, _KMAX)(const xc_func_type *p, double n_s, double sigma_ss, double *out) {
  ...
  if(!(_t6)) XC_CAT(vmt84_f_k, _KMAX)(p, _t4*_t5, _hc1);
