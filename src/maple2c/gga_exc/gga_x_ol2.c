/*
  Generated from python/gga_exc/gga_x_ol2.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py gga_x_ol2
*/

#ifndef _GGA_X_OL2_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _GGA_X_OL2_KERNEL_BODY
#define _KMAX 0
#define _GGA_X_OL2_HELPER_BODIES
#include "gga_x_ol2.c"
#undef _GGA_X_OL2_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _GGA_X_OL2_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_GGA_X_OL2_HELPER_BODIES)
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(ol2_f_k, _KMAX)(const xc_func_type *p, double x, double *out) {
  const gga_x_ol2_params *params = (const gga_x_ol2_params *)(p->params);

  const double _t1 = 0.4e1*x;
  const double _t2 = _t1 + M_CBRT2;
  const double _t3 = params->cc/_t2;
#if _KMAX >= 1
  const double _t4 = 0.027777777777777776*params->bb;
  const double _t5 = params->cc/(_t2 * _t2);
#endif
#if _KMAX >= 2
  const double _t6 = (0.1e1 / (_t2 * _t2 * _t2));
#endif
#if _KMAX >= 3
  const double _t7 = params->cc/(_t2 * _t2 * _t2 * _t2);
#endif

  const double f = _t3*x + params->aa + 0.013888888888888888*params->bb*(x * x);
  out[0] = f;
#if _KMAX >= 1
  const double df_dx = -_t1*_t5 + _t3 + _t4*x;
  out[1] = df_dx;
#endif
#if _KMAX >= 2
  const double d2f_dx2 = _t4 - 0.8e1*_t5 + 0.32e2*_t6*params->cc*x;
  out[2] = d2f_dx2;
#endif
#if _KMAX >= 3
  const double d3f_dx3 = 0.96e2*_t6*params->cc - 0.384e3*_t7*x;
  out[3] = d3f_dx3;
#endif
#if _KMAX >= 4
  const double d4f_dx4 = -0.1536e4*_t7 + 0.6144e4*params->cc*x/xc_powi(_t2, 5);
  out[4] = d4f_dx4;
#endif
}
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(xchan_k, _KMAX)(const xc_func_type *p, double n_s, double sigma_ss, double *out) {
  ...
  XC_CAT(ol2_f_k, _KMAX)(p, _t4*_t5, _hc1);
