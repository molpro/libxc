/*
  Generated from python/lda_exc/lda_x_1d_soft.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py lda_x_1d_soft
*/

#ifndef _LDA_X_1D_SOFT_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _LDA_X_1D_SOFT_KERNEL_BODY
#define _KMAX 0
#define _LDA_X_1D_SOFT_HELPER_BODIES
#include "lda_x_1d_soft.c"
#undef _LDA_X_1D_SOFT_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _LDA_X_1D_SOFT_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_LDA_X_1D_SOFT_HELPER_BODIES)
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(x1d_inter_k, _KMAX)(const xc_func_type *p, double x, double *out) {
  ...
  const double _t1 = xc_bessel_K0( x);
  const double _t2 = 2.0*_t1;
#if _KMAX >= 1
  const double _t3 = xc_bessel_K1( x);
  const double _t4 = 2.0*_t3;
#endif
#if _KMAX >= 2
  const double _t5 = (0.1e1 / x);
  const double _t6 = _t2 + _t4*_t5;
#endif
#if _KMAX >= 3
  const double _t7 = (0.1e1 / (x * x));
  const double _t8 = -_t1 - _t3*_t5;
#endif

  const double f = _t2;
  out[0] = f;
#if _KMAX >= 1
  const double df_dx = -_t4;
  out[1] = df_dx;
#endif
#if _KMAX >= 2
  const double d2f_dx2 = _t6;
  out[2] = d2f_dx2;
#endif
#if _KMAX >= 3
  const double d3f_dx3 = -_t4*_t7 - _t4 + 2.0*_t5*_t8;
  out[3] = d3f_dx3;
#endif
#if _KMAX >= 4
  const double d4f_dx4 = 4.0*_t3/(x * x * x) + 2.0*_t5*(_t3*_t7 + _t3 - _t5*_t8) + _t6 - 4.0*_t7*_t8;
  out[4] = d4f_dx4;
#endif
}
...
FUNC_NAME(const xc_func_type *p, size_t ip, const double *rho, xc_lda_out_params *out)
{
  ...
  const double _t2 = xc_integrate(func2, NULL, 0.0, _hc0[0]);
  ...
  const double _t8 = 0.2e1*my_piecewise3(_t1, 0, _t7*(_t2*_t3 - xc_integrate(func1, NULL, 0.0, _hc0[0])));
  const double zk = _t8;
  ...
  const double _t11 = xc_integrate(func1, NULL, 0.0, _t6);
  const double _t12 = (0.1e1 / (_hc0[0]));
  const double _t13 = xc_integrate(func2, NULL, 0.0, _t6);
  const double _t17 = xc_integrate(func1, NULL, 0.0, _t8);
  const double _t18 = xc_integrate(func2, NULL, 0.0, _t8);
