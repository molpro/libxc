/*
  Generated from python/lda_exc/lda_c_vwn_2.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py lda_c_vwn_2
*/

#ifndef _LDA_C_VWN_2_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _LDA_C_VWN_2_KERNEL_BODY
#define _KMAX 0
#define _LDA_C_VWN_2_HELPER_BODIES
#include "lda_c_vwn_2.c"
#undef _LDA_C_VWN_2_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _LDA_C_VWN_2_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_LDA_C_VWN_2_HELPER_BODIES)
...
FUNC_NAME(const xc_func_type *p, size_t ip, const double *rho, xc_lda_out_params *out)
{
  ...
  const double _t12 = _t10*_t11 + 0.1e1;
  const double _t13 = 0.4e1*_t10;
  ...
  const double _t9 = _hc1[0]*_hc2[0];
  const double _t15 = _t14*_t9;
  const double _t16 = _t15*nb;
  const double _t18 = _hc3[0]*_t17;
  const double _t19 = _hc1[0]*_t14;
  const double _t20 = _t18*_t19;
  const double _t21 = _t20*nb;
  const double _t22 = _hc0[0]*_hc1[0] + _hc4[0] - _t16*na + _t21*na;
  const double zk = _t22;
  if(out->zk != NULL) out->zk[ip*p->dim.zk + 0] += zk;
#if _KMAX >= 1
  const double _t23 = -_t8;
  const double _t24 = _t10*_t7;
  const double _t25 = -_t23 - _t24;
  const double _t26 = (0.1e1 / (_t1 * _t1 * _t1));
  const double _t27 = 0.2e1*_t26;
  const double _t28 = _t11*_t27;
  const double _t29 = 0.2e1*na;
  const double _t30 = 0.2e1*nb;
  const double _t31 = _t29 - _t30;
  const double _t32 = _t10*_t31 - _t28;
  const double _t34 = _t13*na;
  const double _t39 = _t14*na;
  const double _t42 = xc_powr(_t1, -4, 3);
  const double _t45 = (0.1e1 / 0.6e1)*_t42;
  const double _t50 = 0.8e1*na;
  const double _t51 = _t26*nb;
  const double _t52 = _t50*_t51;
  const double _t53 = xc_powr(_t1, -10, 3);
  const double _t54 = na*nb;
  const double _t61 = -_t24 - _t8;
  const double _t63 = -_t31;
  const double _t64 = _t10*_t63 - _t28;
  const double _t70 = _t39*nb;
  const double _t33 = _t32*_t9;
  const double _t35 = _t33*_t34;
  const double _t36 = _t35*nb;
  const double _t37 = _hc1[1]*_t25;
  ...
  const double _t46 = _hc0[2]*_t4;
  const double _t47 = _hc1[0]*_t46;
  const double _t48 = _hc1[0]*_t12;
  const double _t49 = _t18*_t48;
  const double _t55 = _t17*_t48;
  const double _t56 = _hc3[1]*_t4;
  const double _t57 = _t55*_t56;
  const double _t58 = _t53*_t57;
  const double _t59 = (0.2e1 / 0.3e1)*_t58;
  const double _t60 = -0.8e1*_hc1[0]*_hc2[0]*_t12*_t26*na*nb - 0.2e1 / 0.3e1*_hc1[0]*_hc2[2]*_t12*_t2*_t3*_t53*na*nb + (0.1e1 / 0.6e1)*_t44 + _t45*_t47 + _t49*_t52 + _t54*_t59;
  const double _t62 = _t15*na;
  const double _t65 = _t64*_t9;
  const double _t66 = _t65*nb;
  const double _t67 = _t34*_t66;
  const double _t68 = _hc1[1]*_hc2[0];
  const double _t69 = _t61*_t68;
  const double _t71 = _t69*_t70;
  const double dF_dna = _t1*(_hc0[0]*_hc1[1]*_t25 + 0.4e1*_hc1[0]*_hc3[0]*_t10*_t12*_t17*nb + 0.4e1*_hc1[0]*_hc3[0]*_t10*_t17*_t32*na*nb + 0.4e1*_hc1[1]*_hc3[0]*_t10*_t12*_t17*_t25*na*nb - _t16 - _t36 - _t41 - _t60) + _t22;
  const double dF_dnb = _t1*(_hc0[0]*_hc1[1]*_t61 + 0.4e1*_hc1[0]*_hc3[0]*_t10*_t12*_t17*na + 0.4e1*_hc1[0]*_hc3[0]*_t10*_t17*_t64*na*nb + 0.4e1*_hc1[1]*_hc3[0]*_t10*_t12*_t17*_t61*na*nb - _t60 - _t62 - _t67 - _t71) + _t22;
  if(out->vrho != NULL) out->vrho[ip*p->dim.vrho + 0] += dF_dna;
  if(out->vrho != NULL) out->vrho[ip*p->dim.vrho + 1] += dF_dnb;
#endif
#if _KMAX >= 2
  const double _t72 = xc_powr(_t1, -7, 3);
  const double _t73 = (0.2e1 / 0.9e1)*_t72;
  const double _t74 = xc_powr(0.6e1, 2, 3);
  const double _t75 = (0.1e1 / (M_CBRTPI * M_CBRTPI));
  const double _t76 = _t74*_t75;
  const double _t78 = xc_powr(_t1, -8, 3);
  const double _t79 = (0.1e1 / 0.36e2)*_t78;
  const double _t82 = _t76*_t79;
  const double _t83 = (0.1e1 / (_t1 * _t1 * _t1 * _t1));
  const double _t84 = 0.24e2*_t83;
  const double _t88 = _t4*_t73;
  const double _t92 = _t84*na;
  const double _t93 = xc_powr(_t1, -13, 3);
  const double _t99 = xc_powr(_t1, -14, 3);
  const double _t104 = 0.16e2*_t26;
  const double _t109 = (0.1e1 / 0.3e1)*_t42;
  const double _t114 = _t104*na;
  const double _t118 = (0.4e1 / 0.3e1)*_t53;
  const double _t122 = _t114*nb;
  const double _t129 = _t118*_t54;
  const double _t132 = 0.2e1*_t10;
  const double _t133 = -_t132;
  const double _t134 = _t27*_t7;
  const double _t135 = _t133 + _t134;
  const double _t138 = (_t25 * _t25);
  const double _t141 = 0.8e1*nb;
  const double _t144 = _t10*_t141;
  const double _t145 = 0.4e1*_t26;
  const double _t146 = 0.6e1*_t83;
  const double _t147 = _t11*_t146;
  const double _t148 = _t132 + _t147;
  const double _t149 = -_t145*_t31 + _t148;
  const double _t153 = _t10*nb;
  const double _t154 = _t153*_t50;
  const double _t162 = _t34*nb;
  const double _t177 = 0.6e1*_t11*_t83 - _t132 - _t27*_t31 - _t27*_t63;
  const double _t180 = _t14*nb;
  const double _t185 = _t26*_t50;
  const double _t188 = xc_powi(_t1, -5);
  const double _t191 = _t50*nb;
  const double _t202 = _t25*_t61;
  const double _t207 = _t162*_t32;
  const double _t214 = (0.2e1 / 0.3e1)*_t54;
  const double _t232 = _t132 + _t134;
  const double _t234 = (_t61 * _t61);
  const double _t238 = _t10*_t50;
  const double _t239 = -_t145*_t63 + _t148;
  const double _t77 = _hc0[5]*_t76;
  const double _t80 = _t46*_t73 + _t77*_t79;
  const double _t81 = _hc1[0]*_t80;
  const double _t85 = _t12*_t9;
  const double _t86 = _t84*_t85;
  const double _t87 = _t86*na;
  const double _t89 = _hc2[2]*_t88 + _hc2[5]*_t82;
  const double _t90 = _t19*_t89;
  const double _t91 = _t90*na;
  const double _t94 = _hc2[2]*_t4;
  const double _t95 = _t48*_t94;
  const double _t96 = _t93*_t95;
  const double _t97 = (0.8e1 / 0.3e1)*_t96;
  const double _t98 = (0.1e1 / 0.9e1)*_t55;
  const double _t100 = _hc3[6]*_t76;
  const double _t101 = _t100*_t99;
  const double _t102 = _t57*_t93;
  const double _t103 = _hc4[6]*_t82 + _t101*_t54*_t98 + (0.32e2 / 0.9e1)*_t102*_t54 + _t43*_t73 + _t49*_t92*nb - _t54*_t97 + _t81 - _t87*nb - _t91*nb;
  const double _t105 = _t104*_t85;
  const double _t106 = _t105*nb;
  const double _t107 = _t104*_t49;
  const double _t108 = _t107*nb;
  const double _t110 = _hc1[1]*_t46;
  const double _t111 = _t110*_t25;
  const double _t112 = _hc1[0]*_t32;
  const double _t113 = _t112*_t18;
  const double _t115 = _t113*_t114;
  const double _t116 = _hc2[0]*_t12;
  const double _t117 = _t116*_t37;
  const double _t119 = _t118*_t95;
  const double _t120 = _t12*_t37;
  const double _t121 = _t120*_t18;
  const double _t123 = _t118*_t57;
  const double _t124 = _t112*_t94;
  const double _t125 = _t17*_t56;
  const double _t126 = _t112*_t125;
  const double _t127 = _t118*_t126;
  const double _t128 = _t120*_t94;
  const double _t130 = _t120*_t125;
  const double _t131 = _t104*_t33*na*nb + _t106 - _t108 - _t109*_t111 + _t114*_t117*nb - _t115*nb + _t118*_t124*_t54 + _t119*nb - _t121*_t122 - _t123*nb - _t127*_t54 + _t128*_t129 - _t129*_t130;
  const double _t136 = _hc0[0]*_hc1[1];
  const double _t137 = _t135*_t136;
  const double _t139 = _hc0[0]*_hc1[2];
  const double _t140 = _t138*_t139;
  const double _t142 = _t10*_t9;
  const double _t143 = _t142*_t32;
  const double _t150 = _t149*_t9;
  const double _t151 = _t150*_t34;
  const double _t152 = _t32*_t38;
  const double _t155 = _t135*_t68;
  const double _t156 = _t155*_t39;
  const double _t157 = _hc1[2]*_t138;
  const double _t158 = _hc2[0]*_t157;
  const double _t159 = _t158*_t39;
  const double _t160 = _hc1[0]*_t18;
  const double _t161 = _t149*_t160;
  const double _t163 = _hc1[1]*_t135;
  const double _t164 = _t163*_t18;
  const double _t165 = _t157*_t18;
  const double _t166 = _t18*_t37;
  const double _t167 = _t166*_t32;
  const double _t168 = _t113*_t144 - _t117*_t144 + _t121*_t144 + _t137 + _t140 - _t141*_t143 - _t151*nb - _t152*_t154 + _t154*_t167 - _t156*nb - _t159*nb + _t161*_t162 + _t164*_t70 + _t165*_t70;
  const double _t169 = _t136*_t25;
  const double _t170 = _t12*_t142;
  const double _t171 = _t143*_t50;
  const double _t172 = _t106*na - _t108*na - _t109*_t47 + _t119*_t54 - _t123*_t54 - 0.1e1 / 0.3e1*_t44;
  const double _t173 = _t139*_t25;
  const double _t174 = _t173*_t61;
  const double _t175 = _t26*_t85;
  const double _t176 = _t26*_t49;
  const double _t178 = _t9*nb;
  const double _t179 = _t177*_t178;
  const double _t181 = _t113*_t34;
  const double _t182 = _hc1[0]*_t64;
  const double _t183 = _t18*_t182;
  const double _t184 = _t183*nb;
  const double _t186 = _t33*_t51;
  const double _t187 = _t110*_t61;
  const double _t189 = _hc1[1]*_t116;
  const double _t190 = _t188*_t189;
  const double _t192 = _t38*_t64;
  const double _t193 = _t32*_t69;
  const double _t194 = _t160*_t177;
  const double _t195 = _hc1[1]*_t61;
  const double _t196 = _t18*_t195;
  const double _t197 = _t189*_t61;
  const double _t198 = _t53*_t95;
  const double _t199 = (0.2e1 / 0.3e1)*_t198;
  const double _t200 = _t12*_t195;
  const double _t201 = _t18*_t200;
  const double _t203 = _hc1[2]*_t202;
  const double _t204 = _hc2[0]*_t39;
  const double _t205 = _t204*nb;
  const double _t206 = _t166*_t64;
  const double _t208 = _t12*_t18;
  const double _t209 = _hc1[1]*_t208;
  const double _t210 = _t188*_t209;
  const double _t211 = _t191*_t210;
  const double _t212 = _t182*_t94;
  const double _t213 = _t212*_t53;
  const double _t215 = _t124*_t53;
  const double _t216 = _t18*_t70;
  const double _t217 = _t125*_t182;
  const double _t218 = _t217*_t53;
  const double _t219 = _t126*_t53;
  const double _t220 = _t53*_t94;
  const double _t221 = _t120*_t220;
  const double _t222 = _t214*_t220;
  const double _t223 = _t125*_t53;
  const double _t224 = _t120*_t223;
  const double _t225 = _t214*_t223;
  const double _t226 = _t136*_t61;
  const double _t227 = _t118*_t94;
  const double _t228 = _t227*_t54;
  const double _t229 = _t118*_t125;
  const double _t230 = _t200*_t54;
  const double _t231 = _t104*_t66*na + _t105*na - _t107*na - _t109*_t187 - _t114*_t184 + _t114*_t197*nb + _t118*_t212*_t54 + _t119*na - _t122*_t201 - _t123*na - _t129*_t217 + _t200*_t228 - _t229*_t230;
  const double _t233 = _t136*_t232;
  const double _t235 = _t139*_t234;
  const double _t236 = _t142*_t64;
  const double _t237 = _t236*_t50;
  const double _t240 = _t178*_t239;
  const double _t241 = _t64*_t69;
  const double _t242 = _t232*_t68;
  const double _t243 = _hc1[2]*_t234;
  const double _t244 = _hc2[0]*_t243;
  const double _t245 = _t160*_t239;
  const double _t246 = _hc1[1]*_t232;
  const double _t247 = _t18*_t246;
  const double _t248 = _t18*_t243;
  const double _t249 = _t196*_t64;
  const double _t250 = -_t154*_t241 + _t154*_t249 + _t162*_t245 + _t183*_t238 - _t197*_t238 + _t201*_t238 + _t233 + _t235 - _t237 - _t240*_t34 - _t242*_t70 - _t244*_t70 + _t247*_t70 + _t248*_t70;
  const double d2F_dna2 = _t1*(_t103 + _t131 + _t168) + _t113*_t154 - _t117*_t154 + _t121*_t154 - _t141*_t170 + _t144*_t49 + 0.2e1*_t169 - _t171*nb + _t172;
  const double d2F_dna_dnb = _t1*(_t103 - _t111*_t45 - _t113*_t52 + _t117*_t52 - _t121*_t52 + _t13*_t184 - _t13*_t66 + _t134*_t136 + _t141*_t175 - _t141*_t176 - _t15 - _t162*_t192 - _t162*_t193 + _t162*_t194 + _t162*_t206 + _t166*_t39 + _t174 + _t175*_t50 - _t176*_t50 - _t179*_t34 + _t180*_t196 - _t180*_t69 + _t181 - _t183*_t52 + _t185*_t66 + _t186*_t50 - _t187*_t45 - _t190*_t191*_t7 + _t196*_t207 + _t197*_t52 + _t199*na + _t199*nb + _t20 + _t200*_t222 - _t200*_t225 - _t201*_t52 - _t203*_t205 + _t203*_t216 + _t211*_t7 + _t213*_t214 + _t214*_t215 - _t214*_t218 - _t214*_t219 + _t214*_t221 - _t214*_t224 - _t35 - _t40 - _t59*na - _t59*nb) - _t16 + _t166*_t70 + _t169 + _t172 + _t181*nb + _t184*_t34 + _t196*_t70 + _t20*na + _t21 + _t226 - _t36 - _t41 - _t62 - _t67 - _t71;
  const double d2F_dnb2 = _t1*(_t103 + _t231 + _t250) + _t154*_t183 - _t154*_t197 + _t154*_t201 - _t170*_t50 + _t172 + 0.2e1*_t226 - _t237*nb + _t238*_t49;
  if(out->v2rho2 != NULL) out->v2rho2[ip*p->dim.v2rho2 + 0] += d2F_dna2;
  if(out->v2rho2 != NULL) out->v2rho2[ip*p->dim.v2rho2 + 1] += d2F_dna_dnb;
  if(out->v2rho2 != NULL) out->v2rho2[ip*p->dim.v2rho2 + 2] += d2F_dnb2;
#endif
#if _KMAX >= 3
  const double _t251 = 0.6e1*_t26;
  const double _t252 = _t146*_t7;
  const double _t253 = _t251 - _t252;
  const double _t254 = (_t25 * _t25 * _t25);
  const double _t255 = _t145*_t7;
  const double _t256 = -_t13 + _t255;
  const double _t261 = 0.12e2*_t26;
  const double _t262 = 0.24e2*_t188;
  const double _t263 = _t11*_t262;
  const double _t264 = _t261 + _t263;
  const double _t265 = -_t264 + 0.18e2*_t31*_t83;
  const double _t266 = _t265*_t34;
  const double _t268 = 0.12e2*_t10;
  const double _t269 = _t268*nb;
  const double _t274 = 0.24e2*_t153;
  const double _t276 = 0.48e2*_t26;
  const double _t286 = _t269*na;
  const double _t292 = 0.24e2*_t51;
  const double _t294 = _t276*nb;
  const double _t295 = 0.4e1*nb;
  const double _t301 = _t292*na;
  const double _t304 = _t276*na;
  const double _t310 = _t30*na;
  const double _t313 = _t295*na;
  const double _t316 = (0.14e2 / 0.27e2)*_t53;
  const double _t317 = xc_powr(_t1, -11, 3);
  const double _t318 = (0.2e1 / 0.27e2)*_t317;
  const double _t321 = (0.1e1 / M_PI);
  const double _t326 = _t316*_t4;
  const double _t327 = _t318*_t76;
  const double _t329 = _t4*_t45;
  const double _t334 = 0.96e2*na;
  const double _t335 = _t188*_t334;
  const double _t336 = xc_powr(_t1, -16, 3);
  const double _t337 = xc_powi(_t1, -6);
  const double _t341 = xc_powr(_t1, -17, 3);
  const double _t351 = 0.72e2*_t83;
  const double _t360 = _t351*_t54;
  const double _t374 = _t304*nb;
  const double _t376 = (0.2e1 / 0.3e1)*_t72;
  const double _t382 = -_t252 + 0.2e1*_t26;
  const double _t383 = -_t10*_t7;
  const double _t384 = _t23 + _t383;
  const double _t388 = 0.8e1*_t10;
  const double _t391 = 0.12e2*_t83;
  const double _t392 = _t145 - _t263;
  const double _t393 = _t146*_t63 + _t31*_t391 + _t392;
  const double _t396 = _t104*nb;
  const double _t401 = 0.48e2*nb;
  const double _t407 = _t162*_t64;
  const double _t414 = _t7*na;
  const double _t415 = 0.16e2*nb;
  const double _t416 = _t414*_t415;
  const double _t418 = _t401*_t83;
  const double _t430 = 0.32e2*_t7;
  const double _t431 = _t430*_t54;
  const double _t433 = (0.8e1 / 0.3e1)*_t54;
  const double _t434 = (0.16e2 / 0.3e1)*_t54;
  const double _t436 = _t118*nb;
  const double _t445 = xc_powr(_t1, -19, 3);
  const double _t448 = _t118*na;
  const double _t452 = (0.8e1 / 0.3e1)*_t12;
  const double _t453 = _t414*nb;
  const double _t458 = 0.32e2*_t26;
  const double _t462 = _t458*na;
  const double _t463 = _t462*nb;
  const double _t464 = (0.8e1 / 0.3e1)*_t53;
  const double _t474 = _t383 + _t8;
  const double _t475 = _t146*_t31 + _t391*_t63 + _t392;
  const double _t477 = 0.48e2*na;
  const double _t478 = _t477*_t83;
  const double _t483 = _t92*nb;
  const double _t484 = _t418*na;
  const double _t493 = _t337*nb;
  const double _t494 = _t414*_t493;
  const double _t525 = 0.4e1*na;
  const double _t530 = -_t251 - _t252;
  const double _t531 = (_t61 * _t61 * _t61);
  const double _t532 = _t13 + _t255;
  const double _t535 = -_t264 + 0.18e2*_t63*_t83;
  const double _t541 = 0.24e2*_t10;
  const double _t545 = _t286*_t64;
  const double _t553 = _t541*na;
  const double _t555 = _t274*na;
  const double _t257 = 0.12e2*_t142;
  const double _t258 = _t149*_t257;
  const double _t259 = _t258*nb;
  const double _t260 = _hc0[2]*_t42*_t5;
  const double _t267 = _t178*_t266;
  const double _t270 = _t135*_t189;
  const double _t271 = _t269*_t270;
  const double _t272 = _t116*_t157;
  const double _t273 = _t269*_t272;
  const double _t275 = _t152*_t274;
  const double _t277 = _t113*_t276;
  const double _t278 = _t277*nb;
  const double _t279 = _t39*_t68;
  const double _t280 = _t253*_t279;
  const double _t281 = _t280*nb;
  const double _t282 = _hc1[3]*_t254;
  const double _t283 = _t204*_t282;
  const double _t284 = _t283*nb;
  const double _t285 = _t149*_t38;
  const double _t287 = _t285*_t286;
  const double _t288 = _t155*_t32;
  const double _t289 = _t286*_t288;
  const double _t290 = _t158*_t32;
  const double _t291 = _t286*_t290;
  const double _t293 = _t161*_t292;
  const double _t296 = _t219*_t295;
  const double _t297 = _hc1[2]*_t25;
  const double _t298 = _t205*_t297;
  const double _t299 = _t256*_t298;
  const double _t300 = _t135*_t298;
  const double _t302 = _t163*_t208;
  const double _t303 = _t157*_t208;
  const double _t305 = _t167*_t304;
  const double _t306 = _hc1[0]*_t149;
  const double _t307 = _t306*na;
  const double _t308 = _t223*_t30;
  const double _t309 = _t12*_t163;
  const double _t311 = _t223*_t310;
  const double _t312 = _t12*_t157;
  const double _t314 = _t32*_t37;
  const double _t315 = _t223*_t314;
  const double _t319 = _t4*(_hc0[5]*_t88 + _hc0[9]*_t82);
  const double _t320 = -_t316*_t46 - _t318*_t77 - _t319*_t45;
  const double _t322 = _hc4[21]*_t321;
  const double _t323 = _t322*_t83;
  const double _t324 = _hc4[6]*_t76;
  const double _t325 = _t317*_t324;
  const double _t328 = _hc2[5]*_t88 + _hc2[9]*_t82;
  const double _t330 = -_hc2[2]*_t326 - _hc2[5]*_t327 - _t328*_t329;
  const double _t331 = _t19*_t330;
  const double _t332 = _t331*na;
  const double _t333 = _t49*nb;
  const double _t338 = _hc3[21]*_t321;
  const double _t339 = _t337*_t338;
  const double _t340 = _t339*_t54;
  const double _t342 = _t100*_t55;
  const double _t343 = _t341*_t342;
  const double _t344 = _t343*_t54;
  const double _t345 = _t336*_t57;
  const double _t346 = _t345*_t54;
  const double _t347 = -0.96e2*_hc1[0]*_hc2[0]*_t12*_t188*na*nb - 0.12e2*_hc1[0]*_hc2[2]*_t12*_t2*_t3*_t336*na*nb - 0.24e2*_hc1[0]*_t12*_t26*_t89*na*nb - _hc1[0]*_t320 + _t316*_t43 + (0.1e1 / 0.36e2)*_t323 + (0.1e1 / 0.9e1)*_t325 + _t332*nb + _t333*_t335 + _t340*_t98 + (0.10e2 / 0.9e1)*_t344 + (0.524e3 / 0.27e2)*_t346;
  const double _t348 = _t48*_t89;
  const double _t349 = _t268*_t348;
  const double _t350 = _t349*nb;
  const double _t352 = _t351*_t85;
  const double _t353 = _t352*nb;
  const double _t354 = _t112*_t89;
  const double _t355 = _t354*na;
  const double _t356 = _t268*_t355;
  const double _t357 = _t33*na;
  const double _t358 = _t351*_t357;
  const double _t359 = _t286*_t89;
  const double _t361 = _t124*_t93;
  const double _t362 = _t128*_t93;
  const double _t363 = -0.72e2*_hc1[0]*_hc3[0]*_t12*_t17*_t83*nb - 0.72e2*_hc1[0]*_hc3[0]*_t17*_t32*_t83*na*nb - 0.32e2 / 0.3e1*_hc1[0]*_hc3[1]*_t12*_t17*_t2*_t3*_t93*nb - 0.32e2 / 0.3e1*_hc1[0]*_hc3[1]*_t17*_t2*_t3*_t32*_t93*na*nb - 0.1e1 / 0.3e1*_hc1[0]*_hc3[6]*_t12*_t17*_t74*_t75*_t99*nb - 0.1e1 / 0.3e1*_hc1[0]*_hc3[6]*_t17*_t32*_t74*_t75*_t99*na*nb - 0.72e2*_hc1[1]*_hc3[0]*_t12*_t17*_t25*_t83*na*nb - 0.32e2 / 0.3e1*_hc1[1]*_hc3[1]*_t12*_t17*_t2*_t25*_t3*_t93*na*nb - 0.1e1 / 0.3e1*_hc1[1]*_hc3[6]*_t12*_t17*_t25*_t74*_t75*_t99*na*nb - 0.3e1*_hc1[1]*_t25*_t80 + _t117*_t360 + _t120*_t359 + _t141*_t96 + _t191*_t361 + _t191*_t362 + _t350 + _t353 + _t356*nb + _t358*nb;
  const double _t364 = 0.24e2*_t143;
  const double _t365 = _t276*_t85;
  const double _t366 = _t110*_t42;
  const double _t367 = _t25*_t366;
  const double _t368 = _t276*_t49;
  const double _t369 = _t276*_t33;
  const double _t370 = _t189*na;
  const double _t371 = _t135*_t370;
  const double _t372 = _t161*_t269;
  const double _t373 = _t117*_t276;
  const double _t375 = _t167*_t274;
  const double _t377 = _t351*_t49;
  const double _t378 = (0.1e1 / 0.3e1)*_t55;
  const double _t379 = _t101*_t378;
  const double _t380 = (0.32e2 / 0.3e1)*_t102;
  const double _t381 = -_t191*_t96 + (0.1e1 / 0.12e2)*_t324*_t78 - _t350*na - _t353*na + _t376*_t43 + _t377*na*nb + _t379*_t54 + _t380*_t54 + 0.3e1*_t81;
  const double _t385 = 0.8e1*_t142;
  const double _t386 = _t142*_t177;
  const double _t387 = _t10*_t348;
  const double _t389 = _t83*_t85;
  const double _t390 = 0.48e2*_t389;
  const double _t394 = _t178*_t34;
  const double _t395 = _t182*_t89;
  const double _t397 = 0.16e2*_t190;
  const double _t398 = _t397*nb;
  const double _t399 = _t398*_t7;
  const double _t400 = _t33*_t83;
  const double _t402 = _t400*_t401;
  const double _t403 = _t110*_t45;
  const double _t404 = _t45*_t46;
  const double _t405 = _t279*nb;
  const double _t406 = _t149*_t69;
  const double _t408 = _t70*_t89;
  const double _t409 = _t177*_t38;
  const double _t410 = _t120*_t89;
  const double _t411 = _t116*_t203;
  const double _t412 = _t188*_t68;
  const double _t413 = _t32*_t412;
  const double _t417 = _t197*nb;
  const double _t419 = _t117*_t418;
  const double _t420 = (0.16e2 / 0.3e1)*_t96;
  const double _t421 = _hc1[2]*_t205;
  const double _t422 = _t421*_t61;
  const double _t423 = _hc1[3]*_t138;
  const double _t424 = _t384*_t423;
  const double _t425 = _hc2[0]*_t203;
  const double _t426 = _t32*_t425;
  const double _t427 = _t116*_t297;
  const double _t428 = _t188*_t427;
  const double _t429 = _t209*_t337;
  const double _t432 = _t212*_t93;
  const double _t435 = _t223*_t306;
  const double _t437 = _t200*_t94;
  const double _t438 = _t437*_t93;
  const double _t439 = _t229*_t54;
  const double _t440 = _t110*_t93;
  const double _t441 = (0.2e1 / 0.3e1)*_t440;
  const double _t442 = _t109*_t46;
  const double _t443 = _t196*_t32;
  const double _t444 = _t203*_t208;
  const double _t446 = _hc1[0]*_t177;
  const double _t447 = _t446*_t54;
  const double _t449 = _t125*_t436;
  const double _t450 = _t37*_t64;
  const double _t451 = _t195*_t32;
  const double _t454 = _hc1[1]*_t445;
  const double _t455 = _t125*_t454;
  const double _t456 = _t12*_t203;
  const double _t457 = -0.16e2*_hc1[0]*_hc2[0]*_t12*_t26 - 0.16e2*_hc1[0]*_hc2[0]*_t177*_t26*na*nb - 0.16e2*_hc1[0]*_hc2[0]*_t26*_t32*na - 0.16e2*_hc1[0]*_hc2[0]*_t26*_t64*nb - 0.4e1 / 0.3e1*_hc1[0]*_hc2[2]*_t12*_t2*_t3*_t53 - 0.4e1 / 0.3e1*_hc1[0]*_hc2[2]*_t177*_t2*_t3*_t53*na*nb - 0.4e1 / 0.3e1*_hc1[0]*_hc2[2]*_t2*_t3*_t32*_t53*na - 0.4e1 / 0.3e1*_hc1[0]*_hc2[2]*_t2*_t3*_t53*_t64*nb - 0.16e2*_hc1[1]*_hc2[0]*_t12*_t25*_t26*na - 0.16e2*_hc1[1]*_hc2[0]*_t12*_t26*_t61*nb - 0.16e2*_hc1[1]*_hc2[0]*_t25*_t26*_t64*na*nb - 0.16e2*_hc1[1]*_hc2[0]*_t26*_t32*_t61*na*nb - 0.4e1 / 0.3e1*_hc1[1]*_hc2[2]*_t12*_t2*_t25*_t3*_t53*na - 0.8e1 / 0.3e1*_hc1[1]*_hc2[2]*_t12*_t2*_t3*_t445*_t7*na*nb - 0.4e1 / 0.3e1*_hc1[1]*_hc2[2]*_t12*_t2*_t3*_t53*_t61*nb - 0.4e1 / 0.3e1*_hc1[1]*_hc2[2]*_t2*_t25*_t3*_t53*_t64*na*nb - 0.4e1 / 0.3e1*_hc1[1]*_hc2[2]*_t2*_t3*_t32*_t53*_t61*na*nb - 0.16e2*_hc1[2]*_hc2[0]*_t12*_t25*_t26*_t61*na*nb - 0.4e1 / 0.3e1*_hc1[2]*_hc2[2]*_t12*_t2*_t25*_t3*_t53*_t61*na*nb + _t104*_t184 + _t107 + _t114*_t121 + _t115 + _t122*_t194 + _t122*_t206 + _t122*_t443 + _t122*_t444 + _t123 + _t127*na + _t130*_t448 + _t200*_t449 + _t201*_t396 + _t203*_t442 + _t217*_t436 + _t229*_t447 + _t347 + _t439*_t450 + _t439*_t451 + _t439*_t456 + _t441*_t7 + _t452*_t453*_t455;
  const double _t459 = _t458*_t85;
  const double _t460 = _t33*_t458;
  const double _t461 = _t113*_t458;
  const double _t465 = _t464*_t95;
  const double _t466 = _t464*_t57;
  const double _t467 = _t124*_t464;
  const double _t468 = _t126*_t464;
  const double _t469 = _t128*_t464;
  const double _t470 = _t130*_t464;
  const double _t471 = _t386*_t50;
  const double _t472 = _t210*_t415;
  const double _t473 = _t113*_t238 - _t117*_t238 - _t12*_t385 + _t121*_t238 + _t136*_t255 - _t141*_t236 + _t144*_t183 - _t144*_t197 + _t144*_t201 - _t154*_t192 - _t154*_t193 + _t154*_t194 + _t154*_t206 - _t154*_t411 + _t154*_t443 + _t154*_t444 - _t171 + 0.2e1*_t174 + _t381 + _t388*_t49 - _t399*na + _t414*_t472 - _t471*nb;
  const double _t476 = _t357*nb;
  const double _t479 = _t239*_t38;
  const double _t480 = _t177*_t69;
  const double _t481 = _t200*_t89;
  const double _t482 = _t412*_t64;
  const double _t485 = _t232*_t297;
  const double _t486 = _hc1[3]*_t234;
  const double _t487 = _t474*_t486;
  const double _t488 = _t209*_t232;
  const double _t489 = _t425*_t64;
  const double _t490 = _t208*_t243;
  const double _t491 = _hc1[2]*_t61;
  const double _t492 = _t116*_t491;
  const double _t495 = _t209*_t494;
  const double _t496 = _hc1[0]*_t239;
  const double _t497 = _t223*_t496;
  const double _t498 = _t125*_t200;
  const double _t499 = _t12*_t246;
  const double _t500 = _t12*_t243;
  const double _t501 = _t195*_t64;
  const double _t502 = _t458*_t65;
  const double _t503 = _t366*_t61;
  const double _t504 = _t183*_t458;
  const double _t505 = _t212*_t464;
  const double _t506 = _t217*_t464;
  const double _t507 = _t464*_t94;
  const double _t508 = _t200*_t507;
  const double _t509 = _t125*_t464;
  const double _t510 = _t200*_t509;
  const double _t511 = _t349*na;
  const double _t512 = _t352*na;
  const double _t513 = _hc1[1]*_t260;
  const double _t514 = _t269*_t395;
  const double _t515 = _t514*na;
  const double _t516 = _t183*_t276;
  const double _t517 = _t516*na;
  const double _t518 = _t351*_t66;
  const double _t519 = _t518*na;
  const double _t520 = _t50*_t96;
  const double _t521 = _t200*_t359;
  const double _t522 = _t245*_t292;
  const double _t523 = _t201*_t304;
  const double _t524 = _t197*_t360;
  const double _t526 = _t191*_t432;
  const double _t527 = _t223*_t525;
  const double _t528 = _t191*_t438;
  const double _t529 = _t223*_t501;
  const double _t533 = _t239*_t257;
  const double _t534 = _t533*na;
  const double _t536 = _t178*_t535;
  const double _t537 = _t189*_t268;
  const double _t538 = _t232*_t537;
  const double _t539 = _t116*_t268;
  const double _t540 = _t243*_t539;
  const double _t542 = _t241*_t541;
  const double _t543 = _hc1[3]*_t531;
  const double _t544 = _t239*_t69;
  const double _t546 = -_hc0[0]*_hc1[1]*_t530 - _hc0[0]*_hc1[2]*_t232*_t61 - _hc0[0]*_hc1[2]*_t532*_t61 - _hc0[0]*_hc1[3]*_t531 - 0.12e2*_hc1[0]*_hc3[0]*_t10*_t17*_t239*na - 0.4e1*_hc1[0]*_hc3[0]*_t10*_t17*_t535*na*nb - 0.12e2*_hc1[1]*_hc3[0]*_t10*_t12*_t17*_t232*na - 0.4e1*_hc1[1]*_hc3[0]*_t10*_t12*_t17*_t530*na*nb - 0.12e2*_hc1[1]*_hc3[0]*_t10*_t17*_t232*_t64*na*nb - 0.12e2*_hc1[1]*_hc3[0]*_t10*_t17*_t239*_t61*na*nb - 0.24e2*_hc1[1]*_hc3[0]*_t10*_t17*_t61*_t64*na - 0.4e1*_hc1[2]*_hc3[0]*_t10*_t12*_t17*_t232*_t61*na*nb - 0.12e2*_hc1[2]*_hc3[0]*_t10*_t12*_t17*_t234*na - 0.4e1*_hc1[2]*_hc3[0]*_t10*_t12*_t17*_t532*_t61*na*nb - 0.12e2*_hc1[2]*_hc3[0]*_t10*_t17*_t234*_t64*na*nb - 0.4e1*_hc1[3]*_hc3[0]*_t10*_t12*_t17*_t531*na*nb + _t205*_t543 + _t232*_t422 + _t242*_t545 + _t244*_t545 + _t286*_t544 + _t34*_t536 + _t405*_t530 + _t422*_t532 + _t534 + _t538*na + _t540*na + _t542*na;
  const double _t547 = 0.24e2*_t236;
  const double _t548 = _t197*_t541;
  const double _t549 = _t276*_t65;
  const double _t550 = _t241*_t274;
  const double _t551 = _t232*_t370;
  const double _t552 = _t116*_t243;
  const double _t554 = _t213*_t295;
  const double _t556 = _t218*_t295;
  const double _t557 = _t220*_t295;
  const double _t558 = _t200*_t557;
  const double _t559 = _t200*_t223;
  const double d3F_dna3 = _t1*(_hc0[0]*_hc1[1]*_t253 + _hc0[0]*_hc1[2]*_t135*_t25 + _hc0[0]*_hc1[2]*_t25*_t256 + _hc0[0]*_hc1[3]*_t254 + 0.24e2*_hc1[0]*_hc2[0]*_t149*_t26*na*nb + 0.48e2*_hc1[0]*_hc2[0]*_t26*_t32*nb + 0.2e1*_hc1[0]*_hc2[2]*_t149*_t2*_t3*_t53*na*nb + 0.4e1*_hc1[0]*_hc2[2]*_t2*_t3*_t32*_t53*nb + 0.12e2*_hc1[0]*_hc3[0]*_t10*_t149*_t17*nb + 0.4e1*_hc1[0]*_hc3[0]*_t10*_t17*_t265*na*nb + 0.24e2*_hc1[1]*_hc2[0]*_t12*_t135*_t26*na*nb + 0.48e2*_hc1[1]*_hc2[0]*_t12*_t25*_t26*nb + 0.48e2*_hc1[1]*_hc2[0]*_t25*_t26*_t32*na*nb + 0.2e1*_hc1[1]*_hc2[2]*_t12*_t135*_t2*_t3*_t53*na*nb + 0.4e1*_hc1[1]*_hc2[2]*_t12*_t2*_t25*_t3*_t53*nb + 0.4e1*_hc1[1]*_hc2[2]*_t2*_t25*_t3*_t32*_t53*na*nb + 0.12e2*_hc1[1]*_hc3[0]*_t10*_t12*_t135*_t17*nb + 0.4e1*_hc1[1]*_hc3[0]*_t10*_t12*_t17*_t253*na*nb + 0.12e2*_hc1[1]*_hc3[0]*_t10*_t135*_t17*_t32*na*nb + 0.12e2*_hc1[1]*_hc3[0]*_t10*_t149*_t17*_t25*na*nb + 0.24e2*_hc1[1]*_hc3[0]*_t10*_t17*_t25*_t32*nb + 0.24e2*_hc1[2]*_hc2[0]*_t12*_t138*_t26*na*nb + 0.2e1*_hc1[2]*_hc2[2]*_t12*_t138*_t2*_t3*_t53*na*nb + 0.4e1*_hc1[2]*_hc3[0]*_t10*_t12*_t135*_t17*_t25*na*nb + 0.12e2*_hc1[2]*_hc3[0]*_t10*_t12*_t138*_t17*nb + 0.4e1*_hc1[2]*_hc3[0]*_t10*_t12*_t17*_t25*_t256*na*nb + 0.12e2*_hc1[2]*_hc3[0]*_t10*_t138*_t17*_t32*na*nb + 0.4e1*_hc1[3]*_hc3[0]*_t10*_t12*_t17*_t254*na*nb - _t121*_t294 - _t157*_t260 - _t163*_t260 - _t224*_t295 - _t259 - _t267 - _t271 - _t273 - _t275 - _t278 - _t281 - _t284 - _t287 - _t289 - _t291 - _t293*na - _t296 - _t299 - _t300 - _t301*_t302 - _t301*_t303 - _t305*nb - _t307*_t308 - _t309*_t311 - _t311*_t312 - _t313*_t315 - _t347 - _t363) + _t113*_t274 - _t117*_t274 + _t121*_t274 - _t121*_t374 + 0.3e1*_t137 + 0.3e1*_t140 + _t198*_t295 + _t215*_t295*na + _t221*_t295*na - _t224*_t313 - _t259*na - _t269*_t371 - _t273*na - _t275*na - _t278*na + _t286*_t302 + _t286*_t303 - _t295*_t58 - _t296*na - _t364*nb + _t365*nb - _t367 - _t368*nb + _t369*na*nb + _t372*na + _t373*na*nb + _t375*na + _t381;
  const double d3F_dna2_dnb = _t1*(_hc0[0]*_hc1[1]*_t382 + _hc0[0]*_hc1[2]*_t135*_t61 + 0.4e1*_hc0[0]*_hc1[2]*_t25*_t26*_t7 + _hc0[0]*_hc1[3]*_t138*_t384 + 0.8e1*_hc1[0]*_hc2[0]*_t149*_t26*na*nb + 0.16e2*_hc1[0]*_hc2[0]*_t26*_t32*nb + (0.2e1 / 0.3e1)*_hc1[0]*_hc2[2]*_t149*_t2*_t3*_t53*na*nb + (0.4e1 / 0.3e1)*_hc1[0]*_hc2[2]*_t2*_t3*_t32*_t53*nb + 0.4e1*_hc1[0]*_hc3[0]*_t10*_t149*_t17*na + 0.8e1*_hc1[0]*_hc3[0]*_t10*_t17*_t177*nb + 0.8e1*_hc1[0]*_hc3[0]*_t10*_t17*_t32 + 0.4e1*_hc1[0]*_hc3[0]*_t10*_t17*_t393*na*nb + 0.24e2*_hc1[0]*_hc3[0]*_t12*_t17*_t83*na + 0.48e2*_hc1[0]*_hc3[0]*_t12*_t17*_t83*nb + 0.48e2*_hc1[0]*_hc3[0]*_t17*_t32*_t83*na*nb + 0.24e2*_hc1[0]*_hc3[0]*_t17*_t64*_t83*na*nb + (0.32e2 / 0.9e1)*_hc1[0]*_hc3[1]*_t12*_t17*_t2*_t3*_t93*na + (0.64e2 / 0.9e1)*_hc1[0]*_hc3[1]*_t12*_t17*_t2*_t3*_t93*nb + (0.64e2 / 0.9e1)*_hc1[0]*_hc3[1]*_t17*_t2*_t3*_t32*_t93*na*nb + (0.32e2 / 0.9e1)*_hc1[0]*_hc3[1]*_t17*_t2*_t3*_t64*_t93*na*nb + (0.1e1 / 0.9e1)*_hc1[0]*_hc3[6]*_t12*_t17*_t74*_t75*_t99*na + (0.2e1 / 0.9e1)*_hc1[0]*_hc3[6]*_t12*_t17*_t74*_t75*_t99*nb + (0.2e1 / 0.9e1)*_hc1[0]*_hc3[6]*_t17*_t32*_t74*_t75*_t99*na*nb + (0.1e1 / 0.9e1)*_hc1[0]*_hc3[6]*_t17*_t64*_t74*_t75*_t99*na*nb + 0.8e1*_hc1[1]*_hc2[0]*_t12*_t135*_t26*na*nb + 0.16e2*_hc1[1]*_hc2[0]*_t12*_t25*_t26*nb + 0.32e2*_hc1[1]*_hc2[0]*_t12*_t337*_t7*na*nb + 0.16e2*_hc1[1]*_hc2[0]*_t25*_t26*_t32*na*nb + (0.2e1 / 0.3e1)*_hc1[1]*_hc2[2]*_t12*_t135*_t2*_t3*_t53*na*nb + (0.4e1 / 0.3e1)*_hc1[1]*_hc2[2]*_t12*_t2*_t25*_t3*_t53*nb + (0.4e1 / 0.3e1)*_hc1[1]*_hc2[2]*_t2*_t25*_t3*_t32*_t53*na*nb + 0.4e1*_hc1[1]*_hc3[0]*_t10*_t12*_t135*_t17*na + 0.8e1*_hc1[1]*_hc3[0]*_t10*_t12*_t17*_t25 + 0.4e1*_hc1[1]*_hc3[0]*_t10*_t12*_t17*_t382*na*nb + 0.4e1*_hc1[1]*_hc3[0]*_t10*_t135*_t17*_t64*na*nb + 0.4e1*_hc1[1]*_hc3[0]*_t10*_t149*_t17*_t61*na*nb + 0.8e1*_hc1[1]*_hc3[0]*_t10*_t17*_t177*_t25*na*nb + 0.8e1*_hc1[1]*_hc3[0]*_t10*_t17*_t25*_t32*na + 0.8e1*_hc1[1]*_hc3[0]*_t10*_t17*_t25*_t64*nb + 0.8e1*_hc1[1]*_hc3[0]*_t10*_t17*_t32*_t61*nb + 0.16e2*_hc1[1]*_hc3[0]*_t12*_t17*_t188*_t7*nb + 0.48e2*_hc1[1]*_hc3[0]*_t12*_t17*_t25*_t83*na*nb + 0.24e2*_hc1[1]*_hc3[0]*_t12*_t17*_t61*_t83*na*nb + 0.16e2*_hc1[1]*_hc3[0]*_t17*_t188*_t32*_t7*na*nb + (0.64e2 / 0.9e1)*_hc1[1]*_hc3[1]*_t12*_t17*_t2*_t25*_t3*_t93*na*nb + (0.32e2 / 0.9e1)*_hc1[1]*_hc3[1]*_t12*_t17*_t2*_t3*_t61*_t93*na*nb + (0.2e1 / 0.9e1)*_hc1[1]*_hc3[6]*_t12*_t17*_t25*_t74*_t75*_t99*na*nb + (0.1e1 / 0.9e1)*_hc1[1]*_hc3[6]*_t12*_t17*_t61*_t74*_t75*_t99*na*nb + 0.2e1*_hc1[1]*_t25*_t80 + _hc1[1]*_t61*_t80 + 0.8e1*_hc1[2]*_hc2[0]*_t12*_t138*_t26*na*nb + (0.2e1 / 0.3e1)*_hc1[2]*_hc2[2]*_t12*_t138*_t2*_t3*_t53*na*nb + 0.4e1*_hc1[2]*_hc3[0]*_t10*_t12*_t135*_t17*_t61*na*nb + 0.4e1*_hc1[2]*_hc3[0]*_t10*_t12*_t138*_t17*na + 0.8e1*_hc1[2]*_hc3[0]*_t10*_t12*_t17*_t25*_t61*nb + 0.4e1*_hc1[2]*_hc3[0]*_t10*_t138*_t17*_t64*na*nb + 0.8e1*_hc1[2]*_hc3[0]*_t10*_t17*_t25*_t32*_t61*na*nb + 0.16e2*_hc1[2]*_hc3[0]*_t12*_t17*_t188*_t25*_t7*na*nb + 0.4e1*_hc1[3]*_hc3[0]*_t10*_t12*_t138*_t17*_t384*na*nb - _t113*_t396 - _t117*_t388 - _t121*_t396 - _t122*_t167 - _t127*nb - _t130*_t436 - _t135*_t403 - _t135*_t422 - _t141*_t386 - _t141*_t387 - _t144*_t192 - _t144*_t193 - _t144*_t411 - _t151 - _t152*_t238 - _t154*_t354 - _t154*_t409 - _t154*_t410 - _t154*_t426 - _t155*_t407 - _t156 - _t157*_t404 - _t158*_t407 - _t159 - _t161*_t52 - _t162*_t395 - _t162*_t406 - _t195*_t408 - _t205*_t424 - _t214*_t435 - _t225*_t309 - _t225*_t312 - _t302*_t52 - _t303*_t52 - _t314*_t439 - _t32*_t385 - _t361*_t434 - _t362*_t434 - _t382*_t405 - _t390*nb - _t393*_t394 - _t399 - _t402*na - _t413*_t416 - _t416*_t428 - _t417*_t92 - _t419*na - _t420*nb - _t429*_t431 - _t432*_t433 - _t433*_t438 - _t457 - _t66*_t92 - _t87 - _t91 - _t97*na) + _t117*_t463 - _t121*_t463 + _t168 + _t231 - _t333*_t458 - 0.2e1 / 0.3e1*_t367 + _t459*nb + _t460*na*nb - _t461*_t54 + _t465*nb - _t466*nb + _t467*_t54 - _t468*_t54 + _t469*_t54 - _t470*_t54 + _t473;
  const double d3F_dna_dnb2 = _t1*(_hc0[0]*_hc1[2]*_t232*_t25 + 0.4e1*_hc0[0]*_hc1[2]*_t26*_t61*_t7 + _hc0[0]*_hc1[3]*_t234*_t474 + 0.8e1*_hc1[0]*_hc2[0]*_t239*_t26*na*nb + 0.16e2*_hc1[0]*_hc2[0]*_t26*_t64*na + (0.2e1 / 0.3e1)*_hc1[0]*_hc2[2]*_t2*_t239*_t3*_t53*na*nb + (0.4e1 / 0.3e1)*_hc1[0]*_hc2[2]*_t2*_t3*_t53*_t64*na + 0.8e1*_hc1[0]*_hc3[0]*_t10*_t17*_t177*na + 0.4e1*_hc1[0]*_hc3[0]*_t10*_t17*_t239*nb + 0.4e1*_hc1[0]*_hc3[0]*_t10*_t17*_t475*na*nb + 0.8e1*_hc1[0]*_hc3[0]*_t10*_t17*_t64 + 0.48e2*_hc1[0]*_hc3[0]*_t12*_t17*_t83*na + 0.24e2*_hc1[0]*_hc3[0]*_t12*_t17*_t83*nb + 0.24e2*_hc1[0]*_hc3[0]*_t17*_t32*_t83*na*nb + 0.48e2*_hc1[0]*_hc3[0]*_t17*_t64*_t83*na*nb + (0.64e2 / 0.9e1)*_hc1[0]*_hc3[1]*_t12*_t17*_t2*_t3*_t93*na + (0.32e2 / 0.9e1)*_hc1[0]*_hc3[1]*_t12*_t17*_t2*_t3*_t93*nb + (0.32e2 / 0.9e1)*_hc1[0]*_hc3[1]*_t17*_t2*_t3*_t32*_t93*na*nb + (0.64e2 / 0.9e1)*_hc1[0]*_hc3[1]*_t17*_t2*_t3*_t64*_t93*na*nb + (0.2e1 / 0.9e1)*_hc1[0]*_hc3[6]*_t12*_t17*_t74*_t75*_t99*na + (0.1e1 / 0.9e1)*_hc1[0]*_hc3[6]*_t12*_t17*_t74*_t75*_t99*nb + (0.1e1 / 0.9e1)*_hc1[0]*_hc3[6]*_t17*_t32*_t74*_t75*_t99*na*nb + (0.2e1 / 0.9e1)*_hc1[0]*_hc3[6]*_t17*_t64*_t74*_t75*_t99*na*nb + 0.8e1*_hc1[1]*_hc2[0]*_t12*_t188*na*nb + 0.8e1*_hc1[1]*_hc2[0]*_t12*_t232*_t26*na*nb + 0.16e2*_hc1[1]*_hc2[0]*_t12*_t26*_t61*na + 0.56e2*_hc1[1]*_hc2[0]*_t12*_t337*_t7*na*nb + 0.16e2*_hc1[1]*_hc2[0]*_t26*_t61*_t64*na*nb + (0.2e1 / 0.3e1)*_hc1[1]*_hc2[2]*_t12*_t2*_t232*_t3*_t53*na*nb + (0.4e1 / 0.3e1)*_hc1[1]*_hc2[2]*_t12*_t2*_t3*_t53*_t61*na + (0.4e1 / 0.3e1)*_hc1[1]*_hc2[2]*_t2*_t3*_t53*_t61*_t64*na*nb + 0.4e1*_hc1[1]*_hc3[0]*_t10*_t12*_t17*_t232*nb + 0.8e1*_hc1[1]*_hc3[0]*_t10*_t12*_t17*_t61 + 0.8e1*_hc1[1]*_hc3[0]*_t10*_t17*_t177*_t61*na*nb + 0.4e1*_hc1[1]*_hc3[0]*_t10*_t17*_t232*_t32*na*nb + 0.4e1*_hc1[1]*_hc3[0]*_t10*_t17*_t239*_t25*na*nb + 0.8e1*_hc1[1]*_hc3[0]*_t10*_t17*_t25*_t64*na + 0.8e1*_hc1[1]*_hc3[0]*_t10*_t17*_t32*_t61*na + 0.8e1*_hc1[1]*_hc3[0]*_t10*_t17*_t61*_t64*nb + 0.16e2*_hc1[1]*_hc3[0]*_t12*_t17*_t188*_t7*na + 0.24e2*_hc1[1]*_hc3[0]*_t12*_t17*_t25*_t83*na*nb + 0.48e2*_hc1[1]*_hc3[0]*_t12*_t17*_t61*_t83*na*nb + 0.16e2*_hc1[1]*_hc3[0]*_t17*_t188*_t64*_t7*na*nb + (0.32e2 / 0.9e1)*_hc1[1]*_hc3[1]*_t12*_t17*_t2*_t25*_t3*_t93*na*nb + (0.64e2 / 0.9e1)*_hc1[1]*_hc3[1]*_t12*_t17*_t2*_t3*_t61*_t93*na*nb + (0.1e1 / 0.9e1)*_hc1[1]*_hc3[6]*_t12*_t17*_t25*_t74*_t75*_t99*na*nb + (0.2e1 / 0.9e1)*_hc1[1]*_hc3[6]*_t12*_t17*_t61*_t74*_t75*_t99*na*nb + _hc1[1]*_t25*_t80 + 0.2e1*_hc1[1]*_t61*_t80 + 0.8e1*_hc1[2]*_hc2[0]*_t12*_t234*_t26*na*nb + (0.2e1 / 0.3e1)*_hc1[2]*_hc2[2]*_t12*_t2*_t234*_t3*_t53*na*nb + 0.4e1*_hc1[2]*_hc3[0]*_t10*_t12*_t17*_t232*_t25*na*nb + 0.4e1*_hc1[2]*_hc3[0]*_t10*_t12*_t17*_t234*nb + 0.8e1*_hc1[2]*_hc3[0]*_t10*_t12*_t17*_t25*_t61*na + 0.4e1*_hc1[2]*_hc3[0]*_t10*_t17*_t234*_t32*na*nb + 0.8e1*_hc1[2]*_hc3[0]*_t10*_t17*_t25*_t61*_t64*na*nb + 0.16e2*_hc1[2]*_hc3[0]*_t12*_t17*_t188*_t61*_t7*na*nb + 0.4e1*_hc1[3]*_hc3[0]*_t10*_t12*_t17*_t234*_t474*na*nb - _t114*_t183 - _t114*_t201 - _t117*_t483 - _t122*_t249 - _t13*_t240 - _t136*_t252 - _t136*_t27 - _t144*_t241 - _t154*_t395 - _t154*_t480 - _t154*_t481 - _t154*_t489 - _t162*_t354 - _t162*_t479 - _t180*_t242 - _t180*_t244 - _t188*_t416*_t492 - _t192*_t238 - _t193*_t238 - _t197*_t388 - _t197*_t484 - _t205*_t485 - _t205*_t487 - _t207*_t242 - _t207*_t244 - _t211 - _t214*_t497 - _t217*_t448 - _t225*_t499 - _t225*_t500 - _t232*_t403 - _t238*_t411 - _t243*_t404 - _t245*_t52 - _t361*_t433 - _t362*_t433 - _t37*_t408 - _t385*_t64 - _t387*_t50 - _t390*na - _t394*_t475 - _t397*_t414 - _t416*_t482 - _t420*na - _t432*_t434 - _t434*_t438 - _t439*_t501 - _t448*_t498 - _t457 - _t471 - _t476*_t84 - _t478*_t66 - _t488*_t52 - _t490*_t52 - 0.56e2*_t495 - _t86*nb - _t90*nb - _t97*nb) + _t131 + _t197*_t463 - _t201*_t463 + _t250 + _t459*na - _t462*_t49 + _t465*na - _t466*na + _t473 + _t502*na*nb - 0.2e1 / 0.3e1*_t503 - _t504*_t54 + _t505*_t54 - _t506*_t54 + _t508*_t54 - _t510*_t54;
  const double d3F_dnb3 = _t1*(0.24e2*_hc1[0]*_hc2[0]*_t239*_t26*na*nb + 0.48e2*_hc1[0]*_hc2[0]*_t26*_t64*na + 0.2e1*_hc1[0]*_hc2[2]*_t2*_t239*_t3*_t53*na*nb + 0.4e1*_hc1[0]*_hc2[2]*_t2*_t3*_t53*_t64*na + 0.72e2*_hc1[0]*_hc3[0]*_t12*_t17*_t83*na + 0.72e2*_hc1[0]*_hc3[0]*_t17*_t64*_t83*na*nb + (0.32e2 / 0.3e1)*_hc1[0]*_hc3[1]*_t12*_t17*_t2*_t3*_t93*na + (0.32e2 / 0.3e1)*_hc1[0]*_hc3[1]*_t17*_t2*_t3*_t64*_t93*na*nb + (0.1e1 / 0.3e1)*_hc1[0]*_hc3[6]*_t12*_t17*_t74*_t75*_t99*na + (0.1e1 / 0.3e1)*_hc1[0]*_hc3[6]*_t17*_t64*_t74*_t75*_t99*na*nb + 0.24e2*_hc1[1]*_hc2[0]*_t12*_t232*_t26*na*nb + 0.48e2*_hc1[1]*_hc2[0]*_t12*_t26*_t61*na + 0.48e2*_hc1[1]*_hc2[0]*_t26*_t61*_t64*na*nb + 0.2e1*_hc1[1]*_hc2[2]*_t12*_t2*_t232*_t3*_t53*na*nb + 0.4e1*_hc1[1]*_hc2[2]*_t12*_t2*_t3*_t53*_t61*na + 0.4e1*_hc1[1]*_hc2[2]*_t2*_t3*_t53*_t61*_t64*na*nb + 0.72e2*_hc1[1]*_hc3[0]*_t12*_t17*_t61*_t83*na*nb + (0.32e2 / 0.3e1)*_hc1[1]*_hc3[1]*_t12*_t17*_t2*_t3*_t61*_t93*na*nb + (0.1e1 / 0.3e1)*_hc1[1]*_hc3[6]*_t12*_t17*_t61*_t74*_t75*_t99*na*nb + 0.3e1*_hc1[1]*_t61*_t80 + 0.24e2*_hc1[2]*_hc2[0]*_t12*_t234*_t26*na*nb + 0.2e1*_hc1[2]*_hc2[2]*_t12*_t2*_t234*_t3*_t53*na*nb - _t200*_t527 - _t218*_t525 - _t232*_t513 - _t243*_t260 - _t249*_t374 - _t301*_t488 - _t301*_t490 - _t310*_t497 - _t311*_t499 - _t311*_t500 - _t313*_t529 - _t347 - _t511 - _t512 - _t515 - _t517 - _t519 - _t520 - _t521 - _t522*na - _t523 - _t524 - _t526 - _t528 - _t546) + _t183*_t541*na + _t197*_t304*nb + _t198*_t525 + _t201*_t553 + 0.3e1*_t233 + 0.3e1*_t235 + _t245*_t286 + _t249*_t555 - _t269*_t551 + _t286*_t488 + _t286*_t490 - _t286*_t552 - _t313*_t559 + _t365*na - _t368*na + _t381 - _t503 - _t517*nb - _t523*nb - _t525*_t58 - _t534*nb - _t547*na - _t548*na + _t549*na*nb - _t550*na + _t554*na - _t556*na + _t558*na;
  if(out->v3rho3 != NULL) out->v3rho3[ip*p->dim.v3rho3 + 0] += d3F_dna3;
...
#endif
#if _KMAX >= 4
  const double _t560 = _t262*_t7;
  const double _t561 = _t560 - _t84;
  const double _t562 = (_t135 * _t135);
  const double _t563 = (_t25 * _t25 * _t25 * _t25);
  const double _t571 = _t391*_t7;
  const double _t572 = _t261 - _t571;
  const double _t574 = 0.6e1*_t10;
  const double _t575 = 0.6e1*na - 0.6e1*nb;
  const double _t576 = _t26*_t575 - _t574;
  const double _t579 = -_t295 + _t525;
  const double _t580 = -_t13 + _t26*_t579;
  const double _t581 = -_t132 + _t26*_t31;
  const double _t582 = 0.288e3*nb;
  const double _t584 = 0.48e2*_t153;
  const double _t590 = 0.16e2*_t153;
  const double _t592 = 0.96e2*_t51;
  const double _t596 = 0.384e3*nb;
  const double _t598 = _t188*_t596;
  const double _t600 = _t582*_t83;
  const double _t602 = 0.144e3*nb;
  const double _t603 = _t602*_t83;
  const double _t604 = _t603*na;
  const double _t607 = _t32*_t584;
  const double _t614 = _t590*na;
  const double _t616 = _t253*_t590;
  const double _t618 = _t188*_t31;
  const double _t619 = _t11*_t337;
  const double _t620 = _t351 + 0.120e3*_t619;
  const double _t621 = -0.96e2*_t618 + _t620;
  const double _t625 = 0.192e3*_t51;
  const double _t631 = _t598*na;
  const double _t638 = _t607*na;
  const double _t639 = 0.32e2*nb;
  const double _t640 = _t149*_t555;
  const double _t642 = _t50 - 0.8e1*nb;
  const double _t643 = _t148 - _t26*_t642;
  const double _t644 = _t555*_t643;
  const double _t645 = _t265*_t614;
  const double _t646 = _t32*_t614;
  const double _t662 = _t592*na;
  const double _t667 = _t32*_t662;
  const double _t669 = 0.16e2*_t54;
  const double _t690 = _t401*na;
  const double _t691 = _t600*na;
  const double _t700 = (0.128e3 / 0.3e1)*nb;
  const double _t702 = 0.32e2*_t54;
  const double _t706 = _t12*_t141;
  const double _t716 = _t336*_t54;
  const double _t748 = (0.140e3 / 0.81e2)*_t93;
  const double _t749 = (0.7e1 / 0.27e2)*_t99;
  const double _t750 = _t2/(M_PI * M_CBRTPI);
  const double _t751 = 0.480e3*_t337;
  const double _t752 = _t445*_t54;
  const double _t753 = xc_powi(_t1, -7);
  const double _t755 = xc_powr(_t1, -22, 3);
  const double _t763 = 0.96e2*_t153;
  const double _t785 = -_t391 + _t560;
  const double _t787 = 0.4e1*_t26 - _t571;
  const double _t789 = _t254*_t384;
  const double _t794 = _t393*nb;
  const double _t798 = _t251*_t7;
  const double _t799 = _t149*_t268;
  const double _t802 = _t26*na;
  const double _t803 = 0.24e2*_t802;
  const double _t807 = _t401*_t7;
  const double _t810 = _t268*na;
  const double _t812 = _t269*_t64;
  const double _t813 = _t32*_t810;
  const double _t814 = -0.120e3*_t11*_t337;
  const double _t815 = -_t262*_t63 - 0.72e2*_t618 - _t814;
  const double _t817 = 0.4e1*_t53;
  const double _t824 = 0.72e2*_t51;
  const double _t826 = 0.96e2*nb;
  const double _t827 = _t337*_t7;
  const double _t830 = _t188*_t582*na;
  const double _t831 = _t602*_t753;
  const double _t834 = _t351*nb;
  const double _t835 = _t188*_t807;
  const double _t839 = _t177*_t286;
  const double _t841 = _t286*_t382;
  const double _t843 = _t26*_t579;
  const double _t844 = _t286*(_t133 + _t147 - _t26*_t579 + _t843);
  const double _t847 = _t266*nb;
  const double _t850 = 0.2e1*_t7;
  const double _t857 = _t269*_t382;
  const double _t859 = _t301*_t64;
  const double _t864 = _t334*nb;
  const double _t865 = _t827*_t864;
  const double _t871 = _t401*_t414;
  const double _t872 = _t32*_t871;
  const double _t874 = _t301*_t32;
  const double _t875 = _t54*_t560;
  const double _t878 = _t286*_t32;
  const double _t881 = _t191*_t7;
  const double _t882 = _t188*_t881;
  const double _t887 = _t310*_t393;
  const double _t895 = 0.12e2*_t54;
  const double _t922 = _t310*_t382;
  const double _t938 = 0.36e2*_t54;
  const double _t945 = (0.2e1 / 0.3e1)*nb;
  const double _t963 = (0.32e2 / 0.3e1)*_t54;
  const double _t973 = _t351*na;
  const double _t978 = (0.32e2 / 0.3e1)*na;
  const double _t979 = (0.32e2 / 0.3e1)*nb;
  const double _t991 = 0.216e3*_t54;
  const double _t994 = 0.36e2*_t153;
  const double _t999 = _t83*_t991;
  const double _t1013 = 0.24e2*_t54;
  const double _t1034 = _t132 + _t26*_t31;
  const double _t1036 = _t138*_t234;
  const double _t1039 = 0.8e1*_t26;
  const double _t1040 = 0.96e2*_t83;
  const double _t1044 = 0.16e2*_t10;
  const double _t1046 = _t1044*_t32;
  const double _t1048 = _t393*_t50;
  const double _t1053 = 0.192e3*na;
  const double _t1054 = 0.192e3*_t188;
  const double _t1055 = _t334*_t83;
  const double _t1056 = _t458*nb;
  const double _t1057 = _t430*na;
  const double _t1058 = _t430*nb;
  const double _t1061 = _t1044*na;
  const double _t1062 = _t238*_t64;
  const double _t1063 = _t144*_t32;
  const double _t1064 = _t188*_t63;
  const double _t1065 = -0.48e2*_t1064 - 0.48e2*_t618 - _t814 - _t84;
  const double _t1073 = _t1054*_t54;
  const double _t1074 = _t1055*nb;
  const double _t1075 = 0.64e2*_t414;
  const double _t1077 = _t11*_t702/xc_powi(_t1, 8);
  const double _t1078 = _t1058*_t188;
  const double _t1079 = _t84*nb;
  const double _t1080 = _t1046*na;
  const double _t1081 = _t154*_t475;
  const double _t1082 = _t1048*_t153;
  const double _t1083 = _t154*_t64;
  const double _t1084 = _t1083*_t382;
  const double _t1086 = _t154*_t32;
  const double _t1088 = _t144*_t232;
  const double _t1090 = _t162*_t239;
  const double _t1091 = _t149*_t162;
  const double _t1092 = _t148 + _t26*_t642;
  const double _t1093 = _t1092*_t162;
  const double _t1094 = _t162*_t643;
  const double _t1097 = _t122*_t64;
  const double _t1098 = _t122*_t32;
  const double _t1101 = _t1075*_t493;
  const double _t1103 = 0.112e3*_t494;
  const double _t1104 = (0.16e2 / 0.3e1)*na;
  const double _t1105 = (0.16e2 / 0.3e1)*nb;
  const double _t1106 = (0.4e1 / 0.3e1)*_t7;
  const double _t1111 = _t431*_t64;
  const double _t1113 = _t188*_t431;
  const double _t1114 = _t1113*_t32;
  const double _t1115 = _t177*_t614;
  const double _t1130 = _t475*na;
  const double _t1136 = (0.16e2 / 0.3e1)*_t414;
  const double _t1139 = (0.128e3 / 0.9e1)*na;
  const double _t1140 = (0.128e3 / 0.9e1)*nb;
  const double _t1169 = _t1105*_t414;
  const double _t1188 = _t596*_t753;
  const double _t1192 = 0.64e2*_t802;
  const double _t1193 = 0.64e2*_t51;
  const double _t1194 = 0.144e3*_t83*na;
  const double _t1195 = 0.16e2*na;
  const double _t1196 = _t1193*na;
  const double _t1200 = _t474*_t531;
  const double _t1201 = 0.18e2*_t7*_t83;
  const double _t1208 = 0.288e3*na;
  const double _t1209 = _t334*_t493;
  const double _t1210 = _t477*_t7;
  const double _t1212 = -0.72e2*_t1064 - _t262*_t31 - _t814;
  const double _t1216 = _t262*_t64;
  const double _t1217 = _t335*nb;
  const double _t1218 = _t1210*_t188;
  const double _t1219 = _t1130*_t269;
  const double _t1221 = _t162*_t535;
  const double _t1229 = _t262*_t54;
  const double _t1231 = 0.168e3*_t494;
  const double _t1233 = _t188*_t871;
  const double _t1234 = _t1233*_t64;
  const double _t1235 = _t232*_t875;
  const double _t1245 = _t535*na;
  const double _t1246 = _t1245*_t945;
  const double _t1265 = 0.96e2*_t802;
  const double _t1267 = _t560 + _t84;
  const double _t1268 = (_t232 * _t232);
  const double _t1269 = (_t61 * _t61 * _t61 * _t61);
  const double _t1270 = -_t261 - _t571;
  const double _t1271 = _t13 + _t843;
  const double _t1272 = _t26*_t575 + _t574;
  const double _t1273 = 0.48e2*_t10;
  const double _t1274 = 0.384e3*na;
  const double _t1275 = _t1208*_t83;
  const double _t1276 = _t1273*na;
  const double _t1277 = _t1276*_t64;
  const double _t1279 = -0.96e2*_t1064 + _t620;
  const double _t1281 = 0.192e3*_t802;
  const double _t1283 = 0.32e2*na;
  const double _t1284 = _t239*_t555;
  const double _t1285 = _t1092*_t555;
  const double _t1286 = _t1245*_t590;
  const double _t1287 = _t614*_t64;
  const double _t1289 = _t64*_t662;
  const double _t1292 = _t232*_t532;
  const double _t1301 = (0.128e3 / 0.3e1)*na;
  const double _t1305 = _t10*_t334;
  const double _t1306 = _t584*na;
  const double _t1307 = _t1306*_t64;
  const double _t1308 = _t625*na;
  const double _t564 = _hc0[0]*_hc1[4];
  const double _t565 = _hc1[1]*_t320;
  const double _t566 = _t25*_t565;
  const double _t567 = _hc1[1]*_t80;
  const double _t568 = _t135*_t567;
  const double _t569 = _hc1[2]*_t80;
  const double _t570 = _t138*_t569;
  const double _t573 = _t135*_t139;
  const double _t577 = _hc0[0]*_hc1[3];
  const double _t578 = _t138*_t577;
  const double _t583 = _t400*_t582;
  const double _t585 = _t354*_t584;
  const double _t586 = 0.16e2*_t142;
  const double _t587 = _t586*nb;
  const double _t588 = _t265*_t587;
  const double _t589 = _t330*_t48;
  const double _t591 = _t589*_t590;
  const double _t593 = _t150*_t592;
  const double _t594 = _t348*_t592;
  const double _t595 = _t188*_t85;
  const double _t597 = _t595*_t596;
  const double _t599 = _t49*_t598;
  const double _t601 = _t117*_t600;
  const double _t605 = _t161*_t592;
  const double _t606 = _t285*_t584;
  const double _t608 = _t155*_t607;
  const double _t609 = _t584*_t89;
  const double _t610 = _t120*_t609;
  const double _t611 = _t158*_t607;
  const double _t612 = _t274*_t89;
  const double _t613 = _t612*na;
  const double _t615 = _t112*_t330;
  const double _t617 = _t116*_t590;
  const double _t622 = _t160*_t265;
  const double _t623 = _t178*_t265;
  const double _t624 = _t272*_t592;
  const double _t626 = _t152*_t625;
  const double _t627 = _t113*_t600;
  const double _t628 = (0.2e1 / 0.3e1)*_t366;
  const double _t629 = _t42*_t46;
  const double _t630 = (0.2e1 / 0.3e1)*_t629;
  const double _t632 = _t152*na;
  const double _t633 = _t167*_t625;
  const double _t634 = _t208*_t592;
  const double _t635 = _t163*_t634;
  const double _t636 = _t157*_t634;
  const double _t637 = _t37*_t89;
  const double _t641 = _t12*_t613;
  const double _t647 = _t253*_t68;
  const double _t648 = _t120*_t330;
  const double _t649 = _t297*_t617;
  const double _t650 = _hc2[0]*_t282;
  const double _t651 = _hc1[4]*_t205;
  const double _t652 = _t160*_t162;
  const double _t653 = _t220*_t306;
  const double _t654 = _t208*_t282;
  const double _t655 = _t189*_t253;
  const double _t656 = _t116*_t282;
  const double _t657 = _t336*_t95;
  const double _t658 = _t401*_t657;
  const double _t659 = _t149*_t166;
  const double _t660 = _t584*_t659;
  const double _t661 = _t18*_t607;
  const double _t663 = _t121*_t600;
  const double _t664 = (0.4e1 / 0.9e1)*_t55;
  const double _t665 = _t338*_t493;
  const double _t666 = _t297*_t630;
  const double _t668 = _t209*_t253;
  const double _t670 = _t93*_t94;
  const double _t671 = _t306*_t670;
  const double _t672 = _t256*_t297;
  const double _t673 = _hc2[0]*_t646;
  const double _t674 = _t135*_t297;
  const double _t675 = _t154*_t427;
  const double _t676 = _t135*_t421;
  const double _t677 = _t205*_t423;
  const double _t678 = _hc1[1]*_t18;
  const double _t679 = _t678*_t70;
  const double _t680 = _hc1[2]*_t216;
  const double _t681 = _hc1[4]*_t216;
  const double _t682 = _t12*_t220;
  const double _t683 = _t141*_t682;
  const double _t684 = _t220*_t314;
  const double _t685 = _hc1[1]*_t253;
  const double _t686 = _t18*_t646;
  const double _t687 = _t208*_t590;
  const double _t688 = _t427*_t463;
  const double _t689 = _t124*_t336;
  const double _t692 = _t345*nb;
  const double _t693 = _t343*nb;
  const double _t694 = _t112*_t17;
  const double _t695 = _t340*_t694;
  const double _t696 = _t101*_t694;
  const double _t697 = _t696*nb;
  const double _t698 = _hc1[0]*_t265;
  const double _t699 = _t507*_t54;
  const double _t701 = _t126*_t93;
  const double _t703 = _t314*_t670;
  const double _t704 = _t208*_t463;
  const double _t705 = _t669*_t670;
  const double _t707 = _t223*_t706;
  const double _t708 = _t216*_t297;
  const double _t709 = _t135*_t680;
  const double _t710 = _t216*_t423;
  const double _t711 = _t191*_t220;
  const double _t712 = _t149*_t37;
  const double _t713 = _t32*_t711;
  const double _t714 = _t208*_t297;
  const double _t715 = _t128*_t336;
  const double _t717 = _t126*_t716;
  const double _t718 = _t100*_t341*_t54;
  const double _t719 = _t694*_t718;
  const double _t720 = _t509*_t54;
  const double _t721 = _t120*_t17;
  const double _t722 = _t340*_t721;
  const double _t723 = _t101*_t17;
  const double _t724 = _t306*_t723;
  const double _t725 = _t101*_t721;
  const double _t726 = _t725*nb;
  const double _t727 = _hc1[1]*_t54;
  const double _t728 = _t253*_t727;
  const double _t729 = _t12*_t507;
  const double _t730 = _t12*_t699;
  const double _t731 = _t125*_t93;
  const double _t732 = _t306*_t54;
  const double _t733 = _t731*_t732;
  const double _t734 = _t130*_t93;
  const double _t735 = _t191*_t223;
  const double _t736 = _t32*_t735;
  const double _t737 = _t130*_t716;
  const double _t738 = _t718*_t721;
  const double _t739 = _t12*_t728;
  const double _t740 = _t12*_t282;
  const double _t741 = _t214*_t723;
  const double _t742 = _t54*_t723;
  const double _t743 = _t314*_t742;
  const double _t744 = _t54*_t731;
  const double _t745 = (0.64e2 / 0.3e1)*_t744;
  const double _t746 = (0.128e3 / 0.3e1)*_t744;
  const double _t747 = _t12*_t720;
  const double _t754 = _t54*_t55;
  const double _t756 = _hc1[0]*(_t319*_t376 - _t329*(-_hc0[5]*_t326 - _hc0[9]*_t327 - _t329*(_hc0[14]*_t82 + _hc0[9]*_t88)) + _t46*_t748 + _t749*_t77) + (0.1e1 / 0.54e2)*_hc3[56]*_t750*_t754*_t755 + (0.1e1 / 0.216e3)*_hc4[56]*_t336*_t750 + (0.2e1 / 0.9e1)*_t188*_t322 - _t19*_t54*(_hc2[5]*_t749*_t76 + _t328*_t376*_t4 - _t329*(-_hc2[5]*_t326 - _hc2[9]*_t327 - _t329*(_hc2[14]*_t82 + _hc2[9]*_t88)) + _t748*_t94) + (0.40e2 / 0.81e2)*_t324*_t99 + _t333*_t751*na + (0.16e2 / 0.9e1)*_t338*_t753*_t754 - _t348*_t604 + _t43*_t748 + _t463*_t589 - _t54*_t751*_t85 + (0.9680e4 / 0.81e2)*_t57*_t752 - 0.64e2*_t752*_t95 + (0.772e3 / 0.81e2)*_t342*_t54/xc_powr(_t1, 20, 3);
  const double _t757 = _t136*_t253;
  const double _t758 = _t254*_t577;
  const double _t759 = _t25*_t567;
  const double _t760 = _t173*_t256;
  const double _t761 = _t135*_t173;
  const double _t762 = _t142*_t149;
  const double _t764 = _t135*_t366;
  const double _t765 = _t157*_t629;
  const double _t766 = _t49*_t83;
  const double _t767 = _t208*_t584;
  const double _t768 = _t427*_t614;
  const double _t769 = _t101*_t55;
  const double _t770 = _t769*nb;
  const double _t771 = _t163*_t682;
  const double _t772 = _t157*_t682;
  const double _t773 = _t208*_t614;
  const double _t774 = _t54*_t696;
  const double _t775 = _t54*_t725;
  const double _t776 = (0.1e1 / 0.9e1)*_t323;
  const double _t777 = (0.56e2 / 0.27e2)*_t43*_t53;
  const double _t778 = (0.4e1 / 0.9e1)*_t325;
  const double _t779 = _t591*na;
  const double _t780 = _t599*na;
  const double _t781 = _t340*_t664;
  const double _t782 = (0.2096e4 / 0.27e2)*_t346;
  const double _t783 = (0.40e2 / 0.9e1)*_t344;
  const double _t784 = 0.4e1*_hc1[0]*_t320 + _t594*na + _t597*na + _t658*na - _t776 - _t777 - _t778 - _t779 - _t780 - _t781 - _t782 - _t783;
  const double _t786 = _t565*_t61;
  const double _t788 = _t139*_t61;
  const double _t790 = _t202*_t577;
  const double _t791 = _t400*_t602;
  const double _t792 = _t274*_t354;
  const double _t793 = _t152*_t541;
  const double _t795 = _t257*_t794;
  const double _t796 = _t135*_t537;
  const double _t797 = _t157*_t539;
  const double _t800 = _t160*_t799;
  const double _t801 = 0.24e2*_t150;
  const double _t804 = _t179*_t276;
  const double _t805 = _t348*_t51;
  const double _t806 = _t188*_t49;
  const double _t808 = _t274*_t409;
  const double _t809 = _t38*na;
  const double _t811 = _t189*_t382;
  const double _t816 = _t182*_t330;
  const double _t818 = _t160*_t266;
  const double _t819 = _t160*_t268;
  const double _t820 = _t208*_t268;
  const double _t821 = _t178*_t393;
  const double _t822 = _t395*na;
  const double _t823 = _t167*_t541;
  const double _t825 = _t188*_t66;
  const double _t828 = _t189*_t827;
  const double _t829 = _t188*_t357;
  const double _t832 = _t370*_t7;
  const double _t833 = _t209*_t827;
  const double _t836 = _t54*_t68;
  const double _t837 = _t560*_t836;
  const double _t838 = _t268*_t809;
  const double _t840 = _t32*_t68;
  const double _t842 = _t12*_t359;
  const double _t845 = _t116*_t269;
  const double _t846 = _t135*_t491;
  const double _t848 = _t330*_t70;
  const double _t849 = _t204*_t297;
  const double _t851 = _t46*_t93;
  const double _t852 = _t297*_t851;
  const double _t853 = _t220*_t446;
  const double _t854 = _t295*_t853;
  const double _t855 = _t18*_t39;
  const double _t856 = _t657*na;
  const double _t858 = _t149*_t196;
  const double _t860 = _t166*_t177;
  const double _t861 = _t657*nb;
  const double _t862 = _t32*_t678;
  const double _t863 = _t410*na;
  const double _t866 = _t297*_t404;
  const double _t867 = _t339*na;
  const double _t868 = _t201*nb;
  const double _t869 = _t188*_t297;
  const double _t870 = _hc2[0]*_t869;
  const double _t873 = _t209*_t382;
  const double _t876 = _t116*_t875;
  const double _t877 = _t286*_t425;
  const double _t879 = _hc2[0]*_t878;
  const double _t880 = _hc1[2]*_t116;
  const double _t883 = _t880*_t882;
  const double _t884 = _t223*_t446;
  const double _t885 = _hc2[0]*_t407;
  const double _t886 = _hc1[0]*_t220;
  const double _t888 = _t220*_t525;
  const double _t889 = _t450*_t557;
  const double _t890 = _t451*_t557;
  const double _t891 = _t18*_t407;
  const double _t892 = _t454*_t94;
  const double _t893 = _t7*_t892;
  const double _t894 = _t427*_t52;
  const double _t896 = _t212*_t336;
  const double _t897 = _t208*_t269;
  const double _t898 = _t560*_t727;
  const double _t899 = _t18*_t898;
  const double _t900 = _t116*_t301;
  const double _t901 = _t18*_t203;
  const double _t902 = _t32*_t901;
  const double _t903 = _t54*_t689;
  const double _t904 = _t209*_t414;
  const double _t905 = _t345*na;
  const double _t906 = _t343*na;
  const double _t907 = _t17*_t182;
  const double _t908 = _t340*_t907;
  const double _t909 = _t701*nb;
  const double _t910 = _t208*_t301;
  const double _t911 = _hc1[1]*_t12;
  const double _t912 = _t755*_t911;
  const double _t913 = _t912*_t94;
  const double _t914 = _t191*_t670;
  const double _t915 = _t455*_t7;
  const double _t916 = _t208*_t52;
  const double _t917 = _t223*_t295;
  const double _t918 = _hc1[3]*_t202;
  const double _t919 = _t205*_t918;
  const double _t920 = _hc1[0]*_t223;
  const double _t921 = _t223*_t29;
  const double _t923 = _t220*_t310;
  const double _t924 = _t149*_t195;
  const double _t925 = _t163*_t923;
  const double _t926 = _t157*_t923;
  const double _t927 = _t177*_t37;
  const double _t928 = _t557*na;
  const double _t929 = _t203*_t682;
  const double _t930 = _t216*_t491;
  const double _t931 = _t32*_t892;
  const double _t932 = _hc1[2]*_t208;
  const double _t933 = _t882*_t932;
  const double _t934 = _t336*_t437;
  const double _t935 = _t286*_t901;
  const double _t936 = _t18*_t878;
  const double _t937 = _t208*_t875;
  const double _t939 = _t18*_t869;
  const double _t940 = _t217*_t716;
  const double _t941 = _t718*_t907;
  const double _t942 = _t17*_t340;
  const double _t943 = _t54*_t724;
  const double _t944 = (0.2e1 / 0.3e1)*_t682;
  const double _t946 = _t214*_t682;
  const double _t947 = _t734*nb;
  const double _t948 = _t32*_t455;
  const double _t949 = _t223*_t313;
  const double _t950 = _t163*_t311;
  const double _t951 = _t157*_t311;
  const double _t952 = _t310*_t682;
  const double _t953 = _t203*_t32;
  const double _t954 = _t216*_t918;
  const double _t955 = _t445*_t94;
  const double _t956 = _t12*_t955;
  const double _t957 = _t297*_t956;
  const double _t958 = _t498*_t716;
  const double _t959 = _t17*_t200*_t718;
  const double _t960 = (0.2e1 / 0.3e1)*_t223;
  const double _t961 = (0.1e1 / 0.3e1)*_t723;
  const double _t962 = _t54*_t961;
  const double _t964 = _t731*_t963;
  const double _t965 = _t125*_t453*_t912;
  const double _t966 = _t125*_t445;
  const double _t967 = _t12*_t966;
  const double _t968 = _t297*_t967;
  const double _t969 = _t12*_t311;
  const double _t970 = _t12*_t225;
  const double _t971 = _t202*_t569;
  const double _t972 = _t567*_t7;
  const double _t974 = _t12*_t89;
  const double _t975 = _t696*na;
  const double _t976 = _t101*_t907;
  const double _t977 = _t976*nb;
  const double _t980 = _t217*_t93;
  const double _t981 = _t725*na;
  const double _t982 = _t200*nb;
  const double _t983 = (0.32e2 / 0.3e1)*_t447;
  const double _t984 = _t498*_t93;
  const double _t985 = _t100*_t17*_t911/xc_powr(_t1, 23, 3);
  const double _t986 = _t113*_t973 - _t117*_t973 + _t121*_t973 - _t141*_t432 - _t141*_t438 - _t179*_t973 + _t184*_t351 - _t192*_t360 - _t193*_t360 + _t194*_t360 + _t201*_t834 - _t203*_t842 + _t206*_t360 + _t251*_t972 - _t269*_t481 - _t349 - _t351*_t417 - _t352 - _t356 - _t358 - _t359*_t446 - _t359*_t450 - _t359*_t451 - _t360*_t411 + _t360*_t443 + _t360*_t444 - _t361*_t50 - _t362*_t50 + _t377 + _t379 + _t380 - _t410*_t810 + _t414*_t945*_t985 - _t446*_t914 + _t447*_t961 - _t450*_t914 + _t450*_t962 + _t450*_t964 - _t451*_t914 + _t451*_t962 + _t451*_t964 - _t456*_t914 + _t456*_t962 + _t456*_t964 - _t514 - _t518 + _t701*_t978 + _t731*_t983 + _t734*_t978 - _t898*_t974 - 0.8e1*_t96 + _t961*_t982 + 0.3e1*_t971 + (0.1e1 / 0.3e1)*_t975 + (0.1e1 / 0.3e1)*_t977 + _t979*_t980 + _t979*_t984 + (0.1e1 / 0.3e1)*_t981;
  const double _t987 = _t567*_t61;
  const double _t988 = 0.216e3*_t389;
  const double _t989 = _t387*nb;
  const double _t990 = 0.24e2*_t386;
  const double _t992 = _t277*na;
  const double _t993 = _t516*nb;
  const double _t995 = _t440*_t850;
  const double _t996 = _t150*nb;
  const double _t997 = _t197*_t276;
  const double _t998 = _t203*_t629;
  const double _t1000 = _t194*_t304;
  const double _t1001 = _t1000*nb;
  const double _t1002 = _t121*_t304;
  const double _t1003 = _t201*_t294;
  const double _t1004 = 0.24e2*_t96;
  const double _t1005 = _t57*_t817;
  const double _t1006 = _t160*_t794;
  const double _t1007 = _t208*_t810;
  const double _t1008 = _t192*_t304;
  const double _t1009 = _t193*_t304;
  const double _t1010 = _t828*nb;
  const double _t1011 = _t206*_t374;
  const double _t1012 = _t374*_t443;
  const double _t1014 = _t116*_t286;
  const double _t1015 = _t219*_t525;
  const double _t1016 = _t188*_t862;
  const double _t1017 = _t374*_t444;
  const double _t1018 = _t446*na;
  const double _t1019 = _t1018*_t917;
  const double _t1020 = _t224*_t525;
  const double _t1021 = _t200*_t917;
  const double _t1022 = _t12*_t892;
  const double _t1023 = _t208*_t286;
  const double _t1024 = _t208*_t869;
  const double _t1025 = _t54*_t976;
  const double _t1026 = _t54*_t980;
  const double _t1027 = _t12*_t455;
  const double _t1028 = _t1027*_t881;
  const double _t1029 = _t450*_t949;
  const double _t1030 = _t451*_t949;
  const double _t1031 = _t456*_t949;
  const double _t1032 = _t232*_t567;
  const double _t1033 = _t234*_t569;
  const double _t1035 = _t234*_t577;
  const double _t1037 = _t177*_t586;
  const double _t1038 = 0.2e1*_t788;
  const double _t1041 = _t65*na;
  const double _t1042 = _t1041*_t83;
  const double _t1043 = _t190*_t430;
  const double _t1045 = _t1044*_t192;
  const double _t1047 = _t1046*_t69;
  const double _t1049 = _t141*_t142;
  const double _t1050 = _t177*_t9;
  const double _t1051 = _t276*_t348;
  const double _t1052 = 0.192e3*_t595;
  const double _t1059 = _t114*_t161;
  const double _t1060 = _t614*_t89;
  const double _t1066 = _t162*_t89;
  const double _t1067 = _t110*_t336;
  const double _t1068 = _t160*_t475;
  const double _t1069 = _t178*_t475;
  const double _t1070 = _t189*_t232;
  const double _t1071 = _t370*_t827;
  const double _t1072 = _t109*_t110;
  const double _t1076 = _t188*_t430*_t727;
  const double _t1085 = _t200*_t330;
  const double _t1087 = _t116*_t238;
  const double _t1089 = _t144*_t487;
  const double _t1095 = _t166*_t239;
  const double _t1096 = _t1063*_t18;
  const double _t1099 = _t177*_t196;
  const double _t1100 = _t64*_t678;
  const double _t1102 = _t64*_t68;
  const double _t1107 = _t491*_t851;
  const double _t1108 = (0.2e1 / 0.9e1)*_t55;
  const double _t1109 = _t227*nb;
  const double _t1110 = _t446*nb;
  const double _t1112 = _hc2[0]*_t491;
  const double _t1116 = _t154*_t492;
  const double _t1117 = _hc2[0]*_t1086;
  const double _t1118 = _hc2[0]*_t1083;
  const double _t1119 = _t205*_t486;
  const double _t1120 = _t208*_t238;
  const double _t1121 = _t232*_t427;
  const double _t1122 = _t116*_t122;
  const double _t1123 = _t64*_t901;
  const double _t1124 = _t208*_t491;
  const double _t1125 = _t496*_t670;
  const double _t1126 = _t452*_t727;
  const double _t1127 = _t976*na;
  const double _t1128 = _t227*na;
  const double _t1129 = _hc1[0]*_t794;
  const double _t1131 = _hc1[0]*_t1130;
  const double _t1132 = _t1128*_t12;
  const double _t1133 = _t1109*_t12;
  const double _t1134 = _t507*na;
  const double _t1135 = _t507*nb;
  const double _t1137 = _t12*_t893;
  const double _t1138 = _t980*na;
  const double _t1141 = _t1113*_t918;
  const double _t1142 = _t122*_t208;
  const double _t1143 = _t216*_t486;
  const double _t1144 = _t1124*_t154;
  const double _t1145 = _t1086*_t18;
  const double _t1146 = _t1083*_t18;
  const double _t1147 = _t18*_t491;
  const double _t1148 = _t453*_t913;
  const double _t1149 = _t670*_t963;
  const double _t1150 = _t501*_t670;
  const double _t1151 = _t12*_t915;
  const double _t1152 = _t433*_t670;
  const double _t1153 = _t1152*_t12;
  const double _t1154 = _t509*na;
  const double _t1155 = _t509*nb;
  const double _t1156 = _t125*_t448;
  const double _t1157 = _t12*_t449;
  const double _t1158 = (0.2e1 / 0.9e1)*_t200;
  const double _t1159 = _t496*_t54;
  const double _t1160 = (0.1e1 / 0.9e1)*_t723;
  const double _t1161 = _t723*na;
  const double _t1162 = _t12*_t727;
  const double _t1163 = _t1162*_t382;
  const double _t1164 = _t239*_t37;
  const double _t1165 = _t228*_t64;
  const double _t1166 = _t228*_t32;
  const double _t1167 = (0.4e1 / 0.9e1)*_t723;
  const double _t1168 = _t177*_t195;
  const double _t1170 = _t64*_t892;
  const double _t1171 = _t1159*_t731;
  const double _t1172 = _t984*na;
  const double _t1173 = (0.128e3 / 0.9e1)*_t731;
  const double _t1174 = _t455*_t64;
  const double _t1175 = _t439*_t64;
  const double _t1176 = _t32*_t439;
  const double _t1177 = _t1160*_t54;
  const double _t1178 = _t501*_t742;
  const double _t1179 = _t12*_t228;
  const double _t1180 = _t1167*_t54;
  const double _t1181 = _t203*_t64;
  const double _t1182 = _t491*_t956;
  const double _t1183 = (0.32e2 / 0.9e1)*_t54*_t731;
  const double _t1184 = (0.64e2 / 0.9e1)*_t54*_t731;
  const double _t1185 = _t1173*_t54;
  const double _t1186 = _t491*_t967;
  const double _t1187 = _t12*_t439;
  const double _t1189 = -_t1188*_t832 + _t1188*_t904 + _t136*_t560 + _t756;
  const double _t1190 = _t387*na;
  const double _t1191 = _t389*na;
  const double _t1197 = _t116*_t154;
  const double _t1198 = _t223*_t434;
  const double _t1199 = -0.384e3*_hc1[0]*_hc2[0]*_t12*_t188*na*nb - 0.48e2*_hc1[0]*_hc2[2]*_t12*_t2*_t3*_t336*na*nb - 0.96e2*_hc1[0]*_t12*_t26*_t89*na*nb - 0.4e1*_hc1[0]*_t320 + _t776 + _t777 + _t778 + _t779 + _t780 + _t781 + _t782 + _t783;
  const double _t1202 = _t1130*_t257;
  const double _t1203 = _t589*na;
  const double _t1204 = _t139*_t232;
  const double _t1205 = _t239*_t9;
  const double _t1206 = _t348*_t802;
  const double _t1207 = _t595*na;
  const double _t1211 = _t209*_t262;
  const double _t1213 = _t530*_t68;
  const double _t1214 = _hc2[0]*_t180;
  const double _t1215 = _t160*_t535;
  const double _t1220 = _t539*na;
  const double _t1222 = _t1214*_t491;
  const double _t1223 = _hc2[0]*_t543;
  const double _t1224 = _t220*_t496;
  const double _t1225 = _t18*_t180;
  const double _t1226 = _t189*_t530;
  const double _t1227 = _t116*_t543;
  const double _t1228 = _t196*_t239;
  const double _t1230 = _t481*na;
  const double _t1232 = _t404*_t491;
  const double _t1236 = _hc2[0]*_t545;
  const double _t1237 = _t209*_t530;
  const double _t1238 = _t208*_t543;
  const double _t1239 = _t1112*_t207;
  const double _t1240 = _t246*_t682;
  const double _t1241 = _t243*_t682;
  const double _t1242 = _t1225*_t491;
  const double _t1243 = _t18*_t543;
  const double _t1244 = _t492*_t52;
  const double _t1247 = _t12*_t914;
  const double _t1248 = _t1124*_t52;
  const double _t1249 = _t195*_t239;
  const double _t1250 = _t246*_t923;
  const double _t1251 = _t243*_t923;
  const double _t1252 = _t1147*_t207;
  const double _t1253 = _t18*_t545;
  const double _t1254 = _t200*_t942;
  const double _t1255 = _t530*_t727;
  const double _t1256 = _t1161*_t200;
  const double _t1257 = _t246*_t311;
  const double _t1258 = _t243*_t311;
  const double _t1259 = _t1162*_t530;
  const double _t1260 = _t12*_t543;
  const double _t1261 = _t491*_t946;
  const double _t1262 = _t491*_t970;
  const double _t1263 = _t232*_t366;
  const double _t1264 = _t243*_t629;
  const double _t1266 = _t66*_t83;
  const double _t1278 = _t370*_t530;
  const double _t1280 = _t1245*_t160;
  const double _t1282 = _t609*na;
  const double _t1288 = _t1061*_t492;
  const double _t1290 = _t491*_t630;
  const double _t1291 = _t1112*_t1287;
  const double _t1293 = _t220*_t501;
  const double _t1294 = _t1061*_t1124;
  const double _t1295 = _t463*_t492;
  const double _t1296 = _hc1[0]*_t1245;
  const double _t1297 = _t1124*_t463;
  const double _t1298 = _t223*_t50;
  const double _t1299 = _t64*_t711;
  const double _t1300 = _t1147*_t1287;
  const double _t1302 = _t64*_t735;
  const double _t1303 = _t491*_t730;
  const double _t1304 = _t491*_t747;
  const double _t1309 = _t492*_t614;
  const double _t1310 = _t1124*_t614;
  const double d4F_dna4 = _t1*(-_t113*_t631 + _t117*_t631 - _t121*_t631 - _t135*_t649 - _t135*_t666 + _t135*_t688 + _t136*_t561 + _t139*_t562 - _t141*_t435 + _t141*_t653 - _t150*_t604 + _t154*_t253*_t714 - _t155*_t640 - _t157*_t641 + _t157*_t661 + _t157*_t683 - _t157*_t707 + _t157*_t713 - _t157*_t736 - _t158*_t644 + _t161*_t604 - _t163*_t641 + _t163*_t661 + _t163*_t683 - _t163*_t707 + _t163*_t713 - _t163*_t736 + _t164*_t640 - _t164*_t667 + _t165*_t644 - _t165*_t667 + _t166*_t645 + _t167*_t691 + 0.2e1*_t173*_t253 + _t173*_t572 - _t189*_t616 + _t209*_t616 + _t214*_t724 - _t253*_t628 - _t253*_t675 + _t256*_t573 - _t256*_t649 - _t256*_t666 - _t256*_t676 + _t256*_t688 + _t256*_t709 + _t270*_t592 - _t272*_t604 - _t282*_t617 - _t282*_t630 + _t282*_t686 + _t282*_t730 + _t285*_t662 + _t288*_t662 + _t290*_t662 - _t298*_t572 + _t302*_t604 + _t303*_t604 - _t306*_t613 - _t309*_t705 + _t309*_t741 + _t309*_t745 - _t312*_t705 + _t312*_t741 + _t312*_t745 + _t314*_t746 - _t315*_t415 + _t355*_t592 + _t357*_t598 - _t361*_t639 - _t362*_t639 - _t371*_t603 - _t38*_t645 - _t394*_t621 - _t405*_t561 + _t410*_t662 + _t415*_t684 - _t421*_t562 + _t462*_t623 - _t463*_t622 - _t463*_t654 + _t463*_t655 + _t463*_t656 - _t463*_t668 - _t509*_t739 + _t561*_t679 + _t562*_t680 + _t563*_t564 - _t563*_t651 + _t563*_t681 + 0.4e1*_t566 + 0.6e1*_t568 + 0.6e1*_t570 + _t572*_t708 + _t576*_t578 - _t576*_t677 + _t576*_t710 + _t578*_t580 + _t578*_t581 - _t580*_t677 + _t580*_t710 - _t581*_t677 + _t581*_t710 - _t583 - _t585 - _t588 + _t590*_t622 + _t590*_t654 - _t591 + _t593 + _t594 + _t597 - _t599 - _t600*_t632 - _t601 - _t605 - _t606 - _t608 - _t610 - _t611 - _t614*_t615 - _t614*_t648 + _t621*_t652 + _t624 + _t626 + _t627 - _t633 - _t635 - _t636 - _t637*_t638 - _t646*_t647 - _t646*_t650 + _t658 - _t659*_t662 + _t660 + _t663 - _t664*_t665 - _t669*_t671 - _t672*_t673 + _t672*_t686 + _t672*_t687 - _t672*_t704 + _t672*_t730 - _t672*_t747 - _t673*_t674 + _t674*_t686 + _t674*_t687 - _t674*_t704 + _t674*_t730 - _t674*_t747 + _t685*_t686 + _t689*_t690 + _t690*_t715 - 0.2096e4 / 0.27e2*_t692 - 0.40e2 / 0.9e1*_t693 - 0.4e1 / 0.9e1*_t695 + (0.4e1 / 0.3e1)*_t697 + _t698*_t699 - _t698*_t720 + _t700*_t701 + _t700*_t734 - _t702*_t703 + _t711*_t712 - _t712*_t735 - 0.2096e4 / 0.27e2*_t717 - 0.40e2 / 0.9e1*_t719 - _t720*_t740 - 0.4e1 / 0.9e1*_t722 + (0.4e1 / 0.3e1)*_t726 + _t728*_t729 + (0.64e2 / 0.3e1)*_t733 - 0.2096e4 / 0.27e2*_t737 - 0.40e2 / 0.9e1*_t738 + (0.4e1 / 0.3e1)*_t743 + _t756) + _t102*_t700 - _t113*_t625 + _t117*_t625 - _t121*_t625 - _t135*_t768 - _t152*_t763 + _t157*_t767 + _t161*_t584 + _t163*_t767 + _t164*_t638 + _t165*_t638 + _t167*_t763 + 0.192e3*_t186 - _t191*_t435 + _t191*_t653 + _t191*_t771 + _t191*_t772 + _t215*_t415 - _t219*_t415 + _t221*_t415 - _t224*_t415 - _t256*_t768 - _t270*_t584 - _t272*_t584 - _t309*_t735 - _t312*_t735 - _t315*_t669 - _t361*_t702 - _t362*_t702 - _t370*_t616 + _t371*_t592 - _t387*_t401 - _t389*_t582 - _t401*_t762 + (0.128e3 / 0.3e1)*_t54*_t701 + (0.128e3 / 0.3e1)*_t54*_t734 + _t582*_t766 - _t583*na - _t585*na - _t588*na + _t593*na - _t601*na - _t605*na - _t606*na - _t608*na - _t610*na - _t611*na + _t614*_t622 + _t614*_t654 - _t614*_t656 + _t614*_t668 + _t624*na + _t626*na + _t627*na - _t633*na - _t635*na - _t636*na - _t639*_t96 + _t660*na + _t663*na + _t669*_t684 + _t672*_t773 + _t674*_t773 + 0.4e1*_t757 + 0.4e1*_t758 + 0.12e2*_t759 + 0.4e1*_t760 + 0.4e1*_t761 - 0.2e1*_t764 - 0.2e1*_t765 + (0.4e1 / 0.3e1)*_t770 + (0.4e1 / 0.3e1)*_t774 + (0.4e1 / 0.3e1)*_t775 + _t784;
  const double d4F_dna3_dnb = _t1*(_hc1[1]*_t682*_t922 + _t113*_t603 - _t113*_t830 - _t117*_t603 + _t117*_t830 - _t120*_t612 - _t121*_t276 + _t121*_t603 - _t121*_t830 + _t124*_t817 - _t126*_t817 + _t128*_t817 - _t130*_t817 + _t134*_t139*_t256 + _t134*_t573 + _t135*_t790 - _t135*_t849 - _t135*_t866 - _t135*_t883 + _t135*_t894 - _t135*_t919 + _t135*_t933 + _t135*_t954 + _t136*_t785 - _t149*_t837 - _t149*_t877 + _t149*_t899 + _t149*_t935 - _t150*_t360 + _t152*_t294 + _t152*_t304 - _t152*_t604 - _t155*_t812 - _t155*_t839 + _t155*_t859 + _t157*_t820 - _t157*_t842 - _t158*_t812 - _t158*_t813 - _t158*_t844 + _t158*_t859 - _t160*_t301*_t393 + _t161*_t360 - _t161*_t803 - _t162*_t816 + _t163*_t820 - _t163*_t842 + _t164*_t812 + _t164*_t813 + _t164*_t839 - _t164*_t859 - _t164*_t874 + _t165*_t812 + _t165*_t813 + _t165*_t844 - _t165*_t859 - _t165*_t874 + _t166*_t794*_t810 + _t166*_t799*na - _t167*_t294 + _t167*_t604 + _t173*_t382 + _t173*_t787 - _t184*_t335 + _t185*_t623 - _t191*_t671 + _t192*_t294 + _t193*_t294 - _t194*_t294 - _t195*_t848 + _t196*_t847 - 0.1e1 / 0.9e1*_t200*_t942 - _t206*_t294 + _t209*_t857 + _t222*_t698 - _t223*_t911*_t922 - _t225*_t698 - _t225*_t740 - _t253*_t403 - _t253*_t422 + _t253*_t788 + _t253*_t930 + _t256*_t790 - _t256*_t849 - _t256*_t866 - _t256*_t883 + _t256*_t894 - _t256*_t919 + _t256*_t933 + _t256*_t954 - _t258 - _t260*_t424 - _t260*_t846 - _t266*_t9 - _t269*_t406 - _t269*_t589 - _t269*_t811 + _t269*_t858 + _t270*_t292 + _t270*_t803 + _t272*_t292 - _t272*_t360 + _t272*_t803 - _t274*_t426 + _t274*_t860 + _t274*_t902 - _t277 - _t280 - _t282*_t404 + _t282*_t855 + _t282*_t891 + _t282*_t946 - _t283 + _t285*_t301 - _t286*_t615 - _t286*_t648 + _t288*_t301 - _t288*_t810 - _t29*_t435 + _t29*_t653 + _t29*_t771 + _t29*_t772 + _t290*_t301 - _t292*_t302 - _t292*_t303 + _t292*_t370*_t382 + _t292*_t822 - _t293 + _t294*_t411 - _t294*_t443 - _t294*_t444 - _t295*_t315 + _t295*_t684 - _t295*_t884 + _t295*_t929 - _t298*_t382 - _t298*_t787 - _t30*_t435 + _t30*_t653 + _t30*_t771 + _t30*_t772 + _t301*_t406 + _t301*_t481 - _t301*_t659 - _t301*_t858 - _t301*_t873 + _t302*_t360 - _t302*_t803 + _t303*_t360 - _t303*_t803 - _t305 - _t306*_t359 - _t308*_t309 - _t308*_t312 - _t309*_t914 - _t309*_t921 + _t309*_t962 + _t309*_t964 - _t311*_t712 - _t311*_t924 - _t312*_t914 - _t312*_t921 + _t312*_t962 + _t312*_t964 - _t314*_t527 - _t314*_t613 + _t314*_t741 + _t314*_t745 + _t314*_t888 + _t32*_t925 + _t32*_t926 - _t32*_t950 - _t32*_t951 - _t332 + _t334*_t595 - _t334*_t806 + _t334*_t825 + _t335*_t417 - _t335*_t868 + _t348*_t803 + _t355*_t824 - _t361*_t415 - _t362*_t415 + _t369 - _t371*_t834 + _t373 + _t374*_t409 + _t374*_t426 - _t374*_t860 - _t374*_t902 - _t378*_t665 - _t382*_t513 + _t382*_t708 - _t394*_t815 - _t405*_t785 - _t407*_t647 - _t407*_t650 - _t413*_t807 - _t416*_t913 - _t423*_t876 + _t423*_t937 - _t424*_t845 - _t424*_t879 + _t424*_t897 + _t424*_t900 - _t424*_t910 + _t424*_t936 + _t424*_t952 - _t424*_t969 - _t427*_t835 + _t427*_t865 - _t450*_t917 - _t451*_t917 - _t456*_t917 + _t51*_t801 - _t52*_t622 - _t52*_t654 + _t52*_t655 + _t52*_t656 - _t52*_t668 + _t564*_t789 + 0.3e1*_t566 + 0.3e1*_t568 + 0.3e1*_t570 + _t578*_t798 + _t582*_t595 - _t582*_t806 + _t582*_t829 + _t64*_t925 + _t64*_t926 - _t64*_t950 - _t64*_t951 - _t651*_t789 + _t652*_t815 - _t669*_t703 + _t672*_t855 - _t672*_t885 + _t672*_t891 - _t672*_t916 + _t672*_t946 - _t672*_t970 + _t674*_t855 - _t674*_t885 + _t674*_t891 - _t674*_t916 + _t674*_t946 - _t674*_t970 + _t679*_t785 + _t681*_t789 + _t685*_t855 + _t685*_t891 - _t69*_t847 - 0.524e3 / 0.9e1*_t692 - 0.10e2 / 0.3e1*_t693 - 0.1e1 / 0.3e1*_t695 + (0.2e1 / 0.3e1)*_t697 + _t706*_t893 - _t706*_t915 + _t708*_t787 + _t712*_t923 + _t714*_t835 - _t714*_t865 + _t715*_t938 - 0.524e3 / 0.9e1*_t717 - 0.10e2 / 0.3e1*_t719 - 0.1e1 / 0.3e1*_t722 + _t725*_t945 + _t728*_t944 + (0.32e2 / 0.3e1)*_t733 - 0.524e3 / 0.9e1*_t737 - 0.10e2 / 0.3e1*_t738 - _t739*_t960 + _t756 + _t786 - _t791 - _t792 - _t793 + _t794*_t819 - _t794*_t838 - _t795 - _t796 - _t797 - _t799*_t809 + _t800 + _t801*_t802 + _t803*_t821 + _t804 + 0.72e2*_t805 - _t808 + _t818 + _t823 + _t824*_t863 + _t826*_t828 - _t826*_t833 - _t831*_t832 + _t831*_t904 + _t835*_t862 - _t840*_t841 + _t840*_t865 + _t841*_t862 - _t845*_t846 - _t846*_t879 + _t846*_t897 + _t846*_t900 - _t846*_t910 + _t846*_t936 + _t846*_t952 - _t846*_t969 - _t850*_t852 + _t854 + 0.12e2*_t856 + 0.36e2*_t861 - _t862*_t865 - _t867*_t98 - _t870*_t872 + _t872*_t939 + _t881*_t931 - _t881*_t948 + _t881*_t957 - _t881*_t968 + _t886*_t887 - _t887*_t920 + _t889 + _t890 + _t895*_t896 + _t895*_t934 + 0.36e2*_t903 - 0.524e3 / 0.27e2*_t905 - 0.10e2 / 0.9e1*_t906 - 0.1e1 / 0.9e1*_t908 + (0.64e2 / 0.3e1)*_t909 + _t923*_t924 + _t927*_t928 - _t927*_t949 + _t928*_t953 - 0.524e3 / 0.27e2*_t940 - 0.10e2 / 0.9e1*_t941 + (0.1e1 / 0.3e1)*_t943 + (0.64e2 / 0.3e1)*_t947 - _t949*_t953 - 0.524e3 / 0.27e2*_t958 - 0.10e2 / 0.9e1*_t959 + (0.64e2 / 0.3e1)*_t965 + _t986) - _t1001 - _t1002 - _t1003 - _t1004*nb - _t1005 + _t1006*_t810 + _t1007*_t157 + _t1007*_t163 + _t1008*nb + _t1009*nb + _t1010*_t334 - _t1011 - _t1012 - _t1013*_t361 - _t1013*_t362 - _t1014*_t424 - _t1014*_t846 - _t1015 + _t1016*_t871 - _t1017 - _t1019 + _t102*_t639 - _t1020 - _t1021 + _t1022*_t881 + _t1023*_t424 + _t1023*_t846 + _t1024*_t871 + (0.1e1 / 0.3e1)*_t1025 + (0.32e2 / 0.3e1)*_t1026 - _t1028 - _t1029 - _t1030 - _t1031 + _t113*_t541 - _t113*_t592 + _t113*_t999 - _t117*_t541 + _t117*_t592 - _t117*_t999 + _t121*_t541 - _t121*_t592 + _t121*_t999 + 0.3e1*_t135*_t788 + 0.3e1*_t136*_t382 + _t141*_t215 - _t141*_t219 + _t141*_t221 - _t141*_t224 - _t155*_t545 + _t157*_t897 - _t158*_t545 - _t161*_t374 + _t163*_t897 + _t164*_t545 + _t164*_t878 + _t165*_t545 + _t165*_t878 - _t167*_t662 + _t173*_t261*_t7 + _t184*_t973 - _t190*_t807 - _t191*_t315 + _t191*_t684 - _t192*_t274 - _t193*_t274 + _t194*_t274 + _t201*_t360 + _t206*_t274 + _t209*_t841 + _t210*_t807 + _t215*_t525 + _t216*_t282 + _t216*_t672 + _t216*_t674 + _t216*_t685 + _t221*_t525 + _t230*_t961 - _t258*na - _t259 - _t267 - _t271 + _t272*_t374 - _t273 - _t274*_t411 + _t274*_t443 + _t274*_t444 - _t275 - _t281 - _t284 - _t286*_t406 + _t286*_t659 + _t286*_t858 - _t287 - _t289 - _t291 + _t294*_t371 - _t299 - _t300 - _t302*_t374 - _t303*_t374 + _t304*_t996 - _t307*_t917 - _t309*_t949 - _t312*_t949 + _t313*_t653 + _t313*_t771 + _t313*_t772 + _t313*_t929 + _t33*_t592 - _t355*_t994 - _t364 + _t365 - _t368 + _t369*na - _t370*_t857 + _t372 + _t373*na + _t374*_t411 + _t375 + _t377*na + _t379*na + _t380*na + 0.3e1*_t384*_t578 - _t400*_t991 - _t413*_t871 - _t426*_t555 - _t428*_t871 - _t511 - _t512 - _t515 - _t519 - _t520 - _t521 - _t524 - _t526 - _t528 + _t549*nb + _t554 + _t555*_t860 + _t555*_t902 - _t556 + _t558 + _t592*_t632 + _t701*_t702 + _t702*_t734 + _t757 + _t758 + 0.9e1*_t759 + _t760 + _t761 - _t764 - _t765 + 0.216e3*_t766*nb + _t770 + _t774 + _t775 + _t784 - _t793*na - _t795*na - _t796*na - _t797*na + _t800*na + _t804*na - _t808*na + _t817*_t95 + _t818*nb + _t823*na - _t833*_t864 + _t854*na - _t863*_t994 + _t889*na + _t890*na + _t963*_t984 + 0.3e1*_t987 - _t988*nb - 0.36e2*_t989 - _t990*nb - _t992 - _t993 - _t995 + _t997*nb - _t998;
  const double d4F_dna2_dnb2 = 0.2e1*_hc0[0]*_hc1[1]*_t382 + 0.2e1*_hc0[0]*_hc1[2]*_t135*_t61 + 0.2e1*_hc0[0]*_hc1[2]*_t232*_t25 + 0.8e1*_hc0[0]*_hc1[2]*_t25*_t26*_t7 + 0.8e1*_hc0[0]*_hc1[2]*_t26*_t61*_t7 + 0.2e1*_hc0[0]*_hc1[3]*_t138*_t384 + 0.2e1*_hc0[0]*_hc1[3]*_t234*_t474 + 0.64e2*_hc1[0]*_hc2[0]*_t12*_t26 + 0.16e2*_hc1[0]*_hc2[0]*_t149*_t26*na*nb + 0.64e2*_hc1[0]*_hc2[0]*_t177*_t26*na*nb + 0.16e2*_hc1[0]*_hc2[0]*_t239*_t26*na*nb + 0.64e2*_hc1[0]*_hc2[0]*_t26*_t32*na + 0.32e2*_hc1[0]*_hc2[0]*_t26*_t32*nb + 0.32e2*_hc1[0]*_hc2[0]*_t26*_t64*na + 0.64e2*_hc1[0]*_hc2[0]*_t26*_t64*nb + (0.16e2 / 0.3e1)*_hc1[0]*_hc2[2]*_t12*_t2*_t3*_t53 + (0.4e1 / 0.3e1)*_hc1[0]*_hc2[2]*_t149*_t2*_t3*_t53*na*nb + (0.16e2 / 0.3e1)*_hc1[0]*_hc2[2]*_t177*_t2*_t3*_t53*na*nb + (0.4e1 / 0.3e1)*_hc1[0]*_hc2[2]*_t2*_t239*_t3*_t53*na*nb + (0.16e2 / 0.3e1)*_hc1[0]*_hc2[2]*_t2*_t3*_t32*_t53*na + (0.8e1 / 0.3e1)*_hc1[0]*_hc2[2]*_t2*_t3*_t32*_t53*nb + (0.8e1 / 0.3e1)*_hc1[0]*_hc2[2]*_t2*_t3*_t53*_t64*na + (0.16e2 / 0.3e1)*_hc1[0]*_hc2[2]*_t2*_t3*_t53*_t64*nb + 0.8e1*_hc1[0]*_hc3[0]*_t10*_t149*_t17*na + 0.16e2*_hc1[0]*_hc3[0]*_t10*_t17*_t177*na + 0.16e2*_hc1[0]*_hc3[0]*_t10*_t17*_t177*nb + 0.8e1*_hc1[0]*_hc3[0]*_t10*_t17*_t239*nb + 0.16e2*_hc1[0]*_hc3[0]*_t10*_t17*_t32 + 0.8e1*_hc1[0]*_hc3[0]*_t10*_t17*_t393*na*nb + 0.8e1*_hc1[0]*_hc3[0]*_t10*_t17*_t475*na*nb + 0.16e2*_hc1[0]*_hc3[0]*_t10*_t17*_t64 + 0.144e3*_hc1[0]*_hc3[0]*_t12*_t17*_t83*na + 0.144e3*_hc1[0]*_hc3[0]*_t12*_t17*_t83*nb + 0.144e3*_hc1[0]*_hc3[0]*_t17*_t32*_t83*na*nb + 0.144e3*_hc1[0]*_hc3[0]*_t17*_t64*_t83*na*nb + (0.64e2 / 0.3e1)*_hc1[0]*_hc3[1]*_t12*_t17*_t2*_t3*_t93*na + (0.64e2 / 0.3e1)*_hc1[0]*_hc3[1]*_t12*_t17*_t2*_t3*_t93*nb + (0.64e2 / 0.3e1)*_hc1[0]*_hc3[1]*_t17*_t2*_t3*_t32*_t93*na*nb + (0.64e2 / 0.3e1)*_hc1[0]*_hc3[1]*_t17*_t2*_t3*_t64*_t93*na*nb + (0.2e1 / 0.3e1)*_hc1[0]*_hc3[6]*_t12*_t17*_t74*_t75*_t99*na + (0.2e1 / 0.3e1)*_hc1[0]*_hc3[6]*_t12*_t17*_t74*_t75*_t99*nb + (0.2e1 / 0.3e1)*_hc1[0]*_hc3[6]*_t17*_t32*_t74*_t75*_t99*na*nb + (0.2e1 / 0.3e1)*_hc1[0]*_hc3[6]*_t17*_t64*_t74*_t75*_t99*na*nb + 0.16e2*_hc1[1]*_hc2[0]*_t12*_t135*_t26*na*nb + 0.16e2*_hc1[1]*_hc2[0]*_t12*_t188*na*nb + 0.16e2*_hc1[1]*_hc2[0]*_t12*_t232*_t26*na*nb + 0.64e2*_hc1[1]*_hc2[0]*_t12*_t25*_t26*na + 0.32e2*_hc1[1]*_hc2[0]*_t12*_t25*_t26*nb + 0.32e2*_hc1[1]*_hc2[0]*_t12*_t26*_t61*na + 0.64e2*_hc1[1]*_hc2[0]*_t12*_t26*_t61*nb + 0.176e3*_hc1[1]*_hc2[0]*_t12*_t337*_t7*na*nb + 0.32e2*_hc1[1]*_hc2[0]*_t25*_t26*_t32*na*nb + 0.64e2*_hc1[1]*_hc2[0]*_t25*_t26*_t64*na*nb + 0.64e2*_hc1[1]*_hc2[0]*_t26*_t32*_t61*na*nb + 0.32e2*_hc1[1]*_hc2[0]*_t26*_t61*_t64*na*nb + (0.4e1 / 0.3e1)*_hc1[1]*_hc2[2]*_t12*_t135*_t2*_t3*_t53*na*nb + (0.4e1 / 0.3e1)*_hc1[1]*_hc2[2]*_t12*_t2*_t232*_t3*_t53*na*nb + (0.16e2 / 0.3e1)*_hc1[1]*_hc2[2]*_t12*_t2*_t25*_t3*_t53*na + (0.8e1 / 0.3e1)*_hc1[1]*_hc2[2]*_t12*_t2*_t25*_t3*_t53*nb + (0.32e2 / 0.3e1)*_hc1[1]*_hc2[2]*_t12*_t2*_t3*_t445*_t7*na*nb + (0.8e1 / 0.3e1)*_hc1[1]*_hc2[2]*_t12*_t2*_t3*_t53*_t61*na + (0.16e2 / 0.3e1)*_hc1[1]*_hc2[2]*_t12*_t2*_t3*_t53*_t61*nb + (0.8e1 / 0.3e1)*_hc1[1]*_hc2[2]*_t2*_t25*_t3*_t32*_t53*na*nb + (0.16e2 / 0.3e1)*_hc1[1]*_hc2[2]*_t2*_t25*_t3*_t53*_t64*na*nb + (0.16e2 / 0.3e1)*_hc1[1]*_hc2[2]*_t2*_t3*_t32*_t53*_t61*na*nb + (0.8e1 / 0.3e1)*_hc1[1]*_hc2[2]*_t2*_t3*_t53*_t61*_t64*na*nb + 0.8e1*_hc1[1]*_hc3[0]*_t10*_t12*_t135*_t17*na + 0.8e1*_hc1[1]*_hc3[0]*_t10*_t12*_t17*_t232*nb + 0.16e2*_hc1[1]*_hc3[0]*_t10*_t12*_t17*_t25 + 0.8e1*_hc1[1]*_hc3[0]*_t10*_t12*_t17*_t382*na*nb + 0.16e2*_hc1[1]*_hc3[0]*_t10*_t12*_t17*_t61 + 0.8e1*_hc1[1]*_hc3[0]*_t10*_t135*_t17*_t64*na*nb + 0.8e1*_hc1[1]*_hc3[0]*_t10*_t149*_t17*_t61*na*nb + 0.16e2*_hc1[1]*_hc3[0]*_t10*_t17*_t177*_t25*na*nb + 0.16e2*_hc1[1]*_hc3[0]*_t10*_t17*_t177*_t61*na*nb + 0.8e1*_hc1[1]*_hc3[0]*_t10*_t17*_t232*_t32*na*nb + 0.8e1*_hc1[1]*_hc3[0]*_t10*_t17*_t239*_t25*na*nb + 0.16e2*_hc1[1]*_hc3[0]*_t10*_t17*_t25*_t32*na + 0.16e2*_hc1[1]*_hc3[0]*_t10*_t17*_t25*_t64*na + 0.16e2*_hc1[1]*_hc3[0]*_t10*_t17*_t25*_t64*nb + 0.16e2*_hc1[1]*_hc3[0]*_t10*_t17*_t32*_t61*na + 0.16e2*_hc1[1]*_hc3[0]*_t10*_t17*_t32*_t61*nb + 0.16e2*_hc1[1]*_hc3[0]*_t10*_t17*_t61*_t64*nb + 0.32e2*_hc1[1]*_hc3[0]*_t12*_t17*_t188*_t7*na + 0.32e2*_hc1[1]*_hc3[0]*_t12*_t17*_t188*_t7*nb + 0.144e3*_hc1[1]*_hc3[0]*_t12*_t17*_t25*_t83*na*nb + 0.144e3*_hc1[1]*_hc3[0]*_t12*_t17*_t61*_t83*na*nb + 0.32e2*_hc1[1]*_hc3[0]*_t17*_t188*_t32*_t7*na*nb + 0.32e2*_hc1[1]*_hc3[0]*_t17*_t188*_t64*_t7*na*nb + (0.64e2 / 0.3e1)*_hc1[1]*_hc3[1]*_t12*_t17*_t2*_t25*_t3*_t93*na*nb + (0.64e2 / 0.3e1)*_hc1[1]*_hc3[1]*_t12*_t17*_t2*_t3*_t61*_t93*na*nb + (0.2e1 / 0.3e1)*_hc1[1]*_hc3[6]*_t12*_t17*_t25*_t74*_t75*_t99*na*nb + (0.2e1 / 0.3e1)*_hc1[1]*_hc3[6]*_t12*_t17*_t61*_t74*_t75*_t99*na*nb + 0.6e1*_hc1[1]*_t25*_t80 + 0.6e1*_hc1[1]*_t61*_t80 + 0.16e2*_hc1[2]*_hc2[0]*_t12*_t138*_t26*na*nb + 0.16e2*_hc1[2]*_hc2[0]*_t12*_t234*_t26*na*nb + 0.64e2*_hc1[2]*_hc2[0]*_t12*_t25*_t26*_t61*na*nb + (0.4e1 / 0.3e1)*_hc1[2]*_hc2[2]*_t12*_t138*_t2*_t3*_t53*na*nb + (0.4e1 / 0.3e1)*_hc1[2]*_hc2[2]*_t12*_t2*_t234*_t3*_t53*na*nb + (0.16e2 / 0.3e1)*_hc1[2]*_hc2[2]*_t12*_t2*_t25*_t3*_t53*_t61*na*nb + 0.8e1*_hc1[2]*_hc3[0]*_t10*_t12*_t135*_t17*_t61*na*nb + 0.8e1*_hc1[2]*_hc3[0]*_t10*_t12*_t138*_t17*na + 0.8e1*_hc1[2]*_hc3[0]*_t10*_t12*_t17*_t232*_t25*na*nb + 0.8e1*_hc1[2]*_hc3[0]*_t10*_t12*_t17*_t234*nb + 0.16e2*_hc1[2]*_hc3[0]*_t10*_t12*_t17*_t25*_t61*na + 0.16e2*_hc1[2]*_hc3[0]*_t10*_t12*_t17*_t25*_t61*nb + 0.8e1*_hc1[2]*_hc3[0]*_t10*_t138*_t17*_t64*na*nb + 0.8e1*_hc1[2]*_hc3[0]*_t10*_t17*_t234*_t32*na*nb + 0.16e2*_hc1[2]*_hc3[0]*_t10*_t17*_t25*_t32*_t61*na*nb + 0.16e2*_hc1[2]*_hc3[0]*_t10*_t17*_t25*_t61*_t64*na*nb + 0.32e2*_hc1[2]*_hc3[0]*_t12*_t17*_t188*_t25*_t7*na*nb + 0.32e2*_hc1[2]*_hc3[0]*_t12*_t17*_t188*_t61*_t7*na*nb + 0.8e1*_hc1[3]*_hc3[0]*_t10*_t12*_t138*_t17*_t384*na*nb + 0.8e1*_hc1[3]*_hc3[0]*_t10*_t12*_t17*_t234*_t474*na*nb + _t1*(_t10*_t1048*_t160 - _t1006*_t114 + _t101*_t664 + 0.112e3*_t1010 + _t1013*_t715 + _t1013*_t896 + _t1013*_t934 + _t1016*_t1057 - _t1016*_t669 + _t1018*_t507 - _t1018*_t509 + (0.128e3 / 0.9e1)*_t102 + _t1022*_t1136 + _t1024*_t1057 - _t1024*_t669 - _t1027*_t1136 + _t1032 + _t1033 + _t1034*_t578 - _t1034*_t677 + _t1034*_t710 + _t1035*_t581 + _t1036*_t564 - _t1036*_t651 + _t1036*_t681 - _t1037 + _t1038*_t382 + _t1039*_t7*_t790 + _t1039*_t972 + _t104*_t240 + _t1040*_t184 - _t1040*_t417 + _t1040*_t49 - _t1040*_t66 + _t1040*_t868 - 0.48e2*_t1042 - _t1043 + _t1044*_t194 + _t1044*_t206 - _t1044*_t355 - _t1044*_t411 + _t1044*_t444 - _t1045 + _t1046*_t196 - _t1047 - _t1048*_t142 - _t1049*_t475 + _t1050*_t462 + _t1051*na + _t1051*nb + _t1052*na + _t1052*nb - _t1053*_t184*_t188 - _t1053*_t806 + _t1053*_t825 - _t1054*_t333 + _t1054*_t476 + _t1055*_t113 - _t1055*_t117 + _t1055*_t121 - _t1055*_t179 + _t1056*_t192 + _t1056*_t193 - _t1056*_t194 - _t1056*_t206 + _t1056*_t241 - _t1056*_t249 + _t1056*_t411 - _t1056*_t443 - _t1056*_t444 - _t1057*_t413 - _t1057*_t428 - _t1058*_t482 - _t1059 - _t1060*_t446 - _t1060*_t450 - _t1060*_t451 - _t1060*_t456 - _t1061*_t409 - _t1061*_t410 + _t1061*_t860 - _t1062*_t155 - _t1062*_t158 + _t1062*_t164 + _t1062*_t165 - _t1063*_t242 - _t1063*_t244 - _t1065*_t394 + _t1065*_t652 - _t1066*_t306 - _t1066*_t496 + _t1067*_t850 - _t1068*_t122 + _t1068*_t144 + _t1069*_t114 + _t1070*_t396 + 0.64e2*_t1071 - _t1072*_t382 - _t1073*_t113 + _t1073*_t117 - _t1073*_t121 + _t1073*_t197 - _t1073*_t201 - _t1074*_t192 - _t1074*_t193 + _t1074*_t194 + _t1074*_t206 - _t1074*_t411 + _t1074*_t443 + _t1074*_t444 - _t1075*_t429 + _t1076*_t177*_t18 - _t1076*_t974 - _t1077*_t880 + _t1077*_t932 + _t1078*_t1100 + _t1078*_t1124 - _t1078*_t492 - _t1079*_t371 - _t1079*_t551 - _t1080*_t425 + _t1080*_t901 + _t1081*_t166 - _t1081*_t38 + _t1082*_t196 - _t1082*_t69 - _t1083*_t195*_t89 + _t1084*_t678 - _t1084*_t68 - _t1085*_t154 - _t1086*_t637 - _t1087*_t424 - _t1087*_t846 - _t1088*_t427 + _t1088*_t714 - _t1089*_t116 + _t1089*_t208 - _t1090*_t155 + _t1090*_t164 - _t1091*_t242 + _t1091*_t247 - _t1093*_t158 + _t1093*_t165 - _t1094*_t244 + _t1094*_t248 - _t1095*_t122 + _t1095*_t144 + _t1096*_t243 + _t1096*_t246 + _t1097*_t155 + _t1097*_t158 - _t1097*_t164 - _t1097*_t165 + _t1098*_t242 + _t1098*_t244 - _t1098*_t247 - _t1098*_t248 - _t1099*_t463 + _t1099*_t590 - _t1100*_t1101 + _t1101*_t1102 - _t1101*_t1124 + _t1101*_t492 + _t1103*_t427 - _t1103*_t714 + _t1103*_t840 - _t1103*_t862 - _t1104*_t432 - _t1104*_t438 + _t1105*_t1137 - _t1105*_t1151 - _t1105*_t361 - _t1105*_t362 - _t1106*_t1107 - _t1106*_t852 - _t1108*_t665 - _t1108*_t867 + _t1109*_t1131 + _t1109*_t496 + _t1110*_t507 - _t1110*_t509 - _t1111*_t870 + _t1111*_t939 - _t1112*_t1114 + _t1114*_t1147 - _t1115*_t425 + _t1115*_t901 - _t1116*_t382 - _t1117*_t485 - _t1117*_t487 - _t1118*_t424 - _t1118*_t846 - _t1119*_t581 + _t1120*_t424 + _t1120*_t846 + _t1121*_t122 + _t1122*_t424 + _t1122*_t487 + _t1122*_t846 - _t1123*_t463 + _t1123*_t590 - _t1125*_t433 - _t1126*_t955 + _t1126*_t966 + (0.2e1 / 0.9e1)*_t1127 + _t1128*_t1129 - _t1129*_t1156 + _t113*_t418 - _t1131*_t449 + _t1132*_t157 + _t1132*_t163 + _t1133*_t243 + _t1133*_t246 + _t1134*_t314 + _t1134*_t450 + _t1134*_t451 + _t1134*_t456 + _t1135*_t450 + _t1135*_t451 + _t1135*_t456 + _t1135*_t501 + (0.64e2 / 0.9e1)*_t1138 + _t1139*_t701 + _t1139*_t734 + _t114*_t150 + _t114*_t270 + _t114*_t272 - _t114*_t302 - _t114*_t303 + _t114*_t821 + _t1140*_t980 + _t1140*_t984 - _t1141*_t116 + _t1141*_t208 - _t1142*_t424 - _t1142*_t485 - _t1142*_t487 - _t1142*_t846 + _t1143*_t581 + _t1144*_t382 + _t1145*_t485 + _t1145*_t487 + _t1146*_t424 + _t1146*_t846 - 0.88e2 / 0.3e1*_t1148 - _t1149*_t450 - _t1149*_t451 - _t1149*_t456 - _t1150*_t434 - _t1152*_t309 - _t1152*_t312 - _t1153*_t243 - _t1153*_t246 - _t1154*_t314 - _t1154*_t450 - _t1154*_t451 - _t1154*_t456 - _t1155*_t450 - _t1155*_t451 - _t1155*_t456 - _t1155*_t501 - _t1156*_t309 - _t1156*_t312 - _t1157*_t243 - _t1157*_t246 + _t1158*_t1161 - _t1158*_t942 + _t1159*_t1160 + _t1163*_t227 - _t1163*_t229 + _t1164*_t228 - _t1164*_t439 + _t1165*_t157 + _t1165*_t163 + _t1166*_t243 + _t1166*_t246 + _t1167*_t447 + _t1167*_t982 + _t1168*_t699 - _t1168*_t720 + _t1169*_t1170 - _t1169*_t1174 + _t1169*_t1182 - _t1169*_t1186 + _t1169*_t931 - _t1169*_t948 + _t1169*_t957 - _t1169*_t968 + _t117*_t458 + (0.32e2 / 0.9e1)*_t1171 + (0.64e2 / 0.9e1)*_t1172 + _t1173*_t447 - _t1175*_t157 - _t1175*_t163 - _t1176*_t243 - _t1176*_t246 + _t1177*_t309 + _t1177*_t312 + _t1177*_t499 + _t1177*_t500 + (0.2e1 / 0.9e1)*_t1178 + _t1179*_t424 + _t1179*_t485 + _t1179*_t487 + _t1179*_t846 + _t1180*_t450 + _t1180*_t451 + _t1180*_t456 + _t1181*_t699 - _t1181*_t720 + _t1183*_t309 + _t1183*_t312 + _t1183*_t499 + _t1183*_t500 + _t1184*_t314 + _t1184*_t501 + _t1185*_t450 + _t1185*_t451 + _t1185*_t456 - _t1187*_t424 - _t1187*_t485 - _t1187*_t487 - _t1187*_t846 + _t1189 + _t121*_t418 - _t121*_t458 + _t122*_t406 + _t122*_t479 + _t122*_t811 - _t122*_t858 - _t122*_t873 + 0.8e1*_t139*_t619 - _t144*_t354 - _t144*_t410 - _t144*_t479 - _t144*_t589 - _t145*_t173 + _t152*_t462 - _t152*_t484 - _t154*_t615 - _t154*_t648 - _t154*_t816 - _t157*_t408 + _t161*_t483 - _t163*_t408 - _t167*_t462 + _t167*_t484 - _t173*_t571 - _t177*_t412*_t431 + _t179*_t458 + _t183*_t478 + _t192*_t462 + _t193*_t462 - _t194*_t462 + _t197*_t458 - _t197*_t478 - _t201*_t458 + _t201*_t478 - _t206*_t462 + _t210*_t430 + _t227*_t307 + _t228*_t924 - _t229*_t307 + _t232*_t573 - _t232*_t676 + _t232*_t709 - _t238*_t395 - _t238*_t406 - _t238*_t481 - _t238*_t589 - _t238*_t811 + _t238*_t858 + _t238*_t873 - _t240*_t92 - _t241*_t484 - _t243*_t408 - _t245*_t396 + _t245*_t483 - _t246*_t408 + _t249*_t484 - _t272*_t483 + _t294*_t355 + _t294*_t822 + _t302*_t483 + _t303*_t483 - _t334*_t400 - _t361*_t978 - _t362*_t978 - 0.32e2*_t370*_t493 + _t374*_t410 + _t374*_t481 - 0.16e2*_t387 - 0.96e2*_t389 - _t395*_t590 - _t396*_t488 - _t396*_t490 + _t396*_t552 + _t398 - _t402 + _t409*_t463 + _t411*_t462 + _t413*_t669 - _t419 - _t424*_t442 + _t426*_t463 + _t428*_t669 + _t429*_t702 - _t432*_t979 - _t433*_t671 - _t434*_t703 - _t438*_t979 - _t439*_t924 + _t441 - _t442*_t485 - _t442*_t487 - _t442*_t846 - _t443*_t462 - _t444*_t462 - _t449*_t496 + (0.8e1 / 0.9e1)*_t453*_t985 + _t460 - _t461 + _t463*_t480 + _t463*_t489 - _t463*_t860 - _t463*_t902 + _t467 - _t468 + _t469 - _t470 - _t472 - _t480*_t590 - _t481*_t590 + _t483*_t488 + _t483*_t490 - _t483*_t552 - _t489*_t590 + _t502 - _t504 + _t505 - _t506 + _t508 - _t510 + 0.2e1*_t566 + _t568 + _t570 - _t670*_t983 - 0.1048e4 / 0.27e2*_t692 - 0.20e2 / 0.9e1*_t693 - 0.2e1 / 0.9e1*_t695 + (0.2e1 / 0.9e1)*_t697 + _t699*_t927 + _t699*_t953 - 0.1048e4 / 0.27e2*_t717 - 0.20e2 / 0.9e1*_t719 - _t720*_t927 - _t720*_t953 - 0.2e1 / 0.9e1*_t722 + (0.2e1 / 0.9e1)*_t726 + (0.32e2 / 0.9e1)*_t733 - 0.1048e4 / 0.27e2*_t737 - 0.20e2 / 0.9e1*_t738 + (0.2e1 / 0.9e1)*_t743 + 0.2e1*_t786 - 0.112e3*_t833*nb + 0.24e2*_t856 + 0.24e2*_t861 + 0.24e2*_t903 - 0.1048e4 / 0.27e2*_t905 - 0.20e2 / 0.9e1*_t906 - 0.2e1 / 0.9e1*_t908 + (0.64e2 / 0.9e1)*_t909 - _t92*_t996 - 0.1048e4 / 0.27e2*_t940 - 0.20e2 / 0.9e1*_t941 + (0.1e1 / 0.9e1)*_t943 + (0.64e2 / 0.9e1)*_t947 - 0.1048e4 / 0.27e2*_t958 - 0.20e2 / 0.9e1*_t959 - 0.32e2 / 0.3e1*_t96 + (0.328e3 / 0.9e1)*_t965 + 0.4e1*_t971 + (0.4e1 / 0.9e1)*_t975 + (0.4e1 / 0.9e1)*_t977 + (0.4e1 / 0.9e1)*_t981) - _t1027*_t414*_t979 - _t1037*na - _t1037*nb - _t1043*na - _t1043*nb - _t1044*_t117 - _t1044*_t197 - _t1045*na - _t1047*na - _t1049*_t239 - _t1056*_t121 - _t1059*nb - _t1061*_t152 - _t1061*_t411 - _t1070*_t144 - _t1072*_t135 - _t1072*_t232 - _t1083*_t155 - _t1083*_t158 - _t1086*_t242 - _t1086*_t244 - _t1104*_t219 - _t1104*_t224 - _t1105*_t218 - _t1105*_t559 - _t1113*_t492 - _t113*_t1192 - _t1159*_t229 - _t117*_t604 - 0.24e2*_t1190 - 0.144e3*_t1191 - _t1192*_t121 - _t1193*_t183 - _t1193*_t201 - _t1194*_t66 - _t1195*_t96 - _t1196*_t194 - _t1196*_t206 - _t1196*_t443 - _t1196*_t444 - _t1197*_t424 - _t1197*_t487 - _t1197*_t846 - _t1198*_t450 - _t1198*_t451 - _t1198*_t456 - _t1199 - _t120*_t613 - _t122*_t245 - _t122*_t302 - _t122*_t303 - _t122*_t488 - _t122*_t490 - _t136*_t145 - _t136*_t571 - _t142*_t191*_t475 - _t142*_t50*_t794 - 0.16e2*_t143 - _t144*_t552 - _t154*_t406 - _t154*_t479 - _t154*_t811 - _t157*_t442 - _t167*_t463 - _t192*_t590 - _t193*_t590 - _t197*_t604 - _t200*_t613 - _t201*_t462 - _t203*_t617 - _t210*_t669 - _t229*_t732 - _t232*_t675 - _t238*_t270 - _t238*_t272 - _t241*_t590 - _t243*_t442 - _t249*_t463 - 0.64e2*_t26*_t49 - _t274*_t822 - _t309*_t439 - _t312*_t439 - _t314*_t720 - _t361*_t669 - _t362*_t669 - _t389*_t602 - _t409*_t614 - _t413*_t431 - _t415*_t96 - _t425*_t646 - _t428*_t431 - _t431*_t482 - _t432*_t669 - _t434*_t884 - _t438*_t669 - _t439*_t499 - _t439*_t500 - 0.8e1 / 0.3e1*_t440*_t7 - _t461*nb - _t468*nb - _t470*nb - _t480*_t614 - _t489*_t614 - 0.176e3*_t495 - _t50*_t762 - _t501*_t720 - _t504*na - _t506*na - _t510*na - 0.16e2 / 0.3e1*_t58 - _t586*_t64 - _t791*na - _t792*na - 0.24e2*_t989 - 0.4e1 / 0.3e1*_t998;
  const double d4F_dna_dnb3 = 0.3e1*_hc0[0]*_hc1[2]*_t232*_t25 + 0.12e2*_hc0[0]*_hc1[2]*_t26*_t61*_t7 + 0.3e1*_hc0[0]*_hc1[3]*_t234*_t474 + 0.48e2*_hc1[0]*_hc2[0]*_t12*_t26 + 0.48e2*_hc1[0]*_hc2[0]*_t177*_t26*na*nb + 0.48e2*_hc1[0]*_hc2[0]*_t239*_t26*na*nb + 0.48e2*_hc1[0]*_hc2[0]*_t26*_t32*na + 0.96e2*_hc1[0]*_hc2[0]*_t26*_t64*na + 0.48e2*_hc1[0]*_hc2[0]*_t26*_t64*nb + 0.4e1*_hc1[0]*_hc2[2]*_t12*_t2*_t3*_t53 + 0.4e1*_hc1[0]*_hc2[2]*_t177*_t2*_t3*_t53*na*nb + 0.4e1*_hc1[0]*_hc2[2]*_t2*_t239*_t3*_t53*na*nb + 0.4e1*_hc1[0]*_hc2[2]*_t2*_t3*_t32*_t53*na + 0.8e1*_hc1[0]*_hc2[2]*_t2*_t3*_t53*_t64*na + 0.4e1*_hc1[0]*_hc2[2]*_t2*_t3*_t53*_t64*nb + 0.24e2*_hc1[0]*_hc3[0]*_t10*_t17*_t177*na + 0.12e2*_hc1[0]*_hc3[0]*_t10*_t17*_t239*nb + 0.12e2*_hc1[0]*_hc3[0]*_t10*_t17*_t475*na*nb + 0.24e2*_hc1[0]*_hc3[0]*_t10*_t17*_t64 + 0.216e3*_hc1[0]*_hc3[0]*_t12*_t17*_t83*na + 0.216e3*_hc1[0]*_hc3[0]*_t17*_t64*_t83*na*nb + 0.32e2*_hc1[0]*_hc3[1]*_t12*_t17*_t2*_t3*_t93*na + 0.32e2*_hc1[0]*_hc3[1]*_t17*_t2*_t3*_t64*_t93*na*nb + _hc1[0]*_hc3[6]*_t12*_t17*_t74*_t75*_t99*na + _hc1[0]*_hc3[6]*_t17*_t64*_t74*_t75*_t99*na*nb + 0.24e2*_hc1[1]*_hc2[0]*_t12*_t188*na*nb + 0.48e2*_hc1[1]*_hc2[0]*_t12*_t232*_t26*na*nb + 0.48e2*_hc1[1]*_hc2[0]*_t12*_t25*_t26*na + 0.96e2*_hc1[1]*_hc2[0]*_t12*_t26*_t61*na + 0.48e2*_hc1[1]*_hc2[0]*_t12*_t26*_t61*nb + 0.168e3*_hc1[1]*_hc2[0]*_t12*_t337*_t7*na*nb + 0.48e2*_hc1[1]*_hc2[0]*_t25*_t26*_t64*na*nb + 0.48e2*_hc1[1]*_hc2[0]*_t26*_t32*_t61*na*nb + 0.96e2*_hc1[1]*_hc2[0]*_t26*_t61*_t64*na*nb + 0.4e1*_hc1[1]*_hc2[2]*_t12*_t2*_t232*_t3*_t53*na*nb + 0.4e1*_hc1[1]*_hc2[2]*_t12*_t2*_t25*_t3*_t53*na + 0.8e1*_hc1[1]*_hc2[2]*_t12*_t2*_t3*_t445*_t7*na*nb + 0.8e1*_hc1[1]*_hc2[2]*_t12*_t2*_t3*_t53*_t61*na + 0.4e1*_hc1[1]*_hc2[2]*_t12*_t2*_t3*_t53*_t61*nb + 0.4e1*_hc1[1]*_hc2[2]*_t2*_t25*_t3*_t53*_t64*na*nb + 0.4e1*_hc1[1]*_hc2[2]*_t2*_t3*_t32*_t53*_t61*na*nb + 0.8e1*_hc1[1]*_hc2[2]*_t2*_t3*_t53*_t61*_t64*na*nb + 0.12e2*_hc1[1]*_hc3[0]*_t10*_t12*_t17*_t232*nb + 0.24e2*_hc1[1]*_hc3[0]*_t10*_t12*_t17*_t61 + 0.24e2*_hc1[1]*_hc3[0]*_t10*_t17*_t177*_t61*na*nb + 0.12e2*_hc1[1]*_hc3[0]*_t10*_t17*_t232*_t32*na*nb + 0.12e2*_hc1[1]*_hc3[0]*_t10*_t17*_t239*_t25*na*nb + 0.24e2*_hc1[1]*_hc3[0]*_t10*_t17*_t25*_t64*na + 0.24e2*_hc1[1]*_hc3[0]*_t10*_t17*_t32*_t61*na + 0.24e2*_hc1[1]*_hc3[0]*_t10*_t17*_t61*_t64*nb + 0.48e2*_hc1[1]*_hc3[0]*_t12*_t17*_t188*_t7*na + 0.216e3*_hc1[1]*_hc3[0]*_t12*_t17*_t61*_t83*na*nb + 0.48e2*_hc1[1]*_hc3[0]*_t17*_t188*_t64*_t7*na*nb + 0.32e2*_hc1[1]*_hc3[1]*_t12*_t17*_t2*_t3*_t61*_t93*na*nb + _hc1[1]*_hc3[6]*_t12*_t17*_t61*_t74*_t75*_t99*na*nb + 0.9e1*_hc1[1]*_t61*_t80 + 0.48e2*_hc1[2]*_hc2[0]*_t12*_t234*_t26*na*nb + 0.48e2*_hc1[2]*_hc2[0]*_t12*_t25*_t26*_t61*na*nb + 0.4e1*_hc1[2]*_hc2[2]*_t12*_t2*_t234*_t3*_t53*na*nb + 0.4e1*_hc1[2]*_hc2[2]*_t12*_t2*_t25*_t3*_t53*_t61*na*nb + 0.12e2*_hc1[2]*_hc3[0]*_t10*_t12*_t17*_t232*_t25*na*nb + 0.12e2*_hc1[2]*_hc3[0]*_t10*_t12*_t17*_t234*nb + 0.24e2*_hc1[2]*_hc3[0]*_t10*_t12*_t17*_t25*_t61*na + 0.12e2*_hc1[2]*_hc3[0]*_t10*_t17*_t234*_t32*na*nb + 0.24e2*_hc1[2]*_hc3[0]*_t10*_t17*_t25*_t61*_t64*na*nb + 0.48e2*_hc1[2]*_hc3[0]*_t12*_t17*_t188*_t61*_t7*na*nb + 0.12e2*_hc1[3]*_hc3[0]*_t10*_t12*_t17*_t234*_t474*na*nb + _t1*(_hc1[1]*_t1225*_t530 - _t1000 + _t1007*_t485 + _t1007*_t487 + _t1008 + _t1009 - _t1022*_t313 + _t1027*_t313 + 0.3e1*_t1032 + 0.3e1*_t1033 + _t1035*_t798 - 0.144e3*_t1042 + _t1050*_t304 + 0.3e1*_t1067*_t7 + _t1069*_t803 + _t1070*_t292 + _t1070*_t803 + 0.168e3*_t1071 - _t1085*_t286 - _t1095*_t301 + _t1095*_t810 - _t1099*_t374 + _t1099*_t553 + _t1100*_t1218 - _t1100*_t1231 + _t1102*_t1231 - _t1107*_t850 - _t1112*_t1234 + _t1121*_t301 - _t1123*_t374 + _t1123*_t553 + _t1124*_t1218 - _t1124*_t1229 - _t1124*_t1231 + (0.2e1 / 0.3e1)*_t1127 - _t113*_t1217 - _t1130*_t160*_t292 + _t1130*_t30*_t886 + _t1130*_t819 - _t1131*_t308 + _t1137*_t50 + (0.64e2 / 0.3e1)*_t1138 + _t1147*_t1234 - 0.28e2*_t1148 - _t1151*_t50 + _t1159*_t961 - _t1164*_t311 + _t1164*_t923 + _t1168*_t928 - _t1168*_t949 + _t117*_t1217 + _t1170*_t881 + (0.64e2 / 0.3e1)*_t1172 - _t1174*_t881 + _t1181*_t928 - _t1181*_t949 + _t1182*_t881 - _t1186*_t881 + _t1189 + _t1194*_t183 - _t1194*_t197 + _t1194*_t201 - _t1195*_t432 - _t1195*_t438 + _t1200*_t564 - _t1200*_t651 + _t1200*_t681 - _t1201*_t788 - _t1202 - _t1203*_t268 + _t1204*_t798 + _t1205*_t803 + 0.72e2*_t1206 + 0.288e3*_t1207 - _t1208*_t806 + _t1208*_t825 - _t1209*_t189 + _t1209*_t209 - _t121*_t1217 - _t1210*_t482 - _t1211*na - _t1212*_t394 + _t1212*_t652 - _t1213*_t180 - _t1213*_t207 - _t1214*_t543 + _t1215*_t13*nb - _t1215*_t52 - _t1216*_t18*_t727 + _t1216*_t836 - _t1218*_t492 + _t1219*_t196 - _t1219*_t69 - _t1220*_t485 - _t1220*_t487 + _t1221*_t166 - _t1221*_t38 - _t1222*_t232 - _t1222*_t532 - _t1223*_t207 + _t1224*_t29 + _t1224*_t30 + _t1225*_t543 + _t1226*_t52 + _t1227*_t52 + _t1228*_t269 - _t1228*_t301 + _t1229*_t492 + _t1230*_t824 + _t1231*_t492 - _t1232*_t232 - _t1232*_t532 - _t1235*_t880 + _t1235*_t932 - _t1236*_t485 - _t1236*_t487 - _t1237*_t52 - _t1238*_t52 - _t1239*_t232 - _t1239*_t532 + _t1240*_t29 + _t1240*_t30 + _t1241*_t29 + _t1241*_t30 + _t1242*_t232 + _t1242*_t532 + _t1243*_t207 + _t1244*_t232 + _t1244*_t532 + _t1246*_t886 - _t1246*_t920 - _t1247*_t243 - _t1247*_t246 - _t1248*_t232 - _t1248*_t532 - _t1249*_t311 + _t1249*_t923 + _t1250*_t32 + _t1250*_t64 + _t1251*_t32 + _t1251*_t64 + _t1252*_t232 + _t1252*_t532 + _t1253*_t485 + _t1253*_t487 - 0.1e1 / 0.3e1*_t1254 + _t1255*_t944 + (0.2e1 / 0.3e1)*_t1256 - _t1257*_t32 - _t1257*_t64 - _t1258*_t32 - _t1258*_t64 - _t1259*_t960 - _t1260*_t225 + _t1261*_t232 + _t1261*_t532 - _t1262*_t232 - _t1262*_t532 - _t13*_t536 + _t136*_t391 - _t162*_t615 + _t173*_t530 - _t183*_t830 + _t185*_t536 + _t197*_t830 - _t201*_t276 - _t201*_t830 - _t206*_t304 + _t207*_t530*_t678 + _t212*_t817 - _t217*_t817 + _t232*_t790 - _t232*_t919 + _t232*_t954 + _t239*_t292*_t809 - _t239*_t837 - _t239*_t838 - _t239*_t877 + _t239*_t899 + _t239*_t935 + 0.24e2*_t240*_t26 - _t240*_t973 + _t241*_t294 + _t241*_t304 - _t241*_t604 - _t242*_t812 - _t242*_t813 - _t242*_t839 + _t242*_t859 + _t242*_t874 + _t243*_t820 - _t243*_t842 - _t244*_t812 - _t244*_t813 - _t244*_t844 + _t244*_t859 + _t244*_t874 + _t245*_t268 + _t245*_t360 - _t245*_t803 + _t246*_t820 - _t246*_t842 + _t247*_t812 + _t247*_t813 + _t247*_t839 - _t247*_t859 - _t247*_t874 + _t248*_t812 + _t248*_t813 + _t248*_t844 - _t248*_t859 - _t248*_t874 - _t249*_t294 - _t249*_t304 + _t249*_t541 + _t249*_t604 - _t251*_t788 - _t260*_t485 - _t260*_t487 + _t262*_t370 - _t269*_t544 - _t286*_t816 - _t29*_t497 + _t292*_t355 - _t292*_t488 - _t292*_t490 + _t292*_t552 - _t298*_t530 - _t30*_t497 + _t301*_t410 + _t301*_t544 + _t304*_t411 - _t304*_t443 - _t304*_t444 - _t308*_t499 - _t308*_t500 - _t331*nb - _t359*_t496 + _t360*_t488 + _t360*_t490 - _t360*_t552 - _t37*_t848 + _t374*_t480 + _t374*_t489 - _t378*_t867 - _t403*_t530 - _t404*_t543 - 0.168e3*_t414*_t429 + _t437*_t817 + _t440 - _t450*_t527 + _t450*_t888 - _t451*_t527 + _t451*_t888 - _t456*_t527 + _t456*_t888 - _t480*_t553 - _t481*_t553 - _t485*_t910 + _t485*_t952 - _t485*_t969 - _t486*_t876 + _t486*_t937 + _t487*_t900 - _t487*_t910 + _t487*_t952 - _t487*_t969 - _t488*_t803 - _t489*_t553 - _t490*_t803 - _t496*_t914 + _t496*_t964 - _t498*_t817 - _t499*_t921 + _t499*_t962 + _t499*_t964 - _t500*_t921 + _t500*_t962 + _t500*_t964 - _t501*_t527 + _t501*_t557 - _t501*_t613 - _t501*_t705 + _t501*_t741 + _t501*_t745 + _t501*_t888 - _t501*_t917 - _t516 - _t522 + _t525*_t853 - _t525*_t884 + _t530*_t708 + _t532*_t790 - _t532*_t919 + _t532*_t954 - _t533 - _t538 - _t540 - _t541*_t822 - _t542 + _t543*_t946 + _t549 - _t551*_t834 + _t552*_t803 + _t566 + _t595*_t826 - _t665*_t98 + _t689*_t895 - 0.524e3 / 0.27e2*_t692 - 0.10e2 / 0.9e1*_t693 - 0.1e1 / 0.9e1*_t695 + _t715*_t895 - 0.524e3 / 0.27e2*_t717 - 0.10e2 / 0.9e1*_t719 - 0.1e1 / 0.9e1*_t722 - 0.524e3 / 0.27e2*_t737 - 0.10e2 / 0.9e1*_t738 + 0.3e1*_t786 + 0.24e2*_t805 - _t806*_t826 + _t822*_t824 + _t826*_t829 + 0.36e2*_t856 + 0.12e2*_t861 + _t896*_t938 - 0.524e3 / 0.9e1*_t905 - 0.10e2 / 0.3e1*_t906 - 0.1e1 / 0.3e1*_t908 + _t934*_t938 - 0.524e3 / 0.9e1*_t940 - 0.10e2 / 0.3e1*_t941 - 0.524e3 / 0.9e1*_t958 - 0.10e2 / 0.3e1*_t959 + (0.100e3 / 0.3e1)*_t965 + _t986 + _t997) - _t1001 - _t1002 - _t1003 - _t1004*na - _t1005 - _t1011 - _t1012 - _t1013*_t432 - _t1013*_t438 - _t1014*_t487 - _t1015 - _t1017 - _t1019 - _t1020 - _t1021 - _t1028 - _t1029 - _t1030 - _t1031 - _t1070*_t269 - _t1121*_t286 - 0.36e2*_t1190 - _t1199 - _t1201*_t136 - _t1202*nb - _t1210*_t190 - _t1211*_t54 - _t1230*_t994 - _t1231*_t209 - _t1233*_t492 - _t1263 - _t1264 - _t1265*_t183 - _t1265*_t201 - 0.216e3*_t1266*na - _t136*_t251 - _t191*_t529 - _t192*_t553 - _t193*_t553 - _t197*_t999 - _t218*_t50 - _t242*_t878 - _t244*_t878 - _t245*_t374 - _t249*_t662 - _t269*_t552 - _t286*_t479 - _t313*_t497 - _t363 - _t368 - _t374*_t488 - _t374*_t490 - _t411*_t553 - _t480*_t555 - _t482*_t871 - _t489*_t555 - _t499*_t949 - _t50*_t559 - _t500*_t949 - _t533*nb - _t546 - _t547 - _t548 - _t550 - _t556 - _t822*_t994 - _t988*na - _t990*na - _t992 - _t993 - _t995 - _t998;
  const double d4F_dnb4 = _t1*(0.6e1*_t1032 + 0.6e1*_t1033 + _t1034*_t1035 - _t1034*_t1119 + _t1034*_t1143 + _t1035*_t1271 + _t1035*_t1272 + _t1038*_t530 - 0.288e3*_t1042 - _t1044*_t1203 - _t1044*_t1278 + _t1044*_t1280 - _t1061*_t1227 + _t1061*_t1237 + _t1061*_t1238 + _t1070*_t1265 - _t1085*_t614 + _t1100*_t530*_t614 - _t1116*_t530 - _t1119*_t1271 - _t1119*_t1272 - _t1125*_t669 + (0.4e1 / 0.3e1)*_t1127 + _t1135*_t1296 + (0.128e3 / 0.3e1)*_t1138 + _t1143*_t1271 + _t1143*_t1272 + _t1144*_t530 - _t1150*_t702 - _t1155*_t1296 + (0.64e2 / 0.3e1)*_t1171 + (0.4e1 / 0.3e1)*_t1178 - _t1194*_t240 + _t1195*_t1293 - _t1195*_t529 + _t1204*_t532 + _t1205*_t1265 + 0.96e2*_t1206 + 0.384e3*_t1207 - _t1213*_t1287 - _t1215*_t463 - _t1223*_t1287 + _t1224*_t50 + _t1226*_t463 + _t1227*_t463 + _t1228*_t1276 - _t1228*_t662 - _t1237*_t463 - _t1238*_t463 + _t1240*_t50 + _t1241*_t50 + _t1243*_t1287 - _t1245*_t586 + _t1249*_t711 - _t1249*_t735 - 0.4e1 / 0.9e1*_t1254 + _t1255*_t729 + (0.4e1 / 0.3e1)*_t1256 - _t1259*_t509 - _t1260*_t720 - _t1265*_t245 - _t1265*_t488 - _t1265*_t490 + _t1265*_t552 + _t1267*_t136 - _t1267*_t405 + _t1267*_t679 + _t1268*_t139 - _t1268*_t421 + _t1268*_t680 + _t1269*_t564 - _t1269*_t651 + _t1269*_t681 - _t1270*_t422 + _t1270*_t788 + _t1270*_t930 - _t1273*_t822 - _t1274*_t806 + _t1274*_t825 + _t1275*_t183 - _t1275*_t197 + _t1275*_t201 - _t1276*_t481 - _t1276*_t544 - _t1277*_t242 - _t1277*_t244 + _t1277*_t247 + _t1277*_t248 - _t1279*_t394 + _t1279*_t652 + _t1281*_t241 - _t1281*_t249 - _t1282*_t501 - _t1283*_t432 - _t1283*_t438 - _t1284*_t242 + _t1284*_t247 - _t1285*_t244 + _t1285*_t248 + _t1286*_t196 - _t1286*_t69 - _t1288*_t232 - _t1288*_t532 + _t1289*_t242 + _t1289*_t244 - _t1289*_t247 - _t1289*_t248 - _t1290*_t232 - _t1290*_t532 - _t1291*_t232 - _t1291*_t532 - _t1292*_t421 + _t1292*_t680 + _t1294*_t232 + _t1294*_t532 + _t1295*_t232 + _t1295*_t532 - _t1297*_t232 - _t1297*_t532 - _t1298*_t499 - _t1298*_t500 + _t1299*_t243 + _t1299*_t246 + _t1300*_t232 + _t1300*_t532 + _t1301*_t984 - _t1302*_t243 - _t1302*_t246 + _t1303*_t232 + _t1303*_t532 - _t1304*_t232 - _t1304*_t532 - _t183*_t631 + _t197*_t631 - _t201*_t631 - _t241*_t691 - _t243*_t641 + _t245*_t604 - _t246*_t641 + _t249*_t691 + _t462*_t536 + _t477*_t657 + _t481*_t662 + _t488*_t604 + _t490*_t604 - _t496*_t613 + _t496*_t741 - _t497*_t50 - _t499*_t705 + _t499*_t741 + _t499*_t745 - _t500*_t705 + _t500*_t741 + _t500*_t745 + _t501*_t746 - _t530*_t628 - _t543*_t630 + _t543*_t730 + _t544*_t662 - _t551*_t603 - _t552*_t604 + _t592*_t822 - _t614*_t816 - _t664*_t867 + _t690*_t896 + _t690*_t934 + _t756 + 0.4e1*_t786 - 0.2096e4 / 0.27e2*_t905 - 0.40e2 / 0.9e1*_t906 - 0.4e1 / 0.9e1*_t908 - 0.2096e4 / 0.27e2*_t940 - 0.40e2 / 0.9e1*_t941 - 0.2096e4 / 0.27e2*_t958 - 0.40e2 / 0.9e1*_t959) + _t102*_t1301 + (0.4e1 / 0.3e1)*_t1025 + (0.128e3 / 0.3e1)*_t1026 + 0.192e3*_t1041*_t26 - 0.48e2*_t1190 - 0.288e3*_t1191 + _t1195*_t200*_t220 + _t1195*_t213 - _t1195*_t218 - _t1195*_t559 - _t1208*_t1266 + _t1208*_t766 + _t1224*_t191 - _t1227*_t614 + _t1228*_t1306 + _t1237*_t614 + _t1238*_t614 + _t1240*_t191 + _t1241*_t191 - _t1245*_t587 - 0.2e1*_t1263 - 0.2e1*_t1264 + _t1265*_t240 - _t1273*_t551 + _t1276*_t245 + _t1276*_t488 + _t1276*_t490 - _t1276*_t552 - _t1278*_t590 + _t1280*_t590 - _t1281*_t183 + _t1281*_t197 - _t1281*_t201 - _t1282*_t200 - _t1283*_t96 + _t1293*_t669 - _t1305*_t241 + _t1305*_t249 - _t1306*_t544 - _t1307*_t242 - _t1307*_t244 + _t1307*_t247 + _t1307*_t248 + _t1308*_t241 - _t1308*_t249 - _t1309*_t232 - _t1309*_t532 + _t1310*_t232 + _t1310*_t532 + 0.4e1*_t136*_t530 - _t142*_t239*_t477 + _t183*_t691 - _t191*_t497 - _t197*_t691 + _t201*_t691 + (0.4e1 / 0.3e1)*_t230*_t723 + 0.4e1*_t232*_t788 - _t243*_t634*na - _t245*_t662 - _t432*_t702 - _t438*_t702 - _t488*_t662 - _t499*_t735 - _t500*_t735 - _t529*_t669 + 0.4e1*_t531*_t577 + 0.4e1*_t532*_t788 + (0.128e3 / 0.3e1)*_t54*_t984 + _t551*_t592 + _t552*_t662 - _t584*_t822 + (0.4e1 / 0.3e1)*_t769*na + _t784 + 0.12e2*_t987;
  if(out->v4rho4 != NULL) out->v4rho4[ip*p->dim.v4rho4 + 0] += d4F_dna4;
