/*
  Generated from python/lda_exc/lda_x_sloc.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py lda_x_sloc
*/

#ifndef _LDA_X_SLOC_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _LDA_X_SLOC_KERNEL_BODY
#define _KMAX 0
#define _LDA_X_SLOC_HELPER_BODIES
#include "lda_x_sloc.c"
#undef _LDA_X_SLOC_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _LDA_X_SLOC_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_LDA_X_SLOC_HELPER_BODIES)
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(f_sloc_k, _KMAX)(const xc_func_type *p, double rs, double z, double *out) {
  const lda_x_sloc_params *params = (const lda_x_sloc_params *)(p->params);

  const double _t1 = params->b + 0.1e1;
  const double _t2 = (0.1e1 / (rs * rs * rs));
  const double _t3 = params->a*pow((0.3e1 / 0.4e1)*_t2/M_PI, params->b)/(0.2e1*params->b + 0.2e1);
#if _KMAX >= 1
  const double _t6 = 0.3e1*params->b;
  const double _t7 = _t6/rs;
#endif
#if _KMAX >= 2
  const double _t9 = (0.1e1 / (rs * rs));
  const double _t11 = ((params->b) * (params->b));
  const double _t12 = 0.9e1*_t11;
#endif
#if _KMAX >= 3
  const double _t16 = 0.6e1*params->b;
  const double _t18 = ((params->b) * (params->b) * (params->b));
#endif
  double _hc0[(_KMAX) >= 4 ? 15 : (_KMAX) >= 3 ? 10 : (_KMAX) >= 2 ? 6 : (_KMAX) >= 1 ? 3 : 1];
  XC_CAT(opz_pow_n_gen_k, _KMAX)(p, z, _t1, _hc0);
  double _hc1[(_KMAX) >= 4 ? 15 : (_KMAX) >= 3 ? 10 : (_KMAX) >= 2 ? 6 : (_KMAX) >= 1 ? 3 : 1];
  XC_CAT(opz_pow_n_gen_k, _KMAX)(p, -z, _t1, _hc1);
  const double _t4 = _t3*(_hc0[0] + _hc1[0]);
#if _KMAX >= 1
  const double _t5 = _t3*(_hc0[2] - _hc1[2]);
#endif
#if _KMAX >= 2
  const double _t8 = _t3*(_hc0[5] + _hc1[5]);
  const double _t10 = _t4*_t9;
#endif
#if _KMAX >= 3
  const double _t13 = _t3*(_hc0[9] - _hc1[9]);
  const double _t14 = _t5*_t9;
  const double _t15 = _t2*_t4;
  const double _t17 = 0.27e2*_t15;
#endif
#if _KMAX >= 4
  const double _t19 = _t8*_t9;
  const double _t20 = _t2*_t5;
  const double _t21 = 0.27e2*_t20;
  const double _t22 = _t4/(rs * rs * rs * rs);
#endif

  const double f = -_t4;
  out[0] = f;
#if _KMAX >= 1
  const double df_dz = -_t5;
  out[1] = df_dz;
  ...
  out[3] = d2f_dz2;
  const double d2f_drs_dz = _t5*_t7;
  out[4] = d2f_drs_dz;
  const double d2f_drs2 = -_t10*_t12 - _t10*_t6;
  out[5] = d2f_drs2;
#endif
#if _KMAX >= 3
  const double d3f_dz3 = -_t13;
  out[6] = d3f_dz3;
  const double d3f_drs_dz2 = _t7*_t8;
  out[7] = d3f_drs_dz2;
  const double d3f_drs2_dz = -_t12*_t14 - _t14*_t6;
  out[8] = d3f_drs2_dz;
  const double d3f_drs3 = _t11*_t17 + _t15*_t16 + _t17*_t18;
  out[9] = d3f_drs3;
#endif
#if _KMAX >= 4
  const double d4f_dz4 = -_t3*(_hc0[14] + _hc1[14]);
  out[10] = d4f_dz4;
  const double d4f_drs_dz3 = _t13*_t7;
  out[11] = d4f_drs_dz3;
  const double d4f_drs2_dz2 = -_t12*_t19 - _t19*_t6;
  out[12] = d4f_drs2_dz2;
  const double d4f_drs3_dz = _t11*_t21 + _t16*_t20 + _t18*_t21;
  out[13] = d4f_drs3_dz;
  const double d4f_drs4 = -0.99e2*_t11*_t22 - 0.162e3*_t18*_t22 - 0.81e2*_t22*((params->b) * (params->b) * (params->b) * (params->b)) - 0.18e2*_t22*params->b;
  out[14] = d4f_drs4;
#endif
}
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(f_sloc_z0_k, _KMAX)(const xc_func_type *p, double rs, double *out) {
  const lda_x_sloc_params *params = (const lda_x_sloc_params *)(p->params);

  double _hc0[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(opz_pow_n_gen_z0_k, _KMAX)(p, 0, _hc0);
  const double _t1 = (0.1e1 / (rs * rs * rs));
  const double _t2 = _hc0[0]*params->a*pow((0.3e1 / 0.4e1)*_t1/M_PI, params->b)/(0.2e1*params->b + 0.2e1);
#if _KMAX >= 1
  const double _t3 = _t2*params->b;
  const double _t4 = 0.6e1*_t3;
#endif
#if _KMAX >= 2
  const double _t5 = (0.1e1 / (rs * rs));
  const double _t6 = _t2*((params->b) * (params->b));
#endif
#if _KMAX >= 3
  const double _t7 = 0.54e2*_t1;
  const double _t8 = _t2*((params->b) * (params->b) * (params->b));
#endif
#if _KMAX >= 4
  const double _t9 = (0.1e1 / (rs * rs * rs * rs));
#endif

  const double f = -0.2e1*_t2;
  out[0] = f;
#if _KMAX >= 1
  const double df_drs = _t4/rs;
  out[1] = df_drs;
#endif
#if _KMAX >= 2
  const double d2f_drs2 = -_t4*_t5 - 0.18e2*_t5*_t6;
  out[2] = d2f_drs2;
#endif
#if _KMAX >= 3
  const double d3f_drs3 = 0.12e2*_t1*_t3 + _t6*_t7 + _t7*_t8;
  out[3] = d3f_drs3;
#endif
#if _KMAX >= 4
  const double d4f_drs4 = -0.162e3*_t2*_t9*((params->b) * (params->b) * (params->b) * (params->b)) - 0.36e2*_t3*_t9 - 0.198e3*_t6*_t9 - 0.324e3*_t8*_t9;
  out[4] = d4f_drs4;
#endif
}
...
FUNC_NAME(const xc_func_type *p, size_t ip, const double *rho, xc_lda_out_params *out)
{
  ...
  double _hc0[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(f_sloc_z0_k, _KMAX)(p, (0.1e1 / 0.2e1)*_t4, _hc0);
  ...
  double _hc0[(_KMAX) >= 4 ? 15 : (_KMAX) >= 3 ? 10 : (_KMAX) >= 2 ? 6 : (_KMAX) >= 1 ? 3 : 1];
  XC_CAT(f_sloc_k, _KMAX)(p, _t5/xc_powr(_t1, 1, 3), _t6*_t7, _hc0);
  const double zk = _hc0[0];
  if(out->zk != NULL) out->zk[ip*p->dim.zk + 0] += zk;
#if _KMAX >= 1
