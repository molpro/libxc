/*
  Generated from python/lda_exc/lda_c_hl.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py lda_c_hl
*/

#ifndef _LDA_C_HL_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _LDA_C_HL_KERNEL_BODY
#define _KMAX 0
#define _LDA_C_HL_HELPER_BODIES
#include "lda_c_hl.c"
#undef _LDA_C_HL_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _LDA_C_HL_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_LDA_C_HL_HELPER_BODIES)
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(hl_f_k, _KMAX)(const xc_func_type *p, double rs, double zeta, double *out) {
  const lda_c_hl_params *params = (const lda_c_hl_params *)(p->params);

  double _hc0[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(f_zeta_k, _KMAX)(p, zeta, _hc0);
  const double _t1 = (0.1e1 / (params->hl_r[0]));
  const double _t2 = ((params->hl_r[0]) * (params->hl_r[0]));
  const double _t3 = (0.1e1 / _t2);
  const double _t4 = (rs * rs);
  const double _t5 = (0.1e1 / rs);
  const double _t6 = _t5*params->hl_r[0];
  const double _t7 = xc_log1p(_t6);
  const double _t8 = ((params->hl_r[0]) * (params->hl_r[0]) * (params->hl_r[0]));
  const double _t9 = (0.1e1 / _t8);
  const double _t10 = (rs * rs * rs);
  const double _t11 = _t10*_t9 + 0.1e1;
  const double _t12 = params->hl_c[0]*((0.1e1 / 0.2e1)*_t1*rs + _t11*_t7 - _t3*_t4 - 0.1e1 / 0.3e1);
  const double _t13 = (0.1e1 / (params->hl_r[1]));
  const double _t14 = ((params->hl_r[1]) * (params->hl_r[1]));
  const double _t15 = (0.1e1 / _t14);
  const double _t16 = _t5*params->hl_r[1];
  const double _t17 = xc_log1p(_t16);
  const double _t18 = ((params->hl_r[1]) * (params->hl_r[1]) * (params->hl_r[1]));
  const double _t19 = (0.1e1 / _t18);
  const double _t20 = _t10*_t19 + 0.1e1;
  const double _t21 = _t12 - params->hl_c[1]*((0.1e1 / 0.2e1)*_t13*rs - _t15*_t4 + _t17*_t20 - 0.1e1 / 0.3e1);
#if _KMAX >= 1
  const double _t22 = 0.2e1*_t3;
  const double _t23 = (0.1e1 / _t4);
  const double _t24 = _t6 + 0.1e1;
  const double _t25 = (0.1e1 / _t24);
  const double _t26 = _t11*_t25*params->hl_r[0];
  const double _t27 = params->hl_c[0]*((0.1e1 / 0.2e1)*_t1 - _t22*rs - _t23*_t26 + 0.3e1*_t4*_t7*_t9);
  const double _t28 = 0.2e1*_t15;
  const double _t29 = _t16 + 0.1e1;
  const double _t30 = (0.1e1 / _t29);
  const double _t31 = _t20*_t30*params->hl_r[1];
  const double _t32 = _t27 - params->hl_c[1]*((0.1e1 / 0.2e1)*_t13 + 0.3e1*_t17*_t19*_t4 - _t23*_t31 - _t28*rs);
#endif
#if _KMAX >= 2
  const double _t33 = _t25*_t3;
  const double _t34 = (0.1e1 / _t10);
  const double _t35 = (0.1e1 / (rs * rs * rs * rs));
  const double _t36 = (0.1e1 / (_t24 * _t24));
  const double _t37 = _t11*_t2*_t36;
  const double _t38 = params->hl_c[0]*(0.2e1*_t11*_t25*_t34*params->hl_r[0] - _t22 - 0.6e1*_t33 - _t35*_t37 + 0.6e1*_t7*_t9*rs);
  const double _t39 = _t15*_t30;
  const double _t40 = (0.1e1 / (_t29 * _t29));
  const double _t41 = _t14*_t20*_t40;
  const double _t42 = _t38 - params->hl_c[1]*(0.6e1*_t17*_t19*rs + 0.2e1*_t20*_t30*_t34*params->hl_r[1] - _t28 - _t35*_t41 - 0.6e1*_t39);
#endif
#if _KMAX >= 3
  const double _t43 = 0.9e1*_t23;
  const double _t44 = 0.6e1*_t35;
  const double _t45 = xc_powi(rs, -5);
  const double _t46 = xc_powi(rs, -6);
  const double _t47 = 0.2e1*_t46;
  const double _t48 = (0.1e1 / (_t24 * _t24 * _t24));
  const double _t49 = params->hl_c[0]*(-_t1*_t36*_t43 + 0.6e1*_t11*_t2*_t36*_t45 - _t11*_t47*_t48*_t8 - _t26*_t44 + 0.6e1*_t7*_t9);
  const double _t50 = (0.1e1 / (_t29 * _t29 * _t29));
  const double _t51 = _t49 - params->hl_c[1]*(-_t13*_t40*_t43 + 0.6e1*_t14*_t20*_t40*_t45 + 0.6e1*_t17*_t19 - _t18*_t20*_t47*_t50 - _t31*_t44);
#endif
#if _KMAX >= 4
  const double _t52 = 0.24e2*_t35;
  const double _t53 = 0.24e2*_t23;
  const double _t54 = 0.36e2*_t46;
  const double _t55 = xc_powi(rs, -7);
  const double _t56 = 0.6e1/xc_powi(rs, 8);
  const double _t57 = params->hl_c[0]*(0.36e2*_t1*_t34*_t36 + 0.24e2*_t11*_t25*_t45*params->hl_r[0] + 0.24e2*_t11*_t48*_t55*_t8 - _t11*_t56*((params->hl_r[0]) * (params->hl_r[0]) * (params->hl_r[0]) * (params->hl_r[0]))/(_t24 * _t24 * _t24 * _t24) - _t33*_t53 - _t37*_t54 - _t48*_t52);
#endif

  const double f = _hc0[0]*_t21 - _t12;
  out[0] = f;
#if _KMAX >= 1
  const double df_dzeta = _hc0[1]*_t21;
  out[1] = df_dzeta;
  const double df_drs = _hc0[0]*_t32 - _t27;
  out[2] = df_drs;
#endif
#if _KMAX >= 2
  const double d2f_dzeta2 = _hc0[2]*_t21;
  out[3] = d2f_dzeta2;
  const double d2f_drs_dzeta = _hc0[1]*_t32;
  out[4] = d2f_drs_dzeta;
  const double d2f_drs2 = _hc0[0]*_t42 - _t38;
  out[5] = d2f_drs2;
#endif
#if _KMAX >= 3
  const double d3f_dzeta3 = _hc0[3]*_t21;
  out[6] = d3f_dzeta3;
  const double d3f_drs_dzeta2 = _hc0[2]*_t32;
  out[7] = d3f_drs_dzeta2;
  const double d3f_drs2_dzeta = _hc0[1]*_t42;
  out[8] = d3f_drs2_dzeta;
  const double d3f_drs3 = _hc0[0]*_t51 - _t49;
  out[9] = d3f_drs3;
#endif
#if _KMAX >= 4
  const double d4f_dzeta4 = _hc0[4]*_t21;
  out[10] = d4f_dzeta4;
  const double d4f_drs_dzeta3 = _hc0[3]*_t32;
  out[11] = d4f_drs_dzeta3;
  const double d4f_drs2_dzeta2 = _hc0[2]*_t42;
  out[12] = d4f_drs2_dzeta2;
  const double d4f_drs3_dzeta = _hc0[1]*_t51;
  out[13] = d4f_drs3_dzeta;
  const double d4f_drs4 = _hc0[0]*(_t57 - params->hl_c[1]*(0.36e2*_t13*_t34*_t40 + 0.24e2*_t18*_t20*_t50*_t55 + 0.24e2*_t20*_t30*_t45*params->hl_r[1] - _t20*_t56*((params->hl_r[1]) * (params->hl_r[1]) * (params->hl_r[1]) * (params->hl_r[1]))/(_t29 * _t29 * _t29 * _t29) - _t39*_t53 - _t41*_t54 - _t50*_t52)) - _t57;
  out[14] = d4f_drs4;
#endif
}
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(hl_f_zeta0_k, _KMAX)(const xc_func_type *p, double rs, double *out) {
  ...
  const double _t3 = (0.1e1 / _t2);
  const double _t4 = (rs * rs);
  const double _t5 = params->hl_r[0]/rs;
  const double _t6 = xc_log1p(_t5);
  const double _t7 = ((params->hl_r[0]) * (params->hl_r[0]) * (params->hl_r[0]));
  const double _t8 = (0.1e1 / _t7);
  const double _t9 = (rs * rs * rs);
  const double _t10 = _t8*_t9 + 0.1e1;
#if _KMAX >= 1
  const double _t11 = 0.2e1*_t3;
  const double _t12 = (0.1e1 / _t4);
  const double _t13 = _t5 + 0.1e1;
  const double _t14 = (0.1e1 / _t13);
  const double _t15 = _t10*_t14*params->hl_r[0];
#endif
#if _KMAX >= 2
  const double _t16 = _t14*_t3;
  const double _t17 = (0.1e1 / _t9);
  const double _t18 = (0.1e1 / (rs * rs * rs * rs));
  const double _t19 = (0.1e1 / (_t13 * _t13));
  const double _t20 = _t10*_t19*_t2;
#endif
#if _KMAX >= 3
  const double _t21 = xc_powi(rs, -5);
  const double _t22 = (0.1e1 / (_t13 * _t13 * _t13));
  const double _t23 = xc_powi(rs, -6);
#endif

  const double f = -params->hl_c[0]*((0.1e1 / 0.2e1)*_t1*rs + _t10*_t6 - _t3*_t4 - 0.1e1 / 0.3e1);
  out[0] = f;
#if _KMAX >= 1
  const double df_drs = -params->hl_c[0]*((0.1e1 / 0.2e1)*_t1 - _t11*rs - _t12*_t15 + 0.3e1*_t4*_t6*_t8);
  out[1] = df_drs;
#endif
#if _KMAX >= 2
  const double d2f_drs2 = -params->hl_c[0]*(0.2e1*_t10*_t14*_t17*params->hl_r[0] - _t11 - 0.6e1*_t16 - _t18*_t20 + 0.6e1*_t6*_t8*rs);
  out[2] = d2f_drs2;
#endif
#if _KMAX >= 3
  const double d3f_drs3 = -params->hl_c[0]*(-0.9e1*_t1*_t12*_t19 + 0.6e1*_t10*_t19*_t2*_t21 - 0.2e1*_t10*_t22*_t23*_t7 - 0.6e1*_t15*_t18 + 0.6e1*_t6*_t8);
  out[3] = d3f_drs3;
#endif
#if _KMAX >= 4
  const double d4f_drs4 = -params->hl_c[0]*(0.36e2*_t1*_t17*_t19 + 0.24e2*_t10*_t14*_t21*params->hl_r[0] + 0.24e2*_t10*_t22*_t7/xc_powi(rs, 7) - 0.6e1*_t10*((params->hl_r[0]) * (params->hl_r[0]) * (params->hl_r[0]) * (params->hl_r[0]))/((_t13 * _t13 * _t13 * _t13)*xc_powi(rs, 8)) - 0.24e2*_t12*_t16 - 0.24e2*_t18*_t22 - 0.36e2*_t20*_t23);
  out[4] = d4f_drs4;
#endif
}
...
FUNC_NAME(const xc_func_type *p, size_t ip, const double *rho, xc_lda_out_params *out)
{
  ...
  double _hc0[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(hl_f_zeta0_k, _KMAX)(p, (0.1e1 / 0.2e1)*_t4, _hc0);
  ...
  XC_CAT(hl_f_k, _KMAX)(p, _t5/xc_powr(_t1, 1, 3), _t6*_t7, _hc0);
  const double zk = _hc0[0];
  if(out->zk != NULL) out->zk[ip*p->dim.zk + 0] += zk;
#if _KMAX >= 1
