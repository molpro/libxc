/*
  Generated from python/mgga_exc/mgga_x_ktbm.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py mgga_x_ktbm
*/

#ifndef _MGGA_X_KTBM_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _MGGA_X_KTBM_KERNEL_BODY
#define _KMAX 0
#define _MGGA_X_KTBM_HELPER_BODIES
#include "mgga_x_ktbm.c"
#undef _MGGA_X_KTBM_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _MGGA_X_KTBM_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_MGGA_X_KTBM_HELPER_BODIES)
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(ktbm_t_k, _KMAX)(const xc_func_type *p, double t, double *out) {

  const double _t1 = (0.5e1 / 0.9e1)*xc_powr(0.6e1, 1, 3)/(M_PI * M_CBRTPI);

  const double f = _t1*t;
  out[0] = f;
#if _KMAX >= 1
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(ktbm_top_k, _KMAX)(const xc_func_type *p, double p_, double t, double *out) {
  const mgga_x_ktbm_params *params = (const mgga_x_ktbm_params *)(p->params);

  const double _t1 = p_*params->xt;
#if _KMAX >= 1
  const double _t2 = 0.2e1*params->b2t;
  const double _t3 = 0.2e1*params->a2t;
#endif

  const double f = _t1*t + (p_ * p_)*params->a2t + p_*params->at + params->b2t*(t * t) + params->bt*t + params->ct;
  out[0] = f;
#if _KMAX >= 1
  const double df_dt = _t1 + _t2*t + params->bt;
  out[1] = df_dt;
  const double df_dp_ = _t3*p_ + params->at + params->xt*t;
  out[2] = df_dp_;
#endif
#if _KMAX >= 2
  const double d2f_dt2 = _t2;
  out[3] = d2f_dt2;
  const double d2f_dp__dt = params->xt;
  out[4] = d2f_dp__dt;
  const double d2f_dp_2 = _t3;
  out[5] = d2f_dp_2;
#endif
#if _KMAX >= 3
  const double d3f_dt3 = 0;
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(ktbm_bot_k, _KMAX)(const xc_func_type *p, double p_, double t, double *out) {
  const mgga_x_ktbm_params *params = (const mgga_x_ktbm_params *)(p->params);

  const double _t1 = p_*params->xb;
#if _KMAX >= 1
  const double _t2 = 0.2e1*params->b2b;
  const double _t3 = 0.2e1*params->a2b;
#endif

  const double f = _t1*t + (p_ * p_)*params->a2b + p_*params->ab + params->b2b*(t * t) + params->bb*t + params->cb;
  out[0] = f;
#if _KMAX >= 1
  const double df_dt = _t1 + _t2*t + params->bb;
  out[1] = df_dt;
  const double df_dp_ = _t3*p_ + params->ab + params->xb*t;
  out[2] = df_dp_;
#endif
#if _KMAX >= 2
  const double d2f_dt2 = _t2;
  out[3] = d2f_dt2;
  const double d2f_dp__dt = params->xb;
  out[4] = d2f_dp__dt;
  const double d2f_dp_2 = _t3;
  out[5] = d2f_dp_2;
#endif
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(ktbm_f_pf_k, _KMAX)(const xc_func_type *p, double x2, double u, double t, double *out) {

  double _hc0[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(ktbm_t_k, _KMAX)(p, t, _hc0);
  const double _t1 = xc_powr(0.6e1, 1, 3);
  const double _t2 = (0.1e1 / (M_PI * M_CBRTPI));
  const double _t3 = _t1*_t2;
  const double _t4 = (0.1e1 / 0.24e2)*_t3;
  const double _t5 = _t4*x2;
#if _KMAX >= 2
  const double _t12 = ((_hc0[1]) * (_hc0[1]));
  const double _t14 = 0.2e1*_t12;
  ...
  const double _t30 = (0.1e1 / 0.288e3)*_t27;
#endif
#if _KMAX >= 3
  const double _t35 = ((_hc0[1]) * (_hc0[1]) * (_hc0[1]));
  const double _t36 = 0.3e1*_t35;
  const double _t42 = (0.1e1 / 0.12e2)*_t12;
  const double _t51 = (0.1e1 / 0.96e2)*_t27;
  const double _t53 = (0.1e1 / (M_PI * M_PI * M_PI * M_PI));
  const double _t54 = (0.1e1 / 0.768e3)*_t53;
  const double _t58 = (0.1e1 / 0.384e3)*_t53;
#endif
#if _KMAX >= 4
  const double _t60 = ((_hc0[1]) * (_hc0[1]) * (_hc0[1]) * (_hc0[1]));
  const double _t61 = 0.6e1*_t60;
  const double _t62 = 0.24e2*_t60;
  const double _t69 = _t3*_t35;
  const double _t74 = (0.1e1 / 0.4e1)*_t69;
  const double _t80 = (0.1e1 / 0.144e3)*_t27;
  const double _t81 = _t12*_t80;
  const double _t82 = _t12*_t27;
  const double _t83 = (0.1e1 / 0.72e2)*_t82;
  const double _t88 = (0.1e1 / 0.24e2)*_t82;
  const double _t89 = (0.1e1 / 0.48e2)*_t82;
  const double _t94 = _t1/(M_PI * M_PI * M_PI * M_PI * M_PI * M_CBRTPI);
  const double _t96 = (0.1e1 / 0.2304e4)*_t94;
#endif
  double _hc1[(_KMAX) >= 4 ? 15 : (_KMAX) >= 3 ? 10 : (_KMAX) >= 2 ? 6 : (_KMAX) >= 1 ? 3 : 1];
  XC_CAT(ktbm_bot_k, _KMAX)(p, _t5, _hc0[0], _hc1);
  double _hc2[(_KMAX) >= 4 ? 15 : (_KMAX) >= 3 ? 10 : (_KMAX) >= 2 ? 6 : (_KMAX) >= 1 ? 3 : 1];
  XC_CAT(ktbm_top_k, _KMAX)(p, _t5, _hc0[0], _hc2);
  const double _t6 = (0.1e1 / (_hc1[0]));
#if _KMAX >= 1
  const double _t7 = (0.1e1 / ((_hc1[0]) * (_hc1[0])));
  const double _t8 = _hc2[0]*_t7;
  const double _t9 = _hc0[1]*_hc1[1];
  const double _t10 = _hc2[2]*_t4;
  const double _t11 = _t4*_t8;
#endif
#if _KMAX >= 2
  const double _t13 = _hc2[3]*_t12;
  const double _t15 = _hc1[1]*_t7;
  const double _t16 = _hc1[3]*_t12;
  const double _t17 = (0.1e1 / ((_hc1[0]) * (_hc1[0]) * (_hc1[0])));
  const double _t18 = ((_hc1[1]) * (_hc1[1]));
  const double _t19 = _t17*_t18;
  const double _t20 = _t10*_t7;
  const double _t21 = _hc1[2]*_t7;
  const double _t22 = _t21*_t4;
  const double _t23 = _hc0[1]*_hc2[1];
  const double _t24 = _hc0[1]*_hc1[4];
  const double _t29 = _hc2[5]*_t28;
  const double _t31 = _t21*_t30;
  const double _t32 = ((_hc1[2]) * (_hc1[2]));
  const double _t33 = _t17*_t32;
  const double _t34 = _t30*_t33;
#endif
#if _KMAX >= 3
  const double _t37 = _hc1[3]*_t7;
  const double _t38 = ((_hc1[1]) * (_hc1[1]) * (_hc1[1]));
  const double _t39 = (0.1e1 / ((_hc1[0]) * (_hc1[0]) * (_hc1[0]) * (_hc1[0])));
  const double _t40 = _hc2[0]*_t39;
  const double _t41 = _hc2[4]*_t3;
  const double _t43 = _hc1[4]*_t7;
  const double _t44 = _t18*_t40;
  const double _t45 = _t29*_t7;
  const double _t46 = _hc0[1]*_hc2[4];
  const double _t47 = _t24*_t7;
  const double _t48 = _hc1[5]*_t7;
  const double _t49 = _t28*_t48;
  const double _t50 = _t32*_t40;
  const double _t52 = _t50*_t51;
  const double _t55 = _hc2[5]*_t54;
  const double _t56 = _t48*_t54;
  const double _t57 = ((_hc1[2]) * (_hc1[2]) * (_hc1[2]));
  const double _t59 = _t57*_t58;
#endif
#if _KMAX >= 4
  const double _t63 = _hc1[1]*_hc1[3];
  const double _t64 = _hc2[1]*_t17;
  const double _t65 = _hc2[0]/xc_powi((_hc1[0]), 5);
  const double _t66 = _hc2[1]*_t39;
  const double _t67 = _hc2[0]*_t17;
  const double _t68 = _t35*_t41;
  const double _t70 = _hc2[3]*_t69;
  const double _t71 = _hc1[1]*_hc1[2];
  const double _t72 = _t17*_t71;
  const double _t73 = _hc2[2]*_t17;
  const double _t75 = _hc1[4]*_t69;
  const double _t76 = _hc1[2]*_t69;
  const double _t77 = _hc2[2]*_t39;
  const double _t78 = _t38*_t69;
  const double _t79 = (0.3e1 / 0.4e1)*_t76;
  const double _t84 = _hc1[4]*_t83;
  const double _t85 = _hc1[5]*_t64;
  const double _t86 = _hc1[2]*_t73;
  const double _t87 = _hc1[5]*_t67;
  const double _t90 = _hc1[2]*_t58;
  const double _t91 = _t24*_t53;
  const double _t92 = _t53*_t9;
  const double _t93 = (0.1e1 / 0.128e3)*_t32;
  const double _t95 = _hc2[5]*_t94;
#endif

  const double f = _hc2[0]*_t6;
  out[0] = f;
#if _KMAX >= 1
  const double df_dt = _hc0[1]*_hc2[1]*_t6 - _t8*_t9;
  out[1] = df_dt;
  const double df_du = 0;
  out[2] = df_du;
  const double df_dx2 = -_hc1[2]*_t11 + _t10*_t6;
  out[3] = df_dx2;
#endif
#if _KMAX >= 2
  const double d2f_dt2 = _hc2[0]*_t14*_t19 - _hc2[1]*_t14*_t15 + _t13*_t6 - _t16*_t8;
  out[4] = d2f_dt2;
  const double d2f_du_dt = 0;
  out[5] = d2f_du_dt;
  const double d2f_du2 = 0;
  out[6] = d2f_du2;
  const double d2f_dx2_dt = (0.1e1 / 0.12e2)*_hc0[1]*_hc1[1]*_hc1[2]*_hc2[0]*_t1*_t17*_t2 + (0.1e1 / 0.24e2)*_hc0[1]*_hc2[4]*_t1*_t2*_t6 - _t11*_t24 - _t20*_t9 - _t22*_t23;
  out[7] = d2f_dx2_dt;
  const double d2f_dx2_du = 0;
  out[8] = d2f_dx2_du;
  const double d2f_dx22 = -_hc1[5]*_t28*_t8 + _hc2[0]*_t34 - _hc2[2]*_t31 + _t29*_t6;
  out[9] = d2f_dx22;
#endif
#if _KMAX >= 3
  const double d3f_dt3 = 0.6e1*_hc1[1]*_hc1[3]*_hc2[0]*_t17*_t35 + 0.6e1*_hc2[1]*_t17*_t18*_t35 - _hc2[1]*_t36*_t37 - _hc2[3]*_t15*_t36 - 0.6e1*_t35*_t38*_t40;
  out[10] = d3f_dt3;
  ...
  out[13] = d3f_du3;
  const double d3f_dx2_dt2 = (0.1e1 / 0.6e1)*_hc1[1]*_hc1[2]*_hc2[1]*_t1*_t12*_t17*_t2 + (0.1e1 / 0.6e1)*_hc1[1]*_hc1[4]*_hc2[0]*_t1*_t12*_t17*_t2 + (0.1e1 / 0.12e2)*_hc1[2]*_hc1[3]*_hc2[0]*_t1*_t12*_t17*_t2 - 0.1e1 / 0.4e1*_hc1[2]*_t12*_t3*_t44 - _hc2[1]*_t3*_t42*_t43 + (0.1e1 / 0.12e2)*_hc2[2]*_t1*_t12*_t17*_t18*_t2 - _t13*_t22 - _t15*_t41*_t42 - _t16*_t20;
  out[14] = d3f_dx2_dt2;
  const double d3f_dx2_du_dt = 0;
  out[15] = d3f_dx2_du_dt;
  const double d3f_dx2_du2 = 0;
  out[16] = d3f_dx2_du2;
  const double d3f_dx22_dt = (0.1e1 / 0.144e3)*_hc0[1]*_hc1[1]*_hc1[2]*_hc2[2]*_t17*_t25*_t26 + (0.1e1 / 0.288e3)*_hc0[1]*_hc1[1]*_hc1[5]*_hc2[0]*_t17*_t25*_t26 + (0.1e1 / 0.144e3)*_hc0[1]*_hc1[2]*_hc1[4]*_hc2[0]*_t17*_t25*_t26 + (0.1e1 / 0.288e3)*_hc0[1]*_hc2[1]*_t17*_t25*_t26*_t32 - _hc2[2]*_t30*_t47 - _t23*_t49 - _t31*_t46 - _t45*_t9 - _t52*_t9;
  out[17] = d3f_dx22_dt;
  const double d3f_dx22_du = 0;
  out[18] = d3f_dx22_du;
  const double d3f_dx23 = (0.1e1 / 0.384e3)*_hc1[2]*_hc1[5]*_hc2[0]*_t17*_t53 + (0.1e1 / 0.384e3)*_hc2[2]*_t17*_t32*_t53 - _hc2[2]*_t56 - _t21*_t55 - _t40*_t59;
  out[19] = d3f_dx23;
#endif
#if _KMAX >= 4
  const double d4f_dt4 = ((_hc1[1]) * (_hc1[1]) * (_hc1[1]) * (_hc1[1]))*_t62*_t65 + ((_hc1[3]) * (_hc1[3]))*_t61*_t67 - 0.36e2*_hc1[3]*_t44*_t60 + 0.12e2*_hc2[3]*_t19*_t60 - _hc2[3]*_t37*_t61 - _t38*_t62*_t66 + _t62*_t63*_t64;
  out[20] = d4f_dt4;
  ...
  out[24] = d4f_du4;
  const double d4f_dx2_dt3 = (0.1e1 / 0.2e1)*_hc1[1]*_t64*_t75 + _hc1[2]*_t65*_t78 + _hc1[3]*_hc1[4]*_t67*_t74 + (0.1e1 / 0.4e1)*_hc1[3]*_t64*_t76 - _t18*_t66*_t79 + (0.1e1 / 0.4e1)*_t19*_t68 - 0.1e1 / 0.8e1*_t37*_t68 - _t40*_t63*_t79 - 0.1e1 / 0.8e1*_t43*_t70 - 0.3e1 / 0.4e1*_t44*_t75 + _t63*_t73*_t74 + (0.1e1 / 0.4e1)*_t70*_t72 - 0.1e1 / 0.4e1*_t77*_t78;
  out[25] = d4f_dx2_dt3;
  ...
  out[28] = d4f_dx2_du3;
  const double d4f_dx22_dt2 = -_hc1[1]*_t32*_t66*_t89 + _hc1[1]*_t73*_t84 + _hc1[1]*_t81*_t85 - _hc1[2]*_t18*_t77*_t89 + _hc1[2]*_t64*_t84 + ((_hc1[4]) * (_hc1[4]))*_t67*_t81 - _hc1[4]*_t40*_t71*_t88 - _hc1[5]*_t12*_t44*_t51 - _hc2[4]*_t43*_t81 + _hc2[4]*_t72*_t83 + _hc2[5]*_t12*_t19*_t30 + _t13*_t34 - _t13*_t49 + _t16*_t30*_t87 - _t16*_t45 - _t16*_t52 + _t16*_t80*_t86 + _t18*_t32*_t65*_t88;
  out[29] = d4f_dx22_dt2;
  const double d4f_dx22_du_dt = 0;
  out[30] = d4f_dx22_du_dt;
  const double d4f_dx22_du2 = 0;
  out[31] = d4f_dx22_du2;
  const double d4f_dx23_dt = -_hc0[1]*_t59*_t66 + _hc0[1]*_t85*_t90 - 0.1e1 / 0.128e3*_hc1[2]*_hc1[5]*_t40*_t92 + _hc1[5]*_t58*_t73*_t9 + _hc2[5]*_t17*_t9*_t90 + _t24*_t58*_t87 + _t33*_t46*_t58 - _t40*_t91*_t93 - _t46*_t56 - _t47*_t55 + (0.1e1 / 0.96e2)*_t57*_t65*_t92 - _t77*_t92*_t93 + (0.1e1 / 0.192e3)*_t86*_t91;
  out[32] = d4f_dx23_dt;
  const double d4f_dx23_du = 0;
  out[33] = d4f_dx23_du;
  const double d4f_dx24 = ((_hc1[2]) * (_hc1[2]) * (_hc1[2]) * (_hc1[2]))*_t65*_t96 + (0.1e1 / 0.9216e4)*((_hc1[5]) * (_hc1[5]))*_t67*_t94 - 0.1e1 / 0.1536e4*_hc1[5]*_t50*_t94 + _hc1[5]*_t86*_t96 + (0.1e1 / 0.4608e4)*_t33*_t95 - 0.1e1 / 0.9216e4*_t48*_t95 - _t57*_t77*_t96;
  out[34] = d4f_dx24;
#endif
}
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(xchan_k, _KMAX)(const xc_func_type *p, double n_s, double sigma_ss, double lapl_s, double tau_s, double *out) {
  ...
  if(!(_t7)) XC_CAT(ktbm_f_pf_k, _KMAX)(p, _t1*sigma_ss, 0, _t2*tau_s, _hc0);
