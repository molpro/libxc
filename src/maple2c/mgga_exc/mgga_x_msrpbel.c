/*
  Generated from python/mgga_exc/mgga_x_msrpbel.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py mgga_x_msrpbel
*/

#ifndef _MGGA_X_MSRPBEL_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _MGGA_X_MSRPBEL_KERNEL_BODY
#define _KMAX 0
#define _MGGA_X_MSRPBEL_HELPER_BODIES
#include "mgga_x_msrpbel.c"
#undef _MGGA_X_MSRPBEL_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _MGGA_X_MSRPBEL_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_MGGA_X_MSRPBEL_HELPER_BODIES)
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(ms_fa_raw_k, _KMAX)(const xc_func_type *p, double a, double *out) {
  const mgga_x_msrpbel_params *params = (const mgga_x_msrpbel_params *)(p->params);
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(msrpbel_f0_k, _KMAX)(const xc_func_type *p, double p_, double c, double *out) {
  const mgga_x_msrpbel_params *params = (const mgga_x_msrpbel_params *)(p->params);

  const double _t1 = (0.1e1 / (params->kappa));
  const double _t2 = xc_expm1(_t1*(-c - 0.10e2 / 0.81e2*p_));
#if _KMAX >= 1
...
#endif
#if _KMAX >= 3
  const double _t5 = _t3/((params->kappa) * (params->kappa));
#endif
#if _KMAX >= 4
  const double _t6 = _t3/((params->kappa) * (params->kappa) * (params->kappa));
#endif

  const double f = -_t2*params->kappa + 0.1e1;
  out[0] = f;
#if _KMAX >= 1
  const double df_dc = _t3;
  out[1] = df_dc;
  const double df_dp_ = (0.10e2 / 0.81e2)*_t2 + 0.10e2 / 0.81e2;
  out[2] = df_dp_;
#endif
#if _KMAX >= 2
  const double d2f_dc2 = -_t4;
  out[3] = d2f_dc2;
  const double d2f_dp__dc = -0.10e2 / 0.81e2*_t4;
  out[4] = d2f_dp__dc;
  const double d2f_dp_2 = -0.100e3 / 0.6561e4*_t4;
  out[5] = d2f_dp_2;
#endif
#if _KMAX >= 3
  const double d3f_dc3 = _t5;
  out[6] = d3f_dc3;
  const double d3f_dp__dc2 = (0.10e2 / 0.81e2)*_t5;
  out[7] = d3f_dp__dc2;
  const double d3f_dp_2_dc = (0.100e3 / 0.6561e4)*_t5;
  out[8] = d3f_dp_2_dc;
  const double d3f_dp_3 = (0.1000e4 / 0.531441e6)*_t5;
  out[9] = d3f_dp_3;
#endif
#if _KMAX >= 4
  const double d4f_dc4 = -_t6;
  out[10] = d4f_dc4;
  const double d4f_dp__dc3 = -0.10e2 / 0.81e2*_t6;
  out[11] = d4f_dp__dc3;
  const double d4f_dp_2_dc2 = -0.100e3 / 0.6561e4*_t6;
  out[12] = d4f_dp_2_dc2;
  const double d4f_dp_3_dc = -0.1000e4 / 0.531441e6*_t6;
  out[13] = d4f_dp_3_dc;
  const double d4f_dp_4 = -0.10000e5 / 0.43046721e8*_t6;
  out[14] = d4f_dp_4;
#endif
}

GPU_DEVICE_FUNCTION static inline void XC_CAT(msrpbel_f0_delta_k, _KMAX)(const xc_func_type *p, double p_, double c, double *out) {
  const mgga_x_msrpbel_params *params = (const mgga_x_msrpbel_params *)(p->params);

  const double _t1 = (0.1e1 / (params->kappa));
  const double _t2 = (0.10e2 / 0.81e2)*_t1;
  const double _t3 = exp(-_t2*p_);
  const double _t4 = xc_expm1(-_t1*c);
  const double _t5 = _t3*_t4;
#if _KMAX >= 1
  const double _t6 = _t3*(_t4 + 0.1e1);
#endif
#if _KMAX >= 3
  const double _t7 = (0.1e1 / ((params->kappa) * (params->kappa)));
  const double _t8 = _t6*_t7;
#endif
#if _KMAX >= 4
  const double _t9 = (0.1e1 / ((params->kappa) * (params->kappa) * (params->kappa)));
  const double _t10 = _t6*_t9;
#endif

  const double f = -_t5*params->kappa;
  out[0] = f;
#if _KMAX >= 1
  const double df_dc = _t6;
  out[1] = df_dc;
  const double df_dp_ = (0.10e2 / 0.81e2)*_t5;
  out[2] = df_dp_;
#endif
#if _KMAX >= 2
  const double d2f_dc2 = -_t1*_t6;
  out[3] = d2f_dc2;
  const double d2f_dp__dc = -_t2*_t6;
  out[4] = d2f_dp__dc;
  const double d2f_dp_2 = -0.100e3 / 0.6561e4*_t1*_t5;
  out[5] = d2f_dp_2;
#endif
#if _KMAX >= 3
  const double d3f_dc3 = _t8;
  out[6] = d3f_dc3;
  const double d3f_dp__dc2 = (0.10e2 / 0.81e2)*_t8;
  out[7] = d3f_dp__dc2;
  const double d3f_dp_2_dc = (0.100e3 / 0.6561e4)*_t8;
  out[8] = d3f_dp_2_dc;
  const double d3f_dp_3 = (0.1000e4 / 0.531441e6)*_t5*_t7;
  out[9] = d3f_dp_3;
#endif
#if _KMAX >= 4
  const double d4f_dc4 = -_t10;
  out[10] = d4f_dc4;
  const double d4f_dp__dc3 = -0.10e2 / 0.81e2*_t10;
  out[11] = d4f_dp__dc3;
  const double d4f_dp_2_dc2 = -0.100e3 / 0.6561e4*_t10;
  out[12] = d4f_dp_2_dc2;
  const double d4f_dp_3_dc = -0.1000e4 / 0.531441e6*_t10;
  out[13] = d4f_dp_3_dc;
  const double d4f_dp_4 = -0.10000e5 / 0.43046721e8*_t5*_t9;
  out[14] = d4f_dp_4;
#endif
}

GPU_DEVICE_FUNCTION static inline void XC_CAT(mgga_alpha_reg_pf_k, _KMAX)(const xc_func_type *p, double x2, double t, double eta, double *out) {

  const double _t1 = (0.1e1 / 0.8e1)*x2;
  const double _t2 = -_t1 + t;
  const double _t3 = _t1*eta + (0.3e1 / 0.10e2)*xc_powr(0.6e1, 2, 3)*(M_PI * M_CBRTPI);
  const double _t4 = (0.1e1 / _t3);
#if _KMAX >= 1
  const double _t5 = (0.1e1 / (_t3 * _t3));
  const double _t6 = _t1*_t5;
  const double _t7 = (0.1e1 / 0.8e1)*_t5;
  const double _t8 = _t2*_t7;
#endif
#if _KMAX >= 2
  const double _t9 = (x2 * x2);
  const double _t10 = (0.1e1 / (_t3 * _t3 * _t3));
  const double _t11 = _t10*_t9;
  const double _t12 = (0.1e1 / 0.32e2)*_t11;
  const double _t13 = (0.1e1 / 0.64e2)*x2;
  const double _t14 = _t10*eta;
  const double _t15 = _t14*x2;
  const double _t16 = (0.1e1 / 0.32e2)*_t15;
  const double _t17 = (eta * eta);
  const double _t18 = _t10*_t17;
  const double _t19 = (0.1e1 / 0.32e2)*_t18;
#endif
#if _KMAX >= 3
  const double _t20 = (x2 * x2 * x2);
  const double _t21 = (0.1e1 / (_t3 * _t3 * _t3 * _t3));
  const double _t22 = _t20*_t21;
  const double _t23 = (0.3e1 / 0.256e3)*_t22;
  const double _t24 = _t21*_t9;
  const double _t25 = _t24*eta;
  const double _t26 = (0.3e1 / 0.256e3)*_t25;
  const double _t27 = _t17*_t21;
  const double _t28 = _t27*x2;
  const double _t29 = (0.3e1 / 0.256e3)*_t28;
  const double _t30 = (eta * eta * eta);
  const double _t31 = _t21*_t30;
  const double _t32 = (0.3e1 / 0.256e3)*_t31;
#endif
#if _KMAX >= 4
  const double _t33 = (0.3e1 / 0.512e3)*_t2/xc_powi(_t3, 5);
  const double _t34 = (0.9e1 / 0.256e3)*_t2;
#endif

  const double f = _t2*_t4;
  out[0] = f;
#if _KMAX >= 1
  const double df_deta = -_t2*_t6;
  out[1] = df_deta;
  const double df_dt = _t4;
  out[2] = df_dt;
  const double df_dx2 = -0.1e1 / 0.8e1*_t4 - _t8*eta;
  out[3] = df_dx2;
#endif
#if _KMAX >= 2
  const double d2f_deta2 = _t12*_t2;
  out[4] = d2f_deta2;
  const double d2f_dt_deta = -_t6;
  out[5] = d2f_dt_deta;
  const double d2f_dt2 = 0;
  out[6] = d2f_dt2;
  const double d2f_dx2_deta = _t13*_t5 + _t16*_t2 - _t8;
  out[7] = d2f_dx2_deta;
  const double d2f_dx2_dt = -_t7*eta;
  out[8] = d2f_dx2_dt;
  const double d2f_dx22 = _t19*_t2 + (0.1e1 / 0.32e2)*_t5*eta;
  out[9] = d2f_dx22;
#endif
#if _KMAX >= 3
  const double d3f_deta3 = -_t2*_t23;
  out[10] = d3f_deta3;
  const double d3f_dt_deta2 = _t12;
  out[11] = d3f_dt_deta2;
  const double d3f_dt2_deta = 0;
  out[12] = d3f_dt2_deta;
  const double d3f_dt3 = 0;
  out[13] = d3f_dt3;
  const double d3f_dx2_deta2 = (0.1e1 / 0.16e2)*_t10*_t2*x2 - 0.1e1 / 0.256e3*_t11 - _t2*_t26;
  out[14] = d3f_dx2_deta2;
  const double d3f_dx2_dt_deta = _t16 - _t7;
  out[15] = d3f_dx2_dt_deta;
  const double d3f_dx2_dt2 = 0;
  out[16] = d3f_dx2_dt2;
  const double d3f_dx22_deta = (0.1e1 / 0.16e2)*_t10*_t2*eta - 0.1e1 / 0.128e3*_t15 - _t2*_t29 + (0.1e1 / 0.32e2)*_t5;
  out[17] = d3f_dx22_deta;
  const double d3f_dx22_dt = _t19;
  out[18] = d3f_dx22_dt;
  const double d3f_dx23 = -0.3e1 / 0.256e3*_t18 - _t2*_t32;
  out[19] = d3f_dx23;
#endif
#if _KMAX >= 4
  const double d4f_deta4 = _t33*(x2 * x2 * x2 * x2);
  out[20] = d4f_deta4;
  const double d4f_dt_deta3 = -_t23;
  out[21] = d4f_dt_deta3;
  ...
  out[24] = d4f_dt4;
  const double d4f_dx2_deta3 = _t20*_t33*eta + (0.3e1 / 0.2048e4)*_t22 - _t24*_t34;
  out[25] = d4f_dx2_deta3;
  const double d4f_dx2_dt_deta2 = (0.1e1 / 0.16e2)*_t10*x2 - _t26;
  out[26] = d4f_dx2_dt_deta2;
  const double d4f_dx2_dt2_deta = 0;
  out[27] = d4f_dx2_dt2_deta;
  const double d4f_dx2_dt3 = 0;
  out[28] = d4f_dx2_dt3;
  const double d4f_dx22_deta2 = -_t10*_t13 + (0.1e1 / 0.16e2)*_t10*_t2 + _t17*_t33*_t9 - 0.3e1 / 0.64e2*_t2*_t21*eta*x2 + (0.3e1 / 0.1024e4)*_t25;
  out[29] = d4f_dx22_deta2;
  const double d4f_dx22_dt_deta = (0.1e1 / 0.16e2)*_t10*eta - _t29;
  out[30] = d4f_dx22_dt_deta;
  const double d4f_dx22_dt2 = 0;
  out[31] = d4f_dx22_dt2;
  const double d4f_dx23_deta = -0.3e1 / 0.128e3*_t14 - _t27*_t34 + (0.9e1 / 0.2048e4)*_t28 + _t30*_t33*x2;
  out[32] = d4f_dx23_deta;
  const double d4f_dx23_dt = -_t32;
  out[33] = d4f_dx23_dt;
  const double d4f_dx24 = (0.3e1 / 0.512e3)*_t31 + _t33*(eta * eta * eta * eta);
  out[34] = d4f_dx24;
#endif
}

GPU_DEVICE_FUNCTION static inline void XC_CAT(msrpbel_f_pf_k, _KMAX)(const xc_func_type *p, double x2, double u, double t, double *out) {
  const mgga_x_msrpbel_params *params = (const mgga_x_msrpbel_params *)(p->params);

  double _hc0[(_KMAX) >= 4 ? 35 : (_KMAX) >= 3 ? 20 : (_KMAX) >= 2 ? 10 : (_KMAX) >= 1 ? 4 : 1];
  XC_CAT(mgga_alpha_reg_pf_k, _KMAX)(p, x2, t, params->eta, _hc0);
  double _hc1[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(ms_fa_k, _KMAX)(p, _hc0[0], _hc1);
  ...
  const double _t11 = ((_hc0[3]) * (_hc0[3]));
  const double _t12 = xc_powr(0.6e1, 2, 3)/(M_PI * M_PI * M_CBRTPI * M_CBRTPI);
  const double _t13 = (0.1e1 / 0.576e3)*_t12;
  const double _t15 = _hc0[3]*_hc1[1];
#endif
#if _KMAX >= 3
  const double _t18 = ((_hc0[2]) * (_hc0[2]) * (_hc0[2]));
  const double _t22 = _hc1[2]*_t7;
  const double _t24 = _hc0[3]*_hc0[8];
  const double _t26 = _hc0[8]*_hc1[1];
  const double _t27 = _hc0[2]*_hc1[2];
  const double _t29 = (0.1e1 / (M_PI * M_PI * M_PI * M_PI));
  const double _t30 = (0.1e1 / 0.2304e4)*_t29;
  const double _t31 = ((_hc0[3]) * (_hc0[3]) * (_hc0[3]));
  const double _t36 = _hc0[9]*_hc1[1];
  const double _t39 = _hc1[2]*_t11;
#endif
#if _KMAX >= 4
  const double _t41 = 0.3e1*_hc0[8];
  const double _t44 = _hc0[3]*_hc0[9];
  const double _t46 = (0.1e1 / 0.55296e5)*_t1/(M_PI * M_PI * M_PI * M_PI * M_PI * M_CBRTPI);
#endif
  double _hc2[(_KMAX) >= 4 ? 15 : (_KMAX) >= 3 ? 10 : (_KMAX) >= 2 ? 6 : (_KMAX) >= 1 ? 3 : 1];
  XC_CAT(msrpbel_f0_delta_k, _KMAX)(p, _t4, params->c, _hc2);
  double _hc3[(_KMAX) >= 4 ? 15 : (_KMAX) >= 3 ? 10 : (_KMAX) >= 2 ? 6 : (_KMAX) >= 1 ? 3 : 1];
  XC_CAT(msrpbel_f0_k, _KMAX)(p, _t4, 0, _hc3);
#if _KMAX >= 1
  const double _t5 = _hc1[1]*_hc2[0];
  const double _t6 = _hc2[2]*_t3;
#endif
#if _KMAX >= 2
  const double _t8 = _hc1[2]*_hc2[0];
  const double _t9 = _hc0[2]*_t8;
  const double _t14 = _hc2[5]*_t13;
  const double _t16 = _hc2[2]*_t2;
  const double _t17 = (0.1e1 / 0.12e2)*_t16;
#endif
#if _KMAX >= 3
  const double _t19 = _hc1[3]*_hc2[0];
  const double _t20 = 0.2e1*_t9;
  const double _t21 = _t19*_t7;
  const double _t23 = 0.2e1*_t8;
  const double _t25 = _t11*_t19;
  const double _t28 = _hc0[3]*_t17;
  const double _t32 = 0.3e1*_t8;
  const double _t33 = _hc0[3]*_t32;
  const double _t34 = _hc2[9]*_t30;
  const double _t35 = (0.1e1 / 0.8e1)*_t16;
  const double _t37 = _hc2[5]*_t12;
  const double _t38 = (0.1e1 / 0.192e3)*_t37;
#endif
#if _KMAX >= 4
  const double _t40 = _hc1[4]*_hc2[0];
  const double _t42 = _hc0[2]*_t19;
  const double _t43 = (0.1e1 / 0.6e1)*_t16;
  const double _t45 = _hc1[2]*_t16;
  const double _t47 = (0.1e1 / 0.96e2)*_t37;
#endif

  const double f = _hc1[0]*_hc2[0] + _hc3[0];
  out[0] = f;
#if _KMAX >= 1
  const double df_dt = _hc0[2]*_t5;
  out[1] = df_dt;
  const double df_du = 0;
  out[2] = df_du;
  const double df_dx2 = _hc0[3]*_t5 + _hc1[0]*_t6 + _hc3[2]*_t3;
  out[3] = df_dx2;
#endif
#if _KMAX >= 2
  ...
  out[6] = d2f_du2;
  const double d2f_dx2_dt = _hc0[3]*_t9 + _hc0[8]*_t5 + _t10*_t6;
  out[7] = d2f_dx2_dt;
  const double d2f_dx2_du = 0;
  out[8] = d2f_dx2_du;
  const double d2f_dx22 = _hc0[9]*_t5 + _hc1[0]*_t14 + _hc3[5]*_t13 + _t11*_t8 + _t15*_t17;
  out[9] = d2f_dx22;
#endif
#if _KMAX >= 3
  ...
  out[13] = d3f_du3;
  const double d3f_dx2_dt2 = _hc0[3]*_t21 + _hc0[8]*_t20 + _t22*_t6;
  out[14] = d3f_dx2_dt2;
  const double d3f_dx2_du_dt = 0;
  out[15] = d3f_dx2_du_dt;
  const double d3f_dx2_du2 = 0;
  out[16] = d3f_dx2_du2;
  const double d3f_dx22_dt = _hc0[18]*_t5 + _hc0[2]*_t25 + _hc0[9]*_t9 + _t10*_t14 + _t17*_t26 + _t23*_t24 + _t27*_t28;
  out[17] = d3f_dx22_dt;
  const double d3f_dx22_du = 0;
  out[18] = d3f_dx22_du;
  const double d3f_dx23 = _hc0[19]*_t5 + _hc0[9]*_t33 + _hc1[0]*_t34 + _hc3[9]*_t30 + _t15*_t38 + _t19*_t31 + _t35*_t36 + _t35*_t39;
  out[19] = d3f_dx23;
#endif
#if _KMAX >= 4
  const double d4f_dt4 = ((_hc0[2]) * (_hc0[2]) * (_hc0[2]) * (_hc0[2]))*_t40;
  out[20] = d4f_dt4;
  ...
  out[24] = d4f_du4;
  const double d4f_dx2_dt3 = _hc0[3]*_t18*_t40 + _hc1[3]*_t18*_t6 + _t21*_t41;
  out[25] = d4f_dx2_dt3;
  ...
  out[28] = d4f_dx2_du3;
  const double d4f_dx22_dt2 = _hc0[18]*_t20 + ((_hc0[8]) * (_hc0[8]))*_t23 + _hc0[8]*_t27*_t43 + _hc0[9]*_t21 + _hc1[3]*_t28*_t7 + _t11*_t40*_t7 + _t14*_t22 + 0.4e1*_t24*_t42;
  out[29] = d4f_dx22_dt2;
  const double d4f_dx22_du_dt = 0;
  out[30] = d4f_dx22_du_dt;
  const double d4f_dx22_du2 = 0;
  out[31] = d4f_dx22_du2;
  const double d4f_dx23_dt = _hc0[18]*_hc1[1]*_t35 + _hc0[18]*_t33 + _hc0[19]*_t9 + _hc0[2]*_hc1[3]*_t11*_t35 + _hc0[2]*_t31*_t40 + _hc0[3]*_t27*_t38 + _hc0[33]*_t5 + _hc0[8]*_hc0[9]*_t32 + _hc0[9]*_t27*_t35 + _t10*_t34 + (0.1e1 / 0.4e1)*_t24*_t45 + _t25*_t41 + _t26*_t38 + 0.3e1*_t42*_t44;
  out[32] = d4f_dx23_dt;
  const double d4f_dx23_du = 0;
  out[33] = d4f_dx23_du;
  const double d4f_dx24 = 0.4e1*_hc0[19]*_hc0[3]*_t8 + _hc0[19]*_hc1[1]*_t43 + ((_hc0[3]) * (_hc0[3]) * (_hc0[3]) * (_hc0[3]))*_t40 + _hc0[34]*_t5 + ((_hc0[9]) * (_hc0[9]))*_t32 + 0.6e1*_hc0[9]*_t25 + _hc1[0]*_hc2[14]*_t46 + _hc1[3]*_t31*_t43 + (0.1e1 / 0.576e3)*_hc2[9]*_t15*_t29 + _hc3[14]*_t46 + _t36*_t47 + _t39*_t47 + (0.1e1 / 0.2e1)*_t44*_t45;
  out[34] = d4f_dx24;
#endif
}
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(xchan_k, _KMAX)(const xc_func_type *p, double n_s, double sigma_ss, double lapl_s, double tau_s, double *out) {
  ...
  if(!(_t7)) XC_CAT(msrpbel_f_pf_k, _KMAX)(p, _t5*sigma_ss, 0, _t6*tau_s, _hc1);
