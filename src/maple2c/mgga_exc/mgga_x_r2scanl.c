/*
  Generated from python/mgga_exc/mgga_x_r2scanl.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py mgga_x_r2scanl
*/

#ifndef _MGGA_X_R2SCANL_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _MGGA_X_R2SCANL_KERNEL_BODY
#define _KMAX 0
#define _MGGA_X_R2SCANL_HELPER_BODIES
#include "mgga_x_r2scanl.c"
#undef _MGGA_X_R2SCANL_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _MGGA_X_R2SCANL_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_MGGA_X_R2SCANL_HELPER_BODIES)
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(mgga_alpha_deorb_reg_k, _KMAX)(const xc_func_type *p, double x, double a, double eta, double *out) {

  const double _t1 = (x * x);
  const double _t2 = xc_powr(0.6e1, 2, 3);
  const double _t3 = (M_PI * M_CBRTPI);
  const double _t4 = _t2*_t3;
  const double _t5 = (0.3e1 / 0.10e2)*_t4;
  const double _t6 = (0.1e1 / 0.8e1)*_t1*eta + _t5;
  const double _t7 = _t5/_t6;
#if _KMAX >= 1
  const double _t8 = _t4/(_t6 * _t6);
  const double _t9 = (0.3e1 / 0.80e2)*_t1;
  const double _t10 = _t8*_t9;
  const double _t11 = (0.3e1 / 0.40e2)*_t8;
  const double _t12 = _t11*x;
  const double _t13 = _t12*a;
#endif
#if _KMAX >= 2
  const double _t14 = (x * x * x * x);
  const double _t15 = (0.1e1 / (_t6 * _t6 * _t6));
  const double _t16 = _t15*_t4;
  const double _t17 = (0.3e1 / 0.320e3)*_t14*_t16;
  const double _t18 = (x * x * x);
  const double _t19 = (0.3e1 / 0.160e3)*_t16*_t18*eta;
  const double _t20 = _t11*a;
  const double _t21 = (eta * eta);
  const double _t22 = _t16*_t21;
  const double _t23 = _t22*_t9;
#endif
#if _KMAX >= 3
  const double _t24 = xc_powi(x, 6);
  const double _t25 = _t4/(_t6 * _t6 * _t6 * _t6);
  const double _t26 = (0.9e1 / 0.2560e4)*_t24*_t25;
  const double _t27 = xc_powi(x, 5);
  const double _t28 = _t25*_t27;
  const double _t29 = _t28*a;
  const double _t30 = (0.9e1 / 0.1280e4)*eta;
  const double _t31 = _t14*_t25;
  const double _t32 = (0.9e1 / 0.640e3)*_t21*_t31;
  const double _t33 = (eta * eta * eta);
  const double _t34 = _t18*_t25;
  const double _t35 = (0.9e1 / 0.320e3)*_t33*_t34;
#endif
#if _KMAX >= 4
  const double _t36 = _t4*a/xc_powi(_t6, 5);
  const double _t37 = (0.9e1 / 0.80e2)*a;
  const double _t38 = a*eta;
#endif

  const double f = _t7*a;
  out[0] = f;
#if _KMAX >= 1
  const double df_deta = -_t10*a;
  out[1] = df_deta;
  const double df_da = _t7;
  out[2] = df_da;
  const double df_dx = -_t13*eta;
  out[3] = df_dx;
#endif
#if _KMAX >= 2
  const double d2f_deta2 = _t17*a;
  out[4] = d2f_deta2;
  const double d2f_da_deta = -_t10;
  out[5] = d2f_da_deta;
  const double d2f_da2 = 0;
  out[6] = d2f_da2;
  const double d2f_dx_deta = -_t13 + _t19*a;
  out[7] = d2f_dx_deta;
  const double d2f_dx_da = -_t12*eta;
  out[8] = d2f_dx_da;
  const double d2f_dx2 = -_t20*eta + _t23*a;
  out[9] = d2f_dx2;
#endif
#if _KMAX >= 3
  const double d3f_deta3 = -_t26*a;
  out[10] = d3f_deta3;
  const double d3f_da_deta2 = _t17;
  out[11] = d3f_da_deta2;
  const double d3f_da2_deta = 0;
  out[12] = d3f_da2_deta;
  const double d3f_da3 = 0;
  out[13] = d3f_da3;
  const double d3f_dx_deta2 = (0.3e1 / 0.80e2)*_t15*_t18*_t2*_t3*a - _t29*_t30;
  out[14] = d3f_dx_deta2;
  const double d3f_dx_da_deta = -_t12 + _t19;
  out[15] = d3f_dx_da_deta;
  const double d3f_dx_da2 = 0;
  out[16] = d3f_dx_da2;
  const double d3f_dx2_deta = (0.3e1 / 0.32e2)*_t1*_t15*_t2*_t3*a*eta - _t20 - _t32*a;
  out[17] = d3f_dx2_deta;
  const double d3f_dx2_da = -_t11*eta + _t23;
  out[18] = d3f_dx2_da;
  const double d3f_dx3 = (0.9e1 / 0.80e2)*_t15*_t2*_t21*_t3*a*x - _t35*a;
  out[19] = d3f_dx3;
#endif
#if _KMAX >= 4
  const double d4f_deta4 = (0.9e1 / 0.5120e4)*_t36*xc_powi(x, 8);
  out[20] = d4f_deta4;
  const double d4f_da_deta3 = -_t26;
  out[21] = d4f_da_deta3;
  const double d4f_da2_deta2 = 0;
  out[22] = d4f_da2_deta2;
  const double d4f_da3_deta = 0;
  out[23] = d4f_da3_deta;
  const double d4f_da4 = 0;
  out[24] = d4f_da4;
  const double d4f_dx_deta3 = -0.27e2 / 0.1280e4*_t29 + (0.9e1 / 0.2560e4)*_t36*eta*xc_powi(x, 7);
  out[25] = d4f_dx_deta3;
  const double d4f_dx_da_deta2 = (0.3e1 / 0.80e2)*_t15*_t18*_t2*_t3 - _t28*_t30;
  out[26] = d4f_dx_da_deta2;
  const double d4f_dx_da2_deta = 0;
  out[27] = d4f_dx_da2_deta;
  const double d4f_dx_da3 = 0;
  out[28] = d4f_dx_da3;
  const double d4f_dx2_deta2 = _t1*_t16*_t37 + (0.9e1 / 0.1280e4)*_t21*_t24*_t36 - 0.81e2 / 0.1280e4*_t31*_t38;
  out[29] = d4f_dx2_deta2;
  const double d4f_dx2_da_deta = (0.3e1 / 0.32e2)*_t1*_t15*_t2*_t3*eta - _t11 - _t32;
  out[30] = d4f_dx2_da_deta;
  const double d4f_dx2_da2 = 0;
  out[31] = d4f_dx2_da2;
  const double d4f_dx3_deta = (0.9e1 / 0.40e2)*_t16*_t38*x - 0.81e2 / 0.640e3*_t21*_t34*a + (0.9e1 / 0.640e3)*_t27*_t33*_t36;
  out[32] = d4f_dx3_deta;
  const double d4f_dx3_da = (0.9e1 / 0.80e2)*_t15*_t2*_t21*_t3*x - _t35;
  out[33] = d4f_dx3_da;
  const double d4f_dx4 = -0.27e2 / 0.160e3*_t1*_t25*_t33*a + (0.9e1 / 0.320e3)*_t14*_t36*(eta * eta * eta * eta) + _t22*_t37;
  out[34] = d4f_dx4;
#endif
}

GPU_DEVICE_FUNCTION static inline void XC_CAT(scan_h1x_k, _KMAX)(const xc_func_type *p, double x, double *out) {
  const mgga_x_r2scanl_params *params = (const mgga_x_r2scanl_params *)(p->params);
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(r2scanl_f_k, _KMAX)(const xc_func_type *p, double x, double u, double t, double *out) {
  const mgga_x_r2scanl_params *params = (const mgga_x_r2scanl_params *)(p->params);

  double _hc0[(_KMAX) >= 4 ? 15 : (_KMAX) >= 3 ? 10 : (_KMAX) >= 2 ? 6 : (_KMAX) >= 1 ? 3 : 1];
  XC_CAT(pc07_alpha_k, _KMAX)(p, x, u, _hc0);
  double _hc1[(_KMAX) >= 4 ? 35 : (_KMAX) >= 3 ? 20 : (_KMAX) >= 2 ? 10 : (_KMAX) >= 1 ? 4 : 1];
  XC_CAT(mgga_alpha_deorb_reg_k, _KMAX)(p, x, _hc0[0], params->eta, _hc1);
  double _hc2[(_KMAX) >= 4 ? 15 : (_KMAX) >= 3 ? 10 : (_KMAX) >= 2 ? 6 : (_KMAX) >= 1 ? 3 : 1];
  XC_CAT(r2scan_f_a_k, _KMAX)(p, x, _hc1[0], _hc2);
#if _KMAX >= 1
  const double _t1 = _hc1[2]*_hc2[1];
  const double _t2 = _hc0[2]*_hc1[2] + _hc1[3];
#endif
#if _KMAX >= 2
  const double _t3 = ((_hc0[1]) * (_hc0[1]));
  const double _t4 = ((_hc1[2]) * (_hc1[2]));
  const double _t5 = _t3*_t4;
  const double _t6 = _hc1[2]*_hc2[4];
  const double _t7 = _hc0[1]*_hc1[8] + _hc0[4]*_hc1[2];
  const double _t8 = _hc2[3]*_t2;
  const double _t9 = _hc0[1]*_hc1[2];
  const double _t10 = 0.2e1*_hc1[8];
  const double _t11 = _hc0[2]*_t10 + _hc0[5]*_hc1[2] + _hc1[9];
  const double _t12 = _hc2[4] + _t8;
#endif
#if _KMAX >= 3
  const double _t13 = ((_hc1[2]) * (_hc1[2]) * (_hc1[2]));
  const double _t14 = ((_hc0[1]) * (_hc0[1]) * (_hc0[1]))*_t13;
  const double _t15 = _hc2[3]*_t4;
  const double _t16 = 0.3e1*_t15;
  const double _t17 = _hc0[1]*_hc0[3];
  const double _t18 = _hc0[3]*_hc1[8] + _hc0[7]*_hc1[2];
  const double _t19 = _hc0[3]*_hc1[2];
  const double _t20 = _hc2[3]*_t7;
  const double _t21 = 0.2e1*_t9;
  const double _t22 = _hc2[6]*_t2;
  const double _t23 = _hc2[7]*_t2;
  const double _t24 = _hc0[1]*_hc1[18] + _hc0[4]*_t10 + _hc0[8]*_hc1[2];
  const double _t25 = _hc2[3]*_t11;
  const double _t26 = _hc2[7]*_t9 + _t20 + _t22*_t9;
  const double _t27 = 0.3e1*_hc1[18];
  const double _t28 = 0.3e1*_hc1[8];
  const double _t29 = _hc0[2]*_t27 + _hc0[5]*_t28 + _hc0[9]*_hc1[2] + _hc1[19];
  const double _t30 = _hc2[8] + _t23;
  const double _t31 = 0.2e1*_t12;
  const double _t32 = _hc2[7] + _t22;
  const double _t33 = _t2*_t32 + _t25 + _t30;
#endif
#if _KMAX >= 4
  const double _t34 = 0.3e1*_t17*_t4;
  const double _t35 = _hc2[3]*_t18;
  const double _t36 = _hc2[10]*_t2;
  const double _t37 = _hc2[6]*_t7;
  const double _t38 = _hc2[7]*_t7;
  const double _t39 = _hc2[11]*_t2;
  const double _t40 = _hc2[3]*_t24;
  const double _t41 = _hc2[6]*_t11;
  const double _t42 = 0.2e1*_t26;
  const double _t43 = _hc2[12]*_t2;
  const double _t44 = _hc2[7]*_t11;
  const double _t45 = _hc2[3]*_t29;
  const double _t46 = _hc2[12]*_t9 + _t38 + _t39*_t9;
  const double _t47 = _hc2[13] + _t43;
  const double _t48 = 0.2e1*_t11;
  const double _t49 = _hc2[12] + _t39;
  const double _t50 = _t2*_t49 + _t44 + _t47;
#endif
  ...
  out[2] = df_du;
  const double df_dx = _hc2[1]*_t2 + _hc2[2];
  out[3] = df_dx;
#endif
#if _KMAX >= 2
  ...
  out[5] = d2f_du_dt;
  const double d2f_du2 = _hc0[3]*_t1 + _hc2[3]*_t5;
  out[6] = d2f_du2;
  const double d2f_dx_dt = 0;
  out[7] = d2f_dx_dt;
  const double d2f_dx_du = _hc0[1]*_t6 + _hc2[1]*_t7 + _t8*_t9;
  out[8] = d2f_dx_du;
  const double d2f_dx2 = _hc2[1]*_t11 + _hc2[4]*_t2 + _hc2[5] + _t12*_t2;
  out[9] = d2f_dx2;
#endif
#if _KMAX >= 3
  ...
  out[12] = d3f_du2_dt;
  const double d3f_du3 = _hc0[6]*_t1 + _hc2[6]*_t14 + _t16*_t17;
  out[13] = d3f_du3;
  const double d3f_dx_dt2 = 0;
  out[14] = d3f_dx_dt2;
  const double d3f_dx_du_dt = 0;
  out[15] = d3f_dx_du_dt;
  const double d3f_dx_du2 = _hc0[3]*_t6 + _hc2[1]*_t18 + _hc2[7]*_t5 + _t19*_t8 + _t20*_t21 + _t22*_t5;
  out[16] = d3f_dx_du2;
  const double d3f_dx2_dt = 0;
  out[17] = d3f_dx2_dt;
  const double d3f_dx2_du = _hc2[1]*_t24 + _hc2[4]*_t7 + _hc2[8]*_t9 + _t12*_t7 + _t2*_t26 + _t23*_t9 + _t25*_t9;
  out[18] = d3f_dx2_du;
  const double d3f_dx3 = _hc2[1]*_t29 + _hc2[4]*_t11 + _hc2[8]*_t2 + _hc2[9] + _t11*_t31 + _t2*_t30 + _t2*_t33;
  out[19] = d3f_dx3;
#endif
#if _KMAX >= 4
  ...
  out[23] = d4f_du3_dt;
  const double d4f_du4 = ((_hc0[1]) * (_hc0[1]) * (_hc0[1]) * (_hc0[1]))*((_hc1[2]) * (_hc1[2]) * (_hc1[2]) * (_hc1[2]))*_hc2[10] + 0.4e1*_hc0[1]*_hc0[6]*_t15 + _hc0[10]*_t1 + ((_hc0[3]) * (_hc0[3]))*_t16 + 0.6e1*_hc0[3]*_hc2[6]*_t13*_t3;
  out[24] = d4f_du4;
  ...
  const double d4f_dx_du3 = _hc0[6]*_hc1[2]*_t8 + _hc0[6]*_t6 + _hc2[1]*(_hc0[11]*_hc1[2] + _hc0[6]*_hc1[8]) + _hc2[11]*_t14 + _hc2[7]*_t34 + _t14*_t36 + 0.3e1*_t19*_t20 + _t22*_t34 + 0.3e1*_t35*_t9 + 0.3e1*_t37*_t5;
  out[28] = d4f_dx_du3;
  const double d4f_dx2_dt2 = 0;
  out[29] = d4f_dx2_dt2;
  const double d4f_dx2_du_dt = 0;
  out[30] = d4f_dx2_du_dt;
  const double d4f_dx2_du2 = _hc2[1]*(_hc0[12]*_hc1[2] + _hc0[3]*_hc1[18] + _hc0[7]*_t10) + _hc2[12]*_t5 + _hc2[4]*_t18 + _hc2[8]*_t19 + _t12*_t18 + _t19*_t23 + _t19*_t25 + _t2*(_hc2[11]*_t5 + _hc2[7]*_t19 + _t19*_t22 + _t21*_t37 + _t35 + _t36*_t5) + _t21*_t38 + _t21*_t40 + _t39*_t5 + _t41*_t5 + _t42*_t7;
  out[31] = d4f_dx2_du2;
  const double d4f_dx3_dt = 0;
  out[32] = d4f_dx3_dt;
  const double d4f_dx3_du = _hc2[1]*(_hc0[1]*_hc1[33] + _hc0[13]*_hc1[2] + _hc0[4]*_t27 + _hc0[8]*_t28) + _hc2[13]*_t9 + _hc2[4]*_t24 + _hc2[8]*_t7 + _t11*_t42 + _t2*_t46 + _t2*(_t2*(_hc2[11]*_t9 + _t36*_t9 + _t37) + _t32*_t7 + _t40 + _t41*_t9 + _t46) + _t24*_t31 + _t30*_t7 + _t33*_t7 + _t43*_t9 + _t44*_t9 + _t45*_t9;
  out[33] = d4f_dx3_du;
  const double d4f_dx4 = _hc2[1]*(_hc0[14]*_hc1[2] + 0.4e1*_hc0[2]*_hc1[33] + 0.6e1*_hc0[5]*_hc1[18] + 0.4e1*_hc0[9]*_hc1[8] + _hc1[34]) + _hc2[13]*_t2 + _hc2[14] + _hc2[4]*_t29 + _hc2[8]*_t11 + 0.3e1*_t11*_t33 + 0.3e1*_t12*_t29 + _t2*_t47 + _t2*_t50 + _t2*(_t2*(_t2*(_hc2[11] + _t36) + _t41 + _t49) + _t32*_t48 + _t45 + _t50) + _t30*_t48;
  out[34] = d4f_dx4;
#endif
}
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(xchan_k, _KMAX)(const xc_func_type *p, double n_s, double sigma_ss, double lapl_s, double tau_s, double *out) {
  ...
  if(!(_t8)) XC_CAT(r2scanl_f_k, _KMAX)(p, _t5*_t6, _t7*lapl_s, 0, _hc1);
