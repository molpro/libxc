/*
  Generated from python/mgga_exc/mgga_k_csk.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py mgga_k_csk
*/

#ifndef _MGGA_K_CSK_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _MGGA_K_CSK_KERNEL_BODY
#define _KMAX 0
#define _MGGA_K_CSK_HELPER_BODIES
#include "mgga_k_csk.c"
#undef _MGGA_K_CSK_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _MGGA_K_CSK_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_MGGA_K_CSK_HELPER_BODIES)
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(csk_z_k, _KMAX)(const xc_func_type *p, double p_, double q, double *out) {
  ...
  const double f = -0.40e2 / 0.27e2*p_ + (0.20e2 / 0.9e1)*q;
  out[0] = f;
#if _KMAX >= 1
  const double df_dq = 0.20e2 / 0.9e1;
  out[1] = df_dq;
  const double df_dp_ = -0.40e2 / 0.27e2;
  out[2] = df_dp_;
#endif
#if _KMAX >= 2
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(csk_zI_negz_k, _KMAX)(const xc_func_type *p, double z, double *out) {
  const mgga_k_csk_params *params = (const mgga_k_csk_params *)(p->params);
