/*
  Generated from python/mgga_exc/mgga_x_mvs.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py mgga_x_mvs
*/

#ifndef _MGGA_X_MVS_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _MGGA_X_MVS_KERNEL_BODY
#define _KMAX 0
#define _MGGA_X_MVS_HELPER_BODIES
#include "mgga_x_mvs.c"
#undef _MGGA_X_MVS_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _MGGA_X_MVS_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_MGGA_X_MVS_HELPER_BODIES)
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(mvs_fa_k, _KMAX)(const xc_func_type *p, double a, double *out) {
  const mgga_x_mvs_params *params = (const mgga_x_mvs_params *)(p->params);

  const double _t1 = 0.1e1 - a;
  const double _t2 = (a * a);
  const double _t3 = _t2*params->e1 + 0.1e1;
  const double _t4 = (_t3 * _t3) + (a * a * a * a)*params->c1;
  const double _t5 = xc_powr(_t4, -1, 4);
#if _KMAX >= 1
  const double _t6 = xc_powr(_t4, -5, 4);
  const double _t7 = (a * a * a)*params->c1;
  const double _t8 = _t3*params->e1;
  const double _t9 = _t8*a;
  const double _t10 = -_t7 - _t9;
  const double _t11 = _t10*_t6;
#endif
#if _KMAX >= 2
  const double _t12 = _t2*params->c1;
  const double _t13 = ((params->e1) * (params->e1));
  const double _t14 = _t13*_t2;
  const double _t15 = -0.3e1*_t12 - 0.2e1*_t14 - _t8;
  const double _t16 = _t15*_t6;
  const double _t17 = xc_powr(_t4, -9, 4);
  const double _t18 = -0.5e1*_t7 - 0.5e1*_t9;
  const double _t19 = _t17*_t18;
  const double _t20 = _t1*_t10;
#endif
#if _KMAX >= 3
  const double _t21 = 0.6e1*params->c1;
  const double _t22 = 0.6e1*_t13;
  const double _t23 = -_t21*a - _t22*a;
  const double _t24 = _t23*_t6;
  const double _t25 = 0.3e1*_t19;
  const double _t26 = -0.15e2*_t12 - 0.10e2*_t14 - 0.5e1*_t8;
  const double _t27 = _t17*_t26;
  const double _t28 = _t15*_t19;
  const double _t29 = xc_powr(_t4, -13, 4);
  const double _t30 = -0.9e1*_t7 - 0.9e1*_t9;
  const double _t31 = _t18*_t29*_t30;
#endif
#if _KMAX >= 4
  const double _t32 = 0.30e2*a;
  const double _t33 = 0.4e1*_t10;
  const double _t34 = 0.3e1*_t1*_t15;
  const double _t35 = _t20*_t29;
#endif

  const double f = _t1*_t5;
  out[0] = f;
#if _KMAX >= 1
  const double df_da = _t1*_t11 - _t5;
  out[1] = df_da;
#endif
#if _KMAX >= 2
  const double d2f_da2 = _t1*_t16 - 0.2e1*_t11 + _t19*_t20;
  out[2] = d2f_da2;
#endif
#if _KMAX >= 3
  const double d3f_da3 = _t1*_t24 + 0.2e1*_t1*_t28 - _t10*_t25 - 0.3e1*_t16 + _t20*_t27 + _t20*_t31;
  out[3] = d3f_da3;
#endif
#if _KMAX >= 4
  const double d4f_da4 = _t1*_t23*_t25 + _t1*_t6*(-_t21 - _t22) + _t17*_t20*(-_t13*_t32 - _t32*params->c1) + _t18*_t20*_t30*(-0.13e2*_t7 - 0.13e2*_t9)/xc_powr(_t4, 17, 4) + _t18*_t35*(-0.27e2*_t12 - 0.18e2*_t14 - 0.9e1*_t8) - 0.4e1*_t24 + 0.2e1*_t26*_t30*_t35 - _t27*_t33 + _t27*_t34 - 0.8e1*_t28 - _t31*_t33 + _t31*_t34;
  out[4] = d4f_da4;
#endif
}
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(mvs_f_pf_k, _KMAX)(const xc_func_type *p, double x2, double u, double t, double *out) {
  const mgga_x_mvs_params *params = (const mgga_x_mvs_params *)(p->params);

  double _hc0[(_KMAX) >= 4 ? 15 : (_KMAX) >= 3 ? 10 : (_KMAX) >= 2 ? 6 : (_KMAX) >= 1 ? 3 : 1];
  XC_CAT(mgga_alpha_pf_k, _KMAX)(p, x2, t, _hc0);
  double _hc1[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(mvs_fa_k, _KMAX)(p, _hc0[0], _hc1);
  const double _t1 = _hc1[0]*params->k0 + 0.1e1;
  const double _t2 = (x2 * x2);
  const double _t3 = xc_powr(0.6e1, 2, 3);
  const double _t4 = _t3*params->b/(M_PI * M_PI * M_CBRTPI * M_CBRTPI);
  const double _t5 = (0.1e1 / 0.576e3)*_t4;
  const double _t6 = _t2*_t5 + 0.1e1;
  const double _t7 = pow(_t6, -0.1e1 / 0.8e1);
#if _KMAX >= 1
  const double _t8 = _t7*params->k0;
  const double _t9 = _hc1[1]*_t8;
  const double _t10 = pow(_t6, -0.9e1 / 0.8e1);
  const double _t11 = _t10*_t4;
  const double _t12 = (0.1e1 / 0.2304e4)*_t11;
  const double _t13 = _t1*_t12;
#endif
#if _KMAX >= 2
  const double _t14 = ((_hc0[1]) * (_hc0[1]));
  const double _t15 = _hc1[2]*_t8;
  const double _t16 = _hc0[1]*_hc0[2];
  const double _t17 = _hc1[1]*params->k0;
  const double _t18 = _hc0[1]*_t17;
  const double _t19 = _t12*_t18;
  const double _t20 = ((_hc0[2]) * (_hc0[2]));
  const double _t21 = _hc0[2]*_t17;
  const double _t22 = _t21*x2;
  const double _t23 = (0.1e1 / 0.1152e4)*_t11;
  const double _t24 = xc_powr(0.6e1, 1, 3)*((params->b) * (params->b))/((M_PI * M_PI * M_PI * M_PI * M_PI * M_CBRTPI)*pow(_t6, 0.17e2 / 0.8e1));
  const double _t25 = _t1*_t24;
  const double _t26 = (0.1e1 / 0.98304e5)*_t2;
#endif
#if _KMAX >= 3
  const double _t27 = ((_hc0[1]) * (_hc0[1]) * (_hc0[1]));
  const double _t28 = _hc1[3]*_t8;
  const double _t29 = _hc0[2]*_t14;
  const double _t30 = _hc1[2]*params->k0;
  const double _t31 = _t14*_t30;
  const double _t32 = _t12*_t31;
  const double _t33 = _hc0[1]*_t20;
  const double _t34 = _t16*_t30;
  const double _t35 = _t24*_t26;
  const double _t36 = ((_hc0[2]) * (_hc0[2]) * (_hc0[2]));
  const double _t37 = (0.1e1 / 0.768e3)*_t11;
  const double _t38 = _t20*_t30;
  const double _t39 = _t37*x2;
  const double _t40 = xc_powi(M_PI, -8);
  const double _t41 = pow(_t6, -0.25e2 / 0.8e1);
  const double _t42 = ((params->b) * (params->b) * (params->b));
  const double _t43 = _t1*_t40*_t41*_t42;
  const double _t44 = (x2 * x2 * x2);
  const double _t45 = (0.17e2 / 0.37748736e8)*_t44;
  const double _t46 = (0.1e1 / 0.32768e5)*_t25;
  const double _t47 = (0.1e1 / 0.32768e5)*_t24;
  const double _t48 = _t2*_t47;
#endif
#if _KMAX >= 4
  const double _t49 = _hc1[4]*_t8;
  const double _t50 = _hc1[3]*params->k0;
  const double _t51 = _t50*x2;
  const double _t52 = _t40*_t41*_t42;
#endif

  const double f = _t1*_t7;
  out[0] = f;
#if _KMAX >= 1
  const double df_dt = _hc0[1]*_t9;
  out[1] = df_dt;
  const double df_du = 0;
  out[2] = df_du;
  const double df_dx2 = _hc0[2]*_t9 - _t13*x2;
  out[3] = df_dx2;
#endif
#if _KMAX >= 2
  const double d2f_dt2 = _t14*_t15;
  out[4] = d2f_dt2;
  const double d2f_du_dt = 0;
  out[5] = d2f_du_dt;
  const double d2f_du2 = 0;
  out[6] = d2f_du2;
  const double d2f_dx2_dt = _t15*_t16 - _t19*x2;
  out[7] = d2f_dx2_dt;
  const double d2f_dx2_du = 0;
  out[8] = d2f_dx2_du;
  const double d2f_dx22 = -_t13 + _t15*_t20 - _t22*_t23 + _t25*_t26;
  out[9] = d2f_dx22;
#endif
#if _KMAX >= 3
  const double d3f_dt3 = _t27*_t28;
  out[10] = d3f_dt3;
  ...
  out[13] = d3f_du3;
  const double d3f_dx2_dt2 = _t28*_t29 - _t32*x2;
  out[14] = d3f_dx2_dt2;
  const double d3f_dx2_du_dt = 0;
  out[15] = d3f_dx2_du_dt;
  const double d3f_dx2_du2 = 0;
  out[16] = d3f_dx2_du2;
  const double d3f_dx22_dt = _t18*_t35 - _t19 - _t23*_t34*x2 + _t28*_t33;
  out[17] = d3f_dx22_dt;
  const double d3f_dx22_du = 0;
  out[18] = d3f_dx22_du;
  const double d3f_dx23 = -_t21*_t37 + _t21*_t48 + _t28*_t36 - _t38*_t39 - _t43*_t45 + _t46*x2;
  out[19] = d3f_dx23;
#endif
#if _KMAX >= 4
  const double d4f_dt4 = ((_hc0[1]) * (_hc0[1]) * (_hc0[1]) * (_hc0[1]))*_t49;
  out[20] = d4f_dt4;
  ...
  out[24] = d4f_du4;
  const double d4f_dx2_dt3 = _hc0[2]*_t27*_t49 - _t12*_t27*_t51;
  out[25] = d4f_dx2_dt3;
  ...
  out[28] = d4f_dx2_du3;
  const double d4f_dx22_dt2 = _t14*_t20*_t49 - _t23*_t29*_t51 + _t31*_t35 - _t32;
  out[29] = d4f_dx22_dt2;
  const double d4f_dx22_du_dt = 0;
  out[30] = d4f_dx22_du_dt;
  const double d4f_dx22_du2 = 0;
  out[31] = d4f_dx22_du2;
  const double d4f_dx23_dt = _hc0[1]*_t36*_t49 - _t18*_t45*_t52 + _t18*_t47*x2 - _t33*_t39*_t50 - _t34*_t37 + _t34*_t48;
  out[32] = d4f_dx23_dt;
  const double d4f_dx23_du = 0;
  out[33] = d4f_dx23_du;
  const double d4f_dx24 = ((_hc0[2]) * (_hc0[2]) * (_hc0[2]) * (_hc0[2]))*_t49 + (0.425e3 / 0.86973087744e11)*_t1*_t3*((params->b) * (params->b) * (params->b) * (params->b))*(x2 * x2 * x2 * x2)/((M_PI * M_PI * M_PI * M_PI * M_PI * M_PI * M_PI * M_PI * M_PI * M_PI * M_CBRTPI * M_CBRTPI)*pow(_t6, 0.33e2 / 0.8e1)) - _t10*_t36*_t5*_t51 - 0.1e1 / 0.384e3*_t11*_t38 + (0.1e1 / 0.16384e5)*_t2*_t24*_t38 - 0.17e2 / 0.6291456e7*_t2*_t43 - 0.17e2 / 0.9437184e7*_t21*_t44*_t52 + (0.1e1 / 0.8192e4)*_t22*_t24 + _t46;
  out[34] = d4f_dx24;
#endif
}
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(xchan_k, _KMAX)(const xc_func_type *p, double n_s, double sigma_ss, double lapl_s, double tau_s, double *out) {
  ...
  if(!(_t7)) XC_CAT(mvs_f_pf_k, _KMAX)(p, _t5*sigma_ss, 0, _t6*tau_s, _hc1);
