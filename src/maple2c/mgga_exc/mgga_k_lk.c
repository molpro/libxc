/*
  Generated from python/mgga_exc/mgga_k_lk.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py mgga_k_lk
*/

#ifndef _MGGA_K_LK_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _MGGA_K_LK_KERNEL_BODY
#define _KMAX 0
#define _MGGA_K_LK_HELPER_BODIES
#include "mgga_k_lk.c"
#undef _MGGA_K_LK_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _MGGA_K_LK_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_MGGA_K_LK_HELPER_BODIES)

/* ---- helpers (one graded body -> <name>_k<order>) ---- */

GPU_DEVICE_FUNCTION static inline void XC_CAT(lk_delta_k, _KMAX)(const xc_func_type *p, double p_, double q, double *out) {

  const double _t1 = (0.1e1 / 0.9e1)*p_;

  const double f = -_t1*q + (0.8e1 / 0.243e3)*(p_ * p_) + (0.8e1 / 0.81e2)*(q * q);
  out[0] = f;
#if _KMAX >= 1
  const double df_dq = -_t1 + (0.16e2 / 0.81e2)*q;
  out[1] = df_dq;
  const double df_dp_ = (0.16e2 / 0.243e3)*p_ - 0.1e1 / 0.9e1*q;
  out[2] = df_dp_;
#endif
#if _KMAX >= 2
  const double d2f_dq2 = 0.16e2 / 0.81e2;
  out[3] = d2f_dq2;
  const double d2f_dp__dq = -0.1e1 / 0.9e1;
  out[4] = d2f_dp__dq;
  const double d2f_dp_2 = 0.16e2 / 0.243e3;
  out[5] = d2f_dp_2;
#endif
#if _KMAX >= 3
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(lk_f0_k, _KMAX)(const xc_func_type *p, double x1, double x2, double *out) {
  const mgga_k_lk_params *params = (const mgga_k_lk_params *)(p->params);

  const double _t1 = params->kappa + x1;
  const double _t2 = (0.1e1 / _t1);
  const double _t3 = params->kappa + x2;
...
#endif
#if _KMAX >= 2
  const double _t7 = (0.1e1 / (_t3 * _t3 * _t3));
  ...
  const double f = params->kappa*(_t2*x1 + _t4*x2) + 0.1e1;
  out[0] = f;
#if _KMAX >= 1
  const double df_dx2 = params->kappa*(_t4 - _t5*x2);
  out[1] = df_dx2;
  const double df_dx1 = params->kappa*(_t2 - _t6*x1);
  out[2] = df_dx1;
#endif
#if _KMAX >= 2
  const double d2f_dx22 = params->kappa*(-0.2e1*_t5 + 0.2e1*_t7*x2);
  out[3] = d2f_dx22;
  const double d2f_dx1_dx2 = 0;
  out[4] = d2f_dx1_dx2;
  const double d2f_dx12 = params->kappa*(-0.2e1*_t6 + 0.2e1*_t8*x1);
  out[5] = d2f_dx12;
#endif
#if _KMAX >= 3
  const double d3f_dx23 = params->kappa*(0.6e1*_t7 - 0.6e1*_t9*x2);
  out[6] = d3f_dx23;
  const double d3f_dx1_dx22 = 0;
  out[7] = d3f_dx1_dx22;
  const double d3f_dx12_dx2 = 0;
  out[8] = d3f_dx12_dx2;
  const double d3f_dx13 = params->kappa*(-0.6e1*_t10*x1 + 0.6e1*_t8);
  out[9] = d3f_dx13;
#endif
#if _KMAX >= 4
  const double d4f_dx24 = params->kappa*(-0.24e2*_t9 + 0.24e2*x2/xc_powi(_t3, 5));
  out[10] = d4f_dx24;
  const double d4f_dx1_dx23 = 0;
  out[11] = d4f_dx1_dx23;
  const double d4f_dx12_dx22 = 0;
  out[12] = d4f_dx12_dx22;
  const double d4f_dx13_dx2 = 0;
  out[13] = d4f_dx13_dx2;
  const double d4f_dx14 = params->kappa*(-0.24e2*_t10 + 0.24e2*x1/xc_powi(_t1, 5));
  out[14] = d4f_dx14;
#endif
}

GPU_DEVICE_FUNCTION static inline void XC_CAT(lk_x1_k, _KMAX)(const xc_func_type *p, double p_, double q, double *out) {
  const mgga_k_lk_params *params = (const mgga_k_lk_params *)(p->params);

  double _hc0[(_KMAX) >= 4 ? 15 : (_KMAX) >= 3 ? 10 : (_KMAX) >= 2 ? 6 : (_KMAX) >= 1 ? 3 : 1];
  XC_CAT(lk_delta_k, _KMAX)(p, p_, q, _hc0);
  const double _t1 = (0.1e1 / (params->kappa));
#if _KMAX >= 1
  const double _t2 = (0.50e2 / 0.729e3)*_t1;
#endif

  const double f = _hc0[0] + (0.25e2 / 0.729e3)*_t1*(p_ * p_) + (0.5e1 / 0.27e2)*p_;
  out[0] = f;
#if _KMAX >= 1
  const double df_dq = _hc0[1];
  out[1] = df_dq;
  const double df_dp_ = _hc0[2] + _t2*p_ + 0.5e1 / 0.27e2;
  out[2] = df_dp_;
#endif
#if _KMAX >= 2
  const double d2f_dq2 = _hc0[3];
  out[3] = d2f_dq2;
  const double d2f_dp__dq = _hc0[4];
  out[4] = d2f_dp__dq;
  const double d2f_dp_2 = _hc0[5] + _t2;
  out[5] = d2f_dp_2;
#endif
#if _KMAX >= 3
  const double d3f_dq3 = 0;
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(lk_x2_k, _KMAX)(const xc_func_type *p, double p_, double q, double *out) {
  const mgga_k_lk_params *params = (const mgga_k_lk_params *)(p->params);

  double _hc0[(_KMAX) >= 4 ? 15 : (_KMAX) >= 3 ? 10 : (_KMAX) >= 2 ? 6 : (_KMAX) >= 1 ? 3 : 1];
  XC_CAT(lk_delta_k, _KMAX)(p, p_, q, _hc0);
  const double _t1 = (0.1e1 / (params->kappa));
  const double _t2 = (0.10e2 / 0.27e2)*_t1;
  const double _t3 = _hc0[0]*_t2;
  const double _t4 = (0.1e1 / ((params->kappa) * (params->kappa)));
#if _KMAX >= 1
  const double _t5 = _hc0[1]*_t2;
  const double _t6 = _t2*p_;
#endif
#if _KMAX >= 2
  const double _t7 = _hc0[3]*_t2;
  const double _t8 = (0.20e2 / 0.27e2)*_t1;
  const double _t9 = (0.250e3 / 0.6561e4)*_t4;
#endif

  const double f = _t3*p_ + (0.125e3 / 0.19683e5)*_t4*(p_ * p_ * p_);
  out[0] = f;
#if _KMAX >= 1
  const double df_dq = _t5*p_;
  out[1] = df_dq;
  const double df_dp_ = _hc0[2]*_t6 + _t3 + (0.125e3 / 0.6561e4)*_t4*(p_ * p_);
  out[2] = df_dp_;
#endif
#if _KMAX >= 2
  const double d2f_dq2 = _t7*p_;
  out[3] = d2f_dq2;
  const double d2f_dp__dq = _hc0[4]*_t6 + _t5;
  out[4] = d2f_dp__dq;
  const double d2f_dp_2 = _hc0[2]*_t8 + _hc0[5]*_t6 + _t9*p_;
  out[5] = d2f_dp_2;
#endif
#if _KMAX >= 3
  const double d3f_dq3 = 0;
  out[6] = d3f_dq3;
  const double d3f_dp__dq2 = _t7;
  out[7] = d3f_dp__dq2;
  const double d3f_dp_2_dq = _hc0[4]*_t8;
  out[8] = d3f_dp_2_dq;
  const double d3f_dp_3 = (0.10e2 / 0.9e1)*_hc0[5]*_t1 + _t9;
  out[9] = d3f_dp_3;
#endif
#if _KMAX >= 4
  const double d4f_dq4 = 0;
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(lk_f_k, _KMAX)(const xc_func_type *p, double p_, double u, double *out) {

  const double _t1 = xc_powr(0.6e1, 1, 3);
  const double _t2 = _t1/(M_PI * M_CBRTPI);
  const double _t3 = (0.1e1 / 0.24e2)*_t2;
  const double _t4 = _t3*u;
#if _KMAX >= 2
  const double _t7 = xc_powr(0.6e1, 2, 3)/(M_PI * M_PI * M_CBRTPI * M_CBRTPI);
  const double _t8 = (0.1e1 / 0.576e3)*_t7;
#endif
#if _KMAX >= 3
  const double _t19 = (0.1e1 / (M_PI * M_PI * M_PI * M_PI));
  const double _t20 = (0.1e1 / 0.2304e4)*_t19;
  const double _t24 = (0.1e1 / 0.1152e4)*_t19;
  const double _t32 = (0.1e1 / 0.288e3)*_t7;
#endif
#if _KMAX >= 4
  const double _t47 = _t1/(M_PI * M_PI * M_PI * M_PI * M_PI * M_CBRTPI);
  const double _t48 = (0.1e1 / 0.55296e5)*_t47;
  const double _t49 = (0.1e1 / 0.18432e5)*_t47;
  const double _t50 = (0.1e1 / 0.192e3)*_t7;
  const double _t57 = (0.1e1 / 0.768e3)*_t19;
  const double _t58 = (0.1e1 / 0.8e1)*_t2;
  const double _t61 = (0.1e1 / 0.12e2)*_t2;
#endif
  double _hc0[(_KMAX) >= 4 ? 15 : (_KMAX) >= 3 ? 10 : (_KMAX) >= 2 ? 6 : (_KMAX) >= 1 ? 3 : 1];
  XC_CAT(lk_x1_k, _KMAX)(p, p_, _t4, _hc0);
  double _hc1[(_KMAX) >= 4 ? 15 : (_KMAX) >= 3 ? 10 : (_KMAX) >= 2 ? 6 : (_KMAX) >= 1 ? 3 : 1];
  XC_CAT(lk_x2_k, _KMAX)(p, p_, _t4, _hc1);
  double _hc2[(_KMAX) >= 4 ? 15 : (_KMAX) >= 3 ? 10 : (_KMAX) >= 2 ? 6 : (_KMAX) >= 1 ? 3 : 1];
  XC_CAT(lk_f0_k, _KMAX)(p, _hc0[0], _hc1[0], _hc2);
#if _KMAX >= 1
  const double _t5 = _hc2[2]*_t3;
  const double _t6 = _hc2[1]*_t3;
#endif
#if _KMAX >= 2
  const double _t9 = _hc0[3]*_t8;
  const double _t10 = _hc1[3]*_t8;
  const double _t11 = ((_hc0[1]) * (_hc0[1]))*_t8;
  const double _t12 = ((_hc1[1]) * (_hc1[1]))*_t8;
  const double _t13 = _hc0[1]*_t3;
  const double _t14 = _hc0[2]*_t13;
  const double _t15 = _hc1[1]*_t3;
  const double _t16 = _hc1[2]*_t15;
  const double _t17 = ((_hc0[2]) * (_hc0[2]));
  const double _t18 = ((_hc1[2]) * (_hc1[2]));
#endif
#if _KMAX >= 3
  const double _t21 = _hc0[6]*_t20;
  const double _t22 = _hc1[6]*_t20;
  const double _t23 = _hc0[1]*_hc2[5];
  const double _t25 = _hc0[3]*_t24;
  const double _t26 = _hc1[1]*_hc2[3];
  const double _t27 = _hc1[3]*_t24;
  const double _t28 = _hc2[5]*_t9 + _hc2[9]*_t11;
  const double _t29 = _hc2[3]*_t10 + _hc2[6]*_t12;
  const double _t30 = _hc0[7]*_t8;
  const double _t31 = _hc1[7]*_t8;
  const double _t33 = _hc0[4]*_t32;
  const double _t34 = _hc1[4]*_t32;
  const double _t35 = _hc0[5]*_hc2[5];
  const double _t36 = _hc0[4]*_t3;
  const double _t37 = _hc2[5]*_t36;
  const double _t38 = _hc1[5]*_hc2[3];
  const double _t39 = _hc1[4]*_t3;
  const double _t40 = _hc2[3]*_t39;
  const double _t41 = _hc2[9]*_t14 + _t37;
  const double _t42 = _hc2[6]*_t16 + _t40;
  const double _t43 = 0.2e1*_hc0[2];
  const double _t44 = 0.2e1*_hc1[2];
  const double _t45 = _hc2[9]*_t17 + _t35;
  const double _t46 = _hc2[6]*_t18 + _t38;
#endif
#if _KMAX >= 4
  const double _t51 = _hc0[1]*_hc2[9];
  const double _t52 = _hc2[14]*_t11 + _hc2[9]*_t9;
  const double _t53 = _hc2[5]*_t21 + _t13*_t52 + _t25*_t51;
  const double _t54 = _hc1[1]*_hc2[6];
  const double _t55 = _hc2[10]*_t12 + _hc2[6]*_t10;
  const double _t56 = _hc2[3]*_t22 + _t15*_t55 + _t27*_t54;
  const double _t59 = _hc2[5]*_t30;
  const double _t60 = _hc2[3]*_t31;
  const double _t62 = _hc0[9]*_hc2[5];
  const double _t63 = _hc0[8]*_hc2[5];
  const double _t64 = _hc1[9]*_hc2[3];
  const double _t65 = _hc1[8]*_hc2[3];
  const double _t66 = _hc0[5]*_hc2[9];
  const double _t67 = _hc2[9]*_t36;
  const double _t68 = _hc1[5]*_hc2[6];
  const double _t69 = _hc2[6]*_t39;
#endif

  const double f = _hc2[0];
  out[0] = f;
#if _KMAX >= 1
  const double df_du = _hc0[1]*_t5 + _hc1[1]*_t6;
  out[1] = df_du;
  const double df_dp_ = _hc0[2]*_hc2[2] + _hc1[2]*_hc2[1];
  out[2] = df_dp_;
#endif
#if _KMAX >= 2
  const double d2f_du2 = _hc2[1]*_t10 + _hc2[2]*_t9 + _hc2[3]*_t12 + _hc2[5]*_t11;
  out[3] = d2f_du2;
  const double d2f_dp__du = _hc0[4]*_t5 + _hc1[4]*_t6 + _hc2[3]*_t16 + _hc2[5]*_t14;
  out[4] = d2f_dp__du;
  const double d2f_dp_2 = _hc0[5]*_hc2[2] + _hc1[5]*_hc2[1] + _hc2[3]*_t18 + _hc2[5]*_t17;
  out[5] = d2f_dp_2;
#endif
#if _KMAX >= 3
  const double d3f_du3 = _hc2[1]*_t22 + _hc2[2]*_t21 + _t13*_t28 + _t15*_t29 + _t23*_t25 + _t26*_t27;
  out[6] = d3f_du3;
  const double d3f_dp__du2 = _hc0[2]*_t28 + _hc1[2]*_t29 + _hc2[1]*_t31 + _hc2[2]*_t30 + _t23*_t33 + _t26*_t34;
  out[7] = d3f_dp__du2;
  const double d3f_dp_2_du = _hc0[2]*_t37 + _hc0[2]*_t41 + _hc0[8]*_t5 + _hc1[2]*_t40 + _hc1[2]*_t42 + _hc1[8]*_t6 + _t13*_t35 + _t15*_t38;
  out[8] = d3f_dp_2_du;
  const double d3f_dp_3 = _hc0[2]*_t45 + _hc0[9]*_hc2[2] + _hc1[2]*_t46 + _hc1[9]*_hc2[1] + _t35*_t43 + _t38*_t44;
  out[9] = d3f_dp_3;
#endif
#if _KMAX >= 4
  const double d4f_du4 = _hc0[10]*_hc2[2]*_t48 + _hc0[3]*_t28*_t50 + _hc0[6]*_t23*_t49 + _hc1[10]*_hc2[1]*_t48 + _hc1[3]*_t29*_t50 + _hc1[6]*_t26*_t49 + _t13*_t53 + _t15*_t56;
  out[10] = d4f_du4;
  const double d4f_dp__du3 = _hc0[11]*_hc2[2]*_t20 + _hc0[2]*_t53 + _hc0[4]*_t28*_t58 + _hc0[7]*_t23*_t57 + _hc1[11]*_hc2[1]*_t20 + _hc1[2]*_t56 + _hc1[4]*_t29*_t58 + _hc1[7]*_t26*_t57;
  out[11] = d4f_dp__du3;
  const double d4f_dp_2_du2 = _hc0[12]*_hc2[2]*_t8 + _hc0[2]*_t59 + _hc0[2]*(_hc0[2]*_t52 + _t33*_t51 + _t59) + _hc0[4]*_t41*_t61 + _hc0[5]*_t28 + _hc0[8]*_t23*_t32 + _hc1[12]*_hc2[1]*_t8 + _hc1[2]*_t60 + _hc1[2]*(_hc1[2]*_t55 + _t34*_t54 + _t60) + _hc1[4]*_t42*_t61 + _hc1[5]*_t29 + _hc1[8]*_t26*_t32;
  out[12] = d4f_dp_2_du2;
  const double d4f_dp_3_du = _hc0[13]*_t5 + _hc0[2]*_t61*_t63 + _hc0[2]*(_hc0[2]*_t67 + _hc0[2]*(_hc2[14]*_t14 + _t67) + _t13*_t66 + _t3*_t63) + 0.2e1*_hc0[5]*_t41 + _hc1[13]*_t6 + _hc1[2]*_t61*_t65 + _hc1[2]*(_hc1[2]*_t69 + _hc1[2]*(_hc2[10]*_t16 + _t69) + _t15*_t68 + _t3*_t65) + 0.2e1*_hc1[5]*_t42 + _t13*_t62 + _t15*_t64 + _t36*_t45 + _t39*_t46;
  out[13] = d4f_dp_3_du;
  const double d4f_dp_4 = _hc0[14]*_hc2[2] + 0.3e1*_hc0[2]*_t62 + _hc0[2]*(_hc0[2]*(_hc2[14]*_t17 + _t66) + _t43*_t66 + _t62) + 0.3e1*_hc0[5]*_t45 + _hc1[14]*_hc2[1] + 0.3e1*_hc1[2]*_t64 + _hc1[2]*(_hc1[2]*(_hc2[10]*_t18 + _t68) + _t44*_t68 + _t64) + 0.3e1*_hc1[5]*_t46;
  out[14] = d4f_dp_4;
#endif
}
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(xchan_k, _KMAX)(const xc_func_type *p, double n_s, double sigma_ss, double lapl_s, double tau_s, double *out) {
  ...
  if(!(_t7)) XC_CAT(lk_f_k, _KMAX)(p, (0.1e1 / 0.24e2)*_t1*_t6, lapl_s/xc_powr(n_s, 5, 3), _hc1);
