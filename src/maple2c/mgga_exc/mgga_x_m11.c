/*
  Generated from python/mgga_exc/mgga_x_m11.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py mgga_x_m11
*/

#ifndef _MGGA_X_M11_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _MGGA_X_M11_KERNEL_BODY
#define _KMAX 0
#define _MGGA_X_M11_HELPER_BODIES
#include "mgga_x_m11.c"
#undef _MGGA_X_M11_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _MGGA_X_M11_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_MGGA_X_M11_HELPER_BODIES)
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(m11_f_k, _KMAX)(const xc_func_type *p, double rs, double z, double x, double u, double t, double *out) {
  const mgga_x_m11_params *params = (const mgga_x_m11_params *)(p->params);
  ...
  const double _t12 = (0.3e1 / 0.10e2)*_t10*_t11 - t;
  const double _t13 = _t10*_t11;
  const double _t14 = (0.3e1 / 0.10e2)*_t13 + t;
  const double _t15 = (0.1e1 / _t14);
  const double _t16 = _t15*params->a[1];
  const double _t17 = xc_powi(_t14, -10);
  const double _t18 = xc_powi(_t12, 10);
  const double _t19 = _t17*_t18;
  const double _t20 = xc_powi(_t14, -11);
  const double _t21 = xc_powi(_t12, 11);
  const double _t22 = _t20*_t21;
  const double _t23 = (0.1e1 / (_t14 * _t14));
  const double _t24 = (_t12 * _t12);
  const double _t25 = _t23*_t24;
  const double _t26 = (0.1e1 / (_t14 * _t14 * _t14));
  const double _t27 = (_t12 * _t12 * _t12);
  const double _t28 = _t26*_t27;
  const double _t29 = (0.1e1 / (_t14 * _t14 * _t14 * _t14));
  const double _t30 = (_t12 * _t12 * _t12 * _t12);
  const double _t31 = _t29*_t30;
  const double _t32 = xc_powi(_t14, -5);
  const double _t33 = xc_powi(_t12, 5);
  const double _t34 = _t32*_t33;
  const double _t35 = xc_powi(_t14, -6);
  const double _t36 = xc_powi(_t12, 6);
  const double _t37 = _t35*_t36;
  const double _t38 = xc_powi(_t14, -7);
  const double _t39 = xc_powi(_t12, 7);
  const double _t40 = _t38*_t39;
  const double _t41 = xc_powi(_t14, -8);
  const double _t42 = xc_powi(_t12, 8);
  const double _t43 = _t41*_t42;
  const double _t44 = xc_powi(_t14, -9);
  const double _t45 = xc_powi(_t12, 9);
  const double _t46 = _t44*_t45;
  const double _t47 = _t12*_t16 + _t19*params->a[10] + _t22*params->a[11] + _t25*params->a[2] + _t28*params->a[3] + _t31*params->a[4] + _t34*params->a[5] + _t37*params->a[6] + _t40*params->a[7] + _t43*params->a[8] + _t46*params->a[9] + params->a[0];
  const double _t48 = _t15*params->b[1];
  const double _t49 = _t12*_t48 + _t19*params->b[10] + _t22*params->b[11] + _t25*params->b[2] + _t28*params->b[3] + _t31*params->b[4] + _t34*params->b[5] + _t37*params->b[6] + _t40*params->b[7] + _t43*params->b[8] + _t46*params->b[9] + params->b[0];
  const double _t50 = _hc0[0]*_t47 + _hc1[0]*_t49;
#if _KMAX >= 1
  const double _t51 = _t12*_t23;
  const double _t52 = -0.3e1 / 0.5e1*_t13 + 0.2e1*t;
  const double _t53 = 0.10e2*params->a[10];
  const double _t54 = _t17*_t45;
  const double _t55 = _t18*_t20;
  const double _t56 = 0.11e2*params->a[11];
  const double _t57 = xc_powi(_t14, -12);
  const double _t58 = _t21*_t57;
  const double _t59 = _t24*_t26;
  const double _t60 = 0.2e1*_t59;
  const double _t61 = 0.3e1*params->a[3];
  const double _t62 = _t27*_t29;
  const double _t63 = 0.4e1*params->a[4];
  const double _t64 = _t30*_t32;
  const double _t65 = 0.5e1*params->a[5];
  const double _t66 = _t33*_t35;
  const double _t67 = 0.6e1*params->a[6];
  const double _t68 = _t36*_t38;
  const double _t69 = 0.7e1*params->a[7];
  const double _t70 = _t39*_t41;
  const double _t71 = 0.8e1*params->a[8];
  const double _t72 = _t42*_t44;
  const double _t73 = 0.9e1*params->a[9];
  const double _t74 = -_t16 + _t23*_t52*params->a[2] - _t51*params->a[1] - _t53*_t54 - _t53*_t55 - _t54*_t73 - _t55*_t56 - _t56*_t58 - _t59*_t61 - _t60*params->a[2] - _t61*_t62 - _t62*_t63 - _t63*_t64 - _t64*_t65 - _t65*_t66 - _t66*_t67 - _t67*_t68 - _t68*_t69 - _t69*_t70 - _t70*_t71 - _t71*_t72 - _t72*_t73;
  const double _t75 = 0.10e2*params->b[10];
  const double _t76 = 0.11e2*params->b[11];
  const double _t77 = 0.3e1*params->b[3];
  const double _t78 = 0.4e1*params->b[4];
  const double _t79 = 0.5e1*params->b[5];
  const double _t80 = 0.6e1*params->b[6];
  const double _t81 = 0.7e1*params->b[7];
  const double _t82 = 0.8e1*params->b[8];
  const double _t83 = 0.9e1*params->b[9];
  const double _t84 = _t23*_t52*params->b[2] - _t48 - _t51*params->b[1] - _t54*_t75 - _t54*_t83 - _t55*_t75 - _t55*_t76 - _t58*_t76 - _t59*_t77 - _t60*params->b[2] - _t62*_t77 - _t62*_t78 - _t64*_t78 - _t64*_t79 - _t66*_t79 - _t66*_t80 - _t68*_t80 - _t68*_t81 - _t70*_t81 - _t70*_t82 - _t72*_t82 - _t72*_t83;
  const double _t85 = _hc0[0]*_t74 + _hc1[0]*_t84;
  const double _t86 = _hc0[1]*_t47 + _hc1[1]*_t49;
  const double _t87 = _t4*_t50;
  const double _t88 = (0.1e1 / (_t5 * _t5));
  const double _t91 = _t3*_t50;
#endif
#if _KMAX >= 2
  const double _t93 = 0.2e1*_t23;
  const double _t94 = _t12*params->a[1];
  const double _t95 = 0.2e1*_t26;
  const double _t96 = _t26*_t52;
  const double _t97 = 0.4e1*_t96;
  const double _t98 = _t17*_t42;
  const double _t99 = 0.90e2*_t98;
  const double _t100 = _t20*_t45;
  const double _t101 = 0.200e3*_t100;
  const double _t102 = _t18*_t57;
  const double _t103 = 0.110e3*_t102;
  const double _t104 = 0.110e3*_t100;
  const double _t105 = 0.242e3*_t102;
  const double _t106 = xc_powi(_t14, -13);
  const double _t107 = _t106*params->a[11];
  const double _t108 = 0.132e3*_t21;
  const double _t109 = _t24*_t29;
  const double _t110 = 0.6e1*_t109;
  const double _t111 = 0.18e2*_t109;
  const double _t112 = _t27*_t32;
  const double _t113 = 0.12e2*_t112;
  const double _t114 = 0.12e2*_t109;
  const double _t115 = 0.32e2*_t112;
  const double _t116 = _t30*_t35;
  const double _t117 = 0.20e2*_t116;
  const double _t118 = 0.20e2*_t112;
  const double _t119 = 0.50e2*_t116;
  const double _t120 = _t33*_t38;
  const double _t121 = 0.30e2*_t120;
  const double _t122 = 0.30e2*_t116;
  const double _t123 = 0.72e2*_t120;
  const double _t124 = _t36*_t41;
  const double _t125 = 0.42e2*_t124;
  const double _t126 = 0.42e2*_t120;
  const double _t127 = 0.98e2*_t124;
  const double _t128 = _t39*_t44;
  const double _t129 = 0.56e2*_t128;
  const double _t130 = 0.56e2*_t124;
  const double _t131 = 0.128e3*_t128;
  const double _t132 = 0.72e2*_t98;
  const double _t133 = 0.72e2*_t128;
  const double _t134 = 0.162e3*_t98;
  const double _t135 = 0.90e2*_t100;
  const double _t136 = _t101*params->a[10] + _t103*params->a[10] + _t104*params->a[11] + _t105*params->a[11] + _t107*_t108 + _t110*params->a[2] + _t111*params->a[3] + _t113*params->a[3] + _t114*params->a[4] + _t115*params->a[4] + _t117*params->a[4] + _t118*params->a[5] + _t119*params->a[5] + _t121*params->a[5] + _t122*params->a[6] + _t123*params->a[6] + _t125*params->a[6] + _t126*params->a[7] + _t127*params->a[7] + _t129*params->a[7] + _t130*params->a[8] + _t131*params->a[8] + _t132*params->a[8] + _t133*params->a[9] + _t134*params->a[9] + _t135*params->a[9] - _t61*_t96 + _t93*params->a[1] + _t93*params->a[2] + _t94*_t95 - _t97*params->a[2] + _t99*params->a[10];
  const double _t137 = _t12*params->b[1];
  const double _t138 = _t106*params->b[11];
  const double _t139 = _t101*params->b[10] + _t103*params->b[10] + _t104*params->b[11] + _t105*params->b[11] + _t108*_t138 + _t110*params->b[2] + _t111*params->b[3] + _t113*params->b[3] + _t114*params->b[4] + _t115*params->b[4] + _t117*params->b[4] + _t118*params->b[5] + _t119*params->b[5] + _t121*params->b[5] + _t122*params->b[6] + _t123*params->b[6] + _t125*params->b[6] + _t126*params->b[7] + _t127*params->b[7] + _t129*params->b[7] + _t130*params->b[8] + _t131*params->b[8] + _t132*params->b[8] + _t133*params->b[9] + _t134*params->b[9] + _t135*params->b[9] + _t137*_t95 - _t77*_t96 + _t93*params->b[1] + _t93*params->b[2] - _t97*params->b[2] + _t99*params->b[10];
  const double _t140 = _hc0[0]*_t136 + _hc1[0]*_t139;
  const double _t141 = _hc0[1]*_t74 + _hc1[1]*_t84;
  const double _t142 = _hc0[2]*_t47 + _hc1[2]*_t49;
  const double _t145 = ((_hc2[2]) * (_hc2[2]));
  const double _t146 = (0.1e1 / 0.3e1)*_t145;
  const double _t147 = (0.1e1 / (_t5 * _t5 * _t5));
  const double _t151 = (0.1e1 / 0.18e2)*_t50;
  const double _t152 = (0.1e1 / (_t5 * _t5 * _t5 * _t5));
  const double _t153 = _t145*_t152;
  const double _t154 = (rs * rs);
  const double _t155 = M_CBRT2;
  const double _t156 = xc_powr(0.3e1, 2, 3);
  const double _t157 = (0.1e1 / (M_CBRTPI * M_CBRTPI));
  const double _t158 = ((p->cam_omega) * (p->cam_omega));
  const double _t163 = _t147*_t151;
#endif
#if _KMAX >= 3
  const double _t167 = 0.6e1*_t26;
  const double _t168 = 0.12e2*_t26;
  const double _t169 = 0.6e1*_t29;
  const double _t170 = _t17*_t39;
  const double _t171 = 0.720e3*_t170;
  const double _t172 = _t20*_t42;
  const double _t173 = 0.2700e4*_t172;
  const double _t174 = _t45*_t57;
  const double _t175 = 0.3300e4*_t174;
  const double _t176 = _t18*params->a[10];
  const double _t177 = 0.1320e4*_t106;
  const double _t178 = 0.990e3*_t172;
  const double _t179 = 0.3630e4*_t174;
  const double _t180 = 0.4356e4*_t18;
  const double _t181 = _t21*params->a[11];
  const double _t182 = xc_powi(_t14, -14);
  const double _t183 = 0.1716e4*_t182;
  const double _t184 = _t24*_t32;
  const double _t185 = 0.24e2*_t184;
  const double _t186 = 0.108e3*_t184;
  const double _t187 = _t27*_t35;
  const double _t188 = 0.60e2*_t187;
  const double _t189 = 0.144e3*_t184;
  const double _t190 = 0.240e3*_t187;
  const double _t191 = _t30*_t38;
  const double _t192 = 0.120e3*_t191;
  const double _t193 = 0.60e2*_t184;
  const double _t194 = 0.300e3*_t187;
  const double _t195 = 0.450e3*_t191;
  const double _t196 = _t33*_t41;
  const double _t197 = 0.210e3*_t196;
  const double _t198 = 0.120e3*_t187;
  const double _t199 = 0.540e3*_t191;
  const double _t200 = 0.756e3*_t196;
  const double _t201 = _t36*_t44;
  const double _t202 = 0.336e3*_t201;
  const double _t203 = 0.210e3*_t191;
  const double _t204 = 0.882e3*_t196;
  const double _t205 = 0.1176e4*_t201;
  const double _t206 = 0.504e3*_t170;
  const double _t207 = 0.336e3*_t196;
  const double _t208 = 0.1344e4*_t201;
  const double _t209 = 0.1728e4*_t170;
  const double _t210 = 0.720e3*_t172;
  const double _t211 = 0.504e3*_t201;
  const double _t212 = 0.1944e4*_t170;
  const double _t213 = 0.2430e4*_t172;
  const double _t214 = 0.990e3*_t174;
  const double _t215 = -_t107*_t180 - _t167*params->a[1] - _t167*params->a[3] - _t168*params->a[2] - _t169*_t94 - _t171*params->a[10] - _t173*params->a[10] - _t175*params->a[10] - _t176*_t177 - _t178*params->a[11] - _t179*params->a[11] - _t181*_t183 - _t185*params->a[2] - _t186*params->a[3] - _t188*params->a[3] - _t189*params->a[4] - _t190*params->a[4] - _t192*params->a[4] - _t193*params->a[5] - _t194*params->a[5] - _t195*params->a[5] - _t197*params->a[5] - _t198*params->a[6] - _t199*params->a[6] - _t200*params->a[6] - _t202*params->a[6] - _t203*params->a[7] - _t204*params->a[7] - _t205*params->a[7] - _t206*params->a[7] - _t207*params->a[8] - _t208*params->a[8] - _t209*params->a[8] - _t210*params->a[8] - _t211*params->a[9] - _t212*params->a[9] - _t213*params->a[9] - _t214*params->a[9] + 0.18e2*_t29*_t52*params->a[2] + 0.27e2*_t29*_t52*params->a[3] + 0.12e2*_t29*_t52*params->a[4];
  const double _t216 = _t18*params->b[10];
  const double _t217 = _t21*params->b[11];
  const double _t218 = -_t137*_t169 - _t138*_t180 - _t167*params->b[1] - _t167*params->b[3] - _t168*params->b[2] - _t171*params->b[10] - _t173*params->b[10] - _t175*params->b[10] - _t177*_t216 - _t178*params->b[11] - _t179*params->b[11] - _t183*_t217 - _t185*params->b[2] - _t186*params->b[3] - _t188*params->b[3] - _t189*params->b[4] - _t190*params->b[4] - _t192*params->b[4] - _t193*params->b[5] - _t194*params->b[5] - _t195*params->b[5] - _t197*params->b[5] - _t198*params->b[6] - _t199*params->b[6] - _t200*params->b[6] - _t202*params->b[6] - _t203*params->b[7] - _t204*params->b[7] - _t205*params->b[7] - _t206*params->b[7] - _t207*params->b[8] - _t208*params->b[8] - _t209*params->b[8] - _t210*params->b[8] - _t211*params->b[9] - _t212*params->b[9] - _t213*params->b[9] - _t214*params->b[9] + 0.18e2*_t29*_t52*params->b[2] + 0.27e2*_t29*_t52*params->b[3] + 0.12e2*_t29*_t52*params->b[4];
  const double _t219 = _hc0[0]*_t215 + _hc1[0]*_t218;
  const double _t220 = _hc0[1]*_t136 + _hc1[1]*_t139;
  const double _t221 = _hc0[2]*_t74 + _hc1[2]*_t84;
  const double _t222 = _hc0[3]*_t47 + _hc1[3]*_t49;
  const double _t223 = _t4*_t85;
  const double _t224 = (0.1e1 / 0.18e2)*_t85;
  const double _t225 = _t4*_t86;
  const double _t226 = (0.1e1 / 0.18e2)*_t86;
  const double _t227 = (0.1e1 / M_PI);
  const double _t228 = ((p->cam_omega) * (p->cam_omega) * (p->cam_omega));
  const double _t231 = (rs * rs * rs);
  const double _t232 = ((_hc2[2]) * (_hc2[2]) * (_hc2[2]));
  const double _t233 = xc_powi(_t5, -6);
  const double _t234 = _t232*_t233;
  const double _t235 = _t231*_t234;
  const double _t239 = xc_powi(_t5, -5);
  const double _t240 = _t232*_t239;
  const double _t241 = (0.1e1 / 0.3e1)*_t240;
  const double _t244 = _t3*_t85;
  const double _t245 = _hc2[2]*_t147;
  const double _t248 = _t145*_t239;
  const double _t249 = _t154*_t248;
  const double _t250 = _hc2[5]*rs;
  const double _t253 = (0.2e1 / 0.9e1)*_t153;
  const double _t254 = _hc2[2]*_t152;
  const double _t255 = _t254*rs;
  const double _t256 = (0.1e1 / 0.9e1)*_t245;
#endif
#if _KMAX >= 4
  const double _t257 = 0.24e2*_t29;
  const double _t258 = 0.72e2*_t29;
  const double _t259 = 0.24e2*_t32;
  const double _t260 = _t32*_t52;
  const double _t261 = 0.96e2*_t260;
  const double _t262 = 0.216e3*_t260;
  const double _t263 = 0.192e3*_t260;
  const double _t264 = 0.60e2*_t260;
  const double _t265 = _t17*_t36;
  const double _t266 = 0.5040e4*_t265;
  const double _t267 = _t20*_t39;
  const double _t268 = 0.28800e5*_t267;
  const double _t269 = _t42*_t57;
  const double _t270 = 0.59400e5*_t269;
  const double _t271 = _t106*_t45;
  const double _t272 = 0.52800e5*_t271;
  const double _t273 = 0.17160e5*_t182;
  const double _t274 = 0.7920e4*_t267;
  const double _t275 = 0.43560e5*_t269;
  const double _t276 = 0.87120e5*_t45;
  const double _t277 = 0.75504e5*_t18*_t182;
  const double _t278 = 0.24024e5/xc_powi(_t14, 15);
  const double _t279 = _t24*_t35;
  const double _t280 = 0.120e3*_t279;
  const double _t281 = 0.720e3*_t279;
  const double _t282 = _t27*_t38;
  const double _t283 = 0.360e3*_t282;
  const double _t284 = 0.1440e4*_t279;
  const double _t285 = 0.1920e4*_t282;
  const double _t286 = _t30*_t41;
  const double _t287 = 0.840e3*_t286;
  const double _t288 = 0.1200e4*_t279;
  const double _t289 = 0.3600e4*_t282;
  const double _t290 = 0.4200e4*_t286;
  const double _t291 = _t33*_t44;
  const double _t292 = 0.1680e4*_t291;
  const double _t293 = 0.360e3*_t279;
  const double _t294 = 0.2880e4*_t282;
  const double _t295 = 0.7560e4*_t286;
  const double _t296 = 0.8064e4*_t291;
  const double _t297 = 0.3024e4*_t265;
  const double _t298 = 0.840e3*_t282;
  const double _t299 = 0.5880e4*_t286;
  const double _t300 = 0.14112e5*params->a[7];
  const double _t301 = 0.5040e4*_t267;
  const double _t302 = 0.1680e4*_t286;
  const double _t303 = 0.10752e5*_t291;
  const double _t304 = 0.24192e5*_t265;
  const double _t305 = 0.23040e5*_t267;
  const double _t306 = 0.7920e4*_t269;
  const double _t307 = 0.3024e4*_t291;
  const double _t308 = 0.18144e5*_t265;
  const double _t309 = 0.38880e5*_t267;
  const double _t310 = 0.35640e5*_t269;
  const double _t311 = 0.11880e5*_t271;
  const double _t312 = 0.14112e5*params->b[7];
  const double _t313 = _t140*_t4;
  const double _t315 = _t141*_t4;
  const double _t316 = _t142*_t4;
  const double _t320 = ((_hc2[2]) * (_hc2[2]) * (_hc2[2]) * (_hc2[2]));
  const double _t321 = xc_powi(_t5, -7);
  const double _t324 = ((_hc2[5]) * (_hc2[5]));
  const double _t326 = _t239*_t87;
  const double _t335 = _t3*_t86;
  const double _t336 = _t152*_t91;
#endif
  double _hc3[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(attenuation_erf_k, _KMAX)(p, _t4*_t9, _hc3);
#if _KMAX >= 1
  const double _t89 = _hc3[1]*_t8*_t88;
  const double _t90 = _hc2[2]*_t89;
  const double _t92 = _hc3[1]*_t9;
#endif
#if _KMAX >= 2
  const double _t143 = _t4*_t90;
  const double _t144 = _hc2[5]*_t89;
  const double _t148 = _hc3[1]*_t7;
  const double _t149 = _t147*_t148;
  const double _t150 = _t146*_t149;
  const double _t159 = _hc3[2]*_t155*_t156*_t157*_t158;
  const double _t160 = _t154*_t159;
  const double _t161 = _t153*_t160;
  const double _t162 = _t3*_t92;
  const double _t164 = _t159*rs;
  const double _t165 = _t163*_t164;
  const double _t166 = _t159*_t88;
#endif
#if _KMAX >= 3
  const double _t229 = _hc3[3]*_t227*_t228;
  const double _t230 = _t151*_t229;
  const double _t236 = _hc2[9]*_t89;
  const double _t237 = _t148*_t232;
  const double _t238 = _t152*_t237;
  const double _t242 = _t159*_t50;
  const double _t243 = _t154*_t242;
  const double _t246 = _t164*_t245;
  const double _t247 = _t3*_t90;
  const double _t251 = _t159*_t250;
  const double _t252 = _t242*rs;
#endif
#if _KMAX >= 4
  const double _t314 = (0.1e1 / 0.18e2)*_t161;
  const double _t317 = _t229*_t235;
  const double _t318 = _t238*_t4;
  const double _t319 = _t160*_t241;
  const double _t322 = _t229*_t50;
  const double _t323 = _t231*_t322;
  const double _t325 = _t149*_t87;
  const double _t327 = _t152*_t242;
  const double _t328 = (0.1e1 / 0.108e3)*_hc3[4]*((p->cam_omega) * (p->cam_omega) * (p->cam_omega) * (p->cam_omega))/_t11;
  const double _t329 = _t328*_t91;
  const double _t330 = _hc2[5]*_t242;
  const double _t331 = (0.1e1 / 0.18e2)*_t246;
  const double _t332 = _t229*_t249;
  const double _t333 = _t147*_t251;
  const double _t334 = _t164*_t253;
  const double _t337 = (0.1e1 / 0.18e2)*_t166;
  const double _t338 = _t229*_t255;
  const double _t339 = _t159*_t256;
  const double _t340 = _t147*_t229;
#endif

  const double f = _hc3[0]*_t50;
  out[0] = f;
#if _KMAX >= 1
  const double df_dt = _hc3[0]*_t85;
  out[1] = df_dt;
  const double df_du = 0;
  out[2] = df_du;
  const double df_dx = _hc3[0]*_t86;
  out[3] = df_dx;
  const double df_dz = -_t87*_t90;
  out[4] = df_dz;
  const double df_drs = _t91*_t92;
  out[5] = df_drs;
#endif
#if _KMAX >= 2
  const double d2f_dt2 = _hc3[0]*_t140;
  out[6] = d2f_dt2;
  const double d2f_du_dt = 0;
  out[7] = d2f_du_dt;
  const double d2f_du2 = 0;
  out[8] = d2f_du2;
  const double d2f_dx_dt = _hc3[0]*_t141;
  out[9] = d2f_dx_dt;
  const double d2f_dx_du = 0;
  out[10] = d2f_dx_du;
  const double d2f_dx2 = _hc3[0]*_t142;
  out[11] = d2f_dx2;
  const double d2f_dz_dt = -_t143*_t85;
  out[12] = d2f_dz_dt;
  const double d2f_dz_du = 0;
  out[13] = d2f_dz_du;
  const double d2f_dz_dx = -_t143*_t86;
  out[14] = d2f_dz_dx;
  const double d2f_dz2 = -_t144*_t87 + _t150*_t87 + _t151*_t161;
  out[15] = d2f_dz2;
  const double d2f_drs_dt = _t162*_t85;
  out[16] = d2f_drs_dt;
  const double d2f_drs_du = 0;
  out[17] = d2f_drs_du;
  const double d2f_drs_dx = _t162*_t86;
  out[18] = d2f_drs_dx;
  const double d2f_drs_dz = -_hc2[2]*_t165 - _hc2[2]*_t89*_t91;
  out[19] = d2f_drs_dz;
  const double d2f_drs2 = _t151*_t166;
  out[20] = d2f_drs2;
#endif
#if _KMAX >= 3
  const double d3f_dt3 = _hc3[0]*_t219;
  out[21] = d3f_dt3;
  ...
  out[24] = d3f_du3;
  const double d3f_dx_dt2 = _hc3[0]*_t220;
  out[25] = d3f_dx_dt2;
  const double d3f_dx_du_dt = 0;
  out[26] = d3f_dx_du_dt;
  const double d3f_dx_du2 = 0;
  out[27] = d3f_dx_du2;
  const double d3f_dx2_dt = _hc3[0]*_t221;
  out[28] = d3f_dx2_dt;
  const double d3f_dx2_du = 0;
  out[29] = d3f_dx2_du;
  const double d3f_dx3 = _hc3[0]*_t222;
  out[30] = d3f_dx3;
  const double d3f_dz_dt2 = -_t140*_t143;
  out[31] = d3f_dz_dt2;
  const double d3f_dz_du_dt = 0;
  out[32] = d3f_dz_du_dt;
  const double d3f_dz_du2 = 0;
  out[33] = d3f_dz_du2;
  const double d3f_dz_dx_dt = -_t141*_t143;
  out[34] = d3f_dz_dx_dt;
  const double d3f_dz_dx_du = 0;
  out[35] = d3f_dz_dx_du;
  const double d3f_dz_dx2 = -_t142*_t143;
  out[36] = d3f_dz_dx2;
  const double d3f_dz2_dt = -_t144*_t223 + _t150*_t223 + _t161*_t224;
  out[37] = d3f_dz2_dt;
  const double d3f_dz2_du = 0;
  out[38] = d3f_dz2_du;
  const double d3f_dz2_dx = -_t144*_t225 + _t150*_t225 + _t161*_t226;
  out[39] = d3f_dz2_dx;
  const double d3f_dz3 = _hc2[2]*_hc2[5]*_hc3[1]*_t1*_t147*_t2*_t50*_t6*p->cam_omega*rs + (0.1e1 / 0.6e1)*_hc2[2]*_hc2[5]*_hc3[2]*_t152*_t154*_t155*_t156*_t157*_t158*_t50 - _t230*_t235 - _t236*_t87 - _t238*_t87 - _t241*_t243;
  out[40] = d3f_dz3;
  const double d3f_drs_dt2 = _t140*_t162;
  out[41] = d3f_drs_dt2;
  const double d3f_drs_du_dt = 0;
  out[42] = d3f_drs_du_dt;
  const double d3f_drs_du2 = 0;
  out[43] = d3f_drs_du2;
  const double d3f_drs_dx_dt = _t141*_t162;
  out[44] = d3f_drs_dx_dt;
  const double d3f_drs_dx_du = 0;
  out[45] = d3f_drs_dx_du;
  const double d3f_drs_dx2 = _t142*_t162;
  out[46] = d3f_drs_dx2;
  const double d3f_drs_dz_dt = -_t224*_t246 - _t244*_t90;
  out[47] = d3f_drs_dz_dt;
  const double d3f_drs_dz_du = 0;
  out[48] = d3f_drs_dz_du;
  const double d3f_drs_dz_dx = -_t226*_t246 - _t247*_t86;
  out[49] = d3f_drs_dz_dx;
  const double d3f_drs_dz2 = -_t144*_t91 + _t150*_t91 - _t163*_t251 + _t230*_t249 + _t252*_t253;
  out[50] = d3f_drs_dz2;
  const double d3f_drs2_dt = _t166*_t224;
  out[51] = d3f_drs2_dt;
  const double d3f_drs2_du = 0;
  out[52] = d3f_drs2_du;
  const double d3f_drs2_dx = _t166*_t226;
  out[53] = d3f_drs2_dx;
  const double d3f_drs2_dz = -_t230*_t255 - _t242*_t256;
  out[54] = d3f_drs2_dz;
  const double d3f_drs3 = _t163*_t229;
  out[55] = d3f_drs3;
#endif
#if _KMAX >= 4
  const double d4f_dt4 = _hc3[0]*(_hc0[0]*(_t107*_t276 + _t176*_t273 + _t181*_t278 + _t257*params->a[1] + _t257*params->a[4] + _t258*params->a[2] + _t258*params->a[3] + _t259*_t94 - _t261*params->a[2] - _t262*params->a[3] - _t263*params->a[4] - _t264*params->a[5] + _t265*_t300 + _t266*params->a[10] + _t268*params->a[10] + _t270*params->a[10] + _t272*params->a[10] + _t274*params->a[11] + _t275*params->a[11] + _t277*params->a[11] + _t280*params->a[2] + _t281*params->a[3] + _t283*params->a[3] + _t284*params->a[4] + _t285*params->a[4] + _t287*params->a[4] + _t288*params->a[5] + _t289*params->a[5] + _t290*params->a[5] + _t291*_t300 + _t292*params->a[5] + _t293*params->a[6] + _t294*params->a[6] + _t295*params->a[6] + _t296*params->a[6] + _t297*params->a[6] + _t298*params->a[7] + _t299*params->a[7] + _t301*params->a[7] + _t302*params->a[8] + _t303*params->a[8] + _t304*params->a[8] + _t305*params->a[8] + _t306*params->a[8] + _t307*params->a[9] + _t308*params->a[9] + _t309*params->a[9] + _t310*params->a[9] + _t311*params->a[9]) + _hc1[0]*(_t137*_t259 + _t138*_t276 + _t216*_t273 + _t217*_t278 + _t257*params->b[1] + _t257*params->b[4] + _t258*params->b[2] + _t258*params->b[3] - _t261*params->b[2] - _t262*params->b[3] - _t263*params->b[4] - _t264*params->b[5] + _t265*_t312 + _t266*params->b[10] + _t268*params->b[10] + _t270*params->b[10] + _t272*params->b[10] + _t274*params->b[11] + _t275*params->b[11] + _t277*params->b[11] + _t280*params->b[2] + _t281*params->b[3] + _t283*params->b[3] + _t284*params->b[4] + _t285*params->b[4] + _t287*params->b[4] + _t288*params->b[5] + _t289*params->b[5] + _t290*params->b[5] + _t291*_t312 + _t292*params->b[5] + _t293*params->b[6] + _t294*params->b[6] + _t295*params->b[6] + _t296*params->b[6] + _t297*params->b[6] + _t298*params->b[7] + _t299*params->b[7] + _t301*params->b[7] + _t302*params->b[8] + _t303*params->b[8] + _t304*params->b[8] + _t305*params->b[8] + _t306*params->b[8] + _t307*params->b[9] + _t308*params->b[9] + _t309*params->b[9] + _t310*params->b[9] + _t311*params->b[9]));
  out[56] = d4f_dt4;
  ...
  out[60] = d4f_du4;
  const double d4f_dx_dt3 = _hc3[0]*(_hc0[1]*_t215 + _hc1[1]*_t218);
  out[61] = d4f_dx_dt3;
  ...
  out[64] = d4f_dx_du3;
  const double d4f_dx2_dt2 = _hc3[0]*(_hc0[2]*_t136 + _hc1[2]*_t139);
  out[65] = d4f_dx2_dt2;
  const double d4f_dx2_du_dt = 0;
  out[66] = d4f_dx2_du_dt;
  const double d4f_dx2_du2 = 0;
  out[67] = d4f_dx2_du2;
  const double d4f_dx3_dt = _hc3[0]*(_hc0[3]*_t74 + _hc1[3]*_t84);
  out[68] = d4f_dx3_dt;
  const double d4f_dx3_du = 0;
  out[69] = d4f_dx3_du;
  const double d4f_dx4 = _hc3[0]*(_hc0[4]*_t47 + _hc1[4]*_t49);
  out[70] = d4f_dx4;
  const double d4f_dz_dt3 = -_t143*_t219;
  out[71] = d4f_dz_dt3;
  ...
  out[74] = d4f_dz_du3;
  const double d4f_dz_dx_dt2 = -_t143*_t220;
  out[75] = d4f_dz_dx_dt2;
  const double d4f_dz_dx_du_dt = 0;
  out[76] = d4f_dz_dx_du_dt;
  const double d4f_dz_dx_du2 = 0;
  out[77] = d4f_dz_dx_du2;
  const double d4f_dz_dx2_dt = -_t143*_t221;
  out[78] = d4f_dz_dx2_dt;
  const double d4f_dz_dx2_du = 0;
  out[79] = d4f_dz_dx2_du;
  const double d4f_dz_dx3 = -_t143*_t222;
  out[80] = d4f_dz_dx3;
  const double d4f_dz2_dt2 = _t140*_t314 - _t144*_t313 + _t150*_t313;
  out[81] = d4f_dz2_dt2;
  const double d4f_dz2_du_dt = 0;
  out[82] = d4f_dz2_du_dt;
  const double d4f_dz2_du2 = 0;
  out[83] = d4f_dz2_du2;
  const double d4f_dz2_dx_dt = _t141*_t314 - _t144*_t315 + _t150*_t315;
  out[84] = d4f_dz2_dx_dt;
  const double d4f_dz2_dx_du = 0;
  out[85] = d4f_dz2_dx_du;
  const double d4f_dz2_dx2 = _t142*_t314 - _t144*_t316 + _t150*_t316;
  out[86] = d4f_dz2_dx2;
  const double d4f_dz3_dt = _hc2[2]*_hc2[5]*_hc3[1]*_t1*_t147*_t2*_t6*_t85*p->cam_omega*rs + (0.1e1 / 0.6e1)*_hc2[2]*_hc2[5]*_hc3[2]*_t152*_t154*_t155*_t156*_t157*_t158*_t85 - _t223*_t236 - _t224*_t317 - _t318*_t85 - _t319*_t85;
  out[87] = d4f_dz3_dt;
  const double d4f_dz3_du = 0;
  out[88] = d4f_dz3_du;
  const double d4f_dz3_dx = _hc2[2]*_hc2[5]*_hc3[1]*_t1*_t147*_t2*_t6*_t86*p->cam_omega*rs + (0.1e1 / 0.6e1)*_hc2[2]*_hc2[5]*_hc3[2]*_t152*_t154*_t155*_t156*_t157*_t158*_t86 - _t225*_t236 - _t226*_t317 - _t318*_t86 - _t319*_t86;
  out[89] = d4f_dz3_dx;
  const double d4f_dz4 = -_hc2[14]*_t87*_t89 + (0.4e1 / 0.3e1)*_hc2[2]*_hc2[9]*_t325 - _hc2[5]*_t146*_t233*_t323 - 0.6e1*_hc2[5]*_t148*_t153*_t87 + (0.2e1 / 0.9e1)*_hc2[9]*_t243*_t254 + 0.4e1*_t148*_t320*_t326 + (0.1e1 / 0.6e1)*_t154*_t324*_t327 + 0.2e1*_t233*_t243*_t320 - 0.2e1*_t249*_t330 + (0.2e1 / 0.3e1)*_t320*_t321*_t323 + _t320*_t329*(rs * rs * rs * rs)/xc_powi(_t5, 8) + _t324*_t325;
  out[90] = d4f_dz4;
  const double d4f_drs_dt3 = _t162*_t219;
  out[91] = d4f_drs_dt3;
  ...
  out[94] = d4f_drs_du3;
  const double d4f_drs_dx_dt2 = _t162*_t220;
  ...
  const double d4f_drs_dx2_dt = _t162*_t221;
  out[98] = d4f_drs_dx2_dt;
  const double d4f_drs_dx2_du = 0;
  out[99] = d4f_drs_dx2_du;
  const double d4f_drs_dx3 = _t162*_t222;
  out[100] = d4f_drs_dx3;
  const double d4f_drs_dz_dt2 = -_t140*_t247 - _t140*_t331;
  ...
  const double d4f_drs_dz_dx_dt = -_t141*_t247 - _t141*_t331;
  out[104] = d4f_drs_dz_dx_dt;
  const double d4f_drs_dz_dx_du = 0;
  out[105] = d4f_drs_dz_dx_du;
  const double d4f_drs_dz_dx2 = -_t142*_t247 - _t142*_t331;
  out[106] = d4f_drs_dz_dx2;
  const double d4f_drs_dz2_dt = -_t144*_t244 + _t150*_t244 + _t224*_t332 - _t224*_t333 + _t334*_t85;
  out[107] = d4f_drs_dz2_dt;
  const double d4f_drs_dz2_du = 0;
  out[108] = d4f_drs_dz2_du;
  const double d4f_drs_dz2_dx = -_t144*_t335 + _t150*_t335 + _t226*_t332 - _t226*_t333 + _t334*_t86;
  out[109] = d4f_drs_dz2_dx;
  const double d4f_drs_dz3 = _hc2[2]*_hc2[5]*_hc3[1]*_t1*_t147*_t2*_t50*_t6*p->cam_omega + (0.2e1 / 0.3e1)*_hc2[2]*_hc2[5]*_hc3[2]*_t152*_t155*_t156*_t157*_t158*_t50*rs + (0.1e1 / 0.6e1)*_hc2[2]*_hc2[5]*_hc3[3]*_t154*_t227*_t228*_t239*_t50 - _hc2[9]*_t165 - 0.1e1 / 0.2e1*_t154*_t234*_t322 - _t231*_t232*_t321*_t329 - _t236*_t91 - _t237*_t336 - _t240*_t252;
  out[110] = d4f_drs_dz3;
  const double d4f_drs2_dt2 = _t140*_t337;
  out[111] = d4f_drs2_dt2;
  const double d4f_drs2_du_dt = 0;
  out[112] = d4f_drs2_du_dt;
  const double d4f_drs2_du2 = 0;
  out[113] = d4f_drs2_du2;
  const double d4f_drs2_dx_dt = _t141*_t337;
  out[114] = d4f_drs2_dx_dt;
  const double d4f_drs2_dx_du = 0;
  out[115] = d4f_drs2_dx_du;
  const double d4f_drs2_dx2 = _t142*_t337;
  out[116] = d4f_drs2_dx2;
  const double d4f_drs2_dz_dt = -_t224*_t338 - _t339*_t85;
  out[117] = d4f_drs2_dz_dt;
  const double d4f_drs2_dz_du = 0;
  out[118] = d4f_drs2_dz_du;
  const double d4f_drs2_dz_dx = -_t226*_t338 - _t339*_t86;
  out[119] = d4f_drs2_dz_dx;
  const double d4f_drs2_dz2 = _t145*_t154*_t233*_t329 + _t146*_t327 - 0.1e1 / 0.9e1*_t147*_t330 - _t152*_t230*_t250 + (0.1e1 / 0.3e1)*_t248*_t322*rs;
  out[120] = d4f_drs2_dz2;
  const double d4f_drs3_dt = _t224*_t340;
  out[121] = d4f_drs3_dt;
  const double d4f_drs3_du = 0;
  out[122] = d4f_drs3_du;
  const double d4f_drs3_dx = _t226*_t340;
  out[123] = d4f_drs3_dx;
  const double d4f_drs3_dz = -_hc2[2]*_t326*_t328 - 0.1e1 / 0.6e1*_t254*_t322;
  out[124] = d4f_drs3_dz;
  const double d4f_drs4 = _t328*_t336;
  out[125] = d4f_drs4;
#endif
}
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(m11_f_z0_k, _KMAX)(const xc_func_type *p, double rs, double x, double u, double t, double *out) {
  const mgga_x_m11_params *params = (const mgga_x_m11_params *)(p->params);
  ...
  const double _t5 = (0.3e1 / 0.10e2)*_t3*_t4 - t;
  const double _t6 = _t3*_t4;
  const double _t7 = (0.3e1 / 0.10e2)*_t6 + t;
  const double _t8 = (0.1e1 / _t7);
  const double _t9 = _t8*params->a[1];
  const double _t10 = xc_powi(_t7, -10);
  const double _t11 = xc_powi(_t5, 10);
  const double _t12 = _t10*_t11;
  const double _t13 = xc_powi(_t7, -11);
  const double _t14 = xc_powi(_t5, 11);
  const double _t15 = _t13*_t14;
  const double _t16 = (0.1e1 / (_t7 * _t7));
  const double _t17 = (_t5 * _t5);
  const double _t18 = _t16*_t17;
  const double _t19 = (0.1e1 / (_t7 * _t7 * _t7));
  const double _t20 = (_t5 * _t5 * _t5);
  const double _t21 = _t19*_t20;
  const double _t22 = (0.1e1 / (_t7 * _t7 * _t7 * _t7));
  const double _t23 = (_t5 * _t5 * _t5 * _t5);
  const double _t24 = _t22*_t23;
  const double _t25 = xc_powi(_t7, -5);
  const double _t26 = xc_powi(_t5, 5);
  const double _t27 = _t25*_t26;
  const double _t28 = xc_powi(_t7, -6);
  const double _t29 = xc_powi(_t5, 6);
  const double _t30 = _t28*_t29;
  const double _t31 = xc_powi(_t7, -7);
  const double _t32 = xc_powi(_t5, 7);
  const double _t33 = _t31*_t32;
  const double _t34 = xc_powi(_t7, -8);
  const double _t35 = xc_powi(_t5, 8);
  const double _t36 = _t34*_t35;
  const double _t37 = xc_powi(_t7, -9);
  const double _t38 = xc_powi(_t5, 9);
  const double _t39 = _t37*_t38;
  const double _t40 = _t12*params->a[10] + _t15*params->a[11] + _t18*params->a[2] + _t21*params->a[3] + _t24*params->a[4] + _t27*params->a[5] + _t30*params->a[6] + _t33*params->a[7] + _t36*params->a[8] + _t39*params->a[9] + _t5*_t9 + params->a[0];
  const double _t41 = _t8*params->b[1];
  const double _t42 = _t12*params->b[10] + _t15*params->b[11] + _t18*params->b[2] + _t21*params->b[3] + _t24*params->b[4] + _t27*params->b[5] + _t30*params->b[6] + _t33*params->b[7] + _t36*params->b[8] + _t39*params->b[9] + _t41*_t5 + params->b[0];
  const double _t43 = _hc0[0]*_t40 + _hc1[0]*_t42;
#if _KMAX >= 1
  const double _t44 = _t16*_t5;
  const double _t45 = -0.3e1 / 0.5e1*_t6 + 0.2e1*t;
  const double _t46 = 0.10e2*params->a[10];
  const double _t47 = _t10*_t38;
  const double _t48 = _t11*_t13;
  const double _t49 = 0.11e2*params->a[11];
  const double _t50 = xc_powi(_t7, -12);
  const double _t51 = _t14*_t50;
  const double _t52 = _t17*_t19;
  const double _t53 = 0.2e1*_t52;
  const double _t54 = 0.3e1*params->a[3];
  const double _t55 = _t20*_t22;
  const double _t56 = 0.4e1*params->a[4];
  const double _t57 = _t23*_t25;
  const double _t58 = 0.5e1*params->a[5];
  const double _t59 = _t26*_t28;
  const double _t60 = 0.6e1*params->a[6];
  const double _t61 = _t29*_t31;
  const double _t62 = 0.7e1*params->a[7];
  const double _t63 = _t32*_t34;
  const double _t64 = 0.8e1*params->a[8];
  const double _t65 = _t35*_t37;
  const double _t66 = 0.9e1*params->a[9];
  const double _t67 = _t16*_t45*params->a[2] - _t44*params->a[1] - _t46*_t47 - _t46*_t48 - _t47*_t66 - _t48*_t49 - _t49*_t51 - _t52*_t54 - _t53*params->a[2] - _t54*_t55 - _t55*_t56 - _t56*_t57 - _t57*_t58 - _t58*_t59 - _t59*_t60 - _t60*_t61 - _t61*_t62 - _t62*_t63 - _t63*_t64 - _t64*_t65 - _t65*_t66 - _t9;
  const double _t68 = 0.10e2*params->b[10];
  const double _t69 = 0.11e2*params->b[11];
  const double _t70 = 0.3e1*params->b[3];
  const double _t71 = 0.4e1*params->b[4];
  const double _t72 = 0.5e1*params->b[5];
  const double _t73 = 0.6e1*params->b[6];
  const double _t74 = 0.7e1*params->b[7];
  const double _t75 = 0.8e1*params->b[8];
  const double _t76 = 0.9e1*params->b[9];
  const double _t77 = _t16*_t45*params->b[2] - _t41 - _t44*params->b[1] - _t47*_t68 - _t47*_t76 - _t48*_t68 - _t48*_t69 - _t51*_t69 - _t52*_t70 - _t53*params->b[2] - _t55*_t70 - _t55*_t71 - _t57*_t71 - _t57*_t72 - _t59*_t72 - _t59*_t73 - _t61*_t73 - _t61*_t74 - _t63*_t74 - _t63*_t75 - _t65*_t75 - _t65*_t76;
  const double _t78 = _hc0[0]*_t67 + _hc1[0]*_t77;
  const double _t79 = _hc0[1]*_t40 + _hc1[1]*_t42;
#endif
#if _KMAX >= 2
  const double _t81 = 0.2e1*_t16;
  const double _t82 = _t5*params->a[1];
  const double _t83 = 0.2e1*_t19;
  const double _t84 = _t19*_t45;
  const double _t85 = 0.4e1*_t84;
  const double _t86 = _t10*_t35;
  const double _t87 = 0.90e2*_t86;
  const double _t88 = _t13*_t38;
  const double _t89 = 0.200e3*_t88;
  const double _t90 = _t11*_t50;
  const double _t91 = 0.110e3*_t90;
  const double _t92 = 0.110e3*_t88;
  const double _t93 = 0.242e3*_t90;
  const double _t94 = xc_powi(_t7, -13);
  const double _t95 = _t94*params->a[11];
  const double _t96 = 0.132e3*_t14;
  const double _t97 = _t17*_t22;
  const double _t98 = 0.6e1*_t97;
  const double _t99 = 0.18e2*_t97;
  const double _t100 = _t20*_t25;
  const double _t101 = 0.12e2*_t100;
  const double _t102 = 0.12e2*_t97;
  const double _t103 = 0.32e2*_t100;
  const double _t104 = _t23*_t28;
  const double _t105 = 0.20e2*_t104;
  const double _t106 = 0.20e2*_t100;
  const double _t107 = 0.50e2*_t104;
  const double _t108 = _t26*_t31;
  const double _t109 = 0.30e2*_t108;
  const double _t110 = 0.30e2*_t104;
  const double _t111 = 0.72e2*_t108;
  const double _t112 = _t29*_t34;
  const double _t113 = 0.42e2*_t112;
  const double _t114 = 0.42e2*_t108;
  const double _t115 = 0.98e2*_t112;
  const double _t116 = _t32*_t37;
  const double _t117 = 0.56e2*_t116;
  const double _t118 = 0.56e2*_t112;
  const double _t119 = 0.128e3*_t116;
  const double _t120 = 0.72e2*_t86;
  const double _t121 = 0.72e2*_t116;
  const double _t122 = 0.162e3*_t86;
  const double _t123 = 0.90e2*_t88;
  const double _t124 = _t101*params->a[3] + _t102*params->a[4] + _t103*params->a[4] + _t105*params->a[4] + _t106*params->a[5] + _t107*params->a[5] + _t109*params->a[5] + _t110*params->a[6] + _t111*params->a[6] + _t113*params->a[6] + _t114*params->a[7] + _t115*params->a[7] + _t117*params->a[7] + _t118*params->a[8] + _t119*params->a[8] + _t120*params->a[8] + _t121*params->a[9] + _t122*params->a[9] + _t123*params->a[9] - _t54*_t84 + _t81*params->a[1] + _t81*params->a[2] + _t82*_t83 - _t85*params->a[2] + _t87*params->a[10] + _t89*params->a[10] + _t91*params->a[10] + _t92*params->a[11] + _t93*params->a[11] + _t95*_t96 + _t98*params->a[2] + _t99*params->a[3];
  const double _t125 = _t5*params->b[1];
  const double _t126 = _t94*params->b[11];
  const double _t127 = _t101*params->b[3] + _t102*params->b[4] + _t103*params->b[4] + _t105*params->b[4] + _t106*params->b[5] + _t107*params->b[5] + _t109*params->b[5] + _t110*params->b[6] + _t111*params->b[6] + _t113*params->b[6] + _t114*params->b[7] + _t115*params->b[7] + _t117*params->b[7] + _t118*params->b[8] + _t119*params->b[8] + _t120*params->b[8] + _t121*params->b[9] + _t122*params->b[9] + _t123*params->b[9] + _t125*_t83 + _t126*_t96 - _t70*_t84 + _t81*params->b[1] + _t81*params->b[2] - _t85*params->b[2] + _t87*params->b[10] + _t89*params->b[10] + _t91*params->b[10] + _t92*params->b[11] + _t93*params->b[11] + _t98*params->b[2] + _t99*params->b[3];
  const double _t128 = _hc0[0]*_t124 + _hc1[0]*_t127;
  const double _t129 = _hc0[1]*_t67 + _hc1[1]*_t77;
  const double _t130 = _hc0[2]*_t40 + _hc1[2]*_t42;
  const double _t131 = (0.1e1 / 0.18e2)*_t43;
#endif
#if _KMAX >= 3
  const double _t133 = 0.6e1*_t19;
  const double _t134 = 0.12e2*_t19;
  const double _t135 = 0.6e1*_t22;
  const double _t136 = _t10*_t32;
  const double _t137 = 0.720e3*_t136;
  const double _t138 = _t13*_t35;
  const double _t139 = 0.2700e4*_t138;
  const double _t140 = _t38*_t50;
  const double _t141 = 0.3300e4*_t140;
  const double _t142 = _t11*params->a[10];
  const double _t143 = 0.1320e4*_t94;
  const double _t144 = 0.990e3*_t138;
  const double _t145 = 0.3630e4*_t140;
  const double _t146 = 0.4356e4*_t11;
  const double _t147 = _t14*params->a[11];
  const double _t148 = xc_powi(_t7, -14);
  const double _t149 = 0.1716e4*_t148;
  const double _t150 = _t17*_t25;
  const double _t151 = 0.24e2*_t150;
  const double _t152 = 0.108e3*_t150;
  const double _t153 = _t20*_t28;
  const double _t154 = 0.60e2*_t153;
  const double _t155 = 0.144e3*_t150;
  const double _t156 = 0.240e3*_t153;
  const double _t157 = _t23*_t31;
  const double _t158 = 0.120e3*_t157;
  const double _t159 = 0.60e2*_t150;
  const double _t160 = 0.300e3*_t153;
  const double _t161 = 0.450e3*_t157;
  const double _t162 = _t26*_t34;
  const double _t163 = 0.210e3*_t162;
  const double _t164 = 0.120e3*_t153;
  const double _t165 = 0.540e3*_t157;
  const double _t166 = 0.756e3*_t162;
  const double _t167 = _t29*_t37;
  const double _t168 = 0.336e3*_t167;
  const double _t169 = 0.210e3*_t157;
  const double _t170 = 0.882e3*_t162;
  const double _t171 = 0.1176e4*_t167;
  const double _t172 = 0.504e3*_t136;
  const double _t173 = 0.336e3*_t162;
  const double _t174 = 0.1344e4*_t167;
  const double _t175 = 0.1728e4*_t136;
  const double _t176 = 0.720e3*_t138;
  const double _t177 = 0.504e3*_t167;
  const double _t178 = 0.1944e4*_t136;
  const double _t179 = 0.2430e4*_t138;
  const double _t180 = 0.990e3*_t140;
  const double _t181 = -_t133*params->a[1] - _t133*params->a[3] - _t134*params->a[2] - _t135*_t82 - _t137*params->a[10] - _t139*params->a[10] - _t141*params->a[10] - _t142*_t143 - _t144*params->a[11] - _t145*params->a[11] - _t146*_t95 - _t147*_t149 - _t151*params->a[2] - _t152*params->a[3] - _t154*params->a[3] - _t155*params->a[4] - _t156*params->a[4] - _t158*params->a[4] - _t159*params->a[5] - _t160*params->a[5] - _t161*params->a[5] - _t163*params->a[5] - _t164*params->a[6] - _t165*params->a[6] - _t166*params->a[6] - _t168*params->a[6] - _t169*params->a[7] - _t170*params->a[7] - _t171*params->a[7] - _t172*params->a[7] - _t173*params->a[8] - _t174*params->a[8] - _t175*params->a[8] - _t176*params->a[8] - _t177*params->a[9] - _t178*params->a[9] - _t179*params->a[9] - _t180*params->a[9] + 0.18e2*_t22*_t45*params->a[2] + 0.27e2*_t22*_t45*params->a[3] + 0.12e2*_t22*_t45*params->a[4];
  const double _t182 = _t11*params->b[10];
  const double _t183 = _t14*params->b[11];
  const double _t184 = -_t125*_t135 - _t126*_t146 - _t133*params->b[1] - _t133*params->b[3] - _t134*params->b[2] - _t137*params->b[10] - _t139*params->b[10] - _t141*params->b[10] - _t143*_t182 - _t144*params->b[11] - _t145*params->b[11] - _t149*_t183 - _t151*params->b[2] - _t152*params->b[3] - _t154*params->b[3] - _t155*params->b[4] - _t156*params->b[4] - _t158*params->b[4] - _t159*params->b[5] - _t160*params->b[5] - _t161*params->b[5] - _t163*params->b[5] - _t164*params->b[6] - _t165*params->b[6] - _t166*params->b[6] - _t168*params->b[6] - _t169*params->b[7] - _t170*params->b[7] - _t171*params->b[7] - _t172*params->b[7] - _t173*params->b[8] - _t174*params->b[8] - _t175*params->b[8] - _t176*params->b[8] - _t177*params->b[9] - _t178*params->b[9] - _t179*params->b[9] - _t180*params->b[9] + 0.18e2*_t22*_t45*params->b[2] + 0.27e2*_t22*_t45*params->b[3] + 0.12e2*_t22*_t45*params->b[4];
  const double _t185 = _hc0[0]*_t181 + _hc1[0]*_t184;
  const double _t186 = _hc0[1]*_t124 + _hc1[1]*_t127;
  const double _t187 = _hc0[2]*_t67 + _hc1[2]*_t77;
  const double _t188 = _hc0[3]*_t40 + _hc1[3]*_t42;
#endif
#if _KMAX >= 4
  const double _t191 = 0.24e2*_t22;
  const double _t192 = 0.72e2*_t22;
  const double _t193 = 0.24e2*_t25;
  const double _t194 = _t25*_t45;
  const double _t195 = 0.96e2*_t194;
  const double _t196 = 0.216e3*_t194;
  const double _t197 = 0.192e3*_t194;
  const double _t198 = 0.60e2*_t194;
  const double _t199 = _t10*_t29;
  const double _t200 = 0.5040e4*_t199;
  const double _t201 = _t13*_t32;
  const double _t202 = 0.28800e5*_t201;
  const double _t203 = _t35*_t50;
  const double _t204 = 0.59400e5*_t203;
  const double _t205 = _t38*_t94;
  const double _t206 = 0.52800e5*_t205;
  const double _t207 = 0.17160e5*_t148;
  const double _t208 = 0.7920e4*_t201;
  const double _t209 = 0.43560e5*_t203;
  const double _t210 = 0.87120e5*_t38;
  const double _t211 = 0.75504e5*_t11*_t148;
  const double _t212 = 0.24024e5/xc_powi(_t7, 15);
  const double _t213 = _t17*_t28;
  const double _t214 = 0.120e3*_t213;
  const double _t215 = 0.720e3*_t213;
  const double _t216 = _t20*_t31;
  const double _t217 = 0.360e3*_t216;
  const double _t218 = 0.1440e4*_t213;
  const double _t219 = 0.1920e4*_t216;
  const double _t220 = _t23*_t34;
  const double _t221 = 0.840e3*_t220;
  const double _t222 = 0.1200e4*_t213;
  const double _t223 = 0.3600e4*_t216;
  const double _t224 = 0.4200e4*_t220;
  const double _t225 = _t26*_t37;
  const double _t226 = 0.1680e4*_t225;
  const double _t227 = 0.360e3*_t213;
  const double _t228 = 0.2880e4*_t216;
  const double _t229 = 0.7560e4*_t220;
  const double _t230 = 0.8064e4*_t225;
  const double _t231 = 0.3024e4*_t199;
  const double _t232 = 0.840e3*_t216;
  const double _t233 = 0.5880e4*_t220;
  const double _t234 = 0.14112e5*params->a[7];
  const double _t235 = 0.5040e4*_t201;
  const double _t236 = 0.1680e4*_t220;
  const double _t237 = 0.10752e5*_t225;
  const double _t238 = 0.24192e5*_t199;
  const double _t239 = 0.23040e5*_t201;
  const double _t240 = 0.7920e4*_t203;
  const double _t241 = 0.3024e4*_t225;
  const double _t242 = 0.18144e5*_t199;
  const double _t243 = 0.38880e5*_t201;
  const double _t244 = 0.35640e5*_t203;
  const double _t245 = 0.11880e5*_t205;
  const double _t246 = 0.14112e5*params->b[7];
#endif
  double _hc2[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(attenuation_erf_k, _KMAX)(p, _t2*rs, _hc2);
#if _KMAX >= 1
  const double _t80 = _hc2[1]*_t2;
#endif
#if _KMAX >= 2
  const double _t132 = M_CBRT2*xc_powr(0.3e1, 2, 3)*_hc2[2]*((p->cam_omega) * (p->cam_omega))/(M_CBRTPI * M_CBRTPI);
#endif
#if _KMAX >= 3
  const double _t189 = (0.1e1 / 0.18e2)*_t132;
  const double _t190 = _hc2[3]*((p->cam_omega) * (p->cam_omega) * (p->cam_omega))/M_PI;
#endif
#if _KMAX >= 4
  const double _t247 = (0.1e1 / 0.18e2)*_t190;
#endif

  const double f = _hc2[0]*_t43;
  out[0] = f;
#if _KMAX >= 1
  const double df_dt = _hc2[0]*_t78;
  out[1] = df_dt;
  const double df_du = 0;
  out[2] = df_du;
  const double df_dx = _hc2[0]*_t79;
  out[3] = df_dx;
  const double df_drs = _t43*_t80;
  out[4] = df_drs;
#endif
#if _KMAX >= 2
  const double d2f_dt2 = _hc2[0]*_t128;
  out[5] = d2f_dt2;
  const double d2f_du_dt = 0;
  out[6] = d2f_du_dt;
  const double d2f_du2 = 0;
  out[7] = d2f_du2;
  const double d2f_dx_dt = _hc2[0]*_t129;
  out[8] = d2f_dx_dt;
  const double d2f_dx_du = 0;
  out[9] = d2f_dx_du;
  const double d2f_dx2 = _hc2[0]*_t130;
  out[10] = d2f_dx2;
  const double d2f_drs_dt = _t78*_t80;
  out[11] = d2f_drs_dt;
  const double d2f_drs_du = 0;
  out[12] = d2f_drs_du;
  const double d2f_drs_dx = _t79*_t80;
  out[13] = d2f_drs_dx;
  const double d2f_drs2 = _t131*_t132;
  out[14] = d2f_drs2;
#endif
#if _KMAX >= 3
  const double d3f_dt3 = _hc2[0]*_t185;
  out[15] = d3f_dt3;
  const double d3f_du_dt2 = 0;
  out[16] = d3f_du_dt2;
  const double d3f_du2_dt = 0;
  ...
  const double d3f_du3 = 0;
  out[18] = d3f_du3;
  const double d3f_dx_dt2 = _hc2[0]*_t186;
  out[19] = d3f_dx_dt2;
  const double d3f_dx_du_dt = 0;
  out[20] = d3f_dx_du_dt;
  const double d3f_dx_du2 = 0;
  out[21] = d3f_dx_du2;
  const double d3f_dx2_dt = _hc2[0]*_t187;
  out[22] = d3f_dx2_dt;
  const double d3f_dx2_du = 0;
  out[23] = d3f_dx2_du;
  const double d3f_dx3 = _hc2[0]*_t188;
  out[24] = d3f_dx3;
  const double d3f_drs_dt2 = _t128*_t80;
  out[25] = d3f_drs_dt2;
  const double d3f_drs_du_dt = 0;
  out[26] = d3f_drs_du_dt;
  const double d3f_drs_du2 = 0;
  out[27] = d3f_drs_du2;
  const double d3f_drs_dx_dt = _t129*_t80;
  out[28] = d3f_drs_dx_dt;
  const double d3f_drs_dx_du = 0;
  out[29] = d3f_drs_dx_du;
  const double d3f_drs_dx2 = _t130*_t80;
  out[30] = d3f_drs_dx2;
  const double d3f_drs2_dt = _t189*_t78;
  out[31] = d3f_drs2_dt;
  const double d3f_drs2_du = 0;
  out[32] = d3f_drs2_du;
  const double d3f_drs2_dx = _t189*_t79;
  out[33] = d3f_drs2_dx;
  const double d3f_drs3 = _t131*_t190;
  out[34] = d3f_drs3;
#endif
#if _KMAX >= 4
  const double d4f_dt4 = _hc2[0]*(_hc0[0]*(_t142*_t207 + _t147*_t212 + _t191*params->a[1] + _t191*params->a[4] + _t192*params->a[2] + _t192*params->a[3] + _t193*_t82 - _t195*params->a[2] - _t196*params->a[3] - _t197*params->a[4] - _t198*params->a[5] + _t199*_t234 + _t200*params->a[10] + _t202*params->a[10] + _t204*params->a[10] + _t206*params->a[10] + _t208*params->a[11] + _t209*params->a[11] + _t210*_t95 + _t211*params->a[11] + _t214*params->a[2] + _t215*params->a[3] + _t217*params->a[3] + _t218*params->a[4] + _t219*params->a[4] + _t221*params->a[4] + _t222*params->a[5] + _t223*params->a[5] + _t224*params->a[5] + _t225*_t234 + _t226*params->a[5] + _t227*params->a[6] + _t228*params->a[6] + _t229*params->a[6] + _t230*params->a[6] + _t231*params->a[6] + _t232*params->a[7] + _t233*params->a[7] + _t235*params->a[7] + _t236*params->a[8] + _t237*params->a[8] + _t238*params->a[8] + _t239*params->a[8] + _t240*params->a[8] + _t241*params->a[9] + _t242*params->a[9] + _t243*params->a[9] + _t244*params->a[9] + _t245*params->a[9]) + _hc1[0]*(_t125*_t193 + _t126*_t210 + _t182*_t207 + _t183*_t212 + _t191*params->b[1] + _t191*params->b[4] + _t192*params->b[2] + _t192*params->b[3] - _t195*params->b[2] - _t196*params->b[3] - _t197*params->b[4] - _t198*params->b[5] + _t199*_t246 + _t200*params->b[10] + _t202*params->b[10] + _t204*params->b[10] + _t206*params->b[10] + _t208*params->b[11] + _t209*params->b[11] + _t211*params->b[11] + _t214*params->b[2] + _t215*params->b[3] + _t217*params->b[3] + _t218*params->b[4] + _t219*params->b[4] + _t221*params->b[4] + _t222*params->b[5] + _t223*params->b[5] + _t224*params->b[5] + _t225*_t246 + _t226*params->b[5] + _t227*params->b[6] + _t228*params->b[6] + _t229*params->b[6] + _t230*params->b[6] + _t231*params->b[6] + _t232*params->b[7] + _t233*params->b[7] + _t235*params->b[7] + _t236*params->b[8] + _t237*params->b[8] + _t238*params->b[8] + _t239*params->b[8] + _t240*params->b[8] + _t241*params->b[9] + _t242*params->b[9] + _t243*params->b[9] + _t244*params->b[9] + _t245*params->b[9]));
  out[35] = d4f_dt4;
  const double d4f_du_dt3 = 0;
  out[36] = d4f_du_dt3;
  const double d4f_du2_dt2 = 0;
  ...
  const double d4f_du4 = 0;
  out[39] = d4f_du4;
  const double d4f_dx_dt3 = _hc2[0]*(_hc0[1]*_t181 + _hc1[1]*_t184);
  out[40] = d4f_dx_dt3;
  const double d4f_dx_du_dt2 = 0;
  ...
  const double d4f_dx_du3 = 0;
  out[43] = d4f_dx_du3;
  const double d4f_dx2_dt2 = _hc2[0]*(_hc0[2]*_t124 + _hc1[2]*_t127);
  out[44] = d4f_dx2_dt2;
  const double d4f_dx2_du_dt = 0;
  out[45] = d4f_dx2_du_dt;
  const double d4f_dx2_du2 = 0;
  out[46] = d4f_dx2_du2;
  const double d4f_dx3_dt = _hc2[0]*(_hc0[3]*_t67 + _hc1[3]*_t77);
  out[47] = d4f_dx3_dt;
  const double d4f_dx3_du = 0;
  out[48] = d4f_dx3_du;
  const double d4f_dx4 = _hc2[0]*(_hc0[4]*_t40 + _hc1[4]*_t42);
  out[49] = d4f_dx4;
  const double d4f_drs_dt3 = _t185*_t80;
  out[50] = d4f_drs_dt3;
  const double d4f_drs_du_dt2 = 0;
  ...
  const double d4f_drs_du3 = 0;
  out[53] = d4f_drs_du3;
  const double d4f_drs_dx_dt2 = _t186*_t80;
  out[54] = d4f_drs_dx_dt2;
  const double d4f_drs_dx_du_dt = 0;
  out[55] = d4f_drs_dx_du_dt;
  const double d4f_drs_dx_du2 = 0;
  out[56] = d4f_drs_dx_du2;
  const double d4f_drs_dx2_dt = _t187*_t80;
  out[57] = d4f_drs_dx2_dt;
  const double d4f_drs_dx2_du = 0;
  out[58] = d4f_drs_dx2_du;
  const double d4f_drs_dx3 = _t188*_t80;
  out[59] = d4f_drs_dx3;
  const double d4f_drs2_dt2 = _t128*_t189;
  out[60] = d4f_drs2_dt2;
  const double d4f_drs2_du_dt = 0;
  out[61] = d4f_drs2_du_dt;
  const double d4f_drs2_du2 = 0;
  out[62] = d4f_drs2_du2;
  const double d4f_drs2_dx_dt = _t129*_t189;
  out[63] = d4f_drs2_dx_dt;
  const double d4f_drs2_dx_du = 0;
  out[64] = d4f_drs2_dx_du;
  const double d4f_drs2_dx2 = _t130*_t189;
  out[65] = d4f_drs2_dx2;
  const double d4f_drs3_dt = _t247*_t78;
  out[66] = d4f_drs3_dt;
  const double d4f_drs3_du = 0;
  out[67] = d4f_drs3_du;
  const double d4f_drs3_dx = _t247*_t79;
  out[68] = d4f_drs3_dx;
  const double d4f_drs4 = (0.1e1 / 0.108e3)*_hc2[4]*_t1*_t43*((p->cam_omega) * (p->cam_omega) * (p->cam_omega) * (p->cam_omega))/_t4;
  out[69] = d4f_drs4;
#endif
}
...
FUNC_NAME(const xc_func_type *p, size_t ip, const double *rho, const double *sigma, const double *lapl, const double *tau, xc_mgga_out_params *out)
{
  ...
  if(!(_t12)) XC_CAT(m11_f_z0_k, _KMAX)(p, _t5, _t6*_t9, 0, _t11*ta, _hc1);
  ...
  if(!(_t16)) XC_CAT(m11_f_k, _KMAX)(p, _t9, _hc0[0], _t10*_t11, 0, _t12*ta, _hc2);
  double _hc3[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  ...
  if(!(_t17)) XC_CAT(m11_f_k, _KMAX)(p, _t9, _hc3[0], _t13*_t14, 0, _t15*tb, _hc5);
