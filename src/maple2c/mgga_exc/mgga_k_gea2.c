/*
  Generated from python/mgga_exc/mgga_k_gea2.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py mgga_k_gea2
*/

#ifndef _MGGA_K_GEA2_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _MGGA_K_GEA2_KERNEL_BODY
#define _KMAX 0
#define _MGGA_K_GEA2_HELPER_BODIES
#include "mgga_k_gea2.c"
#undef _MGGA_K_GEA2_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _MGGA_K_GEA2_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_MGGA_K_GEA2_HELPER_BODIES)

/* ---- helpers (one graded body -> <name>_k<order>) ---- */

GPU_DEVICE_FUNCTION static inline void XC_CAT(gea2_f0_k, _KMAX)(const xc_func_type *p, double p_, double q, double *out) {


  const double f = (0.5e1 / 0.27e2)*p_ + (0.20e2 / 0.9e1)*q + 0.1e1;
  out[0] = f;
#if _KMAX >= 1
  const double df_dq = 0.20e2 / 0.9e1;
  out[1] = df_dq;
  const double df_dp_ = 0.5e1 / 0.27e2;
  out[2] = df_dp_;
#endif
#if _KMAX >= 2
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(gea2_f_k, _KMAX)(const xc_func_type *p, double p_, double u, double *out) {

  const double _t1 = (0.1e1 / 0.24e2)*xc_powr(0.6e1, 1, 3)/(M_PI * M_CBRTPI);
  double _hc0[(_KMAX) >= 4 ? 15 : (_KMAX) >= 3 ? 10 : (_KMAX) >= 2 ? 6 : (_KMAX) >= 1 ? 3 : 1];
  XC_CAT(gea2_f0_k, _KMAX)(p, p_, _t1*u, _hc0);
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(xchan_k, _KMAX)(const xc_func_type *p, double n_s, double sigma_ss, double lapl_s, double tau_s, double *out) {
  ...
  XC_CAT(gea2_f_k, _KMAX)(p, (0.1e1 / 0.24e2)*_t1*_t6, lapl_s/xc_powr(n_s, 5, 3), _hc1);
...
FUNC_NAME(const xc_func_type *p, size_t ip, const double *rho, const double *sigma, const double *lapl, const double *tau, xc_mgga_out_params *out)
{
  ...
  const double d3F_dgaa2_dla = (0.1e1 / 0.16e2)*_hc1[23];
  const double d3F_dgaa_dla2 = (0.1e1 / 0.8e1)*_hc1[21];
  ...
  const double d4F_dgaa3_dla = (0.1e1 / 0.64e2)*_hc1[48];
  const double d4F_dgaa2_dla2 = (0.1e1 / 0.32e2)*_hc1[46];
  const double d4F_dgaa_dla3 = (0.1e1 / 0.16e2)*_hc1[43];
  ...
  const double d3F_dgaa2_dla = _hc2[23];
  const double d3F_dgbb2_dlb = _hc3[23];
  const double d3F_dgaa_dla2 = _hc2[21];
  const double d3F_dgbb_dlb2 = _hc3[21];
  ...
  const double d4F_dgaa3_dla = _hc2[48];
  const double d4F_dgbb3_dlb = _hc3[48];
  const double d4F_dgaa2_dla2 = _hc2[46];
  const double d4F_dgbb2_dlb2 = _hc3[46];
  const double d4F_dgaa_dla3 = _hc2[43];
  const double d4F_dgbb_dlb3 = _hc3[43];
