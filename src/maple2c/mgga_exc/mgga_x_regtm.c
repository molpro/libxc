/*
  Generated from python/mgga_exc/mgga_x_regtm.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py mgga_x_regtm
*/

#ifndef _MGGA_X_REGTM_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _MGGA_X_REGTM_KERNEL_BODY
#define _KMAX 0
#define _MGGA_X_REGTM_HELPER_BODIES
#include "mgga_x_regtm.c"
#undef _MGGA_X_REGTM_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _MGGA_X_REGTM_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_MGGA_X_REGTM_HELPER_BODIES)
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(regtm_f_k, _KMAX)(const xc_func_type *p, double a, double p_, double *out) {
  ...
  const double _t6 = exp(-0.3e1*p_);
  const double _t7 = _t5*_t6;
  const double _t8 = _t2*_t7;
#if _KMAX >= 1
  const double _t9 = (_t1 * _t1);
  const double _t10 = _t7*_t9;
  const double _t11 = xc_powr(_t4, -5, 2);
  const double _t12 = _t11*_t6;
  const double _t13 = _t12*_t2;
  const double _t14 = 6.5268750000000004*_t13;
#endif
#if _KMAX >= 2
  const double _t15 = 0.2e1*a - 0.2e1;
  const double _t16 = _t15*_t7;
  const double _t17 = _t12*_t9;
  const double _t18 = _t17*a;
  const double _t19 = xc_powr(_t4, -7, 2);
  const double _t20 = _t2*_t6;
  const double _t21 = _t19*_t20;
  const double _t22 = _t21*_t3;
#endif
#if _KMAX >= 3
  const double _t23 = 58.741875000000007*_t13;
  const double _t24 = 213.00048632812502*_t21;
  const double _t25 = _t6*_t9;
  const double _t26 = _t19*_t3;
  const double _t27 = _t25*_t26;
  const double _t28 = xc_powr(_t4, -9, 2);
  const double _t29 = _t28*(a * a * a);
  const double _t30 = _t20*_t29;
#endif
#if _KMAX >= 4
  const double _t31 = _t12*_t15;
#endif

  const double f = _t8;
  out[0] = f;
#if _KMAX >= 1
  const double df_dp_ = -0.3e1*_t8;
  out[1] = df_dp_;
  const double df_da = -0.3e1*_t10 - _t14*a;
  out[2] = df_da;
#endif
#if _KMAX >= 2
  const double d2f_dp_2 = 0.9e1*_t8;
  out[3] = d2f_dp_2;
  const double d2f_da_dp_ = 0.9e1*_t10 + 19.580625000000001*_t13*a;
  out[4] = d2f_da_dp_;
  const double d2f_da2 = -_t14 - 0.3e1*_t16 + 39.161250000000003*_t18 + 71.000162109375012*_t22;
  out[5] = d2f_da2;
#endif
#if _KMAX >= 3
  const double d3f_dp_3 = -0.27e2*_t8;
  out[6] = d3f_dp_3;
  const double d3f_da_dp_2 = -0.27e2*_t10 - _t23*a;
  out[7] = d3f_da_dp_2;
  const double d3f_da2_dp_ = 19.580625000000001*_t11*_t2*_t6 + 0.9e1*_t15*_t5*_t6 - 117.48375000000001*_t18 - _t24*_t3;
  out[8] = d3f_da2_dp_;
  const double d3f_da3 = 58.741875000000007*_t11*_t15*_t6*a + 58.741875000000007*_t11*_t6*_t9 + 213.00048632812502*_t19*_t2*_t6*a - 639.00145898437506*_t27 - 1081.2880938244632*_t30 - 0.6e1*_t7;
  out[9] = d3f_da3;
#endif
#if _KMAX >= 4
  const double d4f_dp_4 = 0.81e2*_t8;
  out[10] = d4f_dp_4;
  const double d4f_da_dp_3 = 0.81e2*_t10 + 176.22562500000004*_t13*a;
  out[11] = d4f_da_dp_3;
  const double d4f_da2_dp_2 = -0.27e2*_t16 + 352.45125000000007*_t18 + 639.00145898437506*_t22 - _t23;
  out[12] = d4f_da2_dp_2;
  const double d4f_da3_dp_ = -176.22562500000004*_t17 - 639.00145898437506*_t21*a + 1917.0043769531253*_t27 + 3243.8642814733894*_t30 - 176.22562500000004*_t31*a + 0.18e2*_t7;
  out[13] = d4f_da3_dp_;
  const double d4f_da4 = 156.64500000000001*_t12*a - 1278.0029179687504*_t15*_t26*_t6 - 2556.0058359375007*_t19*_t25*a - 6487.7285629467788*_t20*_t28*_t3 + 21172.29668214163*_t20*(a * a * a * a)/xc_powr(_t4, 11, 2) + _t24 + 12975.457125893558*_t25*_t29 + 117.48375000000001*_t31;
  out[14] = d4f_da4;
#endif
}

GPU_DEVICE_FUNCTION static inline void XC_CAT(regtm_zp_k, _KMAX)(const xc_func_type *p, double a, double p_, double *out) {

  double _hc0[(_KMAX) >= 4 ? 15 : (_KMAX) >= 3 ? 10 : (_KMAX) >= 2 ? 6 : (_KMAX) >= 1 ? 3 : 1];
  XC_CAT(regtm_f_k, _KMAX)(p, a, p_, _hc0);
  const double _t1 = _hc0[0] + p_;
  const double _t2 = (0.1e1 / _t1);
  const double _t3 = (0.3e1 / 0.5e1)*_t2;
  const double _t4 = _t3*a + 0.1e1;
#if _KMAX >= 1
  const double _t5 = -_hc0[1] - 0.1e1;
  const double _t6 = (0.1e1 / (_t1 * _t1));
  const double _t7 = (0.3e1 / 0.5e1)*_t6;
  const double _t8 = _t5*_t7;
  const double _t9 = (0.1e1 / (_t4 * _t4));
  const double _t10 = _t9*a;
  const double _t11 = _t7*a;
  const double _t12 = _hc0[2]*_t11 - _t3;
#endif
#if _KMAX >= 2
  const double _t13 = _hc0[3]*_t7;
  const double _t14 = (0.1e1 / (_t1 * _t1 * _t1 * _t1));
  const double _t15 = (_t5 * _t5);
  const double _t16 = (0.1e1 / (_t4 * _t4 * _t4));
  const double _t17 = (a * a);
  const double _t18 = _t16*_t17;
  const double _t19 = _t15*_t18;
  const double _t20 = (0.18e2 / 0.25e2)*_t19;
  const double _t21 = (0.1e1 / (_t1 * _t1 * _t1));
  const double _t22 = -0.2e1*_hc0[1] - 0.2e1;
  const double _t23 = _t21*_t22;
  const double _t24 = _t23*_t5;
  const double _t25 = (0.3e1 / 0.5e1)*_t24;
  const double _t26 = (0.6e1 / 0.5e1)*_t6;
  const double _t27 = _t26*_t5;
  const double _t28 = _t12*_t16;
  const double _t29 = _t28*a;
  const double _t30 = _hc0[2]*a;
  const double _t31 = _t23*_t30;
  const double _t32 = _hc0[4]*_t11 + (0.3e1 / 0.5e1)*_t31 - _t8;
  const double _t33 = _hc0[2]*_t26;
  const double _t34 = ((_hc0[2]) * (_hc0[2]));
  const double _t35 = _t21*_t34;
  const double _t36 = _t35*a;
  const double _t37 = _hc0[5]*_t11 + _t33 - 0.6e1 / 0.5e1*_t36;
  const double _t38 = -0.6e1 / 0.5e1*_t2 + _t33*a;
  const double _t39 = _t16*_t38;
#endif
#if _KMAX >= 3
  const double _t40 = _hc0[6]*_t7;
  const double _t41 = (0.6e1 / 0.5e1)*_t23;
  const double _t42 = _hc0[3]*_t41;
  const double _t43 = _hc0[3]*_t21;
  const double _t44 = (0.6e1 / 0.5e1)*_t5;
  const double _t45 = _t43*_t44;
  const double _t46 = _t14*_t18;
  const double _t47 = (0.54e2 / 0.25e2)*_t46;
  const double _t48 = _hc0[3]*_t5;
  const double _t49 = xc_powi(_t1, -6);
  const double _t50 = (0.1e1 / (_t4 * _t4 * _t4 * _t4));
  const double _t51 = (a * a * a);
  const double _t52 = (_t5 * _t5 * _t5);
  const double _t53 = _t50*_t51*_t52;
  const double _t54 = xc_powi(_t1, -5);
  const double _t55 = _t19*_t54;
  const double _t56 = -0.3e1*_hc0[1] - 0.3e1;
  const double _t57 = _t14*_t56;
  const double _t58 = _t22*_t57;
  const double _t59 = (0.3e1 / 0.5e1)*_t58;
  const double _t60 = _t5*_t59;
  const double _t61 = _hc0[3]*_t26;
  const double _t62 = _hc0[2]*_t6;
  const double _t63 = _t62*a;
  const double _t64 = _t15*_t50;
  const double _t65 = _t14*_t17;
  const double _t66 = _t64*_t65;
  const double _t67 = (0.6e1 / 0.5e1)*_t24;
  const double _t68 = _t16*_t32;
  const double _t69 = (0.12e2 / 0.5e1)*_t6;
  const double _t70 = _t5*_t69*a;
  const double _t71 = (0.6e1 / 0.5e1)*_t43;
  const double _t72 = _hc0[4]*a;
  const double _t73 = _hc0[7]*_t11 + _t13 - _t25 + _t30*_t59 - _t30*_t71 + _t41*_t72;
  const double _t74 = _t16*_t37;
  const double _t75 = _t74*a;
  const double _t76 = _t12*_t50;
  const double _t77 = _t38*_t76;
  const double _t78 = (0.9e1 / 0.5e1)*_t6;
  const double _t79 = _t5*_t78;
  const double _t80 = _t79*a;
  const double _t81 = _hc0[4]*_t26;
  const double _t82 = _hc0[2]*_t41;
  const double _t83 = -_t27 + _t81*a + _t82*a;
  const double _t84 = (0.12e2 / 0.5e1)*_hc0[2];
  const double _t85 = _hc0[5]*_t23;
  const double _t86 = (0.3e1 / 0.5e1)*a;
  const double _t87 = _t34*_t57;
  const double _t88 = _t87*a;
  const double _t89 = _hc0[8]*_t11 - _t21*_t72*_t84 + _t81 + _t82 + _t85*_t86 - 0.6e1 / 0.5e1*_t88;
  const double _t90 = (0.12e2 / 0.5e1)*_t62;
  const double _t91 = _t26*a;
  const double _t92 = _hc0[5]*_t91 - 0.12e2 / 0.5e1*_t36 + _t90;
  const double _t93 = 0.2e1*_t39;
  const double _t94 = _hc0[5]*_t78;
  const double _t95 = (0.18e2 / 0.5e1)*_t35;
  const double _t96 = (0.18e2 / 0.5e1)*a;
  const double _t97 = _hc0[2]*_t21;
  const double _t98 = _hc0[5]*_t97;
  const double _t99 = ((_hc0[2]) * (_hc0[2]) * (_hc0[2]));
  const double _t100 = _t14*_t99;
  const double _t101 = _hc0[9]*_t11 + _t100*_t96 + _t94 - _t95 - _t96*_t98;
  const double _t102 = _t78*a;
  const double _t103 = _hc0[2]*_t102 - 0.9e1 / 0.5e1*_t2;
  const double _t104 = _t103*_t38;
#endif
#if _KMAX >= 4
  const double _t105 = ((_hc0[3]) * (_hc0[3]));
  const double _t106 = _t21*_t96;
  const double _t107 = (0.9e1 / 0.5e1)*_t23;
  const double _t108 = _t21*a;
  const double _t109 = _hc0[6]*_t108;
  const double _t110 = xc_powi(_t4, -5);
  const double _t111 = _t49*_t51;
  const double _t112 = (0.9e1 / 0.5e1)*_hc0[3];
  const double _t113 = _t10*_t48;
  const double _t114 = _t14*_t22;
  const double _t115 = _t53/xc_powi(_t1, 7);
  const double _t116 = -0.4e1*_hc0[1] - 0.4e1;
  const double _t117 = _t116*_t54;
  const double _t118 = _t18*_t48;
  const double _t119 = _t22*_t54;
  const double _t120 = _t20*_t49;
  const double _t121 = _t120*(-0.5e1*_hc0[1] - 0.5e1);
  const double _t122 = _t54*a;
  const double _t123 = _t116*_t122*_t56;
  const double _t124 = (0.3e1 / 0.5e1)*_t123*_t22;
  const double _t125 = (0.12e2 / 0.5e1)*_t23;
  const double _t126 = (0.54e2 / 0.25e2)*_t15*_t17*_t76;
  const double _t127 = _hc0[3]*_t96;
  const double _t128 = _t5*_t6*_t96;
  const double _t129 = (0.6e1 / 0.5e1)*_hc0[2];
  const double _t130 = _hc0[5]*_t6;
  const double _t131 = _t110*_t12;
  const double _t132 = _t128*_t38*_t50;
  const double _t133 = 0.2e1*_t83;
  const double _t134 = _hc0[7]*_t26;
  const double _t135 = _t43*_t84;
  const double _t136 = _hc0[5]*a;
  const double _t137 = _t14*_t34;
  const double _t138 = (0.24e2 / 0.5e1)*_t72;
  const double _t139 = _hc0[2]*_t57;
  const double _t140 = _t104*_t131;
  const double _t141 = _t103*_t76;
  const double _t142 = _t104*_t50;
  const double _t143 = _hc0[2]*_t108;
#endif

  const double f = (0.1e1 / _t4);
  out[0] = f;
#if _KMAX >= 1
  const double df_dp_ = -_t10*_t8;
  out[1] = df_dp_;
  const double df_da = _t12*_t9;
  out[2] = df_da;
#endif
#if _KMAX >= 2
  const double d2f_dp_2 = _t10*_t13 - _t10*_t25 + _t14*_t20;
  out[3] = d2f_dp_2;
  const double d2f_da_dp_ = -_t27*_t29 + _t32*_t9;
  out[4] = d2f_da_dp_;
  const double d2f_da2 = _t12*_t39 + _t37*_t9;
  out[5] = d2f_da2;
#endif
#if _KMAX >= 3
  const double d3f_dp_3 = _t10*_t40 + _t10*_t42 + _t10*_t45 - _t10*_t60 - _t47*_t48 - 0.162e3 / 0.125e3*_t49*_t53 + _t55*(-0.72e2 / 0.25e2*_hc0[1] - 0.72e2 / 0.25e2) + _t55*(-0.36e2 / 0.25e2*_hc0[1] - 0.36e2 / 0.25e2);
  out[6] = d3f_dp_3;
  const double d3f_da_dp_2 = _t29*_t61 - _t29*_t67 + _t66*(-0.162e3 / 0.125e3*_t2 + (0.162e3 / 0.125e3)*_t63) - _t68*_t70 + _t73*_t9;
  out[7] = d3f_da_dp_2;
  const double d3f_da2_dp_ = -_t27*_t75 + _t28*_t83 + _t32*_t39 - _t77*_t80 + _t89*_t9;
  out[8] = d3f_da2_dp_;
  const double d3f_da3 = _t101*_t9 + _t104*_t76 + _t28*_t92 + _t37*_t93;
  out[9] = d3f_da3;
#endif
#if _KMAX >= 4
  const double d4f_dp_4 = _hc0[10]*_t11*_t9 + (0.972e3 / 0.125e3)*_hc0[3]*_t111*_t64 - 0.144e3 / 0.25e2*_hc0[3]*_t55 + _hc0[6]*_t10*_t107 - 0.72e2 / 0.25e2*_hc0[6]*_t46*_t5 + _t10*_t112*_t58 - _t105*_t106*_t9 + _t105*_t47 + _t109*_t44*_t9 + (0.9e1 / 0.5e1)*_t113*_t114 + (0.12e2 / 0.5e1)*_t113*_t57 + _t115*((0.324e3 / 0.125e3)*_hc0[1] + 0.324e3 / 0.125e3) + _t115*((0.648e3 / 0.125e3)*_hc0[1] + 0.648e3 / 0.125e3) + _t115*((0.972e3 / 0.125e3)*_hc0[1] + 0.972e3 / 0.125e3) + _t116*_t121 - 0.18e2 / 0.5e1*_t117*_t118 - 0.72e2 / 0.25e2*_t118*_t119 + _t120*_t22*_t56 + _t121*_t22 - _t124*_t5*_t9 + (0.1944e4 / 0.625e3)*_t110*(_t5 * _t5 * _t5 * _t5)*(a * a * a * a)/xc_powi(_t1, 8);
  out[10] = d4f_dp_4;
  const double d4f_da_dp_3 = _hc0[3]*_t125*_t29 + _hc0[6]*_t28*_t91 + _t110*_t111*_t52*((0.1944e4 / 0.625e3)*_t2 - 0.1944e4 / 0.625e3*_t63) + _t117*_t126 + _t119*_t126 + _t127*_t6*_t68 - _t128*_t16*_t73 - _t24*_t68*_t96 + (0.12e2 / 0.5e1)*_t29*_t43*_t5 - _t29*_t44*_t58 - 0.162e3 / 0.25e2*_t48*_t65*_t76 + _t66*((0.486e3 / 0.125e3)*_t31 + (0.486e3 / 0.125e3)*_t6*_t72 + (0.162e3 / 0.25e2)*_t6*((0.3e1 / 0.5e1)*_hc0[1] + 0.3e1 / 0.5e1)) + _t9*(_hc0[11]*_t11 + _hc0[2]*_t124 - _hc0[3]*_t57*_t84*a - _hc0[4]*_t43*_t96 + _hc0[7]*_t107*a - _t109*_t129 - _t112*_t114*_t30 + _t40 + _t42 + _t45 + (0.9e1 / 0.5e1)*_t58*_t72 - _t60);
  out[11] = d4f_da_dp_3;
  const double d4f_da2_dp_2 = _hc0[3]*_t102*_t77 - _t128*_t76*_t83 + (0.108e3 / 0.25e2)*_t131*_t15*_t38*_t65 - _t132*_t32 + _t133*_t68 - _t16*_t70*_t89 - 0.9e1 / 0.5e1*_t24*_t77*a + _t28*(_t125*_t72 + _t134*a - _t135*a + (0.6e1 / 0.5e1)*_t30*_t58 + _t61 - _t67) + _t39*_t73 + _t61*_t75 + _t66*((0.162e3 / 0.125e3)*_t130*a - 0.324e3 / 0.125e3*_t36 + (0.324e3 / 0.125e3)*_t62) - _t67*_t75 + _t9*(_hc0[12]*_t11 - 0.12e2 / 0.5e1*((_hc0[4]) * (_hc0[4]))*_t108 + _hc0[4]*_t125 + _hc0[5]*_t58*_t86 - _hc0[7]*_t108*_t84 + _hc0[8]*_t41*a - 0.6e1 / 0.5e1*_t123*_t34 + _t127*_t137 + _t129*_t58 + _t134 - _t135 - _t136*_t71 - _t138*_t139);
  out[12] = d4f_da2_dp_2;
  const double d4f_da3_dp_ = -_t101*_t16*_t27*a - _t132*_t37 + _t133*_t74 - _t140*_t70 + _t141*_t83 + _t142*_t32 + _t28*(_hc0[4]*_t69 + _hc0[8]*_t91 + _t136*_t41 - _t138*_t97 + _t23*_t84 - 0.12e2 / 0.5e1*_t88) + _t68*_t92 - _t76*_t80*_t92 + _t77*(_hc0[4]*_t102 + (0.9e1 / 0.5e1)*_t31 - _t79) + _t89*_t93 + _t9*(_hc0[13]*_t11 - _hc0[4]*_hc0[5]*_t106 - 0.36e2 / 0.5e1*_hc0[4]*_t97 - _hc0[5]*_t139*_t96 + _hc0[8]*_t78 - _hc0[8]*_t96*_t97 + _hc0[9]*_t23*_t86 + _t117*_t96*_t99 + (0.54e2 / 0.5e1)*_t137*_t72 + (0.9e1 / 0.5e1)*_t85 - 0.18e2 / 0.5e1*_t87);
  out[13] = d4f_da3_dp_;
  const double d4f_da4 = 0.3e1*_t101*_t39 + _t140*(-0.12e2 / 0.5e1*_t2 + _t90*a) + 0.2e1*_t141*_t92 + 0.3e1*_t142*_t37 + _t28*(-0.36e2 / 0.5e1*_hc0[5]*_t143 + _hc0[9]*_t91 + (0.36e2 / 0.5e1)*_t100*a + (0.18e2 / 0.5e1)*_t130 - 0.36e2 / 0.5e1*_t35) + 0.3e1*_t74*_t92 + _t77*((0.18e2 / 0.5e1)*_t62 + _t94*a - _t95*a) + _t9*(_hc0[14]*_t11 - 0.72e2 / 0.5e1*((_hc0[2]) * (_hc0[2]) * (_hc0[2]) * (_hc0[2]))*_t122 - ((_hc0[5]) * (_hc0[5]))*_t106 - 0.24e2 / 0.5e1*_hc0[9]*_t143 + _hc0[9]*_t69 + (0.72e2 / 0.5e1)*_t100 + (0.108e3 / 0.5e1)*_t136*_t137 - 0.72e2 / 0.5e1*_t98);
  out[14] = d4f_da4;
#endif
}
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(tm_w_pf_k, _KMAX)(const xc_func_type *p, double x2, double t, double *out) {
  ...
  XC_CAT(regtm_zp_k, _KMAX)(p, _hc0[0], _hc1[0], _hc2);
