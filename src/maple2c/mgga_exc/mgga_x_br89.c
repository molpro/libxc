/*
  Generated from python/mgga_exc/mgga_x_br89.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py mgga_x_br89
*/

#ifndef _MGGA_X_BR89_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _MGGA_X_BR89_KERNEL_BODY
#define _KMAX 0
#define _MGGA_X_BR89_HELPER_BODIES
#include "mgga_x_br89.c"
#undef _MGGA_X_BR89_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _MGGA_X_BR89_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_MGGA_X_BR89_HELPER_BODIES)
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(br89_f_pf_k, _KMAX)(const xc_func_type *p, double x2, double u, double t, double *out) {
  ...
  const double _t12 = params->at*(-_t11*_t9 + _t3*_t6 + _t7*_t8) + 0.1e1;
  const double _t13 = (0.1e1 / 0.2e1)*_hc3[0];
#if _KMAX >= 1
  const double _t14 = (0.1e1 / 0.2e1)*_t12;
  const double _t15 = _hc0[1]*_hc3[1];
  const double _t16 = _hc1[1]*_hc2[1]*_t15;
  const double _t17 = (0.1e1 / (_t5 * _t5));
  const double _t18 = (_t3 * _t3);
  const double _t19 = (0.1e1 / (_t5 * _t5 * _t5 * _t5));
  const double _t20 = (_t3 * _t3 * _t3 * _t3);
  const double _t21 = xc_powi(_t5, -6);
  const double _t22 = 0.6e1*_t10*_t18 - _t17*_t3 + 0.6e1*_t19*_t9 - 0.5e1*_t20*_t7 - 0.5e1*_t21*_t8 - _t6;
  const double _t23 = _t13*params->at;
  const double _t24 = _hc0[2]*_hc2[1];
  const double _t25 = _hc3[1]*_t14;
  const double _t26 = _hc1[1]*_t25;
  const double _t27 = _hc0[3]*_hc2[1];
#endif
#if _KMAX >= 2
  const double _t28 = ((_hc0[1]) * (_hc0[1]));
  const double _t29 = ((_hc1[1]) * (_hc1[1]));
  const double _t30 = _t28*_t29;
  const double _t31 = _hc2[2]*_t25;
  const double _t32 = ((_hc2[1]) * (_hc2[1]));
  const double _t33 = _hc3[2]*_t32;
  const double _t34 = _t14*_t33;
  const double _t35 = _t22*params->at;
  const double _t36 = -0.3e1 / 0.5e1*_t4 + 0.2e1*t;
  const double _t37 = xc_powi(_t5, -7);
  const double _t38 = 0.6e1*_t10*_t36 + _t11*_t3 + 0.2e1*_t17 - 0.36e2*_t18*_t19 + 0.50e2*_t20*_t21 + 0.30e2*_t37*_t8 - 0.4e1*_t7*_t9;
  const double _t39 = _t14*_t15;
  const double _t40 = _hc0[2]*_t29;
  const double _t41 = _hc2[2]*_t40;
  const double _t42 = _hc0[1]*_t34;
  const double _t43 = _hc3[1]*_t35;
  const double _t44 = (0.1e1 / 0.2e1)*_t43;
  const double _t45 = _hc1[1]*_t44;
  const double _t46 = ((_hc0[2]) * (_hc0[2]));
  const double _t47 = _t29*_t46;
  const double _t48 = _hc0[3]*_t29;
  const double _t49 = _hc2[2]*_t48;
  const double _t50 = _hc0[3]*_t40;
  const double _t51 = ((_hc0[3]) * (_hc0[3]));
  const double _t52 = _t29*_t51;
#endif
#if _KMAX >= 3
  const double _t53 = ((_hc0[1]) * (_hc0[1]) * (_hc0[1]));
  const double _t54 = ((_hc1[1]) * (_hc1[1]) * (_hc1[1]));
  const double _t55 = _t53*_t54;
  const double _t56 = _hc2[3]*_t25;
  const double _t57 = (0.3e1 / 0.2e1)*_hc3[2];
  const double _t58 = _t12*_t57;
  const double _t59 = _hc2[2]*_t58;
  const double _t60 = _hc2[1]*_t59;
  const double _t61 = ((_hc2[1]) * (_hc2[1]) * (_hc2[1]))*_hc3[3];
  const double _t62 = _t14*_t61;
  const double _t63 = (0.3e1 / 0.2e1)*_t30;
  const double _t64 = _t33*_t35;
  const double _t65 = xc_powi(_t5, -8);
  const double _t66 = 0.6e1*_t10 + 0.156e3*_t18*_t7 - 0.6e1*_t19*_t3 - 0.54e2*_t19*_t36 - 0.450e3*_t20*_t37 - 0.180e3*_t21*_t9 - 0.210e3*_t65*_t8;
  const double _t67 = _t38*params->at;
  const double _t68 = (0.3e1 / 0.2e1)*_t67;
  const double _t69 = _t28*_t54;
  const double _t70 = _hc0[2]*_t69;
  const double _t71 = _t59*_t69;
  const double _t72 = _t15*_t35;
  const double _t73 = _hc0[1]*_t64;
  const double _t74 = _hc3[1]*_t67;
  const double _t75 = (0.1e1 / 0.2e1)*_t74;
  const double _t76 = _hc1[1]*_t75;
  const double _t77 = _t46*_t54;
  const double _t78 = _hc2[3]*_t39;
  const double _t79 = _hc0[1]*_t60;
  const double _t80 = _t62*_t77;
  const double _t81 = _hc2[2]*_t44;
  const double _t82 = (0.1e1 / 0.2e1)*_t64;
  const double _t83 = ((_hc0[2]) * (_hc0[2]) * (_hc0[2]));
  const double _t84 = _t54*_t83;
  const double _t85 = _hc0[3]*_t69;
  const double _t86 = _hc0[2]*_hc0[3];
  const double _t87 = _t54*_t86;
  const double _t88 = _t24*_t59;
  const double _t89 = _hc0[1]*_hc0[3]*_t54;
  const double _t90 = _hc0[3]*_t41;
  const double _t91 = _hc0[3]*_t77;
  const double _t92 = _t27*_t77;
  const double _t93 = _t51*_t54;
  const double _t94 = _t62*_t93;
  const double _t95 = _hc0[2]*_t93;
  const double _t96 = ((_hc0[3]) * (_hc0[3]) * (_hc0[3]));
  const double _t97 = _t54*_t96;
#endif
#if _KMAX >= 4
  const double _t98 = ((_hc1[1]) * (_hc1[1]) * (_hc1[1]) * (_hc1[1]));
  const double _t99 = ((_hc0[1]) * (_hc0[1]) * (_hc0[1]) * (_hc0[1]))*_t98;
  const double _t100 = _hc2[4]*_t25;
  const double _t101 = _t12*_t99;
  const double _t102 = 0.2e1*_hc2[3];
  const double _t103 = _hc2[1]*_hc3[2];
  const double _t104 = _t102*_t103;
  const double _t105 = ((_hc2[1]) * (_hc2[1]) * (_hc2[1]) * (_hc2[1]))*_hc3[4]*_t14;
  const double _t106 = ((_hc2[2]) * (_hc2[2]))*_t58;
  const double _t107 = 0.3e1*_hc2[2];
  const double _t108 = _hc3[3]*_t107*_t32;
  const double _t109 = _hc2[2]*_t35;
  const double _t110 = _t35*_t61;
  const double _t111 = _t66*params->at;
  const double _t112 = _t33*_t67;
  const double _t113 = _hc0[2]*_t53;
  const double _t114 = _t113*_t98;
  const double _t115 = _hc3[2]*_t24;
  const double _t116 = _t12*_t98;
  const double _t117 = _t102*_t116;
  const double _t118 = _t117*_t53;
  const double _t119 = _t106*_t98;
  const double _t120 = _t108*_t12;
  const double _t121 = (0.3e1 / 0.2e1)*_t70;
  const double _t122 = _hc2[3]*_t43;
  const double _t123 = (0.9e1 / 0.2e1)*_t109*_t69;
  const double _t124 = (0.1e1 / 0.2e1)*_hc1[1]*_hc3[1]*_t111;
  const double _t125 = _t15*_t68;
  const double _t126 = _hc0[1]*_t33*_t68;
  const double _t127 = _t28*_t46;
  const double _t128 = _t127*_t98;
  const double _t129 = _hc2[3]*_t72;
  const double _t130 = _hc0[1]*_t110;
  const double _t131 = _t107*_t35;
  const double _t132 = _hc0[1]*_t103*_t131;
  const double _t133 = _hc2[2]*_t75;
  const double _t134 = (0.1e1 / 0.2e1)*_t112;
  const double _t135 = _t83*_t98;
  const double _t136 = _hc2[4]*_t39;
  const double _t137 = _hc0[1]*_t83;
  const double _t138 = _t104*_t116;
  const double _t139 = _t105*_t98;
  const double _t140 = _t120*_t98;
  const double _t141 = _hc2[3]*_t44;
  const double _t142 = _t109*_t57;
  const double _t143 = _hc2[1]*_t142;
  const double _t144 = (0.1e1 / 0.2e1)*_t110;
  const double _t145 = ((_hc0[2]) * (_hc0[2]) * (_hc0[2]) * (_hc0[2]))*_t98;
  const double _t146 = _t12*_t145;
  const double _t147 = _hc0[3]*_t53;
  const double _t148 = _t100*_t98;
  const double _t149 = _hc3[2]*_t27;
  const double _t150 = (0.3e1 / 0.2e1)*_t85;
  const double _t151 = _t28*_t86;
  const double _t152 = _t115*_t117;
  const double _t153 = _hc0[3]*_t119;
  const double _t154 = _t136*_t98;
  const double _t155 = _hc0[1]*_t46;
  const double _t156 = _hc0[3]*_t155;
  const double _t157 = _hc0[3]*_t135;
  const double _t158 = _t28*_t51;
  const double _t159 = _hc0[2]*_t51;
  const double _t160 = _hc0[1]*_t159;
  const double _t161 = _t46*_t51;
  const double _t162 = _hc0[1]*_t96;
  const double _t163 = _hc0[2]*_t96;
  const double _t164 = ((_hc0[3]) * (_hc0[3]) * (_hc0[3]) * (_hc0[3]))*_t98;
  const double _t165 = _t12*_t164;
#endif

  const double f = -_t12*_t13;
  out[0] = f;
#if _KMAX >= 1
  const double df_dt = -_t14*_t16 - _t22*_t23;
  out[1] = df_dt;
  const double df_du = -_t24*_t26;
  out[2] = df_du;
  const double df_dx2 = -_t26*_t27;
  out[3] = df_dx2;
#endif
#if _KMAX >= 2
  const double d2f_dt2 = -_t16*_t35 - _t23*_t38 - _t30*_t31 - _t30*_t34;
  out[4] = d2f_dt2;
  const double d2f_du_dt = -_t24*_t45 - _t39*_t41 - _t40*_t42;
  out[5] = d2f_du_dt;
  const double d2f_du2 = -_t31*_t47 - _t34*_t47;
  out[6] = d2f_du2;
  const double d2f_dx2_dt = -_t27*_t45 - _t39*_t49 - _t42*_t48;
  out[7] = d2f_dx2_dt;
  const double d2f_dx2_du = -_t31*_t50 - _t34*_t50;
  out[8] = d2f_dx2_du;
  const double d2f_dx22 = -_t31*_t52 - _t34*_t52;
  out[9] = d2f_dx22;
#endif
#if _KMAX >= 3
  const double d3f_dt3 = -_hc2[2]*_t43*_t63 - _t16*_t68 - _t23*_t66 - _t55*_t56 - _t55*_t60 - _t55*_t62 - _t63*_t64;
  out[10] = d3f_dt3;
  const double d3f_du_dt2 = -_t24*_t71 - _t24*_t76 - _t40*_t73 - _t41*_t72 - _t56*_t70 - _t62*_t70;
  out[11] = d3f_du_dt2;
  const double d3f_du2_dt = -_hc0[1]*_t80 - _t47*_t81 - _t47*_t82 - _t77*_t78 - _t77*_t79;
  out[12] = d3f_du2_dt;
  const double d3f_du3 = -_t56*_t84 - _t60*_t84 - _t62*_t84;
  out[13] = d3f_du3;
  const double d3f_dx2_dt2 = -_t27*_t71 - _t27*_t76 - _t48*_t73 - _t49*_t72 - _t56*_t85 - _t62*_t85;
  out[14] = d3f_dx2_dt2;
  const double d3f_dx2_du_dt = -_hc0[1]*_t62*_t87 - _t44*_t90 - _t50*_t82 - _t78*_t87 - _t88*_t89;
  out[15] = d3f_dx2_du_dt;
  const double d3f_dx2_du2 = -_hc0[3]*_t80 - _t56*_t91 - _t59*_t92;
  out[16] = d3f_dx2_du2;
  const double d3f_dx22_dt = -_hc0[1]*_t94 - _t52*_t81 - _t52*_t82 - _t78*_t93 - _t79*_t93;
  out[17] = d3f_dx22_dt;
  const double d3f_dx22_du = -_hc0[2]*_t94 - _t56*_t95 - _t88*_t93;
  out[18] = d3f_dx22_du;
  const double d3f_dx23 = -_t56*_t97 - _t60*_t97 - _t62*_t97;
  out[19] = d3f_dx23;
#endif
#if _KMAX >= 4
  const double d4f_dt4 = -_t100*_t99 - _t101*_t104 - _t101*_t108 - _t102*_t43*_t55 - 0.6e1*_t103*_t109*_t55 - _t105*_t99 - _t106*_t99 - _t107*_t30*_t74 - 0.2e1*_t110*_t55 - 0.2e1*_t111*_t16 - 0.3e1*_t112*_t30 - _t23*(-0.240e3*_t18*_t21 - 0.120e3*_t19 + 0.4200e4*_t20*_t65 + 0.24e2*_t3*_t7 + 0.372e3*_t36*_t7 + 0.2880e4*_t37*_t9 + 0.1680e4*_t8/xc_powi(_t5, 9));
  out[20] = d4f_dt4;
  const double d4f_du_dt3 = -_t100*_t114 - _t105*_t114 - _t110*_t121 - _t113*_t119 - _t114*_t120 - _t115*_t118 - _t115*_t123 - _t121*_t122 - _t124*_t24 - _t125*_t41 - _t126*_t40;
  out[21] = d4f_du_dt3;
  const double d4f_du2_dt2 = -_t100*_t128 - _t104*_t12*_t128 - _t105*_t128 - _t119*_t127 - _t120*_t128 - _t129*_t77 - _t130*_t77 - _t132*_t77 - _t133*_t47 - _t134*_t47;
  out[22] = d4f_du2_dt2;
  const double d4f_du3_dt = -_t119*_t137 - _t135*_t136 - _t137*_t138 - _t137*_t139 - _t137*_t140 - _t141*_t84 - _t143*_t84 - _t144*_t84;
  out[23] = d4f_du3_dt;
  const double d4f_du4 = -_t100*_t145 - _t104*_t146 - _t105*_t145 - _t106*_t145 - _t108*_t146;
  out[24] = d4f_du4;
  const double d4f_dx2_dt3 = -_t110*_t150 - _t118*_t149 - _t119*_t147 - _t122*_t150 - _t123*_t149 - _t124*_t27 - _t125*_t49 - _t126*_t48 - _t139*_t147 - _t140*_t147 - _t147*_t148;
  out[25] = d4f_dx2_dt3;
  const double d4f_dx2_du_dt2 = -_hc0[2]*_t153*_t28 - _hc0[3]*_t152*_t28 - _t115*_t131*_t89 - _t129*_t87 - _t130*_t87 - _t134*_t50 - _t139*_t151 - _t140*_t151 - _t148*_t151 - _t75*_t90;
  out[26] = d4f_dx2_du_dt2;
  const double d4f_dx2_du2_dt = -_hc0[3]*_t154*_t46 - _t117*_t149*_t155 - _t139*_t156 - _t140*_t156 - _t141*_t91 - _t142*_t92 - _t144*_t91 - _t153*_t155;
  out[27] = d4f_dx2_du2_dt;
  const double d4f_dx2_du3 = -_t100*_t157 - _t102*_t12*_t135*_t149 - _t105*_t157 - _t120*_t157 - _t153*_t83;
  out[28] = d4f_dx2_du3;
  const double d4f_dx22_dt2 = -_t119*_t158 - _t129*_t93 - _t130*_t93 - _t132*_t93 - _t133*_t52 - _t134*_t52 - _t138*_t158 - _t139*_t158 - _t140*_t158 - _t148*_t158;
  out[29] = d4f_dx22_dt2;
  const double d4f_dx22_du_dt = -_hc0[1]*_t152*_t51 - _t119*_t160 - _t139*_t160 - _t140*_t160 - _t141*_t95 - _t142*_t24*_t93 - _t144*_t95 - _t154*_t159;
  out[30] = d4f_dx22_du_dt;
  const double d4f_dx22_du2 = -_t119*_t161 - _t138*_t161 - _t139*_t161 - _t140*_t161 - _t148*_t161;
  out[31] = d4f_dx22_du2;
  const double d4f_dx23_dt = -_t119*_t162 - _t138*_t162 - _t139*_t162 - _t140*_t162 - _t141*_t97 - _t143*_t97 - _t144*_t97 - _t154*_t96;
  out[32] = d4f_dx23_dt;
  const double d4f_dx23_du = -_t119*_t163 - _t139*_t163 - _t140*_t163 - _t148*_t163 - _t152*_t96;
  out[33] = d4f_dx23_du;
  const double d4f_dx24 = -_t100*_t164 - _t104*_t165 - _t105*_t164 - _t106*_t164 - _t108*_t165;
  out[34] = d4f_dx24;
#endif
}
