/*
  Generated from python/mgga_exc/mgga_x_gvt4.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py mgga_x_gvt4
*/

#ifndef _MGGA_X_GVT4_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _MGGA_X_GVT4_KERNEL_BODY
#define _KMAX 0
#define _MGGA_X_GVT4_HELPER_BODIES
#include "mgga_x_gvt4.c"
#undef _MGGA_X_GVT4_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _MGGA_X_GVT4_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_MGGA_X_GVT4_HELPER_BODIES)
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(gvt4_gamm_pf_k, _KMAX)(const xc_func_type *p, double alpha, double x2, double z, double *out) {

  const double _t1 = x2 + z;
  ...
  out[4] = d2f_dz2;
  const double d2f_dx2_dz = 0;
  out[5] = d2f_dx2_dz;
  const double d2f_dx22 = 0;
  out[6] = d2f_dx22;
  const double d2f_dalpha_dz = 0.1e1;
  out[7] = d2f_dalpha_dz;
  const double d2f_dalpha_dx2 = 0.1e1;
  out[8] = d2f_dalpha_dx2;
  ...
  out[10] = d3f_dz3;
  const double d3f_dx2_dz2 = 0;
  out[11] = d3f_dx2_dz2;
  const double d3f_dx22_dz = 0;
  out[12] = d3f_dx22_dz;
  const double d3f_dx23 = 0;
  out[13] = d3f_dx23;
  const double d3f_dalpha_dz2 = 0;
  out[14] = d3f_dalpha_dz2;
  const double d3f_dalpha_dx2_dz = 0;
  out[15] = d3f_dalpha_dx2_dz;
  const double d3f_dalpha_dx22 = 0;
  out[16] = d3f_dalpha_dx22;
  const double d3f_dalpha2_dz = 0;
  out[17] = d3f_dalpha2_dz;
  const double d3f_dalpha2_dx2 = 0;
  out[18] = d3f_dalpha2_dx2;
  ...
  out[20] = d4f_dz4;
  const double d4f_dx2_dz3 = 0;
  out[21] = d4f_dx2_dz3;
  const double d4f_dx22_dz2 = 0;
  out[22] = d4f_dx22_dz2;
  const double d4f_dx23_dz = 0;
  out[23] = d4f_dx23_dz;
  const double d4f_dx24 = 0;
  out[24] = d4f_dx24;
  const double d4f_dalpha_dz3 = 0;
  out[25] = d4f_dalpha_dz3;
  const double d4f_dalpha_dx2_dz2 = 0;
  out[26] = d4f_dalpha_dx2_dz2;
  const double d4f_dalpha_dx22_dz = 0;
  out[27] = d4f_dalpha_dx22_dz;
  const double d4f_dalpha_dx23 = 0;
  out[28] = d4f_dalpha_dx23;
  const double d4f_dalpha2_dz2 = 0;
  out[29] = d4f_dalpha2_dz2;
  const double d4f_dalpha2_dx2_dz = 0;
  out[30] = d4f_dalpha2_dx2_dz;
  const double d4f_dalpha2_dx22 = 0;
  out[31] = d4f_dalpha2_dx22;
  const double d4f_dalpha3_dz = 0;
  out[32] = d4f_dalpha3_dz;
  const double d4f_dalpha3_dx2 = 0;
  out[33] = d4f_dalpha3_dx2;
  const double d4f_dalpha4 = 0;
  out[34] = d4f_dalpha4;
#endif
}

GPU_DEVICE_FUNCTION static inline void XC_CAT(gvt4_f_pf_k, _KMAX)(const xc_func_type *p, double x2, double u, double t, double *out) {

  const double _t1 = xc_powr(0.6e1, 2, 3);
  const double _t2 = (M_PI * M_CBRTPI);
  const double _t3 = _t1*_t2;
  const double _t4 = -0.3e1 / 0.5e1*_t3 + 0.2e1*t;
  const double _t6 = -0.0037501955999999998*_t3 + 0.012500651999999999*t - 0.003556788*x2;
  const double _t8 = 0.00035748220000000002*(_t4 * _t4) - 0.0001282732*_t4*x2 - 2.3545180000000001e-5*(x2 * x2);
  const double _t9 = (0.2e1 / 0.3e1)*xc_powr(0.2e1, 2, 3)*M_CBRTPI/M_CBRT3;
#if _KMAX >= 1
  const double _t10 = -0.00085795727999999998*_t3 + 0.0028598576000000001*t - 0.00025654639999999999*x2;
  const double _t12 = 7.6963919999999998e-5*_t1*_t2 - 0.00025654639999999999*t - 4.7090360000000003e-5*x2;
#endif
#if _KMAX >= 2
  const double _t19 = 0.12e2*_t10;
#endif
  double _hc0[(_KMAX) >= 4 ? 35 : (_KMAX) >= 3 ? 20 : (_KMAX) >= 2 ? 10 : (_KMAX) >= 1 ? 4 : 1];
  XC_CAT(gvt4_gamm_pf_k, _KMAX)(p, 0.0018672599999999999, x2, _t4, _hc0);
  ...
  const double _t13 = _hc0[2]*_t7;
  const double _t14 = _hc0[2]*_t11;
  const double _t15 = 0.3e1*_t14;
#endif
#if _KMAX >= 2
  const double _t16 = _hc0[1]*_t7;
  const double _t17 = ((_hc0[1]) * (_hc0[1]));
  const double _t18 = _hc0[1]*_t11;
  const double _t20 = _t11*_t17;
  const double _t21 = xc_powi((_hc0[0]), -5);
  const double _t22 = _t17*_t21;
  const double _t23 = 0.48e2*_t22;
  const double _t24 = _hc0[2]*_t18;
  const double _t25 = _hc0[1]*_hc0[2]*_t21;
  const double _t26 = ((_hc0[2]) * (_hc0[2]));
  const double _t27 = _t11*_t26;
  const double _t28 = _t21*_t26;
#endif
#if _KMAX >= 3
  const double _t29 = ((_hc0[1]) * (_hc0[1]) * (_hc0[1]));
  const double _t30 = xc_powi((_hc0[0]), -6);
  const double _t31 = 0.48e2*_t25;
  const double _t32 = ((_hc0[2]) * (_hc0[2]) * (_hc0[2]));
#endif
#if _KMAX >= 4
  const double _t33 = _t21*_t29;
  const double _t34 = ((_hc0[1]) * (_hc0[1]) * (_hc0[1]) * (_hc0[1]));
  const double _t35 = 0.1920e4*_t30;
  const double _t36 = _t8/xc_powi((_hc0[0]), 7);
  const double _t37 = _hc0[2]*_t22;
  const double _t38 = _t12*_t30;
  const double _t39 = _t10*_t30;
  const double _t40 = _hc0[2]*_t17;
  const double _t41 = _t30*_t6;
  const double _t42 = _hc0[2]*_t29;
  const double _t43 = _hc0[1]*_t28;
  const double _t44 = _hc0[1]*_t26;
  const double _t45 = _t17*_t26;
  const double _t46 = _t21*_t32;
  const double _t47 = 0.240e3*_t32;
  const double _t48 = ((_hc0[2]) * (_hc0[2]) * (_hc0[2]) * (_hc0[2]));
#endif

  const double f = _t9*(-_t5*_t6 - _t7*_t8 + 0.9800683/_hc0[0]);
  out[0] = f;
#if _KMAX >= 1
  const double df_dt = _t9*(0.6e1*_hc0[1]*_t11*_t8 - 1.9601366*_hc0[1]*_t5 + 0.4e1*_hc0[1]*_t6*_t7 - _t10*_t7 - 0.012500651999999999*_t5);
  out[1] = df_dt;
  const double df_du = 0;
  out[2] = df_du;
  const double df_dx2 = _t9*(-0.9800683*_hc0[2]*_t5 - _t12*_t7 + 0.2e1*_t13*_t6 + _t15*_t8 + 0.003556788*_t5);
  out[3] = df_dx2;
#endif
#if _KMAX >= 2
  const double d2f_dt2 = _t9*(0.10000521599999999*_t16 + 7.8405464*_t17*_t7 + _t18*_t19 - 0.24e2*_t20*_t6 - _t23*_t8 - 0.0028598576000000001*_t7);
  out[4] = d2f_dt2;
  const double d2f_du_dt = 0;
  out[5] = d2f_du_dt;
  const double d2f_du2 = 0;
  out[6] = d2f_du2;
  const double d2f_dx2_dt = _t9*(3.9202732*_hc0[2]*_t16 + _t10*_t15 + 0.6e1*_t12*_t18 + 0.025001303999999999*_t13 - 0.014227152*_t16 - 0.12e2*_t24*_t6 - 0.24e2*_t25*_t8 + 0.00025654639999999999*_t7);
  out[7] = d2f_dx2_dt;
  const double d2f_dx2_du = 0;
  out[8] = d2f_dx2_du;
  const double d2f_dx22 = _t9*(0.6e1*_t12*_t14 - 0.014227152*_t13 + 1.9601366*_t26*_t7 - 0.6e1*_t27*_t6 - 0.12e2*_t28*_t8 + 4.7090360000000003e-5*_t7);
  out[9] = d2f_dx22;
#endif
#if _KMAX >= 3
  const double d3f_dt3 = _t9*(0.051477436800000005*_hc0[1]*_t11 - 0.144e3*_t10*_t22 - 47.043278399999998*_t11*_t29 - 0.90004694399999985*_t20 + 0.192e3*_t21*_t29*_t6 + 0.480e3*_t29*_t30*_t8);
  out[10] = d3f_dt3;
  ...
  out[13] = d3f_du3;
  const double d3f_dx2_dt2 = _t9*(0.0085795728000000009*_hc0[2]*_t11 + 0.96e2*_hc0[2]*_t17*_t21*_t6 + 0.240e3*_hc0[2]*_t17*_t30*_t8 - 23.521639199999999*_hc0[2]*_t20 - _t10*_t31 + 0.085362911999999999*_t11*_t17 - _t12*_t23 - 0.0030785567999999999*_t18 - 0.30001564799999997*_t24);
  out[14] = d3f_dx2_dt2;
  const double d3f_dx2_du_dt = 0;
  out[15] = d3f_dx2_du_dt;
  const double d3f_dx2_du2 = 0;
  out[16] = d3f_dx2_du2;
  const double d3f_dx22_dt = _t9*(0.085362911999999999*_hc0[1]*_hc0[2]*_t11 + 0.48e2*_hc0[1]*_t21*_t26*_t6 + 0.120e3*_hc0[1]*_t26*_t30*_t8 - _t12*_t31 - 0.0015392784*_t14 - 11.7608196*_t18*_t26 - 0.00028254216*_t18 - _t19*_t28 - 0.075003911999999992*_t27);
  out[17] = d3f_dx22_dt;
  const double d3f_dx22_du = 0;
  out[18] = d3f_dx22_du;
  const double d3f_dx23 = _t9*(0.064022183999999996*_t11*_t26 - 5.8804097999999998*_t11*_t32 - 0.36e2*_t12*_t28 - 0.00042381324000000001*_t14 + 0.24e2*_t21*_t32*_t6 + 0.60e2*_t30*_t32*_t8);
  out[19] = d3f_dx23;
#endif
#if _KMAX >= 4
  const double d4f_dt4 = _t9*(_t10*_t29*_t35 + 376.34622719999999*_t21*_t34 - 0.82363898880000008*_t22 + 9.600500735999999*_t33 - _t34*_t35*_t6 - 0.5760e4*_t34*_t36);
  out[20] = d4f_dt4;
  ...
  out[24] = d4f_du4;
  const double d4f_dx2_dt3 = _t9*(188.17311359999999*_hc0[2]*_t33 + 0.036942681599999999*_t22 - 0.20590974720000002*_t25 + 0.480e3*_t29*_t38 - 0.68290329599999999*_t33 - 0.2880e4*_t36*_t42 + 3.6001877759999994*_t37 + 0.720e3*_t39*_t40 - 0.960e3*_t41*_t42);
  out[25] = d4f_dx2_dt3;
  ...
  out[28] = d4f_dx2_du3;
  const double d4f_dx22_dt2 = _t9*(94.086556799999997*_t22*_t26 + 0.00226033728*_t22 + 0.024628454399999999*_t25 - 0.034318291200000003*_t28 - 0.1440e4*_t36*_t45 - 0.68290329599999999*_t37 + 0.480e3*_t38*_t40 + 0.240e3*_t39*_t44 - 0.480e3*_t41*_t45 + 1.2000625919999999*_t43);
  out[29] = d4f_dx22_dt2;
  const double d4f_dx22_du_dt = 0;
  out[30] = d4f_dx22_du_dt;
  const double d4f_dx22_du2 = 0;
  out[31] = d4f_dx22_du2;
  const double d4f_dx23_dt = _t9*(-0.720e3*_hc0[1]*_t32*_t36 - _hc0[1]*_t41*_t47 + 47.043278399999998*_hc0[1]*_t46 + 0.00339050592*_t25 + 0.0092356703999999998*_t28 + 0.60e2*_t32*_t39 + 0.360e3*_t38*_t44 - 0.51217747199999997*_t43 + 0.30001564799999997*_t46);
  out[32] = d4f_dx23_dt;
  const double d4f_dx23_du = 0;
  out[33] = d4f_dx23_du;
  const double d4f_dx24 = _t9*(23.521639199999999*_t21*_t48 + 0.00339050592*_t28 - 0.360e3*_t36*_t48 + _t38*_t47 - 0.120e3*_t41*_t48 - 0.341451648*_t46);
  out[34] = d4f_dx24;
#endif
}
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(xchan_k, _KMAX)(const xc_func_type *p, double n_s, double sigma_ss, double lapl_s, double tau_s, double *out) {
  ...
  if(!(_t7)) XC_CAT(gvt4_f_pf_k, _KMAX)(p, _t1*sigma_ss, 0, _t2*tau_s, _hc0);
