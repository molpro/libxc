/*
  Generated from python/mgga_exc/mgga_x_tau_hcth.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py mgga_x_tau_hcth
*/

#ifndef _MGGA_X_TAU_HCTH_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _MGGA_X_TAU_HCTH_KERNEL_BODY
#define _KMAX 0
#define _MGGA_X_TAU_HCTH_HELPER_BODIES
#include "mgga_x_tau_hcth.c"
#undef _MGGA_X_TAU_HCTH_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _MGGA_X_TAU_HCTH_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_MGGA_X_TAU_HCTH_HELPER_BODIES)
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(hcth_ux_pf_k, _KMAX)(const xc_func_type *p, double x2, double *out) {

  double _hc0[(_KMAX) >= 4 ? 15 : (_KMAX) >= 3 ? 10 : (_KMAX) >= 2 ? 6 : (_KMAX) >= 1 ? 3 : 1];
  XC_CAT(b97_u_k, _KMAX)(p, 0.0040000000000000001, x2, _hc0);
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(hcth_gxnl_pf_k, _KMAX)(const xc_func_type *p, double x2, double *out) {
  const mgga_x_tau_hcth_params *params = (const mgga_x_tau_hcth_params *)(p->params);

  double _hc0[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(hcth_ux_pf_k, _KMAX)(p, x2, _hc0);
  const double _t1 = ((_hc0[0]) * (_hc0[0]));
#if _KMAX >= 1
  const double _t2 = 0.2e1*params->cx_nlocal[2];
  const double _t3 = _hc0[0]*_t2;
  const double _t4 = _hc0[1]*params->cx_nlocal[3];
  const double _t5 = 0.3e1*_t1;
#endif
#if _KMAX >= 2
  const double _t6 = ((_hc0[1]) * (_hc0[1]));
  const double _t7 = 0.6e1*params->cx_nlocal[3];
  const double _t8 = _t5*params->cx_nlocal[3];
#endif
#if _KMAX >= 3
  const double _t9 = 0.6e1*params->cx_nlocal[2];
  ...
  const double f = ((_hc0[0]) * (_hc0[0]) * (_hc0[0]))*params->cx_nlocal[3] + _hc0[0]*params->cx_nlocal[1] + _t1*params->cx_nlocal[2] + params->cx_nlocal[0];
  out[0] = f;
#if _KMAX >= 1
  const double df_dx2 = _hc0[1]*_t3 + _hc0[1]*params->cx_nlocal[1] + _t4*_t5;
  out[1] = df_dx2;
#endif
#if _KMAX >= 2
  const double d2f_dx22 = _hc0[0]*_t6*_t7 + _hc0[2]*_t3 + _hc0[2]*_t8 + _hc0[2]*params->cx_nlocal[1] + _t2*_t6;
  out[2] = d2f_dx22;
#endif
#if _KMAX >= 3
  const double d3f_dx23 = ((_hc0[1]) * (_hc0[1]) * (_hc0[1]))*_t7 + _hc0[1]*_hc0[2]*_t9 + _hc0[2]*_t10*_t4 + _hc0[3]*_t3 + _hc0[3]*_t8 + _hc0[3]*params->cx_nlocal[1];
  out[3] = d3f_dx23;
#endif
#if _KMAX >= 4
  const double d4f_dx24 = 0.24e2*_hc0[0]*_hc0[3]*_t4 + 0.8e1*_hc0[1]*_hc0[3]*params->cx_nlocal[2] + 0.36e2*_hc0[2]*_t6*params->cx_nlocal[3] + _hc0[4]*_t3 + _hc0[4]*_t8 + _hc0[4]*params->cx_nlocal[1] + _t10*_t11*params->cx_nlocal[3] + _t11*_t9;
  out[4] = d4f_dx24;
#endif
}

GPU_DEVICE_FUNCTION static inline void XC_CAT(hcth_gxl_pf_k, _KMAX)(const xc_func_type *p, double x2, double *out) {
  const mgga_x_tau_hcth_params *params = (const mgga_x_tau_hcth_params *)(p->params);
  ...
  const double _t1 = ((_hc0[0]) * (_hc0[0]));
#if _KMAX >= 1
  const double _t2 = 0.2e1*params->cx_local[2];
  const double _t3 = _hc0[0]*_t2;
  const double _t4 = _hc0[1]*params->cx_local[3];
  const double _t5 = 0.3e1*_t1;
#endif
#if _KMAX >= 2
  const double _t6 = ((_hc0[1]) * (_hc0[1]));
  const double _t7 = 0.6e1*params->cx_local[3];
  const double _t8 = _t5*params->cx_local[3];
#endif
#if _KMAX >= 3
  const double _t9 = 0.6e1*params->cx_local[2];
  const double _t10 = 0.18e2*_hc0[0];
#endif
#if _KMAX >= 4
  const double _t11 = ((_hc0[2]) * (_hc0[2]));
  ...
  const double f = ((_hc0[0]) * (_hc0[0]) * (_hc0[0]))*params->cx_local[3] + _hc0[0]*params->cx_local[1] + _t1*params->cx_local[2] + params->cx_local[0];
  out[0] = f;
#if _KMAX >= 1
  const double df_dx2 = _hc0[1]*_t3 + _hc0[1]*params->cx_local[1] + _t4*_t5;
  out[1] = df_dx2;
#endif
#if _KMAX >= 2
  const double d2f_dx22 = _hc0[0]*_t6*_t7 + _hc0[2]*_t3 + _hc0[2]*_t8 + _hc0[2]*params->cx_local[1] + _t2*_t6;
  out[2] = d2f_dx22;
#endif
#if _KMAX >= 3
  const double d3f_dx23 = ((_hc0[1]) * (_hc0[1]) * (_hc0[1]))*_t7 + _hc0[1]*_hc0[2]*_t9 + _hc0[2]*_t10*_t4 + _hc0[3]*_t3 + _hc0[3]*_t8 + _hc0[3]*params->cx_local[1];
  out[3] = d3f_dx23;
#endif
#if _KMAX >= 4
  const double d4f_dx24 = 0.24e2*_hc0[0]*_hc0[3]*_t4 + 0.8e1*_hc0[1]*_hc0[3]*params->cx_local[2] + 0.36e2*_hc0[2]*_t6*params->cx_local[3] + _hc0[4]*_t3 + _hc0[4]*_t8 + _hc0[4]*params->cx_local[1] + _t10*_t11*params->cx_local[3] + _t11*_t9;
  out[4] = d4f_dx24;
#endif
}

GPU_DEVICE_FUNCTION static inline void XC_CAT(hcth_f_pf_k, _KMAX)(const xc_func_type *p, double x2, double u, double t, double *out) {

  double _hc0[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(hcth_gxnl_pf_k, _KMAX)(p, x2, _hc0);
  double _hc1[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(hcth_gxl_pf_k, _KMAX)(p, x2, _hc1);
  ...
  const double _t5 = (0.3e1 / 0.10e2)*_t4 + t;
  const double _t6 = (0.1e1 / _t5);
  const double _t7 = xc_powi(_t5, -5);
  const double _t8 = xc_powi(_t3, 5);
  const double _t9 = (_t3 * _t3 * _t3);
  const double _t10 = (0.1e1 / (_t5 * _t5 * _t5));
  const double _t11 = 0.2e1*_t10;
  const double _t12 = -_t11*_t9 + _t3*_t6 + _t7*_t8;
#if _KMAX >= 1
  const double _t13 = (0.1e1 / (_t5 * _t5));
  const double _t14 = (_t3 * _t3);
  const double _t15 = (0.1e1 / (_t5 * _t5 * _t5 * _t5));
  const double _t16 = (_t3 * _t3 * _t3 * _t3);
  const double _t17 = xc_powi(_t5, -6);
  const double _t18 = 0.6e1*_t10*_t14 - _t13*_t3 + 0.6e1*_t15*_t9 - 0.5e1*_t16*_t7 - 0.5e1*_t17*_t8 - _t6;
#endif
#if _KMAX >= 2
  const double _t19 = -0.3e1 / 0.5e1*_t4 + 0.2e1*t;
  const double _t20 = xc_powi(_t5, -7);
  const double _t21 = 0.6e1*_t10*_t19 + _t11*_t3 + 0.2e1*_t13 - 0.36e2*_t14*_t15 + 0.50e2*_t16*_t17 + 0.30e2*_t20*_t8 - 0.4e1*_t7*_t9;
#endif
#if _KMAX >= 3
  const double _t22 = xc_powi(_t5, -8);
  const double _t23 = 0.6e1*_t10 + 0.156e3*_t14*_t7 - 0.54e2*_t15*_t19 - 0.6e1*_t15*_t3 - 0.450e3*_t16*_t20 - 0.180e3*_t17*_t9 - 0.210e3*_t22*_t8;
#endif

  const double f = _hc0[0]*_t12 + _hc1[0];
  out[0] = f;
#if _KMAX >= 1
  const double df_dt = _hc0[0]*_t18;
  out[1] = df_dt;
  const double df_du = 0;
  out[2] = df_du;
  const double df_dx2 = _hc0[1]*_t12 + _hc1[1];
  out[3] = df_dx2;
#endif
#if _KMAX >= 2
  const double d2f_dt2 = _hc0[0]*_t21;
  out[4] = d2f_dt2;
  const double d2f_du_dt = 0;
  out[5] = d2f_du_dt;
  const double d2f_du2 = 0;
  out[6] = d2f_du2;
  const double d2f_dx2_dt = _hc0[1]*_t18;
  out[7] = d2f_dx2_dt;
  const double d2f_dx2_du = 0;
  out[8] = d2f_dx2_du;
  const double d2f_dx22 = _hc0[2]*_t12 + _hc1[2];
  out[9] = d2f_dx22;
#endif
#if _KMAX >= 3
  const double d3f_dt3 = _hc0[0]*_t23;
  out[10] = d3f_dt3;
  ...
  out[13] = d3f_du3;
  const double d3f_dx2_dt2 = _hc0[1]*_t21;
  out[14] = d3f_dx2_dt2;
  const double d3f_dx2_du_dt = 0;
  out[15] = d3f_dx2_du_dt;
  const double d3f_dx2_du2 = 0;
  out[16] = d3f_dx2_du2;
  const double d3f_dx22_dt = _hc0[2]*_t18;
  out[17] = d3f_dx22_dt;
  const double d3f_dx22_du = 0;
  out[18] = d3f_dx22_du;
  const double d3f_dx23 = _hc0[3]*_t12 + _hc1[3];
  out[19] = d3f_dx23;
#endif
#if _KMAX >= 4
  const double d4f_dt4 = _hc0[0]*(-0.240e3*_t14*_t17 - 0.120e3*_t15 + 0.4200e4*_t16*_t22 + 0.372e3*_t19*_t7 + 0.2880e4*_t20*_t9 + 0.24e2*_t3*_t7 + 0.1680e4*_t8/xc_powi(_t5, 9));
  out[20] = d4f_dt4;
  ...
  out[24] = d4f_du4;
  const double d4f_dx2_dt3 = _hc0[1]*_t23;
  out[25] = d4f_dx2_dt3;
  ...
  out[28] = d4f_dx2_du3;
  const double d4f_dx22_dt2 = _hc0[2]*_t21;
  out[29] = d4f_dx22_dt2;
  const double d4f_dx22_du_dt = 0;
  out[30] = d4f_dx22_du_dt;
  const double d4f_dx22_du2 = 0;
  out[31] = d4f_dx22_du2;
  const double d4f_dx23_dt = _hc0[3]*_t18;
  out[32] = d4f_dx23_dt;
  const double d4f_dx23_du = 0;
  out[33] = d4f_dx23_du;
  const double d4f_dx24 = _hc0[4]*_t12 + _hc1[4];
  out[34] = d4f_dx24;
#endif
}
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(xchan_k, _KMAX)(const xc_func_type *p, double n_s, double sigma_ss, double lapl_s, double tau_s, double *out) {
  ...
  if(!(_t7)) XC_CAT(hcth_f_pf_k, _KMAX)(p, _t1*sigma_ss, 0, _t2*tau_s, _hc0);
