/*
  Generated from python/mgga_exc/mgga_x_sregtm_v3.py via
  python/libxc_codegen.py with Python 3.9.25, SymPy 1.14.0.
  Do not edit -- regenerate via
  scripts/sympy2c/compile.py mgga_x_sregtm_v3
*/

#ifndef _MGGA_X_SREGTM_V3_KERNEL_BODY
#define maple2c_order 4
#define MAPLE2C_FLAGS (XC_FLAGS_I_HAVE_EXC | XC_FLAGS_I_HAVE_VXC | XC_FLAGS_I_HAVE_FXC | XC_FLAGS_I_HAVE_KXC | XC_FLAGS_I_HAVE_LXC)

/* ---- top-level dispatch ---- */

#define _MGGA_X_SREGTM_V3_KERNEL_BODY
#define _KMAX 0
#define _MGGA_X_SREGTM_V3_HELPER_BODIES
#include "mgga_x_sregtm_v3.c"
#undef _MGGA_X_SREGTM_V3_HELPER_BODIES
#undef _KMAX
#if XC_MAXORDER >= 1
#define _KMAX 1
#define _MGGA_X_SREGTM_V3_HELPER_BODIES
...
#undef FUNC_NAME
#endif
#elif defined(_MGGA_X_SREGTM_V3_HELPER_BODIES)
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(sregtm_zrev_k, _KMAX)(const xc_func_type *p, double a, double p_, double *out) {

  const double _t1 = 0.5e1*p_ + 0.58567999999999998;
  const double _t2 = a > 0;
  const double _t3 = _t1 + 0.3e1*my_piecewise3(_t2, a, 0);
  const double _t4 = (0.1e1 / _t3);
#if _KMAX >= 1
  const double _t5 = (0.1e1 / (_t3 * _t3));
  const double _t6 = _t1*_t5;
  const double _t7 = my_piecewise3(_t2, 0.1e1, 0);
#endif
#if _KMAX >= 2
...
#endif
#if _KMAX >= 3
  const double _t11 = (0.1e1 / (_t3 * _t3 * _t3 * _t3));
  const double _t12 = _t1*_t11;
  const double _t13 = (_t7 * _t7 * _t7);
#endif
#if _KMAX >= 4
  const double _t14 = _t1/xc_powi(_t3, 5);
#endif

  const double f = _t1*_t4;
  out[0] = f;
#if _KMAX >= 1
  const double df_dp_ = 0.5e1*_t4 - 0.5e1*_t6;
  out[1] = df_dp_;
  const double df_da = -0.3e1*_t6*_t7;
  out[2] = df_da;
#endif
#if _KMAX >= 2
  const double d2f_dp_2 = -0.50e2*_t5 + 0.50e2*_t9;
  out[3] = d2f_dp_2;
  const double d2f_da_dp_ = -0.15e2*_t5*_t7 + 0.30e2*_t7*_t9;
  out[4] = d2f_da_dp_;
  const double d2f_da2 = 0.18e2*_t10*_t9;
  out[5] = d2f_da2;
#endif
#if _KMAX >= 3
  const double d3f_dp_3 = -0.750e3*_t12 + 0.750e3*_t8;
  out[6] = d3f_dp_3;
  const double d3f_da_dp_2 = -0.450e3*_t12*_t7 + 0.300e3*_t7*_t8;
  out[7] = d3f_da_dp_2;
  const double d3f_da2_dp_ = -0.270e3*_t10*_t12 + 0.90e2*_t10*_t8;
  out[8] = d3f_da2_dp_;
  const double d3f_da3 = -0.162e3*_t12*_t13;
  out[9] = d3f_da3;
#endif
#if _KMAX >= 4
  const double d4f_dp_4 = -0.15000e5*_t11 + 0.15000e5*_t14;
  out[10] = d4f_dp_4;
  const double d4f_da_dp_3 = -0.6750e4*_t11*_t7 + 0.9000e4*_t14*_t7;
  out[11] = d4f_da_dp_3;
  const double d4f_da2_dp_2 = -0.2700e4*_t10*_t11 + 0.5400e4*_t10*_t14;
  out[12] = d4f_da2_dp_2;
  const double d4f_da3_dp_ = -0.810e3*_t11*_t13 + 0.3240e4*_t13*_t14;
  out[13] = d4f_da3_dp_;
  const double d4f_da4 = 0.1944e4*_t14*(_t7 * _t7 * _t7 * _t7);
  out[14] = d4f_da4;
#endif
}

GPU_DEVICE_FUNCTION static inline void XC_CAT(regtm_w_k, _KMAX)(const xc_func_type *p, double zp, double *out) {

  const double _t1 = (zp * zp * zp);
  const double _t2 = _t1 + 0.1e1;
  const double _t3 = (0.1e1 / (_t2 * _t2));
  const double _t4 = (zp * zp);
  const double _t5 = 0.3e1*_t1 + _t4;
#if _KMAX >= 1
  const double _t6 = 0.9e1*_t4 + 0.2e1*zp;
  const double _t7 = (0.1e1 / (_t2 * _t2 * _t2));
  const double _t8 = _t5*_t7;
#endif
#if _KMAX >= 2
  const double _t9 = 0.18e2*zp + 0.2e1;
  const double _t10 = 0.12e2*_t8;
  const double _t11 = _t4*_t7;
  const double _t12 = (zp * zp * zp * zp);
  const double _t13 = (0.1e1 / (_t2 * _t2 * _t2 * _t2));
#endif
#if _KMAX >= 3
  const double _t14 = _t6*_t7;
  const double _t15 = xc_powi(zp, 6);
  const double _t16 = xc_powi(_t2, -5);
  ...
  out[0] = f;
#if _KMAX >= 1
  const double df_dzp = _t3*_t6 - 0.6e1*_t4*_t8;
  out[1] = df_dzp;
#endif
#if _KMAX >= 2
  const double d2f_dzp2 = -_t10*zp - 0.12e2*_t11*_t6 + 0.54e2*_t12*_t13*_t5 + _t3*_t9;
  out[2] = d2f_dzp2;
#endif
#if _KMAX >= 3
  const double d3f_dzp3 = 0.324e3*_t1*_t13*_t5 - _t10 - 0.18e2*_t11*_t9 + 0.162e3*_t12*_t13*_t6 - 0.36e2*_t14*zp - 0.648e3*_t15*_t17 + 0.18e2*_t3;
  out[3] = d3f_dzp3;
#endif
#if _KMAX >= 4
  const double d4f_dzp4 = 0.1296e4*_t1*_t13*_t6 - 0.432e3*_t11 + 0.324e3*_t12*_t13*_t9 + 0.1080e4*_t13*_t4*_t5 - 0.48e2*_t14 - 0.2592e4*_t15*_t16*_t6 - 0.7776e4*_t17*xc_powi(zp, 5) - 0.72e2*_t7*_t9*zp + 0.9720e4*_t5*xc_powi(zp, 8)/xc_powi(_t2, 6);
  out[4] = d4f_dzp4;
#endif
}

GPU_DEVICE_FUNCTION static inline void XC_CAT(regtm_one_minus_w_k, _KMAX)(const xc_func_type *p, double zp, double *out) {

  const double _t1 = 0.1e1 - zp;
  const double _t2 = (zp * zp * zp);
  const double _t3 = _t2 + 0.1e1;
  const double _t4 = (0.1e1 / (_t3 * _t3));
  const double _t5 = (zp * zp * zp * zp);
  const double _t6 = xc_powi(zp, 5);
  const double _t7 = -_t2 - _t5 - _t6 + zp + 0.1e1;
  const double _t8 = _t4*_t7;
#if _KMAX >= 1
  const double _t9 = (zp * zp);
  const double _t10 = -0.4e1*_t2 - 0.5e1*_t5 - 0.3e1*_t9 + 0.1e1;
  const double _t11 = (0.1e1 / (_t3 * _t3 * _t3));
  const double _t12 = _t11*_t7;
  const double _t13 = _t1*_t12;
#endif
#if _KMAX >= 2
  const double _t14 = 0.12e2*_t9;
  const double _t15 = -_t14 - 0.20e2*_t2 - 0.6e1*zp;
  const double _t16 = _t15*_t4;
  const double _t17 = 0.12e2*_t13;
  const double _t18 = _t10*_t11;
  const double _t19 = _t1*_t18;
  const double _t20 = (0.1e1 / (_t3 * _t3 * _t3 * _t3));
  const double _t21 = _t20*_t7;
  const double _t22 = _t21*_t5;
#endif
#if _KMAX >= 3
  const double _t23 = -0.60e2*_t9 - 0.24e2*zp - 0.6e1;
  const double _t24 = _t11*_t9;
  const double _t25 = _t15*_t24;
  const double _t26 = xc_powi(zp, 6);
  const double _t27 = xc_powi(_t3, -5);
  const double _t28 = _t27*_t7;
  const double _t29 = _t26*_t28;
#endif
#if _KMAX >= 4
  const double _t30 = _t20*_t5;
  const double _t31 = _t1*_t10;
#endif

  const double f = _t1*_t8;
  out[0] = f;
#if _KMAX >= 1
  const double df_dzp = _t1*_t10*_t4 - 0.6e1*_t13*_t9 - _t8;
  out[1] = df_dzp;
#endif
#if _KMAX >= 2
  const double d2f_dzp2 = _t1*_t16 + 0.54e2*_t1*_t22 - 0.2e1*_t10*_t4 + _t12*_t14 - _t14*_t19 - _t17*zp;
  out[2] = d2f_dzp2;
#endif
#if _KMAX >= 3
  const double d3f_dzp3 = 0.162e3*_t1*_t10*_t20*_t5 + 0.324e3*_t1*_t2*_t20*_t7 + _t1*_t23*_t4 - 0.18e2*_t1*_t25 - 0.648e3*_t1*_t29 + 0.36e2*_t10*_t11*_t9 + 0.36e2*_t11*_t7*zp - 0.3e1*_t16 - _t17 - 0.36e2*_t19*zp - 0.162e3*_t22;
  out[3] = d3f_dzp3;
#endif
#if _KMAX >= 4
  const double d4f_dzp4 = -0.72e2*_t1*_t11*_t15*zp + 0.324e3*_t1*_t15*_t30 + 0.1080e4*_t1*_t21*_t9 - 0.24e2*_t1*_t23*_t24 - 0.7776e4*_t1*_t28*_t6 + _t1*_t4*(-0.120e3*zp - 0.24e2) + 0.9720e4*_t1*_t7*xc_powi(zp, 8)/xc_powi(_t3, 6) - 0.648e3*_t10*_t30 + 0.48e2*_t12 + 0.144e3*_t18*zp - 0.48e2*_t19 + 0.1296e4*_t2*_t20*_t31 - 0.1296e4*_t2*_t21 - 0.4e1*_t23*_t4 + 0.72e2*_t25 - 0.2592e4*_t26*_t27*_t31 + 0.2592e4*_t29;
  out[4] = d4f_dzp4;
#endif
}
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(tm_w_pf_k, _KMAX)(const xc_func_type *p, double x2, double t, double *out) {
  ...
  XC_CAT(sregtm_zrev_k, _KMAX)(p, _hc0[0], _hc1[0], _hc2);
  double _hc3[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(regtm_w_k, _KMAX)(p, _hc2[0], _hc3);
#if _KMAX >= 1
  const double _t1 = _hc0[1]*_hc2[2];
  const double _t2 = _hc0[2]*_hc2[2] + _hc1[1]*_hc2[1];
#endif
#if _KMAX >= 2
  const double _t3 = ((_hc0[1]) * (_hc0[1]));
  const double _t4 = _hc2[5]*_t3;
  const double _t5 = ((_hc2[2]) * (_hc2[2]));
  const double _t6 = _t3*_t5;
  const double _t7 = _hc0[2]*_hc2[5];
  const double _t8 = _hc0[1]*_t7;
  const double _t9 = _hc1[1]*_hc2[4];
  const double _t10 = _hc0[1]*_t9;
  const double _t11 = _t10 + _t8;
  const double _t12 = _hc3[2]*_t2;
  const double _t13 = (_t2 * _t2);
  const double _t14 = _hc0[2]*(_t7 + _t9);
  const double _t15 = _hc1[1]*(_hc0[2]*_hc2[4] + _hc1[1]*_hc2[3]);
  const double _t16 = _t14 + _t15;
#endif
#if _KMAX >= 3
  const double _t17 = ((_hc0[1]) * (_hc0[1]) * (_hc0[1]));
  const double _t18 = _hc2[9]*_t17;
  const double _t19 = ((_hc2[2]) * (_hc2[2]) * (_hc2[2]))*_t17;
  const double _t20 = 0.3e1*_hc3[2];
  const double _t21 = _hc2[2]*_hc2[5]*_t17;
  const double _t22 = _hc3[2]*_t11;
  const double _t23 = 0.2e1*_t1;
  const double _t24 = _hc0[2]*_hc2[9];
  const double _t25 = _t24*_t3;
  const double _t26 = _hc1[1]*_hc2[8];
  const double _t27 = _t26*_t3;
  const double _t28 = _t25 + _t27;
  const double _t29 = _hc3[3]*_t6;
  const double _t30 = _hc3[3]*_t13;
  const double _t31 = 0.2e1*_t10 + 0.2e1*_t8;
  const double _t32 = _hc3[2]*_t16;
  const double _t33 = _hc0[2]*(_hc0[1]*_t24 + _hc0[1]*_t26);
  const double _t34 = _hc0[2]*_hc2[8];
  const double _t35 = _hc1[1]*_hc2[7];
  const double _t36 = _hc1[1]*(_hc0[1]*_t34 + _hc0[1]*_t35);
  const double _t37 = _t33 + _t36;
  const double _t38 = (_t2 * _t2 * _t2);
  const double _t39 = 0.2e1*_t14 + 0.2e1*_t15;
  const double _t40 = _t34 + _t35;
  const double _t41 = _hc0[2]*(_hc0[2]*(_t24 + _t26) + _hc1[1]*_t40);
  const double _t42 = _hc1[1]*(_hc0[2]*_t40 + _hc1[1]*(_hc0[2]*_hc2[7] + _hc1[1]*_hc2[6]));
  const double _t43 = _t41 + _t42;
#endif
#if _KMAX >= 4
  const double _t44 = ((_hc0[1]) * (_hc0[1]) * (_hc0[1]) * (_hc0[1]));
  const double _t45 = _hc0[2]*_hc2[14];
  const double _t46 = _hc1[1]*_hc2[13];
  const double _t47 = _hc3[3]*_t2;
  const double _t48 = _hc3[2]*_t1;
  const double _t49 = _hc0[2]*_hc2[13];
  const double _t50 = _hc1[1]*_hc2[12];
  const double _t51 = _t1*_t47;
  const double _t52 = _hc0[1]*_t49 + _hc0[1]*_t50;
  const double _t53 = _hc0[2]*_hc2[12];
  const double _t54 = _hc1[1]*_hc2[11];
  const double _t55 = _t49 + _t50;
  const double _t56 = _t53 + _t54;
  const double _t57 = _hc0[2]*_t55 + _hc1[1]*_t56;
#endif

  const double f = _hc3[0];
  out[0] = f;
#if _KMAX >= 1
  const double df_dt = _hc3[1]*_t1;
  out[1] = df_dt;
  const double df_dx2 = _hc3[1]*_t2;
  out[2] = df_dx2;
#endif
#if _KMAX >= 2
  const double d2f_dt2 = _hc3[1]*_t4 + _hc3[2]*_t6;
  out[3] = d2f_dt2;
  const double d2f_dx2_dt = _hc3[1]*_t11 + _t1*_t12;
  out[4] = d2f_dx2_dt;
  const double d2f_dx22 = _hc3[1]*_t16 + _hc3[2]*_t13;
  out[5] = d2f_dx22;
#endif
#if _KMAX >= 3
  const double d3f_dt3 = _hc3[1]*_t18 + _hc3[3]*_t19 + _t20*_t21;
  out[6] = d3f_dt3;
  const double d3f_dx2_dt2 = _hc3[1]*_t28 + _t12*_t4 + _t2*_t29 + _t22*_t23;
  out[7] = d3f_dx2_dt2;
  const double d3f_dx22_dt = _hc3[1]*_t37 + _t1*_t30 + _t1*_t32 + _t12*_t31;
  out[8] = d3f_dx22_dt;
  const double d3f_dx23 = _hc3[1]*_t43 + _hc3[3]*_t38 + _t12*_t16 + _t12*_t39;
  out[9] = d3f_dx23;
#endif
#if _KMAX >= 4
  const double d4f_dt4 = _hc2[14]*_hc3[1]*_t44 + ((_hc2[2]) * (_hc2[2]) * (_hc2[2]) * (_hc2[2]))*_hc3[4]*_t44 + 0.4e1*_hc2[2]*_hc2[9]*_hc3[2]*_t44 + ((_hc2[5]) * (_hc2[5]))*_t20*_t44 + 0.6e1*_hc2[5]*_hc3[3]*_t44*_t5;
  out[10] = d4f_dt4;
  const double d4f_dx2_dt3 = _hc3[1]*(_t17*_t45 + _t17*_t46) + _hc3[4]*_t19*_t2 + _t1*_t20*_t28 + 0.3e1*_t11*_t29 + _t12*_t18 + 0.3e1*_t21*_t47 + 0.3e1*_t22*_t4;
  out[11] = d4f_dx2_dt3;
  const double d4f_dx22_dt2 = _hc3[1]*(_hc0[2]*(_t3*_t45 + _t3*_t46) + _hc1[1]*(_t3*_t49 + _t3*_t50)) + _hc3[4]*_t13*_t6 + _t12*(0.2e1*_t25 + 0.2e1*_t27) + _t16*_t29 + _t22*_t31 + _t23*_t31*_t47 + _t30*_t4 + _t32*_t4 + 0.2e1*_t37*_t48;
  out[12] = d4f_dx22_dt2;
  const double d4f_dx23_dt = _hc3[1]*(_hc0[2]*(_hc0[2]*(_hc0[1]*_t45 + _hc0[1]*_t46) + _hc1[1]*_t52) + _hc1[1]*(_hc0[2]*_t52 + _hc1[1]*(_hc0[1]*_t53 + _hc0[1]*_t54))) + _hc3[4]*_t1*_t38 + _t12*_t37 + _t12*(0.2e1*_t33 + 0.2e1*_t36) + _t16*_t22 + _t16*_t51 + _t22*_t39 + _t30*(0.3e1*_t10 + 0.3e1*_t8) + _t39*_t51 + _t43*_t48;
  out[13] = d4f_dx23_dt;
  const double d4f_dx24 = _hc3[1]*(_hc0[2]*(_hc0[2]*(_hc0[2]*(_t45 + _t46) + _hc1[1]*_t55) + _hc1[1]*_t57) + _hc1[1]*(_hc0[2]*_t57 + _hc1[1]*(_hc0[2]*_t56 + _hc1[1]*(_hc0[2]*_hc2[11] + _hc1[1]*_hc2[10])))) + _hc3[2]*(_t16 * _t16) + _hc3[4]*(_t2 * _t2 * _t2 * _t2) + 0.2e1*_t12*_t43 + _t12*(0.2e1*_t41 + 0.2e1*_t42) + _t16*_t30 + _t30*_t39 + _t30*(0.3e1*_t14 + 0.3e1*_t15) + _t32*_t39;
  out[14] = d4f_dx24;
#endif
}
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(tm_qtilde_pf_k, _KMAX)(const xc_func_type *p, double x2, double t, double *out) {
  ...
  out[1] = df_dt;
  const double df_dx2 = (0.2e1 / 0.3e1)*_hc0[1] + (0.9e1 / 0.20e2)*_hc1[2];
  out[2] = df_dx2;
#endif
#if _KMAX >= 2
  const double d2f_dt2 = 0;
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(tm_tratio_pf_k, _KMAX)(const xc_func_type *p, double x2, double t, double *out) {
  ...
  const double _t2 = _hc0[2]*_hc2[5];
  const double _t3 = _hc1[1]*_hc2[4];
#endif
#if _KMAX >= 3
  const double _t4 = ((_hc0[1]) * (_hc0[1]) * (_hc0[1]));
  const double _t5 = _hc0[2]*_hc2[9];
  const double _t6 = _hc1[1]*_hc2[8];
  const double _t7 = _hc0[2]*_hc2[8];
  const double _t8 = _hc1[1]*_hc2[7];
  const double _t9 = _t7 + _t8;
#endif
#if _KMAX >= 4
  const double _t10 = _hc0[2]*_hc2[14];
  const double _t11 = _hc1[1]*_hc2[13];
  const double _t12 = _hc0[2]*_hc2[13];
  const double _t13 = _hc1[1]*_hc2[12];
  const double _t14 = _hc0[1]*_t12 + _hc0[1]*_t13;
  const double _t15 = _hc0[2]*_hc2[12];
  const double _t16 = _hc1[1]*_hc2[11];
  const double _t17 = _t12 + _t13;
  const double _t18 = _t15 + _t16;
  const double _t19 = _hc0[2]*_t17 + _hc1[1]*_t18;
#endif

  const double f = _hc2[0];
  out[0] = f;
#if _KMAX >= 1
  const double df_dt = _hc0[1]*_hc2[2];
  out[1] = df_dt;
  const double df_dx2 = _hc0[2]*_hc2[2] + _hc1[1]*_hc2[1];
  out[2] = df_dx2;
#endif
#if _KMAX >= 2
  const double d2f_dt2 = _hc2[5]*_t1;
  out[3] = d2f_dt2;
  const double d2f_dx2_dt = _hc0[1]*_t2 + _hc0[1]*_t3;
  out[4] = d2f_dx2_dt;
  const double d2f_dx22 = _hc0[2]*(_t2 + _t3) + _hc1[1]*(_hc0[2]*_hc2[4] + _hc1[1]*_hc2[3]);
  out[5] = d2f_dx22;
#endif
#if _KMAX >= 3
  const double d3f_dt3 = _hc2[9]*_t4;
  out[6] = d3f_dt3;
  const double d3f_dx2_dt2 = _t1*_t5 + _t1*_t6;
  out[7] = d3f_dx2_dt2;
  const double d3f_dx22_dt = _hc0[2]*(_hc0[1]*_t5 + _hc0[1]*_t6) + _hc1[1]*(_hc0[1]*_t7 + _hc0[1]*_t8);
  out[8] = d3f_dx22_dt;
  const double d3f_dx23 = _hc0[2]*(_hc0[2]*(_t5 + _t6) + _hc1[1]*_t9) + _hc1[1]*(_hc0[2]*_t9 + _hc1[1]*(_hc0[2]*_hc2[7] + _hc1[1]*_hc2[6]));
  out[9] = d3f_dx23;
#endif
#if _KMAX >= 4
  const double d4f_dt4 = ((_hc0[1]) * (_hc0[1]) * (_hc0[1]) * (_hc0[1]))*_hc2[14];
  out[10] = d4f_dt4;
  const double d4f_dx2_dt3 = _t10*_t4 + _t11*_t4;
  out[11] = d4f_dx2_dt3;
  const double d4f_dx22_dt2 = _hc0[2]*(_t1*_t10 + _t1*_t11) + _hc1[1]*(_t1*_t12 + _t1*_t13);
  out[12] = d4f_dx22_dt2;
  const double d4f_dx23_dt = _hc0[2]*(_hc0[2]*(_hc0[1]*_t10 + _hc0[1]*_t11) + _hc1[1]*_t14) + _hc1[1]*(_hc0[2]*_t14 + _hc1[1]*(_hc0[1]*_t15 + _hc0[1]*_t16));
  out[13] = d4f_dx23_dt;
  const double d4f_dx24 = _hc0[2]*(_hc0[2]*(_hc0[2]*(_t10 + _t11) + _hc1[1]*_t17) + _hc1[1]*_t19) + _hc1[1]*(_hc0[2]*_t19 + _hc1[1]*(_hc0[2]*_t18 + _hc1[1]*(_hc0[2]*_hc2[11] + _hc1[1]*_hc2[10])));
  out[14] = d4f_dx24;
#endif
}
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(tm_fx_SC_pf_k, _KMAX)(const xc_func_type *p, double x2, double t, double *out) {
  ...
  const double _t41 = -_hc0[5]*_t16 + _hc0[5]*_t7 + _hc2[5]*_t13 - _hc2[5]*_t15 + _t22*_t39 + (0.292e3 / 0.2025e4)*_t35 + (0.100e3 / 0.729e3)*_t36 + _t37*_t38 - _t37*_t40;
  ...
  const double _t77 = (0.292e3 / 0.225e3)*_hc0[2];
  const double _t78 = (0.73e2 / 0.75e2)*_hc0[0]*_hc2[4]*_t3 + (0.73e2 / 0.75e2)*_hc0[1]*_hc2[2]*_t3 - _hc0[1]*_t77 + (0.73e2 / 0.75e2)*_hc0[2]*_hc2[1]*_t3 + (0.73e2 / 0.75e2)*_hc0[4]*_hc2[0]*_t3 - _hc0[4]*_t25 - 0.73e2 / 0.75e2*_hc2[4]*_t9 - _t20*_t43 - _t26*_t38 - _t56*_t71;
  const double _t79 = _t24*_t44;
  const double _t80 = _t17*_t44;
  const double _t81 = (0.292e3 / 0.2025e4)*_hc0[5];
  const double _t82 = _hc2[5]*_t22;
  const double _t83 = _hc2[5]*_t10;
  const double _t84 = (0.292e3 / 0.675e3)*_hc0[2];
  const double _t85 = _hc0[4]*_t37;
  const double _t86 = _hc0[5]*_t8;
  const double _t87 = (0.146e3 / 0.675e3)*_t39;
  const double _t88 = (0.73e2 / 0.675e3)*_hc2[5];
  const double _t89 = _hc0[1]*_t81 + _hc0[1]*_t83 + _hc0[1]*_t87 + _hc0[2]*_t66 - _hc0[8]*_t16 + _hc0[8]*_t7 + _hc2[0]*_t85 + _hc2[0]*_t86 + _hc2[1]*_t82 + _hc2[2]*_t68 + _hc2[8]*_t13 - _hc2[8]*_t15 - _t11*_t88 - _t3*_t85 - _t3*_t86 + _t38*_t69 - _t40*_t69 + _t71*_t84;
  const double _t90 = _hc2[2]*_t47;
  const double _t91 = (0.73e2 / 0.225e3)*_hc2[5];
  const double _t92 = (0.73e2 / 0.225e3)*_hc0[5];
  const double _t93 = _hc2[0]*_hc2[2];
  const double _t94 = (0.146e3 / 0.225e3)*_t39;
  const double _t95 = _hc2[2]*_t3;
  const double _t96 = _hc0[2]*_t94 + _hc0[5]*_t84 - _hc0[9]*_t16 + _hc0[9]*_t7 + _hc2[5]*_t90 + _hc2[9]*_t13 - _hc2[9]*_t15 + _t38*_t91 - _t40*_t91 + _t92*_t93 - _t92*_t95;
  const double _t97 = (0.73e2 / 0.75e2)*_hc2[5];
  const double _t98 = (0.146e3 / 0.75e2)*_hc0[2];
  const double _t99 = (0.73e2 / 0.75e2)*_hc0[0]*_hc2[5]*_t3 + (0.146e3 / 0.75e2)*_hc0[2]*_hc2[2]*_t3 + (0.73e2 / 0.75e2)*_hc0[5]*_hc2[0]*_t3 - _hc0[5]*_t25 - 0.292e3 / 0.225e3*_t35 - 0.100e3 / 0.81e2*_t36 - _t39*_t56 - _t9*_t97 - _t93*_t98;
  const double _t100 = 0.2e1*_t79;
  const double _t101 = (0.1387e4 / 0.675e3)*_hc2[2];
  const double _t102 = (0.1387e4 / 0.675e3)*_hc0[0]*_hc2[2]*_t3 + (0.1387e4 / 0.675e3)*_hc0[2]*_hc2[0]*_t3 - _hc0[2]*_t61 - _t101*_t9 - 0.19e2*_t14 - 0.950e3 / 0.729e3*_t42;
  const double _t103 = _t60*_t80;
#endif
#if _KMAX >= 4
  const double _t104 = (0.146e3 / 0.25e2)*_hc0[0];
  const double _t105 = (0.73e2 / 0.75e2)*_hc0[0]*_hc2[6]*_t3 + (0.73e2 / 0.25e2)*_hc0[1]*_hc2[3]*_t3 + (0.73e2 / 0.25e2)*_hc0[3]*_hc2[1]*_t3 + (0.73e2 / 0.75e2)*_hc0[6]*_hc2[0]*_t3 - _hc0[6]*_t25 - 0.73e2 / 0.25e2*_hc2[1]*_t51 - 0.73e2 / 0.25e2*_hc2[3]*_t20 - 0.73e2 / 0.75e2*_hc2[6]*_t9 - _t104*_t48 - 0.292e3 / 0.75e2*_t46 - 0.146e3 / 0.25e2*_t52;
  const double _t106 = 0.3e1*_t28;
  const double _t107 = (0.584e3 / 0.675e3)*_hc2[1];
  const double _t108 = _hc2[6]*_t70;
  const double _t109 = (0.146e3 / 0.225e3)*_hc2[3];
  const double _t110 = (0.292e3 / 0.675e3)*_hc2[1];
  const double _t111 = _hc0[6]*_t3;
  const double _t112 = _t24*_t57;
  const double _t113 = 0.3e1*_t112;
  const double _t114 = _t12*_t27;
  const double _t115 = (0.2774e4 / 0.675e3)*_hc0[0];
  const double _t116 = (0.1387e4 / 0.675e3)*_hc0[0]*_hc2[3]*_t3 + (0.2774e4 / 0.675e3)*_hc0[1]*_hc2[1]*_t3 + (0.1387e4 / 0.675e3)*_hc0[3]*_hc2[0]*_t3 - _hc0[3]*_t61 - 0.2774e4 / 0.675e3*_hc2[1]*_t20 - 0.1387e4 / 0.675e3*_hc2[3]*_t9 - _t115*_t21 - 0.5548e4 / 0.2025e4*_t18;
  const double _t117 = _t116*_t60;
  const double _t118 = 0.3e1*_t65;
  const double _t119 = 0.2e1*_t57;
  const double _t120 = pow(_t5, -0.39e2 / 0.10e2);
  const double _t121 = (0.8468e4 / 0.2025e4)*_hc0[0];
  const double _t122 = _t120*((0.2117e4 / 0.675e3)*_hc0[0]*_hc2[1]*_t3 + (0.2117e4 / 0.675e3)*_hc0[1]*_hc2[0]*_t3 - _hc0[1]*_t121 - 0.2117e4 / 0.675e3*_hc2[1]*_t9);
  const double _t123 = _t122*_t63;
  const double _t124 = _t17*_t27;
  const double _t125 = _t17*_t64;
  const double _t126 = (0.292e3 / 0.675e3)*_hc0[4];
  const double _t127 = _hc2[4]*_t47;
  const double _t128 = (0.73e2 / 0.225e3)*_hc2[7];
  const double _t129 = _hc2[1]*_hc2[4];
  const double _t130 = _hc0[1]*_hc2[2];
  const double _t131 = (0.146e3 / 0.225e3)*_hc0[2];
  const double _t132 = (0.73e2 / 0.225e3)*_hc2[4];
  const double _t133 = (0.146e3 / 0.225e3)*_t71;
  const double _t134 = _hc0[4]*_t49;
  const double _t135 = _hc0[7]*_t50;
  const double _t136 = _t24*_t34;
  const double _t137 = 0.2e1*_t78;
  const double _t138 = _t34*_t44;
  const double _t139 = 0.2e1*_t64;
  const double _t140 = (0.584e3 / 0.225e3)*_hc0[4];
  const double _t141 = (0.292e3 / 0.75e2)*_hc0[0];
  const double _t142 = _hc2[2]*_hc2[3];
  const double _t143 = (0.73e2 / 0.75e2)*_hc2[0];
  const double _t144 = _hc0[4]*_hc2[0];
  const double _t145 = (0.146e3 / 0.675e3)*_hc2[8];
  const double _t146 = _hc2[1]*_hc2[5];
  const double _t147 = (0.146e3 / 0.675e3)*_hc2[7];
  const double _t148 = _hc2[4]*_t126;
  const double _t149 = _hc0[5]*_hc2[3];
  const double _t150 = _hc0[7]*_t37;
  const double _t151 = _hc0[8]*_t19;
  const double _t152 = _t24*_t41;
  const double _t153 = _t41*_t44;
  const double _t154 = _t102*_t60;
  const double _t155 = _t154*_t17;
  const double _t156 = _t102*_t80;
  const double _t157 = (0.292e3 / 0.225e3)*_hc0[5];
  const double _t158 = _hc2[2]*_hc2[4];
  const double _t159 = (0.292e3 / 0.75e2)*_hc0[2];
  const double _t160 = (0.146e3 / 0.225e3)*_hc2[5];
  const double _t161 = (0.73e2 / 0.225e3)*_hc2[8];
  const double _t162 = _hc2[4]*_t92;
  const double _t163 = (0.73e2 / 0.225e3)*_hc0[8];
  const double _t164 = _hc0[9]*_t8;
  const double _t165 = _hc2[9]*_t84;
  const double _t166 = _hc2[2]*_hc2[5];
  const double _t167 = _hc0[5]*_t160;
  const double _t168 = (0.292e3 / 0.675e3)*_hc0[9];
#endif
  ...
  out[7] = d3f_dx2_dt2;
  const double d3f_dx22_dt = _t28*_t41 + _t34*_t79 + _t45*_t78 + _t6*_t89 + _t64*_t80;
  out[8] = d3f_dx22_dt;
  const double d3f_dx23 = _t100*_t41 + _t102*_t103 + _t45*_t99 + _t6*_t96;
  out[9] = d3f_dx23;
#endif
#if _KMAX >= 4
  const double d4f_dt4 = _t105*_t58 + _t106*_t54 + _t113*_t23 + _t114*_t117 + _t114*_t123 + _t118*_t23 + _t119*_t12*_t64 + _t6*(_hc0[0]*_hc2[6]*_t107 + (0.1168e4 / 0.2025e4)*_hc0[1]*_hc0[6] + (0.584e3 / 0.225e3)*_hc0[1]*_t48 - _hc0[10]*_t16 + _hc0[10]*_t7 + (0.292e3 / 0.675e3)*((_hc0[3]) * (_hc0[3])) + (0.292e3 / 0.225e3)*_hc0[3]*_t21 + _hc0[6]*_hc2[0]*_t110 + _hc2[0]*_t108 + _hc2[10]*_t13 - _hc2[10]*_t15 + ((_hc2[3]) * (_hc2[3]))*_t47 - _t108*_t3 + _t109*_t51 - _t109*_t53 - _t110*_t111);
  out[10] = d4f_dt4;
  const double d4f_dx2_dt3 = _t105*_t45 + _t106*_t76 + _t113*_t34 + _t117*_t124 + _t118*_t34 + _t119*_t125 + _t123*_t124 + _t6*((0.292e3 / 0.225e3)*_hc0[1]*_t129 - _hc0[11]*_t16 + _hc0[11]*_t7 + _hc0[3]*_t126 + _hc0[3]*_t133 + (0.146e3 / 0.225e3)*_hc0[4]*_t21 + _hc0[6]*_t29 + _hc0[6]*_t31 + _hc0[7]*_t70 + _hc2[0]*_t134 + _hc2[0]*_t135 + _hc2[1]*_hc2[7]*_t47 + _hc2[11]*_t13 - _hc2[11]*_t15 + _hc2[3]*_t127 + _hc2[6]*_t30 + _hc2[6]*_t72 - _hc2[6]*_t75 + _t109*_t130 - _t11*_t128 - _t111*_t33 + _t128*_t20 + _t131*_t48 + _t132*_t51 - _t132*_t53 - _t134*_t3 - _t135*_t3);
  out[11] = d4f_dx2_dt3;
  const double d4f_dx22_dt2 = _t103*_t116 + _t112*_t41 + _t123*_t80 + _t125*_t137 + _t136*_t137 + _t138*_t139 + _t41*_t65 + _t45*((0.73e2 / 0.75e2)*_hc0[0]*_hc2[7]*_t3 + (0.146e3 / 0.75e2)*_hc0[1]*_hc2[4]*_t3 - _hc0[1]*_t140 - 0.292e3 / 0.75e2*_hc0[1]*_t71 - _hc0[2]*_hc2[3]*_t143 + (0.73e2 / 0.75e2)*_hc0[2]*_hc2[3]*_t3 - _hc0[3]*_hc2[2]*_t143 + (0.73e2 / 0.75e2)*_hc0[3]*_hc2[2]*_t3 - _hc0[3]*_t77 + (0.146e3 / 0.75e2)*_hc0[4]*_hc2[1]*_t3 + (0.73e2 / 0.75e2)*_hc0[7]*_hc2[0]*_t3 - _hc0[7]*_t25 - _hc2[0]*_hc2[4]*_t55 - 0.146e3 / 0.75e2*_hc2[1]*_t144 - 0.73e2 / 0.75e2*_hc2[7]*_t9 - _t129*_t141 - _t142*_t56 - _t21*_t98) + _t59*_t89 + _t6*((0.584e3 / 0.2025e4)*_hc0[1]*_hc0[8] - _hc0[12]*_t16 + _hc0[12]*_t7 + (0.584e3 / 0.2025e4)*_hc0[2]*_hc0[7] + _hc0[2]*_hc2[4]*_t107 + _hc0[3]*_t81 + _hc0[3]*_t83 + _hc0[3]*_t87 + (0.584e3 / 0.2025e4)*((_hc0[4]) * (_hc0[4])) + (0.584e3 / 0.675e3)*_hc0[4]*_t71 + _hc0[5]*_t74 + _hc2[0]*_t148 + _hc2[0]*_t150 + _hc2[0]*_t151 + _hc2[1]*_hc2[8]*_t67 + _hc2[12]*_t13 - _hc2[12]*_t15 + _hc2[2]*_hc2[7]*_t67 + _hc2[3]*_t82 + ((_hc2[4]) * (_hc2[4]))*_t67 + (0.584e3 / 0.675e3)*_hc2[4]*_t130 + _t10*_t149 - _t11*_t145 + _t142*_t84 + _t145*_t20 + _t146*_t70 + _t147*_t38 - _t147*_t40 - _t148*_t3 - 0.73e2 / 0.675e3*_t149*_t3 - _t150*_t3 - _t151*_t3 - _t53*_t88) + _t76*_t79;
  out[12] = d4f_dx22_dt2;
  const double d4f_dx23_dt = _t100*_t89 + _t103*((0.1387e4 / 0.675e3)*_hc0[0]*_hc2[4]*_t3 - 0.5548e4 / 0.2025e4*_hc0[1]*_hc0[2] + (0.1387e4 / 0.675e3)*_hc0[1]*_hc2[2]*_t3 + (0.1387e4 / 0.675e3)*_hc0[2]*_hc2[1]*_t3 + (0.1387e4 / 0.675e3)*_hc0[4]*_hc2[0]*_t3 - _hc0[4]*_t61 - 0.1387e4 / 0.675e3*_hc2[4]*_t9 - _t101*_t20 - _t115*_t71 - _t38*_t62) + _t122*_t156 + _t125*_t99 + _t136*_t99 + _t137*_t152 + _t138*_t154 + _t139*_t153 + _t155*_t78 + _t28*_t96 + _t45*((0.73e2 / 0.75e2)*_hc0[0]*_hc2[8]*_t3 + (0.73e2 / 0.75e2)*_hc0[1]*_hc2[5]*_t3 - _hc0[1]*_t157 + (0.146e3 / 0.75e2)*_hc0[2]*_hc2[4]*_t3 - _hc0[2]*_t140 + (0.146e3 / 0.75e2)*_hc0[4]*_hc2[2]*_t3 - 0.146e3 / 0.75e2*_hc0[4]*_t93 - _hc0[5]*_hc2[0]*_t26 + (0.73e2 / 0.75e2)*_hc0[5]*_hc2[1]*_t3 + (0.73e2 / 0.75e2)*_hc0[8]*_hc2[0]*_t3 - _hc0[8]*_t25 - _hc2[0]*_hc2[4]*_t98 - 0.73e2 / 0.75e2*_hc2[8]*_t9 - _t141*_t158 - _t146*_t56 - _t159*_t71 - _t20*_t97 - _t39*_t55) + _t6*((0.292e3 / 0.2025e4)*_hc0[1]*_hc0[9] + _hc0[1]*_hc2[9]*_t10 - _hc0[13]*_t16 + _hc0[13]*_t7 - _hc0[4]*_t3*_t91 + _hc0[4]*_t94 + _hc0[5]*_t126 + _hc0[5]*_t133 + _hc0[8]*_t84 + _hc2[0]*_t162 + _hc2[0]*_t164 + _hc2[1]*_hc2[9]*_t22 + _hc2[13]*_t13 - _hc2[13]*_t15 + _hc2[5]*_t127 + _hc2[8]*_t90 - 0.73e2 / 0.675e3*_hc2[9]*_t11 + _t130*_t160 + _t131*_t146 + _t144*_t91 + _t158*_t77 + _t161*_t38 - _t161*_t40 - _t162*_t3 + _t163*_t93 - _t163*_t95 - _t164*_t3);
  out[13] = d4f_dx23_dt;
  const double d4f_dx24 = _t103*((0.1387e4 / 0.675e3)*_hc0[0]*_hc2[5]*_t3 + (0.2774e4 / 0.675e3)*_hc0[2]*_hc2[2]*_t3 + (0.1387e4 / 0.675e3)*_hc0[5]*_hc2[0]*_t3 - _hc0[5]*_t61 - 0.2774e4 / 0.675e3*_hc2[2]*_t38 - 0.1387e4 / 0.675e3*_hc2[5]*_t9 - _t115*_t39 - 0.5548e4 / 0.2025e4*_t35 - 0.1900e4 / 0.729e3*_t36) + _t120*_t156*((0.2117e4 / 0.675e3)*_hc0[0]*_hc2[2]*_t3 + (0.2117e4 / 0.675e3)*_hc0[2]*_hc2[0]*_t3 - _hc0[2]*_t121 - 0.2117e4 / 0.675e3*_hc2[2]*_t9 - 0.29e2*_t14 - 0.1450e4 / 0.729e3*_t42) + 0.3e1*_t152*_t99 + 0.3e1*_t153*_t154 + 0.2e1*_t155*_t99 + _t45*((0.73e2 / 0.75e2)*_hc0[0]*_hc2[9]*_t3 + (0.73e2 / 0.25e2)*_hc0[2]*_hc2[5]*_t3 - 0.146e3 / 0.25e2*_hc0[2]*_t39 + (0.73e2 / 0.25e2)*_hc0[5]*_hc2[2]*_t3 - _hc0[5]*_t159 - 0.73e2 / 0.25e2*_hc0[5]*_t93 + (0.73e2 / 0.75e2)*_hc0[9]*_hc2[0]*_t3 - _hc0[9]*_t25 - 0.73e2 / 0.25e2*_hc2[5]*_t38 - 0.73e2 / 0.75e2*_hc2[9]*_t9 - _t104*_t166) + _t6*((0.584e3 / 0.675e3)*_hc0[0]*_hc2[2]*_hc2[9] - _hc0[14]*_t16 + _hc0[14]*_t7 + (0.1168e4 / 0.2025e4)*_hc0[2]*_hc0[9] + (0.584e3 / 0.225e3)*_hc0[2]*_t166 + (0.292e3 / 0.675e3)*((_hc0[5]) * (_hc0[5])) + _hc2[0]*_t165 + _hc2[0]*_t167 + _hc2[14]*_t13 - _hc2[14]*_t15 + ((_hc2[5]) * (_hc2[5]))*_t47 + _t157*_t39 - _t165*_t3 - _t167*_t3 + _t168*_t93 - _t168*_t95) + 0.3e1*_t79*_t96;
  out[14] = d4f_dx24;
#endif
}
...
GPU_DEVICE_FUNCTION static inline void XC_CAT(tm_one_minus_w_pf_k, _KMAX)(const xc_func_type *p, double x2, double t, double *out) {
  ...
  XC_CAT(sregtm_zrev_k, _KMAX)(p, _hc0[0], _hc1[0], _hc2);
  double _hc3[(_KMAX) >= 4 ? 5 : (_KMAX) >= 3 ? 4 : (_KMAX) >= 2 ? 3 : (_KMAX) >= 1 ? 2 : 1];
  XC_CAT(regtm_one_minus_w_k, _KMAX)(p, _hc2[0], _hc3);
#if _KMAX >= 1
  const double _t1 = _hc0[1]*_hc2[2];
  const double _t2 = _hc0[2]*_hc2[2] + _hc1[1]*_hc2[1];
#endif
